...
void write_synthetic_report_table_for_bottom_margin_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2779: bottom-margin settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_bottom_margin_field_json(
    ...
    expect(delete_result.ok, "#2779: deleted bottom-margin settings fixture should mark settings deleted");
}
...
void run_bottom_margin_update_case(
    ...
    if (deleted) {
        write_synthetic_report_table_for_deleted_bottom_margin_field_json(asset_path);
    } else {
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " bottom margin field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " bottom margin field update " << extension << " stderr:\n"
...
void run_bottom_margin_clear_case(
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " bottom margin field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " bottom margin field clear " << extension << " stderr:\n"
    ...
    if (!deleted) {
        ...
        expect_not_contains(clear_process.stdout_text,
                            ...
                            issue_prefix + " clear should remove direct bottom-margin settings");
    }
}

void test_studio_host_json_preserves_bottom_margin_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_bottom_margin_record_settings_tests";
    ...
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        "bottom_margin_update",
        ".frx",
        "34",
        false,
        "report",
        "#2779: report/label record-selected bottom-margin settings success");
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        "bottom_margin_update",
    ...
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        "deleted_bottom_margin_update",
        ".frx",
        "34",
        true,
        "report",
        "#2779: report/label record-selected deleted bottom-margin settings success");
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        "deleted_bottom_margin_update",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_bottom_margin_record_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_bottom_margin_record_selection(argv[1]);
