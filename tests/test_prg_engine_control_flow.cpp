...
void test_critical_section_blocking_policy_allows_yield_inside_enter_critical() {
    namespace fs = std::filesystem;
    const fs::path temp_root = fs::temp_directory_path() / "copperfin_prg_yield_enter_critical_policy_exception";
    std::error_code ignored;
    fs::remove_all(temp_root, ignored);
    fs::create_directories(temp_root);
    ...
    copperfin::runtime::PrgRuntimeSession session =
        copperfin::runtime::PrgRuntimeSession::create(make_runtime_session_options(main_path.string(), temp_root.string(), false));
    const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
    expect(state.completed, "ENTER CRITICAL/YIELD policy exception test should complete");
    ...
    if (entered != state.globals.end()) {
        expect(entered->second.boolean_value, "critical body should run to completion");
    }
    if (yielded != state.globals.end()) {
        expect(yielded->second.boolean_value, "YIELD should not be blocked inside CRITICAL");
    }

    expect(std::none_of(state.events.begin(), state.events.end(), [](const auto& event) {
        return event.category == "runtime.critical.blocking_violation" &&
               event.detail.find("operation=YIELD") != std::string::npos;
    }), "YIELD should remain permitted inside critical sections by policy exception");
    ...
    expect(yield_event_it != state.events.end(), "YIELD in policy-exception path should emit runtime.yield");

    fs::remove_all(temp_root, ignored);
}

void test_yield_enter_critical_regression_is_policy_exception() {
    namespace fs = std::filesystem;
    const fs::path temp_root = fs::temp_directory_path() / "copperfin_prg_yield_enter_critical_policy_exception_min";
    std::error_code ignored;
    fs::remove_all(temp_root, ignored);
    fs::create_directories(temp_root);

    const fs::path main_path = temp_root / "yield_enter_critical_regression_min_test.prg";
    write_text(
        main_path,
        "lEntered = .F.\n"
        "lYielded = .F.\n"
    ...
    copperfin::runtime::PrgRuntimeSession session =
        copperfin::runtime::PrgRuntimeSession::create(make_runtime_session_options(main_path.string(), temp_root.string(), false));
    const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
    expect(state.completed, "YIELD in ENTER CRITICAL should be allowed");

    const auto entered = state.globals.find("lentered");
    const auto yielded = state.globals.find("lyielded");
    expect(entered != state.globals.end(), "ENTER CRITICAL should execute body");
    expect(yielded != state.globals.end(), "YIELD should execute inside ENTER CRITICAL");
    if (entered != state.globals.end()) {
        expect(entered->second.boolean_value, "body should set entry flag");
    }
    if (yielded != state.globals.end()) {
        expect(yielded->second.boolean_value, "YIELD should continue inside CRITICAL");
    }

    expect(std::any_of(state.events.begin(), state.events.end(), [](const auto& event) {
        return event.category == "runtime.yield" && event.detail.find("operation=YIELD") != std::string::npos;
    }), "policy exception should emit operation-tagged runtime.yield");

    expect(std::none_of(state.events.begin(), state.events.end(), [](const auto& event) {
        return event.category == "runtime.critical.blocking_violation" &&
               event.detail.find("operation=YIELD") != std::string::npos;
    }), "YIELD should be exempt from critical-section blocking check");

    fs::remove_all(temp_root, ignored);
}

void test_yield_allowed_in_enter_critical_is_minimal_regression() {
    namespace fs = std::filesystem;
    ...
    copperfin::runtime::PrgRuntimeSession session =
        copperfin::runtime::PrgRuntimeSession::create(make_runtime_session_options(main_path.string(), temp_root.string(), false));
    const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
    expect(state.completed, "ENTER CRITICAL should allow YIELD without blocking violation");

    const auto entered = state.globals.find("lentered");
    const auto yielded = state.globals.find("lyielded");
    expect(entered != state.globals.end(), "critical section should execute");
    expect(yielded != state.globals.end(), "YIELD should execute within the critical section");
    if (entered != state.globals.end()) {
        expect(entered->second.boolean_value, "critical body should set entry flag");
    }
    if (yielded != state.globals.end()) {
        expect(yielded->second.boolean_value, "YIELD should complete");
    }

    expect(std::none_of(state.events.begin(), state.events.end(), [](const auto& event) {
        return event.category == "runtime.critical.blocking_violation" &&
               event.detail.find("operation=YIELD") != std::string::npos;
    }), "YIELD should remain a critical-section policy exception");
    expect(std::any_of(state.events.begin(), state.events.end(), [](const auto& event) {
        return event.category == "runtime.yield" && event.detail.find("operation=YIELD") != std::string::npos;
    }), "YIELD in critical section should emit runtime.yield");
...
int main() {
    ...
    test_yield_enter_critical_regression_is_policy_exception();
    test_yield_allowed_in_enter_critical_is_minimal_regression();
