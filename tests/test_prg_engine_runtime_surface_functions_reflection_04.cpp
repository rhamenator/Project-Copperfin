...
namespace copperfin::runtime_surface_tests
{
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_inherited_external_parent_object_block_derived_startup_child_dotted";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "inherited_external_parent_object_block_derived_startup_child_dotted.prg";
        write_text(
            main_path,
            "oCreate = CREATEOBJECT('ChildForm')\n"
            "oLeaf = NEWOBJECT('LeafForm')\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("inherited external-parent object-block derived-startup child dotted script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        expect(create_child_class_library != state.globals.end() &&
               ...
               "inherited external-parent object-block derived-startup child dotted access should leave the CREATEOBJECT child ClassLibrary empty");
        ...
        expect(leaf_child_class_library != state.globals.end() &&
               ...
               "inherited external-parent object-block derived-startup child dotted access should leave the leaf child ClassLibrary empty");

        expect(state.ole_objects.size() == 5U,
               "inherited external-parent object-block derived-startup child dotted script should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            const auto &create_parent = state.ole_objects[0];
            const auto &create_child = state.ole_objects[1];
            ...
            const auto &leaf_child = state.ole_objects[3];
            expect(create_parent.prog_id == "ChildForm",
                   "inherited external-parent object-block derived-startup child dotted access should preserve CREATEOBJECT parent identity");
            expect(create_parent.source == main_path.string(),
                   "inherited external-parent object-block derived-startup child dotted access should preserve CREATEOBJECT parent source");
            expect(create_child.prog_id == "SaveButton",
                   "inherited external-parent object-block derived-startup child dotted access should preserve CREATEOBJECT child identity");
            expect(create_child.base_class_name == "Custom",
                   "inherited external-parent object-block derived-startup child dotted access should preserve the CREATEOBJECT child builtin base");
            expect(create_child.source == main_path.string(),
                   "inherited external-parent object-block derived-startup child dotted access should preserve CREATEOBJECT child derived-startup source identity");
            expect(create_child.class_library.empty(),
                   "inherited external-parent object-block derived-startup child dotted access should preserve empty CREATEOBJECT child ClassLibrary semantics");
            expect(leaf_parent.prog_id == "LeafForm",
                   "inherited external-parent object-block derived-startup child dotted access should preserve NEWOBJECT leaf identity");
            expect(leaf_parent.source == main_path.string(),
                   "inherited external-parent object-block derived-startup child dotted access should preserve NEWOBJECT leaf source");
            expect(leaf_child.prog_id == "SaveButton",
                   "inherited external-parent object-block derived-startup child dotted access should preserve leaf child identity");
            expect(leaf_child.base_class_name == "Custom",
                   "inherited external-parent object-block derived-startup child dotted access should preserve the leaf child builtin base");
            expect(leaf_child.source == main_path.string(),
                   "inherited external-parent object-block derived-startup child dotted access should preserve leaf child derived-startup source identity");
            expect(leaf_child.class_library.empty(),
                   "inherited external-parent object-block derived-startup child dotted access should preserve empty leaf child ClassLibrary semantics");
            ...
            if (create_parent_caption != create_parent.properties.end())
            {
                expect(copperfin::runtime::format_value(create_parent_caption->second) == "Done-Saved",
                       "inherited external-parent object-block derived-startup child dotted access should let dotted child method calls update the CREATEOBJECT owner");
            }
            else
            {
                expect(false, "inherited external-parent object-block derived-startup child dotted access should preserve the CREATEOBJECT owner caption");
            }
            if (create_child_caption != create_child.properties.end())
            {
                expect(copperfin::runtime::format_value(create_child_caption->second) == "Go",
                       "inherited external-parent object-block derived-startup child dotted access should preserve the CREATEOBJECT child dotted assignment");
            }
            else
            {
                expect(false, "inherited external-parent object-block derived-startup child dotted access should preserve the CREATEOBJECT child caption");
            }
            if (leaf_parent_caption != leaf_parent.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_parent_caption->second) == "Ready-Saved",
                       "inherited external-parent object-block derived-startup child dotted access should let dotted child method calls update the leaf owner");
            }
            else
            {
                expect(false, "inherited external-parent object-block derived-startup child dotted access should preserve the leaf owner caption");
            }
            if (leaf_child_caption != leaf_child.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_child_caption->second) == "Ship",
                       "inherited external-parent object-block derived-startup child dotted access should preserve the leaf child dotted assignment");
            }
            else
            {
                expect(false, "inherited external-parent object-block derived-startup child dotted access should preserve the leaf child caption");
            }
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM behavior should remain stable while inherited external-parent object-block derived-startup child dotted access lands");
        }
        ...
        expect(has_addobject_event,
               "inherited external-parent object-block derived-startup child dotted access should emit child materialization events");
        ...
        expect(has_owner_caption_invoke_event,
               "inherited external-parent object-block derived-startup child dotted access should dispatch direct dotted child method calls");
        ...
        expect(has_save_invoke_event,
               "inherited external-parent object-block derived-startup child dotted access should keep child THISFORM owner dispatch stable");
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_inherited_external_parent_object_block_derived_startup_external_base";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "inherited_external_parent_object_block_derived_startup_external_base.prg";
        write_text(
            main_path,
            "oCreate = CREATEOBJECT('ChildForm')\n"
            "oLeaf = NEWOBJECT('LeafForm')\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("inherited external-parent object-block derived-startup external-base script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        expect(leaf_members_union != state.globals.end() &&
               ...
               "inherited external-parent object-block derived-startup external-base should keep leaf union member enumeration including inherited child methods");

        expect(state.ole_objects.size() == 5U,
               "inherited external-parent object-block derived-startup external-base script should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            const auto &create_parent = state.ole_objects[0];
            const auto &create_child = state.ole_objects[1];
            ...
            const auto &leaf_child = state.ole_objects[3];
            expect(create_parent.prog_id == "ChildForm",
                   "inherited external-parent object-block derived-startup external-base should preserve CREATEOBJECT parent identity");
            expect(create_parent.source == main_path.string(),
                   "inherited external-parent object-block derived-startup external-base should preserve CREATEOBJECT parent source");
            expect(create_child.prog_id == "SaveButton",
                   "inherited external-parent object-block derived-startup external-base should materialize the CREATEOBJECT child");
            expect(create_child.base_class_name == "ParentButton",
                   "inherited external-parent object-block derived-startup external-base should preserve the CREATEOBJECT child immediate external base");
            expect(create_child.source == main_path.string(),
                   "inherited external-parent object-block derived-startup external-base should preserve derived-startup child source identity");
            expect(create_child.class_library == button_library_path.string(),
                   "inherited external-parent object-block derived-startup external-base should preserve the CREATEOBJECT child external ClassLibrary path");
            expect(create_child.class_hierarchy.size() == 5U,
                   "inherited external-parent object-block derived-startup external-base should preserve the CREATEOBJECT child class hierarchy");
            if (create_child.class_hierarchy.size() == 5U)
            {
                expect(create_child.class_hierarchy[0] == "SAVEBUTTON",
                       "inherited external-parent object-block derived-startup external-base should store the CREATEOBJECT child class first");
                expect(create_child.class_hierarchy[1] == "PARENTBUTTON",
                       "inherited external-parent object-block derived-startup external-base should store the immediate external base second");
                expect(create_child.class_hierarchy[2] == "ROOTBUTTON",
                       "inherited external-parent object-block derived-startup external-base should store the deeper external base third");
                expect(create_child.class_hierarchy[3] == "CUSTOM",
                       "inherited external-parent object-block derived-startup external-base should store the builtin base token fourth");
                expect(create_child.class_hierarchy[4] == "OBJECT",
                       "inherited external-parent object-block derived-startup external-base should store the terminal object token fifth");
            }
            expect(!create_child.properties.contains("classlibrary"),
                   "inherited external-parent object-block derived-startup external-base should not materialize a CREATEOBJECT child ClassLibrary shadow");
            expect(leaf_parent.prog_id == "LeafForm",
                   "inherited external-parent object-block derived-startup external-base should preserve NEWOBJECT leaf identity");
            expect(leaf_parent.source == main_path.string(),
                   "inherited external-parent object-block derived-startup external-base should preserve NEWOBJECT leaf source");
            expect(leaf_child.prog_id == "SaveButton",
                   "inherited external-parent object-block derived-startup external-base should materialize the leaf child");
            expect(leaf_child.base_class_name == "ParentButton",
                   "inherited external-parent object-block derived-startup external-base should preserve the leaf child immediate external base");
            expect(leaf_child.source == main_path.string(),
                   "inherited external-parent object-block derived-startup external-base should preserve leaf child source identity");
            expect(leaf_child.class_library == button_library_path.string(),
                   "inherited external-parent object-block derived-startup external-base should preserve the leaf child external ClassLibrary path");
            expect(leaf_child.class_hierarchy.size() == 5U,
                   "inherited external-parent object-block derived-startup external-base should preserve the leaf child class hierarchy");
            if (leaf_child.class_hierarchy.size() == 5U)
            {
                expect(leaf_child.class_hierarchy[0] == "SAVEBUTTON",
                       "inherited external-parent object-block derived-startup external-base should store the leaf child class first");
                expect(leaf_child.class_hierarchy[1] == "PARENTBUTTON",
                       "inherited external-parent object-block derived-startup external-base should store the leaf immediate external base second");
                expect(leaf_child.class_hierarchy[2] == "ROOTBUTTON",
                       "inherited external-parent object-block derived-startup external-base should store the leaf deeper external base third");
                expect(leaf_child.class_hierarchy[3] == "CUSTOM",
                       "inherited external-parent object-block derived-startup external-base should store the leaf builtin base token fourth");
                expect(leaf_child.class_hierarchy[4] == "OBJECT",
                       "inherited external-parent object-block derived-startup external-base should store the leaf terminal object token fifth");
            }
            expect(!leaf_child.properties.contains("classlibrary"),
                   "inherited external-parent object-block derived-startup external-base should not materialize a leaf child ClassLibrary shadow");
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM behavior should remain stable while inherited external-parent object-block derived-startup external-base lands");
        }
        ...
        expect(has_addobject_event,
               "inherited external-parent object-block derived-startup external-base should emit child materialization events");
        ...
        expect(has_save_invoke_event,
               "inherited external-parent object-block derived-startup external-base should keep materialized children usable through THISFORM owner dispatch");
