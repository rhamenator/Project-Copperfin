...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2769: deleted-states success layout fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2769: deleted-states success layout fixture should seed deleted rows");
}
...
void write_synthetic_report_table_for_stable_settings_and_section_json(
    ...
    expect(settings_unique_id_result.ok,
           "#2769: settings+section success fixture should seed a settings unique id");
    ...
    expect(section_unique_id_result.ok,
           "#2769: settings+section success fixture should seed a section unique id");
    expect(!dbf_record_deleted(report_path, 0U) && !dbf_record_deleted(report_path, 1U),
           "#2769: settings+section success fixture should preserve live settings and section rows");
}
...
void write_synthetic_report_table_for_stable_deleted_settings_and_section_json(
    ...
    expect(settings_delete_result.ok,
           "#2769: settings+section restore fixture should mark settings deleted");
    ...
    expect(section_delete_result.ok,
           "#2769: settings+section restore fixture should mark section deleted");
    expect(dbf_record_deleted(report_path, 0U) && dbf_record_deleted(report_path, 1U),
           "#2769: settings+section restore fixture should preserve deleted settings and section rows");
}

void run_deleted_states_delete_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("settings_section_deleted_states_delete" + extension);
    ...
    if (delete_process.exit_code != 0) {
        std::cerr << "studio host " << label << " settings+section deleted-states delete " << extension
    ...
    expect(dbf_record_deleted(asset_path, 0U) && dbf_record_deleted(asset_path, 1U),
           issue_prefix + " delete should mark settings and section rows deleted");
    ...
    expect_contains(delete_process.stdout_text, "\"previewBoundsAvailable\": true",
                    issue_prefix + " delete should expose live preview availability");
    ...
    expect_contains(delete_process.stdout_text, "\"deletedPreviewBoundsRight\": 2700",
                    issue_prefix + " delete should preserve deleted preview right bounds");
    expect_contains(delete_process.stdout_text, "\"deletedPreviewBoundsBottom\": 2900",
                    issue_prefix + " delete should preserve deleted preview bottom bounds");
    ...
    expect_contains(delete_process.stdout_text, "\"pageSetupAvailable\": false",
                    issue_prefix + " delete should clear live page setup");
    expect_contains(delete_process.stdout_text, "\"sectionCount\": 1",
                    issue_prefix + " delete should remove the section from live counts");
...
void run_deleted_states_restore_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& title,
    const std::string& label,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("settings_section_deleted_states_restore" + extension);
    ...
    if (restore_process.exit_code != 0) {
        std::cerr << "studio host " << label << " settings+section deleted-states restore " << extension
    ...
    expect(!dbf_record_deleted(asset_path, 0U) && !dbf_record_deleted(asset_path, 1U),
           issue_prefix + " restore should restore settings and section rows");
    ...
    expect_contains(restore_process.stdout_text, "\"pageSetupAvailable\": true",
                    issue_prefix + " restore should restore live page setup");
    expect_contains(restore_process.stdout_text, "\"sectionCount\": 2",
                    issue_prefix + " restore should restore live section counts");
    expect_contains(restore_process.stdout_text, "\"deletedSectionCount\": 0",
                    issue_prefix + " restore should clear deleted section counts");
    expect_contains_in_order(
        restore_process.stdout_text,
        {
            "\"settings\": [",
...
void test_studio_host_json_preserves_settings_section_deleted_states(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_settings_section_deleted_states_tests";
    ...
    run_deleted_states_delete_case(
        studio_host_path,
        temp_root,
        ".frx",
        "settings_section_deleted_states_delete.frx",
        "report",
        "#2769: report/label settings+section deleted-states success");
    run_deleted_states_delete_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "settings_section_deleted_states_delete.lbx",
        "label",
        "#2769: report/label settings+section deleted-states success");
    run_deleted_states_restore_case(
        studio_host_path,
        temp_root,
        ".frx",
        "settings_section_deleted_states_restore.frx",
        "report",
        "#2769: report/label settings+section deleted-states success");
    run_deleted_states_restore_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "settings_section_deleted_states_restore.lbx",
        "label",
        "#2769: report/label settings+section deleted-states success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_settings_section_deleted_states <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_settings_section_deleted_states(argv[1]);
