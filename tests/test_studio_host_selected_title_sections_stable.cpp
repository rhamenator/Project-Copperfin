...
void write_synthetic_report_table_for_stable_title_section_json(
    ...
    expect(create_result.ok, "#2807: stable title section fixture should be created");
}

void run_title_section_stable_selection(
...
void run_deleted_title_section_stable_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    expect(delete_result.ok && dbf_record_deleted(asset_path, 1U),
           issue_prefix + " fixture should mark the title section deleted");
...
void test_studio_host_json_preserves_selected_title_sections_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_title_sections_stable_tests";
    ...
    run_title_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_title_sections.frx",
        "report",
        "#2807: stable-selected live report title section selection");
    run_title_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_title_sections.lbx",
        "label",
        "#2807: stable-selected live label title section selection");
    run_deleted_title_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_deleted_title_sections.frx",
        "report",
        "#2807: stable-selected deleted report title section selection");
    run_deleted_title_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_deleted_title_sections.lbx",
        "label",
        "#2807: stable-selected deleted label title section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_title_sections_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_title_sections_stable_selection(argv[1]);
