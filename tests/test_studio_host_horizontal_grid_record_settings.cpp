...
void write_synthetic_report_table_for_grid_horizontal_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2781: horizontal-grid settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_grid_horizontal_field_json(
    ...
    expect(delete_result.ok, "#2781: deleted horizontal-grid settings fixture should mark settings deleted");
}

void run_grid_horizontal_update_case(
    ...
    if (deleted) {
        write_synthetic_report_table_for_deleted_grid_horizontal_field_json(asset_path);
    } else {
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " horizontal grid field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " horizontal grid field update " << extension << " stderr:\n"
    ...
    expect(grid_property.ok && grid_property.exists && grid_property.value == updated_grid,
           issue_prefix + " update should persist the GRIDH field");
    ...
    if (!deleted) {
        ...
        expect_contains(update_process.stdout_text, "\"gridHorizontalAvailable\": true",
                        issue_prefix + " update should expose effective horizontal-grid availability");
        expect_contains(update_process.stdout_text, "\"gridHorizontal\": " + updated_grid,
                        issue_prefix + " update should expose the effective horizontal-grid value");
...
void run_grid_horizontal_clear_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& asset_stem,
    const std::string& extension,
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " horizontal grid field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " horizontal grid field clear " << extension << " stderr:\n"
    ...
    expect(grid_property.ok && grid_property.exists && grid_property.value.empty(),
           issue_prefix + " clear should blank the GRIDH field");
    ...
    if (!deleted) {
        ...
        expect_contains(clear_process.stdout_text, "\"gridHorizontal\": 0",
                        issue_prefix + " clear should clear horizontal grid spacing");
        ...
        expect_contains(clear_process.stdout_text, "\"gridHorizontalAvailable\": false",
                        issue_prefix + " clear should clear effective horizontal-grid availability");
        expect_contains(clear_process.stdout_text, "\"gridHorizontal\": 0",
                        issue_prefix + " clear should reset the effective horizontal-grid value");
        ...
        expect_not_contains(clear_process.stdout_text,
                            ...
                            issue_prefix + " clear should remove direct horizontal-grid settings");
    }
}

void test_studio_host_json_preserves_horizontal_grid_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_horizontal_grid_record_settings_tests";
    ...
    run_grid_horizontal_update_case(
        studio_host_path,
        temp_root,
        "grid_horizontal_update",
        ".frx",
        "16",
        false,
        "report",
        "#2781: report/label record-selected horizontal-grid settings success");
    run_grid_horizontal_update_case(
        studio_host_path,
        temp_root,
        "grid_horizontal_update",
        ...
        "label",
        "#2781: report/label record-selected horizontal-grid settings success");
    run_grid_horizontal_clear_case(
        studio_host_path,
        temp_root,
        "grid_horizontal_clear",
        ".frx",
        false,
        "report",
        "#2781: report/label record-selected horizontal-grid settings success");
    ...
    run_grid_horizontal_update_case(
        studio_host_path,
        temp_root,
        "deleted_grid_horizontal_update",
        ".frx",
        "16",
        true,
        "report",
        "#2781: report/label record-selected deleted horizontal-grid settings success");
    run_grid_horizontal_update_case(
        studio_host_path,
        temp_root,
        "deleted_grid_horizontal_update",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_horizontal_grid_record_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_horizontal_grid_record_selection(argv[1]);
