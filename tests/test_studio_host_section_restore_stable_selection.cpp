...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2794: stable section restore layout fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_section_json(const std::filesystem::path& report_path) {
    ...
    expect(delete_result.ok, "#2794: stable section restore fixture should mark section deleted");
}
...
void write_synthetic_report_table_for_stable_deleted_section_json(const std::filesystem::path& report_path) {
    ...
    expect(unique_id_result.ok, "#2794: stable deleted section fixture should seed a deleted section unique id");
    expect(dbf_record_deleted(report_path, 1U),
           "#2794: stable deleted section fixture should preserve the deleted section state");
}
...
void run_section_restore_case(
    ...
    expect_contains(restore_process.stdout_text, "\"containingSectionId\": \"deleted-section-guid\"",
                    issue_prefix + " should preserve stable containing-section ids after restore");
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should rehydrate selected-section metadata");
}

void test_studio_host_json_preserves_section_restore_stable_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_section_restore_stable_selection_tests";
    ...
    run_section_restore_case(
        ...
        "report",
        "#2794: stable-selected report section restore");
    run_section_restore_case(
        studio_host_path,
        temp_root,
        "section_restore_stable.lbx",
        "label",
        "#2794: stable-selected label section restore");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_section_restore_stable_selection <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_section_restore_stable_selection(argv[1]);
