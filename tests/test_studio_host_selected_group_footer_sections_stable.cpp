...
void write_synthetic_report_table_for_stable_group_section_expression_json(
    ...
    expect(create_result.ok,
           "#3101: stable-selected group-footer section fixture should be created");
}

void run_group_footer_section_stable_selection(
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host stable-selected " << label << " group-footer section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host stable-selected " << label << " group-footer section stderr:\n"
                  << process.stderr_text << "\n";
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose selected expression metadata");
}

void run_deleted_group_footer_section_stable_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    expect(delete_result.ok && dbf_record_deleted(asset_path, 3U),
           issue_prefix + " fixture should mark the group-footer section deleted");
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host stable-selected deleted " << label << " group-footer section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host stable-selected deleted " << label << " group-footer section stderr:\n"
                  << process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
...
void test_studio_host_json_preserves_selected_group_footer_sections_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_group_footer_sections_stable_tests";
    ...
    run_group_footer_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_group_footer_section_stable.frx",
        "report",
        "#3101: stable-selected live report group-footer section selection");
    run_group_footer_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_group_footer_section_stable.lbx",
        "label",
        "#3101: stable-selected live label group-footer section selection");
    run_deleted_group_footer_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_group_footer_section_stable.frx",
        "report",
        "#3101: stable-selected deleted report group-footer section selection");
    run_deleted_group_footer_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_group_footer_section_stable.lbx",
        "label",
        "#3101: stable-selected deleted label group-footer section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_group_footer_sections_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_group_footer_sections_stable_selection(argv[1]);
