...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2761: deleted update-batch rollback layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2761: deleted update-batch rollback fixture should mark target rows deleted");
}
...
void run_deleted_update_batch_rollback_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("deleted_report_object_update_batch_rollback" + extension);
    ...
    if (rollback_process.exit_code != 4) {
        std::cerr << "studio host deleted update-batch rollback " << extension << " stdout:\n"
                  << rollback_process.stdout_text << "\n";
        std::cerr << "studio host deleted update-batch rollback " << extension << " stderr:\n"
    ...
    expect(visual_object_property(asset_path, "middle-field-guid", "EXPR") == "middle.value" &&
           ...
           issue_prefix + " should roll back earlier property mutations");
}

void test_studio_host_json_preserves_deleted_update_batch_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_deleted_update_batch_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_deleted_update_batch_rollback_case(
            studio_host_path,
            temp_root,
            extension,
            "#2761: deleted report/label update-batch rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_update_batch_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_update_batch_errors(argv[1]);
