...
void write_synthetic_report_table_for_layout_subtree_deleted_state_json(
    ...
    expect(create_result.ok, "#2756: subtree duplicate layout fixture should be created");
}
...
void run_success_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_subtree_duplicate" + extension);
    ...
    if (duplicate_process.exit_code != 0) {
        std::cerr << "studio host " << label << " subtree duplicate stdout:\n"
                  << duplicate_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " subtree duplicate stderr:\n"
    ...
    expect(visual_object_count(asset_path) == before_count + 1U &&
               ...
               !visual_object_deleted(asset_path, "middle-copy-guid") &&
               visual_object_order(asset_path) ==
                   "left-field-guid,middle-field-guid,right-field-guid,middle-copy-guid",
           issue_prefix + " should append one copied flat layout row");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host " << label << " subtree duplicate reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " subtree duplicate reopen stderr:\n"
    ...
    expect_contains(reopen_process.stdout_text, "\"deletedPreviewBoundsAvailable\": false",
                    issue_prefix + " should keep deleted preview unavailable");
    ...
    expect_contains(reopen_process.stdout_text, "\"liveObjectCount\": 4",
                    issue_prefix + " should refresh live object counts");
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectSectionAvailable\": true",
                    issue_prefix + " should expose copied-object section metadata");
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should refresh selected copied-object metadata");
}

void test_studio_host_json_preserves_subtree_duplicate(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_subtree_duplicate_tests";
    ...
    run_success_case(
        ...
        "report",
        "#2756: report/label stable visual-object duplicate-subtree");
    run_success_case(
        studio_host_path,
        temp_root,
        ".lbx",
...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "Usage: test_studio_host_subtree_duplicate <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_subtree_duplicate(argv[1]);
    if (failures != 0) {
