...
void write_synthetic_report_table_for_stable_group_section_expression_json(
    ...
    expect(create_result.ok,
           "#3101: stable-selected group-header section fixture should be created");
}

void run_group_header_section_stable_selection(
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host stable-selected " << label << " group-header section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host stable-selected " << label << " group-header section stderr:\n"
                  << process.stderr_text << "\n";
...
void run_deleted_group_header_section_stable_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host stable-selected deleted " << label << " group-header section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host stable-selected deleted " << label << " group-header section stderr:\n"
                  << process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
...
void test_studio_host_json_preserves_selected_group_header_sections_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_group_header_sections_stable_tests";
    ...
    run_group_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_group_header_section_stable.frx",
        "report",
        "#3101: stable-selected live report group-header section selection");
    run_group_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_group_header_section_stable.lbx",
        "label",
        "#3101: stable-selected live label group-header section selection");
    run_deleted_group_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_group_header_section_stable.frx",
        "report",
        "#3101: stable-selected deleted report group-header section selection");
    run_deleted_group_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_group_header_section_stable.lbx",
        "label",
        "#3101: stable-selected deleted label group-header section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_group_header_sections_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_group_header_sections_stable_selection(argv[1]);
