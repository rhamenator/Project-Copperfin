...
void write_sort_settings_fixture(
    ...
    expect(create_result.ok, "#2910: TAG sort-settings fixture should be created");
}
...
void test_updates_report_sort_settings_by_record_selection(const std::string& studio_host_path) {
    ...
    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_report_sort_settings_update_record_json_tests";
    ...
    const auto run_update = [&](const fs::path& asset_path,
        ...
        if (update_process.exit_code != 0) {
            std::cerr << "studio host " << label << " record TAG update stdout:\n"
                      << update_process.stdout_text << "\n";
            std::cerr << "studio host " << label << " record TAG update stderr:\n"
                      << update_process.stderr_text << "\n";
            std::cerr << "fixture root: " << temp_root << "\n";
        }

        expect(update_process.exit_code == 0,
               "#2910: report/label record TAG update should exit successfully");
        const auto reopen_process = run_process_capture(
            studio_host_path,
            {"--path", asset_path.string(), "--record", "0", "--json"},
            temp_root);
        expect(reopen_process.exit_code == 0,
               "#2910: report/label record TAG update reopen should exit successfully");
        expect_live_sort_setting_json(
            reopen_process.stdout_text,
            title,
            "customer.region",
            "#2910: report/label record TAG update reopen JSON",
            label_asset);
    };

    run_update(temp_root / "sort_update_record.frx", "sort_update_record.frx", "report", false);
    run_update(temp_root / "sort_update_record.lbx", "sort_update_record.lbx", "label", true);
...
void test_clears_report_sort_settings_by_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_report_sort_settings_clear_record_json_tests";
    std::error_code ignored;
    fs::remove_all(temp_root, ignored);
    fs::create_directories(temp_root);
    ScopedDefaultLocaleCatalogEnvironment default_locale_environment;
    ...
    const auto run_clear = [&](const fs::path& asset_path,
        ...
        if (clear_process.exit_code != 0) {
            std::cerr << "studio host " << label << " record TAG clear stdout:\n"
                      << clear_process.stdout_text << "\n";
            std::cerr << "studio host " << label << " record TAG clear stderr:\n"
                      << clear_process.stderr_text << "\n";
            std::cerr << "fixture root: " << temp_root << "\n";
        }

        expect(clear_process.exit_code == 0,
               "#2910: report/label record TAG clear should exit successfully");
        const auto reopen_process = run_process_capture(
            studio_host_path,
            {"--path", asset_path.string(), "--record", "0", "--json"},
            temp_root);
        expect(reopen_process.exit_code == 0,
               "#2910: report/label record TAG clear reopen should exit successfully");
        expect_contains(reopen_process.stdout_text, "\"documentTitle\": \"" + title + "\"",
                        "#2910: report/label record TAG clear reopen JSON should preserve document titles");
        if (label_asset) {
            expect_contains(reopen_process.stdout_text, "\"isLabel\": true",
                            "#2910: label record TAG clear reopen JSON should retain label identity");
        }
        expect_contains(reopen_process.stdout_text, "\"selectedReportSettingsAvailable\": true",
                        "#2910: report/label record TAG clear reopen JSON should preserve selected-settings availability");
        expect_contains(reopen_process.stdout_text, "\"selectedReportSelectionKind\": \"settings\"",
                        "#2910: report/label record TAG clear reopen JSON should preserve settings selection kind");
        expect_sort_settings_page_summary(
            reopen_process.stdout_text,
            "#2910: report/label record TAG clear reopen JSON");
        expect_contains(reopen_process.stdout_text, "\"settingCount\": 2",
                        "#2910: report/label record TAG clear reopen JSON should remove TAG from live setting counts");
        expect_contains(reopen_process.stdout_text, "\"deletedSettingCount\": 0",
                        "#2910: report/label record TAG clear reopen JSON should keep deleted setting counts empty");
        expect_contains_in_order(
            reopen_process.stdout_text,
            {
                "\"settings\": [",
            ...
            "#2910: report/label record TAG clear reopen JSON should preserve remaining live settings");
        expect_contains_in_order(
            reopen_process.stdout_text,
            {
                "\"selectedReportSettings\": [",
            ...
            "#2910: report/label record TAG clear reopen JSON should preserve remaining selected settings");
        expect_not_contains(reopen_process.stdout_text,
                            "\"name\": \"TAG\", \"recordIndex\": 0, \"fieldIndex\": 3",
                            "#2910: report/label record TAG clear reopen JSON should remove TAG provenance");
    };

    run_clear(temp_root / "sort_clear_record.frx", "sort_clear_record.frx", "report", false);
    run_clear(temp_root / "sort_clear_record.lbx", "sort_clear_record.lbx", "label", true);
...
void test_updates_deleted_report_sort_settings_by_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_deleted_report_sort_settings_update_record_json_tests";
    std::error_code ignored;
    fs::remove_all(temp_root, ignored);
    fs::create_directories(temp_root);
    ScopedDefaultLocaleCatalogEnvironment default_locale_environment;
    ...
    const auto run_update = [&](const fs::path& asset_path,
        ...
        if (update_process.exit_code != 0) {
            std::cerr << "studio host " << label << " record deleted TAG update stdout:\n"
                      << update_process.stdout_text << "\n";
            std::cerr << "studio host " << label << " record deleted TAG update stderr:\n"
                      << update_process.stderr_text << "\n";
            std::cerr << "fixture root: " << temp_root << "\n";
        }

        expect(update_process.exit_code == 0,
               "#2910: report/label record deleted TAG update should exit successfully");
        const auto reopen_process = run_process_capture(
            studio_host_path,
            {"--path", asset_path.string(), "--record", "0", "--json"},
            temp_root);
        expect(reopen_process.exit_code == 0,
               "#2910: report/label record deleted TAG update reopen should exit successfully");
        expect_deleted_sort_setting_json(
            reopen_process.stdout_text,
            title,
            "customer.region",
            "#2910: report/label record deleted TAG update reopen JSON",
            label_asset);
    };

    run_update(temp_root / "deleted_sort_update_record.frx",
               "deleted_sort_update_record.frx",
               "report",
               false);
    run_update(temp_root / "deleted_sort_update_record.lbx",
               "deleted_sort_update_record.lbx",
               "label",
               true);
...
void test_clears_deleted_report_sort_settings_by_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_deleted_report_sort_settings_clear_record_json_tests";
    std::error_code ignored;
    fs::remove_all(temp_root, ignored);
    fs::create_directories(temp_root);
    ScopedDefaultLocaleCatalogEnvironment default_locale_environment;
    ...
    const auto run_clear = [&](const fs::path& asset_path,
        ...
        if (clear_process.exit_code != 0) {
            std::cerr << "studio host " << label << " record deleted TAG clear stdout:\n"
                      << clear_process.stdout_text << "\n";
            std::cerr << "studio host " << label << " record deleted TAG clear stderr:\n"
                      << clear_process.stderr_text << "\n";
            std::cerr << "fixture root: " << temp_root << "\n";
        }

        expect(clear_process.exit_code == 0,
               "#2910: report/label record deleted TAG clear should exit successfully");
        const auto reopen_process = run_process_capture(
            studio_host_path,
            {"--path", asset_path.string(), "--record", "0", "--json"},
            temp_root);
        expect(reopen_process.exit_code == 0,
               "#2910: report/label record deleted TAG clear reopen should exit successfully");
        expect_contains(reopen_process.stdout_text, "\"documentTitle\": \"" + title + "\"",
                        "#2910: report/label record deleted TAG clear reopen JSON should preserve document titles");
        if (label_asset) {
            expect_contains(reopen_process.stdout_text, "\"isLabel\": true",
                            "#2910: label record deleted TAG clear reopen JSON should retain label identity");
        }
        expect_contains(reopen_process.stdout_text, "\"selectedReportSettingsAvailable\": true",
                        "#2910: report/label record deleted TAG clear reopen JSON should preserve selected-settings availability");
        expect_contains(reopen_process.stdout_text, "\"selectedReportSelectionKind\": \"settings\"",
                        "#2910: report/label record deleted TAG clear reopen JSON should preserve settings selection kind");
        expect_sort_settings_page_summary(
            reopen_process.stdout_text,
            "#2910: report/label record deleted TAG clear reopen JSON");
        expect_contains(reopen_process.stdout_text, "\"settingCount\": 0",
                        "#2910: report/label record deleted TAG clear reopen JSON should keep live setting counts empty");
        expect_contains(reopen_process.stdout_text, "\"deletedSettingCount\": 2",
                        "#2910: report/label record deleted TAG clear reopen JSON should remove TAG from deleted setting counts");
        expect_contains_in_order(
            reopen_process.stdout_text,
            {
                "\"deletedSettings\": [",
            ...
            "#2910: report/label record deleted TAG clear reopen JSON should preserve remaining deleted settings");
        expect_contains_in_order(
            reopen_process.stdout_text,
            {
                "\"selectedReportSettings\": [",
            ...
            "#2910: report/label record deleted TAG clear reopen JSON should preserve remaining selected deleted settings");
        expect_not_contains(reopen_process.stdout_text,
                            "\"name\": \"TAG\", \"recordIndex\": 0, \"fieldIndex\": 3",
                            "#2910: report/label record deleted TAG clear reopen JSON should remove deleted TAG provenance");
    };

    run_clear(temp_root / "deleted_sort_clear_record.frx",
              "deleted_sort_clear_record.frx",
              "report",
              false);
    run_clear(temp_root / "deleted_sort_clear_record.lbx",
              "deleted_sort_clear_record.lbx",
              "label",
              true);
...
void test_clears_report_sort_settings_preserve_unsupported_expr_lines_by_record_selection(
    ...
    const auto run_clear = [&](const fs::path& asset_path,
        ...
        expect_contains(reopen_process.stdout_text, "\"selectedReportSettingsAvailable\": true",
                        "#3099: record TAG clear reopen JSON should preserve selected-settings availability");
        expect_contains(reopen_process.stdout_text, "\"selectedReportSelectionKind\": \"settings\"",
                        "#3099: record TAG clear reopen JSON should preserve settings selection kind");
        if (deleted) {
            expect_contains(reopen_process.stdout_text, "\"documentTitle\": \"" + title + "\"",
            ...
            expect_sort_settings_page_summary(
                reopen_process.stdout_text,
                "#3099: record deleted TAG clear reopen JSON");
            expect_contains(reopen_process.stdout_text, "\"settingCount\": 0",
            ...
            expect_sort_settings_page_summary(
                reopen_process.stdout_text,
                "#3099: record TAG clear reopen JSON");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_report_sort_settings_record <studio_host_path>\n";
        return 1;
    }

    test_updates_report_sort_settings_by_record_selection(argv[1]);
