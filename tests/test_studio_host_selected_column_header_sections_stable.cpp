...
void write_synthetic_report_table_for_stable_column_section_json(
    ...
    expect(create_result.ok, "#2809: stable column-header section fixture should be created");
}

void run_column_header_section_stable_selection(
...
void run_deleted_column_header_section_stable_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
...
void test_studio_host_json_preserves_selected_column_header_sections_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_column_header_sections_stable_tests";
    ...
    run_column_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_column_header_sections.frx",
        "report",
        "#2809: stable-selected live report column-header section selection");
    run_column_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_column_header_sections.lbx",
        "label",
        "#2809: stable-selected live label column-header section selection");
    run_deleted_column_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_deleted_column_header_sections.frx",
        "report",
        "#2809: stable-selected deleted report column-header section selection");
    run_deleted_column_header_section_stable_selection(
        studio_host_path,
        temp_root,
        "stable_deleted_column_header_sections.lbx",
        "label",
        "#2809: stable-selected deleted label column-header section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_column_header_sections_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_column_header_sections_stable_selection(argv[1]);
