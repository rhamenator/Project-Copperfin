...
void write_synthetic_report_table_for_column_setup_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2789: column-setup settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_column_setup_json(
    ...
    expect(delete_result.ok, "#2789: deleted column-setup settings fixture should mark settings deleted");
}
...
void run_column_setup_update_case(
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " column setup update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " column setup update " << extension << " stderr:\n"
    ...
    if (!deleted) {
        ...
        expect_contains_in_order(
            ...
            issue_prefix + " update should refresh setting provenance");
    } else {
        expect_empty_report_layout_preview_bounds(
            update_process.stdout_text,
        ...
        expect_contains_in_order(
            update_process.stdout_text,
            {
                "\"deletedSettings\": [",
                "\"name\": \"COLS\", \"recordIndex\": 0, \"fieldIndex\": 2, \"sourceLineIndex\": 0",
                "\"value\": \"" + updated_settings.substr(5, updated_settings.find('\n') - 5) + "\"",
                "\"name\": \"COLWIDTH\", \"recordIndex\": 0, \"fieldIndex\": 2, \"sourceLineIndex\": 1",
                "\"value\": \"" +
                    updated_settings.substr(
                        updated_settings.find("COLWIDTH=") + 9,
                    ...
                    "\"",
                "\"name\": \"COLSPACING\", \"recordIndex\": 0, \"fieldIndex\": 2, \"sourceLineIndex\": 2",
                "\"value\": \"" + updated_settings.substr(updated_settings.rfind('=') + 1) + "\""
            },
...
void run_column_setup_clear_case(
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " column setup clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " column setup clear " << extension << " stderr:\n"
    ...
    if (!deleted) {
        ...
        expect_contains(clear_process.stdout_text,
                        ...
                        issue_prefix + " clear should preserve and blank the EXPR field carrier");
    }
...
void test_studio_host_json_preserves_column_setup_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_column_setup_record_settings_tests";
    ...
    run_column_setup_update_case(
        ...
        false,
        "report",
        "#2789: report/label record-selected column-setup settings success");
    run_column_setup_update_case(
        studio_host_path,
        temp_root,
        "column_setup",
    ...
    run_column_setup_update_case(
        studio_host_path,
        temp_root,
        "deleted_column_setup",
        ".frx",
        "COLS=4\nCOLWIDTH=1800\nCOLSPACING=240",
        true,
        "report",
        "#2789: report/label record-selected deleted column-setup settings success");
    run_column_setup_update_case(
        studio_host_path,
        temp_root,
        "deleted_column_setup",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_column_setup_record_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_column_setup_record_selection(argv[1]);
