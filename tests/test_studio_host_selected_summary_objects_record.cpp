...
void write_synthetic_report_table_for_stable_summary_object_json(
    ...
    expect(create_result.ok, "#2804: stable summary object fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_summary_object_json(
    ...
    expect(delete_result.ok, "#2804: deleted summary object fixture should mark summary object deleted");
}

void run_summary_object_record_selection(
    ...
    if (object_process.exit_code != 0) {
        std::cerr << "studio host " << label << " record-selected summary object stdout:\n"
                  << object_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " record-selected summary object stderr:\n"
...
void run_deleted_summary_object_record_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    if (object_process.exit_code != 0) {
        std::cerr << "studio host " << label << " record-selected deleted summary object stdout:\n"
                  << object_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " record-selected deleted summary object stderr:\n"
                  << object_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
...
void test_studio_host_json_preserves_selected_summary_objects_record_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_summary_objects_record_tests";
    ...
    run_summary_object_record_selection(
        studio_host_path,
        temp_root,
        "selected_summary_object_record.frx",
        "report",
        "#2804: record-selected live report summary object selection");
    run_summary_object_record_selection(
        studio_host_path,
        temp_root,
        "selected_summary_object_record.lbx",
        "label",
        "#2804: record-selected live label summary object selection");
    run_deleted_summary_object_record_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_summary_object_record.frx",
        "report",
        "#2804: record-selected deleted report summary object selection");
    run_deleted_summary_object_record_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_summary_object_record.lbx",
        "label",
        "#2804: record-selected deleted label summary object selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_summary_objects_record <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_summary_objects_record_selection(argv[1]);
