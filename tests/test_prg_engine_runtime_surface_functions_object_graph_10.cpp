...
namespace copperfin::runtime_surface_tests
{
    void test_inherited_release_detaches_object_block_external_base_child_subtree_while_owner_and_sibling_remain_alive()
    {
        ...
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_inherited_release_object_block_external_base_child_subtree";
        ...
        const fs::path main_path = temp_root / "inherited_release_object_block_external_base_child_subtree.prg";
        write_text(
            main_path,
            "cDestroyLog = ''\n"
            ...
            "lDictSet = SETPEM(oDict, 'comparemode', 120)\n"
        ...
        expect(state.completed,
               std::string("inherited Release object-block external-base child subtree script should complete: ") + state.message +
        ...
        expect(released_child != state.globals.end() &&
               ...
               "inherited Release object-block external-base child subtree should invalidate GETPEM() on the owner's released child slot");
        ...
        expect(held_child_parent != state.globals.end() &&
               ...
               "inherited Release object-block external-base child subtree should invalidate GETPEM() on the held child's Parent");
        ...
        expect(held_child_class_library != state.globals.end() &&
               ...
               "inherited Release object-block external-base child subtree should invalidate GETPEM() on the held child's ClassLibrary");
        ...
        expect(held_grandchild_parent != state.globals.end() &&
               ...
               "inherited Release object-block external-base child subtree should invalidate GETPEM() on the released grandchild's Parent");

        expect(state.ole_objects.size() == 3U,
               "inherited Release object-block external-base child subtree should leave owner, surviving sibling, and COM objects registered");
        if (state.ole_objects.size() == 3U)
        {
            expect(state.ole_objects[0].prog_id == "ChildForm",
                   "inherited Release object-block external-base child subtree should keep the owner object alive");
            expect(!state.ole_objects[0].properties.contains("cmdsave"),
                   "inherited Release object-block external-base child subtree should detach the released child from the owner");
            expect(state.ole_objects[0].properties.contains("cmdcancel"),
                   "inherited Release object-block external-base child subtree should preserve the surviving sibling on the owner");
            expect(state.ole_objects[1].prog_id == "CancelButton",
                   "inherited Release object-block external-base child subtree should preserve the surviving sibling object");
            expect(state.ole_objects[2].prog_id == "Scripting.Dictionary",
                   "COM NEWOBJECT should remain stable while inherited object-block external-base subtree Release lands");
        }
        ...
        expect(has_child_destroy_event,
               "inherited Release object-block external-base child subtree should dispatch the child Destroy method");
        ...
        expect(has_grandchild_destroy_event,
               "inherited Release object-block external-base child subtree should dispatch descendant Destroy methods");
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_native_prg_release_owner_object_block_external_base_child_subtree";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "native_release_owner_object_block_external_base_child_subtree.prg";
        write_text(
            main_path,
            "cDestroyLog = ''\n"
            "cChildParentCaptionDuringDestroy = ''\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("native owner Release object-block external-base child subtree script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        expect(child_after_release != state.globals.end() &&
                   child_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release object-block external-base child subtree should invalidate GETPEM() for the released child slot");

        const auto child_parent_after_release = state.globals.find("xchildparentafterrelease");
        expect(child_parent_after_release != state.globals.end() &&
                   child_parent_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release object-block external-base child subtree should invalidate GETPEM() for the released child's Parent");

        const auto child_class_library_after_release = state.globals.find("xchildclasslibraryafterrelease");
        expect(child_class_library_after_release != state.globals.end() &&
                   child_class_library_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release object-block external-base child subtree should invalidate GETPEM() for the released child's ClassLibrary");

        const auto grandchild_parent_after_release = state.globals.find("xgrandchildparentafterrelease");
        expect(grandchild_parent_after_release != state.globals.end() &&
                   grandchild_parent_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release object-block external-base child subtree should invalidate GETPEM() for the released grandchild's Parent");

        const auto form_caption_after_release = state.globals.find("xformcaptionafterrelease");
        expect(form_caption_after_release != state.globals.end() &&
                   form_caption_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release object-block external-base child subtree should invalidate GETPEM() for the released owner");

        expect(state.ole_objects.size() == 1U,
               "native owner Release object-block external-base child subtree should leave only the COM object registered after recursive release");
        if (state.ole_objects.size() == 1U)
        {
            expect(state.ole_objects[0].prog_id == "Scripting.Dictionary",
                   "COM NEWOBJECT should remain stable while native owner object-block external-base Release lands");
        }
        ...
        expect(has_grandchild_destroy_event,
               "native owner Release object-block external-base child subtree should dispatch the grandchild Destroy method");
        ...
        expect(has_child_destroy_event,
               "native owner Release object-block external-base child subtree should dispatch the child Destroy method");
        ...
        expect(has_form_destroy_event,
               "native owner Release object-block external-base child subtree should dispatch the owner Destroy method");
