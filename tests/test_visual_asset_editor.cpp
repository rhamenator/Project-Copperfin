...
void test_set_visual_object_disabled_item_back_color_assigns_numeric_value() {
    namespace fs = std::filesystem;
    const fs::path temp_dir = fs::temp_directory_path() /
        ("copperfin_visual_editor_disabled_item_back_color_tests_" + std::to_string(_getpid()));
    std::error_code ignored;
    fs::remove_all(temp_dir, ignored);
    fs::create_directories(temp_dir);

    const fs::path table_path = temp_dir / "disabled_item_back_color.scx";
    const std::vector<copperfin::vfp::DbfFieldDescriptor> fields{
        {.name = "OBJNAME", .type = 'C', .length = 20U},
        {.name = "NAME", .type = 'C', .length = 20U},
        {.name = "UNIQUEID", .type = 'C', .length = 20U},
    ...
    const auto create_result = copperfin::vfp::create_dbf_table_file(table_path.string(), fields, records);
    expect(create_result.ok, "#819: disabled-item back-color fixture should be writable");

    const auto disabled_item_back_color_for = [&](const std::string& path, const std::string& unique_id) {
        const auto result = copperfin::vfp::query_visual_object_property({
            .path = path,
            .record_index = 0U,
            .object_name = {},
            .unique_id = unique_id,
            .property_name = "DisabledItemBackColor"
        });
        expect(result.ok && result.exists, "#819: disabled-item back-color fixture property should be readable");
        return result.value;
    };
    const auto disabled_item_back_color = [&](const std::string& unique_id) {
        return disabled_item_back_color_for(table_path.string(), unique_id);
    };
    const auto disabled_item_back_color_state = [&]() {
        return disabled_item_back_color("customer-guid") + "," +
            disabled_item_back_color("orders-guid") + "," +
            disabled_item_back_color("other-guid");
    };

    auto color_result = copperfin::vfp::set_visual_object_disabled_item_back_color({
        .path = table_path.string(),
        .objects = {
            {.record_index = 0U, .object_name = "cboCustomer", .unique_id = {}},
            {.record_index = 1U, .object_name = {}, .unique_id = {}}
        },
        .disabled_item_back_color = 65280
    });
    expect(color_result.ok, "#819: disabled-item back-color assignment should support object-name and record-index selectors");
    expect(disabled_item_back_color("customer-guid") == "65280" &&
            disabled_item_back_color("orders-guid") == "65280" &&
            disabled_item_back_color("other-guid") == "255",
        "#819: direct disabled-item back-color assignment should write raw numeric text and preserve unrelated objects");

    auto undo_result = copperfin::vfp::undo_visual_object_property(table_path.string());
    expect(undo_result.ok, "#819: first disabled-item back-color write should remain undo-backed");
    undo_result = copperfin::vfp::undo_visual_object_property(table_path.string());
    expect(undo_result.ok, "#819: second disabled-item back-color write should remain undo-backed");
    expect(disabled_item_back_color_state() == "16777215,12632256,255",
        "#819: disabled-item back-color undo should restore original direct values");

    color_result = copperfin::vfp::set_visual_object_disabled_item_back_color({
        .path = table_path.string(),
        .objects = {
            {.record_index = 0U, .object_name = {}, .unique_id = "customer-guid"},
            {.record_index = 0U, .object_name = {}, .unique_id = "orders-guid"}
        },
        .disabled_item_back_color = 0
    });
    expect(color_result.ok, "#819: disabled-item back-color assignment should support UNIQUEID selectors");
    expect(disabled_item_back_color("customer-guid") == "0" &&
            disabled_item_back_color("orders-guid") == "0",
        "#819: direct disabled-item back-color assignment should store zero as an unquoted numeric value");

    const std::string committed_state = disabled_item_back_color_state();
    color_result = copperfin::vfp::set_visual_object_disabled_item_back_color({
        .path = table_path.string(),
        .objects = {},
        .disabled_item_back_color = 1
    });
    expect(!color_result.ok, "#819: disabled-item back-color assignment should reject empty selections");
    expect(disabled_item_back_color_state() == committed_state,
        "#819: empty-selection failures should not mutate disabled-item back colors");
    ...
    expect(!color_result.ok, "#819: disabled-item back-color assignment should reject negative values");
    expect(disabled_item_back_color_state() == committed_state,
        "#819: negative-value failures should not mutate disabled-item back colors");
    ...
    expect(!color_result.ok, "#819: disabled-item back-color assignment should reject missing selected objects");
    expect(disabled_item_back_color_state() == committed_state,
        "#819: missing-object failures should not mutate disabled-item back colors");
    ...
    expect(!color_result.ok, "#819: disabled-item back-color assignment should reject duplicate selected objects");
    expect(disabled_item_back_color_state() == committed_state,
        "#819: duplicate-selection failures should not mutate disabled-item back colors");

    const fs::path blob_path = temp_dir / "disabled_item_back_color_blob.scx";
    const std::vector<copperfin::vfp::DbfFieldDescriptor> blob_fields{
        {.name = "OBJNAME", .type = 'C', .length = 20U},
        {.name = "UNIQUEID", .type = 'C', .length = 20U},
        {.name = "PROPERTIES", .type = 'M', .length = 4U}
    ...
    const std::vector<std::vector<std::string>> blob_records{
        {"cboBlob", "blob-guid", "DisabledItemBackColor = 16777215\r\nCaption = \"Customer\"\r\n"},
        {"cboNoColor", "no-color-guid", "Caption = \"No color\"\r\n"},
        {"cboOther", "other-guid", "DisabledItemBackColor = 255\r\n"}
    };
    const auto blob_create = copperfin::vfp::create_dbf_table_file(blob_path.string(), blob_fields, blob_records);
    expect(blob_create.ok, "#819: disabled-item back-color property-blob fixture should be writable");

    const auto blob_disabled_item_back_color_state = [&](const std::string& unique_id) {
        return copperfin::vfp::query_visual_object_property({
            .path = blob_path.string(),
            .record_index = 0U,
            .object_name = {},
    ...
    expect(color_result.ok, "#819: disabled-item back-color assignment should support existing and absent serialized properties");
    auto blob_color = blob_disabled_item_back_color_state("blob-guid");
    auto appended_color = blob_disabled_item_back_color_state("no-color-guid");
    auto other_color = blob_disabled_item_back_color_state("other-guid");
    expect(blob_color.ok && blob_color.exists && blob_color.value == "65280" &&
            appended_color.ok && appended_color.exists && appended_color.value == "65280" &&
            other_color.ok && other_color.exists && other_color.value == "255",
        "#819: serialized disabled-item back-color assignment should write unquoted numeric values and preserve unrelated objects");

    undo_result = copperfin::vfp::undo_visual_object_property(blob_path.string());
    expect(undo_result.ok, "#819: appended serialized disabled-item back-color write should remain undo-backed");
    undo_result = copperfin::vfp::undo_visual_object_property(blob_path.string());
    expect(undo_result.ok, "#819: existing serialized disabled-item back-color write should remain undo-backed");
    blob_color = blob_disabled_item_back_color_state("blob-guid");
    appended_color = blob_disabled_item_back_color_state("no-color-guid");
    expect(blob_color.ok && blob_color.exists && blob_color.value == "16777215" &&
            appended_color.ok && !appended_color.exists,
        "#819: serialized disabled-item back-color undo should restore existing values and remove appended properties");

    const fs::path incomplete_path = temp_dir / "missing_disableditembackcolor.scx";
    const std::vector<copperfin::vfp::DbfFieldDescriptor> incomplete_fields{
        {.name = "OBJNAME", .type = 'C', .length = 20U},
        {.name = "UNIQUEID", .type = 'C', .length = 20U}
    };
    ...
    const auto incomplete_create = copperfin::vfp::create_dbf_table_file(
        ...
        incomplete_fields,
        incomplete_records);
    expect(incomplete_create.ok, "#819: missing-DisabledItemBackColor fixture should be writable");
    ...
    expect(!color_result.ok,
        "#819: disabled-item back-color assignment should reject objects without a writable DisabledItemBackColor carrier");
...
void test_set_visual_object_disabled_item_fore_color_assigns_numeric_value() {
    namespace fs = std::filesystem;
    const fs::path temp_dir = fs::temp_directory_path() /
        ("copperfin_visual_editor_disabled_item_fore_color_tests_" + std::to_string(_getpid()));
    std::error_code ignored;
    fs::remove_all(temp_dir, ignored);
    fs::create_directories(temp_dir);

    const fs::path table_path = temp_dir / "disabled_item_fore_color.scx";
    const std::vector<copperfin::vfp::DbfFieldDescriptor> fields{
        {.name = "OBJNAME", .type = 'C', .length = 20U},
        {.name = "NAME", .type = 'C', .length = 20U},
        {.name = "UNIQUEID", .type = 'C', .length = 20U},
    ...
    const auto create_result = copperfin::vfp::create_dbf_table_file(table_path.string(), fields, records);
    expect(create_result.ok, "#820: disabled-item fore-color fixture should be writable");

    const auto disabled_item_fore_color_for = [&](const std::string& path, const std::string& unique_id) {
        const auto result = copperfin::vfp::query_visual_object_property({
            .path = path,
            .record_index = 0U,
            .object_name = {},
            .unique_id = unique_id,
            .property_name = "DisabledItemForeColor"
        });
        expect(result.ok && result.exists, "#820: disabled-item fore-color fixture property should be readable");
        return result.value;
    };
    const auto disabled_item_fore_color = [&](const std::string& unique_id) {
        return disabled_item_fore_color_for(table_path.string(), unique_id);
    };
    const auto disabled_item_fore_color_state = [&]() {
        return disabled_item_fore_color("customer-guid") + "," +
            disabled_item_fore_color("orders-guid") + "," +
            disabled_item_fore_color("other-guid");
    };

    auto color_result = copperfin::vfp::set_visual_object_disabled_item_fore_color({
        .path = table_path.string(),
        .objects = {
            {.record_index = 0U, .object_name = "cboCustomer", .unique_id = {}},
            {.record_index = 1U, .object_name = {}, .unique_id = {}}
        },
        .disabled_item_fore_color = 65280
    });
    expect(color_result.ok, "#820: disabled-item fore-color assignment should support object-name and record-index selectors");
    expect(disabled_item_fore_color("customer-guid") == "65280" &&
            disabled_item_fore_color("orders-guid") == "65280" &&
            disabled_item_fore_color("other-guid") == "255",
        "#820: direct disabled-item fore-color assignment should write raw numeric text and preserve unrelated objects");

    auto undo_result = copperfin::vfp::undo_visual_object_property(table_path.string());
    expect(undo_result.ok, "#820: first disabled-item fore-color write should remain undo-backed");
    undo_result = copperfin::vfp::undo_visual_object_property(table_path.string());
    expect(undo_result.ok, "#820: second disabled-item fore-color write should remain undo-backed");
    expect(disabled_item_fore_color_state() == "0,16777215,255",
        "#820: disabled-item fore-color undo should restore original direct values");

    color_result = copperfin::vfp::set_visual_object_disabled_item_fore_color({
        .path = table_path.string(),
        .objects = {
            {.record_index = 0U, .object_name = {}, .unique_id = "customer-guid"},
            {.record_index = 0U, .object_name = {}, .unique_id = "orders-guid"}
        },
        .disabled_item_fore_color = 0
    });
    expect(color_result.ok, "#820: disabled-item fore-color assignment should support UNIQUEID selectors");
    expect(disabled_item_fore_color("customer-guid") == "0" &&
            disabled_item_fore_color("orders-guid") == "0",
        "#820: direct disabled-item fore-color assignment should store zero as an unquoted numeric value");

    const std::string committed_state = disabled_item_fore_color_state();
    color_result = copperfin::vfp::set_visual_object_disabled_item_fore_color({
        .path = table_path.string(),
        .objects = {},
        .disabled_item_fore_color = 1
    });
    expect(!color_result.ok, "#820: disabled-item fore-color assignment should reject empty selections");
    expect(disabled_item_fore_color_state() == committed_state,
        "#820: empty-selection failures should not mutate disabled-item fore colors");
    ...
    expect(!color_result.ok, "#820: disabled-item fore-color assignment should reject negative values");
    expect(disabled_item_fore_color_state() == committed_state,
        "#820: negative-value failures should not mutate disabled-item fore colors");
    ...
    expect(!color_result.ok, "#820: disabled-item fore-color assignment should reject missing selected objects");
    expect(disabled_item_fore_color_state() == committed_state,
        "#820: missing-object failures should not mutate disabled-item fore colors");
    ...
    expect(!color_result.ok, "#820: disabled-item fore-color assignment should reject duplicate selected objects");
    expect(disabled_item_fore_color_state() == committed_state,
        "#820: duplicate-selection failures should not mutate disabled-item fore colors");

    const fs::path blob_path = temp_dir / "disabled_item_fore_color_blob.scx";
    const std::vector<copperfin::vfp::DbfFieldDescriptor> blob_fields{
        {.name = "OBJNAME", .type = 'C', .length = 20U},
        {.name = "UNIQUEID", .type = 'C', .length = 20U},
        {.name = "PROPERTIES", .type = 'M', .length = 4U}
    ...
    const std::vector<std::vector<std::string>> blob_records{
        {"cboBlob", "blob-guid", "DisabledItemForeColor = 0\r\nCaption = \"Customer\"\r\n"},
        {"cboNoColor", "no-color-guid", "Caption = \"No color\"\r\n"},
        {"cboOther", "other-guid", "DisabledItemForeColor = 255\r\n"}
    };
    const auto blob_create = copperfin::vfp::create_dbf_table_file(blob_path.string(), blob_fields, blob_records);
    expect(blob_create.ok, "#820: disabled-item fore-color property-blob fixture should be writable");

    const auto blob_disabled_item_fore_color_state = [&](const std::string& unique_id) {
        return copperfin::vfp::query_visual_object_property({
            .path = blob_path.string(),
            .record_index = 0U,
            .object_name = {},
    ...
    expect(color_result.ok, "#820: disabled-item fore-color assignment should support existing and absent serialized properties");
    auto blob_color = blob_disabled_item_fore_color_state("blob-guid");
    auto appended_color = blob_disabled_item_fore_color_state("no-color-guid");
    auto other_color = blob_disabled_item_fore_color_state("other-guid");
    expect(blob_color.ok && blob_color.exists && blob_color.value == "65280" &&
            appended_color.ok && appended_color.exists && appended_color.value == "65280" &&
            other_color.ok && other_color.exists && other_color.value == "255",
        "#820: serialized disabled-item fore-color assignment should write unquoted numeric values and preserve unrelated objects");

    undo_result = copperfin::vfp::undo_visual_object_property(blob_path.string());
    expect(undo_result.ok, "#820: appended serialized disabled-item fore-color write should remain undo-backed");
    undo_result = copperfin::vfp::undo_visual_object_property(blob_path.string());
    expect(undo_result.ok, "#820: existing serialized disabled-item fore-color write should remain undo-backed");
    blob_color = blob_disabled_item_fore_color_state("blob-guid");
    appended_color = blob_disabled_item_fore_color_state("no-color-guid");
    expect(blob_color.ok && blob_color.exists && blob_color.value == "0" &&
            appended_color.ok && !appended_color.exists,
        "#820: serialized disabled-item fore-color undo should restore existing values and remove appended properties");

    const fs::path incomplete_path = temp_dir / "missing_disableditemforecolor.scx";
    const std::vector<copperfin::vfp::DbfFieldDescriptor> incomplete_fields{
        {.name = "OBJNAME", .type = 'C', .length = 20U},
        {.name = "UNIQUEID", .type = 'C', .length = 20U}
    };
    ...
    const auto incomplete_create = copperfin::vfp::create_dbf_table_file(
        ...
        incomplete_fields,
        incomplete_records);
    expect(incomplete_create.ok, "#820: missing-DisabledItemForeColor fixture should be writable");
    ...
    expect(!color_result.ok,
        "#820: disabled-item fore-color assignment should reject objects without a writable DisabledItemForeColor carrier");
