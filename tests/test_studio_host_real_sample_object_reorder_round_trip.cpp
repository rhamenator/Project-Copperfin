...
struct RealReorderSample {
    std::filesystem::path primary;
    std::filesystem::path sidecar;
    std::size_t source_record_index = 0U;
    std::size_t reordered_record_index = 0U;
...
void expect_reordered_selection_json(
    const std::string& json_text,
    const RealReorderSample& sample,
    ...
    expect_contains(
        json_text,
        "\"commandUndoAvailable\": false",
        context + " should keep command undo unavailable");
    expect_contains(
        json_text,
        "\"commandUndoLabel\": \"\"",
        context + " should keep the command undo label empty");
    if (sample.is_label) {
...
void exercise_real_sample_object_reorder_round_trip(
    ...
    expect(!copy_error, "#3655: should copy the real primary asset into temp space");
    copy_error.clear();
    fs::copy_file(sample.sidecar, copied_sidecar, fs::copy_options::overwrite_existing, copy_error);
    expect(!copy_error, "#3655: should copy the real sidecar asset into temp space");
    expect(make_writable(copied_primary), "#3655: copied primary asset should become writable");
    expect(make_writable(copied_sidecar), "#3655: copied sidecar asset should become writable");
    ...
    if (initial_process.exit_code != 0) {
        std::cerr << "studio host real sample initial reorder read stdout:\n"
                  << initial_process.stdout_text << "\n";
        std::cerr << "studio host real sample initial reorder read stderr:\n"
                  << initial_process.stderr_text << "\n";
    }
    expect(initial_process.exit_code == 0, "#3655: real sample object reorder baseline read should succeed");
    expect_reordered_selection_json(
        initial_process.stdout_text,
        sample,
        sample.source_record_index,
        "#3655: initial real sample reorder baseline");
    ...
    const auto reorder_process = run_process_capture(
        studio_host_path,
        {
            ...
            "--placement", "after",
            "--target-unique-id", sample.target_unique_id,
            "--json"
        },
        temp_root);
    if (reorder_process.exit_code != 0) {
        std::cerr << "studio host real sample reorder stdout:\n"
                  << reorder_process.stdout_text << "\n";
        std::cerr << "studio host real sample reorder stderr:\n"
                  << reorder_process.stderr_text << "\n";
    }
    expect(reorder_process.exit_code == 0, "#3655: real sample object reorder should succeed");
    expect_reordered_selection_json(
        reorder_process.stdout_text,
        sample,
        sample.reordered_record_index,
        "#3655: reordered real sample object read");
    ...
    const auto reopen_process = run_process_capture(
        studio_host_path,
        {"--path", copied_primary.string(), "--unique-id", sample.unique_id, "--json"},
        temp_root);
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host real sample reorder reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host real sample reorder reopen stderr:\n"
                  << reopen_process.stderr_text << "\n";
    }
    expect(reopen_process.exit_code == 0, "#3655: real sample object reorder reopen should succeed");
    expect_reordered_selection_json(
        reopen_process.stdout_text,
        sample,
        sample.reordered_record_index,
        "#3655: reopened reordered real sample object read");
}

void test_real_vfp9_report_and_label_sample_object_reorder_round_trip(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path reports_root = copperfin::test_support::find_vfp9_reports_root();
    if (reports_root.empty()) {
        std::cerr << "SKIP: #3655 real VFP9 report samples were not found\n";
        return;
    }

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_real_vfp9_sample_object_reorder_round_trip_tests";
    ...
    exercise_real_sample_object_reorder_round_trip(
        studio_host_path,
        temp_root,
        {
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "usage: test_studio_host_real_sample_object_reorder_round_trip <studio-host>\n";
        return 1;
    }

    test_real_vfp9_report_and_label_sample_object_reorder_round_trip(argv[1]);
    if (failures != 0) {
