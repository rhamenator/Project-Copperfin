...
void test_library_output_package_emits_module_definition_from_prg_routines() {
    ...
    if (result.ok) {
        ...
        expect(wrapper_source.find("CopperfinRuntimeBridgeStubEmissionWrapper stub_emission_wrapper;") != std::string::npos,
               "library-output wrapper source should carry the stub-emission wrapper contract through the response-read plan.");
        ...
        expect(wrapper_source.find("const auto stub_emission_wrapper =\n        copperfin_runtime_bridge_build_stub_emission_wrapper(") != std::string::npos,
               "library-output wrapper source should build a shared stub-emission wrapper before building the response-read plan.");
        ...
        expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.stub_emission_wrapper.native_return_surface") != std::string::npos,
               "library-output wrapper source should read the stub-emission return surface through the response-read plan.");
        expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.stub_emission_wrapper.return_adapter") != std::string::npos,
               "library-output wrapper source should read the stub-emission return adapter through the response-read plan.");
        expect(wrapper_source.find("copperfin_build_runtime_bridge_native_int_return_surface(),") != std::string::npos,
        ...
        expect(wrapper_source.find("request_write_plan,\n        stub_emission_wrapper);") != std::string::npos,
               "library-output wrapper source should pass the built wrapper into the response-read-plan builder.");
        ...
        expect(wrapper_source.find("const auto interpreted_result_plan = copperfin_build_runtime_bridge_interpreted_result_plan(\n        response_parse_plan);") != std::string::npos,
               "library-output wrapper source should build the interpreted-result plan directly from the response-parse plan once the wrapper contract is upstream.");
        ...
        expect(wrapper_source.find("const auto final_return_adoption_plan = copperfin_build_runtime_bridge_final_return_adoption_plan(\n        return_emission_plan,\n        copperfin_build_runtime_bridge_placeholder_return_statement(placeholder_return_binding));") != std::string::npos,
               "library-output wrapper source should build the final-return-adoption plan directly from the return-emission plan once the wrapper contract is carried by response read.");
        expect(wrapper_source.find("const auto return_activation_plan = copperfin_build_runtime_bridge_return_activation_plan(\n        final_return_adoption_plan);") != std::string::npos,
               "library-output wrapper source should build the return-activation plan directly from the final-return-adoption plan once the wrapper contract is carried by response read.");
        expect(wrapper_source.find("const auto stub_return_plan = copperfin_build_runtime_bridge_stub_return_plan(\n        return_activation_plan);") != std::string::npos,
               "library-output wrapper source should build the stub-return plan directly from the return-activation plan once the wrapper contract is carried by response read.");
        ...
        expect(wrapper_source.find("\"lparameters\", \"tcMode\", 1U, reinterpret_cast<void*>(&InitLibrary));") != std::string::npos,
        ...
        expect(wrapper_source.find("const auto result = copperfin_build_runtime_bridge_result(") != std::string::npos,
               "library-output wrapper source should build a bridge result from the call");
...
void test_fll_output_package_emits_api_manifest_from_prg_routines() {
    ...
    if (result.ok) {
        ...
        expect(wrapper_source.find("CopperfinRuntimeBridgeStubEmissionWrapper stub_emission_wrapper;") != std::string::npos,
               "fll-output wrapper source should carry the stub-emission wrapper contract through the response-read plan.");
        expect(wrapper_source.find("static CopperfinRuntimeBridgeReturnActivationAdmission copperfin_runtime_bridge_admit_return_activation(") != std::string::npos,
        ...
        expect(wrapper_source.find("const auto stub_emission_wrapper =\n        copperfin_runtime_bridge_build_stub_emission_wrapper(") != std::string::npos,
               "fll-output wrapper source should build a shared stub-emission wrapper before building the response-read plan.");
        expect(wrapper_source.find("return copperfin_runtime_bridge_emit_stub_return_shared(") != std::string::npos,
               "fll-output wrapper source should route FLL stub emission through the shared emitter helper.");
        expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.stub_emission_wrapper.native_return_surface") != std::string::npos,
               "fll-output wrapper source should read the stub-emission return surface through the response-read plan.");
        expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.stub_emission_wrapper.return_adapter") != std::string::npos,
               "fll-output wrapper source should read the stub-emission return adapter through the response-read plan.");
        ...
        expect(wrapper_source.find("request_write_plan,\n        stub_emission_wrapper);") != std::string::npos,
               "fll-output wrapper source should pass the built wrapper into the response-read-plan builder.");
        expect(wrapper_source.find("const auto response_artifact = copperfin_build_runtime_bridge_response_artifact(\n        response_read_plan);") != std::string::npos,
        ...
        expect(wrapper_source.find("const auto interpreted_result_plan = copperfin_build_runtime_bridge_interpreted_result_plan(\n        response_parse_plan);") != std::string::npos,
               "fll-output wrapper source should build the interpreted-result plan directly from the response-parse plan once the wrapper contract is upstream.");
        expect(wrapper_source.find("const auto native_return_plan = copperfin_build_runtime_bridge_native_return_plan(\n        result,\n        interpreted_result_plan);") != std::string::npos,
        ...
        expect(wrapper_source.find("const auto final_return_adoption_plan = copperfin_build_runtime_bridge_final_return_adoption_plan(\n        return_emission_plan,\n        copperfin_build_runtime_bridge_placeholder_return_statement(placeholder_return_binding));") != std::string::npos,
               "fll-output wrapper source should build the final-return-adoption plan directly from the return-emission plan once the wrapper contract is carried by response read.");
        expect(wrapper_source.find("const auto return_activation_plan = copperfin_build_runtime_bridge_return_activation_plan(\n        final_return_adoption_plan);") != std::string::npos,
               "fll-output wrapper source should build the return-activation plan directly from the final-return-adoption plan once the wrapper contract is carried by response read.");
        expect(wrapper_source.find("const auto stub_return_plan = copperfin_build_runtime_bridge_stub_return_plan(\n        return_activation_plan);") != std::string::npos,
               "fll-output wrapper source should build the stub-return plan directly from the return-activation plan once the wrapper contract is carried by response read.");
        expect(wrapper_source.find("static int copperfin_runtime_bridge_emit_stub_return(\n") == std::string::npos,
        ...
        expect(wrapper_source.find("const auto result = copperfin_build_runtime_bridge_result(") != std::string::npos,
               "fll-output wrapper source should build a bridge result from the call");
        expect(wrapper_source.find("const auto placeholder_return_binding =") != std::string::npos,
