...
{
    void test_native_right_button_events_dispatch_around_right_click()
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() /
                                   "copperfin_runtime_right_button_events";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);

        const fs::path main_path = temp_root / "right_button_events.prg";
        write_text(
            main_path,
            ...
            "nY = 0\n"
            "cSequence = ''\n"
            "oForm = CREATEOBJECT('RightEventsForm')\n"
            "oPlain = CREATEOBJECT('Form')\n"
            "nFormHwnd = oForm.hWnd\n"
            "nPlainHwnd = oPlain.hWnd\n"
            "READ EVENTS\n"
            "RETURN\n"
            "DEFINE CLASS RightEventsForm AS Form\n"
            "    FUNCTION MouseDown\n"
        ...
        expect(state.reason == copperfin::runtime::DebugPauseReason::event_loop &&
               ...
               std::string("Right-button fixture should pause in READ EVENTS: ") + state.message);
        ...
        expect(form_it != state.globals.end() && plain_it != state.globals.end(),
               "Right-button fixture should publish both native hWnd values");
        ...
        const auto unknown = session.dispatch_windows_message(
            static_cast<std::intptr_t>(987658),
            0x0204,
            0x000c,
            lparam);
        expect(!unknown.has_value(), "Unknown right-button targets should remain unhandled");
        ...
        const auto plain = session.dispatch_windows_message(
            ...
            0x000c,
            lparam);
        expect(!plain.has_value(), "Objects without right-button handlers should remain unhandled");
        ...
        expect(down.has_value() && *down == 0,
               "Right-button MouseDown should return the modeled default result");
        state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.reason == copperfin::runtime::DebugPauseReason::event_loop &&
                   state.waiting_for_events,
               std::string("Right-button MouseDown should restore the event loop: ") + state.message);
        ...
        expect(up.has_value() && *up == 0,
               "Right-button MouseUp and RightClick should return the modeled default result");
        state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.reason == copperfin::runtime::DebugPauseReason::event_loop &&
                   state.waiting_for_events,
               std::string("Right-button MouseUp dispatch should restore the event loop: ") + state.message);
            ...
            return std::count_if(
                ...
                [&](const copperfin::runtime::RuntimeEvent &event)
                {
                    return event.category == category &&
                           event.detail.find("RightEventsForm") != std::string::npos;
                });
        };
        expect(event_count("prg.event.mousedown") == 1,
               "Right-button MouseDown should emit one stable event");
        expect(event_count("prg.event.mouseup") == 1,
               "Right-button MouseUp should emit one stable event");
        expect(event_count("prg.event.rightclick") == 1,
               "RightClick should emit one stable event");
    }
}
