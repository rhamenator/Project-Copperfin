...
void write_synthetic_report_table_for_layout_subtree_deleted_state_json(
    ...
    expect(create_result.ok, "#2751: subtree duplicate layout fixture should be created");
}
...
void run_missing_selector_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_subtree_duplicate_missing" + extension);
    ...
    if (missing_process.exit_code != 4) {
        std::cerr << "studio host subtree duplicate missing-selector " << extension << " stdout:\n"
                  << missing_process.stdout_text << "\n";
        std::cerr << "studio host subtree duplicate missing-selector " << extension << " stderr:\n"
...
void test_studio_host_json_preserves_subtree_duplicate_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_subtree_duplicate_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_missing_selector_case(
            ...
            temp_root,
            extension,
            "#2751: subtree duplicate missing-selector");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_subtree_duplicate_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_subtree_duplicate_errors(argv[1]);
