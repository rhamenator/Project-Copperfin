...
void write_synthetic_report_table_for_stable_nested_group_section_expression_json(
    ...
    expect(create_result.ok, "#3100: stable nested group section fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_nested_group_footer_expression_json(
    ...
    expect(delete_result.ok && dbf_record_deleted(report_path, 4U),
           "#3100: deleted nested group footer fixture should mark the inner group footer deleted");
}

void run_nested_group_section_stable_selection(
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable-selected nested group section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable-selected nested group section stderr:\n"
                  << process.stderr_text << "\n";
...
void run_deleted_nested_group_section_stable_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    const auto process = run_process_capture(
        studio_host_path,
        {"--path", asset_path.string(), "--unique-id", "country-footer-guid", "--json"},
        temp_root);

    if (process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable-selected deleted nested group section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable-selected deleted nested group section stderr:\n"
                  << process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
...
void test_studio_host_json_preserves_selected_nested_group_sections_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_nested_group_sections_stable_tests";
    ...
    run_nested_group_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_nested_group_sections_stable.frx",
        "report",
        "#3100: stable-selected live report nested group section selection");
    run_nested_group_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_nested_group_sections_stable.lbx",
        "label",
        "#3100: stable-selected live label nested group section selection");
    run_deleted_nested_group_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_nested_group_sections_stable.frx",
        "report",
        "#3100: stable-selected deleted report nested group section selection");
    run_deleted_nested_group_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_nested_group_sections_stable.lbx",
        "label",
        "#3100: stable-selected deleted label nested group section selection");
...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "usage: test_studio_host_selected_nested_group_sections_stable <copperfin_studio_host>\n";
        return 2;
    }

    test_studio_host_json_preserves_selected_nested_group_sections_stable_selection(argv[1]);
