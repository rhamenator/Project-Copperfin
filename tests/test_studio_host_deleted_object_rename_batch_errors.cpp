...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2743: rename-batch rollback layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2743: rename-batch rollback fixture should mark target rows deleted");
}

void run_rename_batch_rollback_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("deleted_object_rename_batch_rollback" + extension);
    ...
    if (rollback_process.exit_code != 4) {
        std::cerr << "studio host rename-batch rollback " << extension << " stdout:\n"
                  << rollback_process.stdout_text << "\n";
        std::cerr << "studio host rename-batch rollback " << extension << " stderr:\n"
    ...
    expect_contains(rollback_process.stdout_text,
                    "The requested identity value already exists in the asset.",
                    issue_prefix + " should report the current identity-value collision error");
    expect(visual_object_order(asset_path) == "left-field-guid,middle-field-guid,right-field-guid",
           issue_prefix + " should roll back earlier rename mutations");
}

void test_studio_host_json_preserves_deleted_object_rename_batch_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_object_rename_batch_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_rename_batch_rollback_case(
            studio_host_path,
            temp_root,
            extension,
            "#2743: deleted object rename-batch rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_rename_batch_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_rename_batch_errors(argv[1]);
