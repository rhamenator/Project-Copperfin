...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2744: reorder-batch rollback layout fixture should be created");
}
...
void run_reorder_batch_rollback_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_reorder_batch_rollback" + extension);
...
void test_studio_host_json_preserves_object_reorder_batch_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_object_reorder_batch_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_reorder_batch_rollback_case(
            ...
            temp_root,
            extension,
            "#2744: object reorder-batch rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_object_reorder_batch_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_object_reorder_batch_errors(argv[1]);
