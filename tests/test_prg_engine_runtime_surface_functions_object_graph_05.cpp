...
namespace copperfin::runtime_surface_tests
{
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_native_prg_release_owner_declarative_external_base_child_subtree";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "native_release_owner_declarative_external_base_child_subtree.prg";
        write_text(
            main_path,
            "cDestroyLog = ''\n"
            "cChildParentCaptionDuringDestroy = ''\n"
            ...
            "xFormCaptionAfterRelease = GETPEM(oForm, 'Caption')\n"
            "oDict = NEWOBJECT('Scripting.Dictionary', 'vbscript.dll')\n"
            "lDictSet = SETPEM(oDict, 'comparemode', 117)\n"
            "nDictCompare = GETPEM(oDict, 'comparemode')\n"
            "RETURN\n"
            "DEFINE CLASS DemoForm AS Custom\n"
            "    Caption = 'MainForm'\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("native owner Release declarative external-base child subtree script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        check("lformhascaptionafterrelease", "false");
        check("ldictset", "true");
        check("ndictcompare", "117");

        const auto child_after_release = state.globals.find("xchildafterrelease");
        expect(child_after_release != state.globals.end() &&
                   child_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release declarative external-base child subtree should invalidate GETPEM() for the released child slot");

        const auto child_parent_after_release = state.globals.find("xchildparentafterrelease");
        expect(child_parent_after_release != state.globals.end() &&
                   child_parent_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release declarative external-base child subtree should invalidate GETPEM() for the released child's Parent");

        const auto child_class_library_after_release = state.globals.find("xchildclasslibraryafterrelease");
        expect(child_class_library_after_release != state.globals.end() &&
                   child_class_library_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release declarative external-base child subtree should invalidate GETPEM() for the released child's ClassLibrary");

        const auto grandchild_parent_after_release = state.globals.find("xgrandchildparentafterrelease");
        expect(grandchild_parent_after_release != state.globals.end() &&
                   grandchild_parent_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release declarative external-base child subtree should invalidate GETPEM() for the released grandchild's Parent");

        const auto form_caption_after_release = state.globals.find("xformcaptionafterrelease");
        expect(form_caption_after_release != state.globals.end() &&
                   form_caption_after_release->second.kind == copperfin::runtime::PrgValueKind::empty,
               "native owner Release declarative external-base child subtree should invalidate GETPEM() for the released owner");

        expect(state.ole_objects.size() == 1U,
               "native owner Release declarative external-base child subtree should leave only the COM object registered after recursive release");
        if (state.ole_objects.size() == 1U)
        {
            expect(state.ole_objects[0].prog_id == "Scripting.Dictionary",
                   "COM NEWOBJECT should remain stable while native owner declarative external-base Release lands");
        }
        ...
        expect(has_grandchild_destroy_event,
               "native owner Release declarative external-base child subtree should dispatch the grandchild Destroy method");
        ...
        expect(has_child_destroy_event,
               "native owner Release declarative external-base child subtree should dispatch the child Destroy method");
        ...
        expect(has_form_destroy_event,
               "native owner Release declarative external-base child subtree should dispatch the owner Destroy method");
