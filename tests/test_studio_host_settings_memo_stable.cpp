...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2776: synthetic report/label layout settings fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2776: synthetic report/label layout settings fixture should mark deleted objects");
}
...
void write_synthetic_report_table_for_deleted_settings_json(const std::filesystem::path& report_path) {
    ...
    expect(delete_result.ok, "#2776: deleted settings fixture should mark report settings deleted");
}
...
void write_synthetic_report_table_for_stable_settings_json(const std::filesystem::path& report_path) {
    ...
    expect(unique_id_result.ok, "#2776: stable settings fixture should seed a settings unique id");
    expect(!dbf_record_deleted(report_path, 0U),
           "#2776: stable settings fixture should preserve the live settings state");
}
...
void run_settings_memo_update_case(
    ...
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("settings_memo_stable_update") + extension);
    if (deleted) {
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable settings memo update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable settings memo update " << extension << " stderr:\n"
...
void run_settings_memo_clear_case(
    ...
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("settings_memo_stable_clear") + extension);
    if (deleted) {
        write_synthetic_report_table_for_stable_deleted_settings_json(asset_path);
    } else {
        write_synthetic_report_table_for_stable_settings_json(asset_path);
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable settings memo clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable settings memo clear " << extension << " stderr:\n"
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " clear should preserve remaining selected direct-field provenance");
...
void test_studio_host_json_preserves_settings_memo_stable_selection(const std::string& studio_host_path) {
    ...
    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_settings_memo_stable_selection_tests";
    ...
    run_settings_memo_update_case(
        ...
        temp_root,
        ".frx",
        "settings_memo_stable_update.frx",
        updated_settings,
        false,
        "report",
        "#2776: report/label stable settings memo success");
    run_settings_memo_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "settings_memo_stable_update.lbx",
        updated_settings,
        false,
        "label",
        "#2776: report/label stable settings memo success");
    run_settings_memo_clear_case(
        ...
        temp_root,
        ".frx",
        "settings_memo_stable_clear.frx",
        false,
        "report",
        "#2776: report/label stable settings memo success");
    run_settings_memo_clear_case(
        ...
        temp_root,
        ".lbx",
        "settings_memo_stable_clear.lbx",
        false,
        "label",
        "#2776: report/label stable settings memo success");
    run_settings_memo_update_case(
        ...
        temp_root,
        ".frx",
        "deleted_settings_memo_stable_update.frx",
        updated_settings,
        true,
        "report",
    ...
    run_settings_memo_update_case(
        ...
        temp_root,
        ".lbx",
        "deleted_settings_memo_stable_update.lbx",
        updated_settings,
        true,
        "label",
        "#2776: report/label stable deleted settings memo success");
    run_settings_memo_clear_case(
        ...
        temp_root,
        ".frx",
        "deleted_settings_memo_stable_clear.frx",
        true,
        "report",
        "#2776: report/label stable deleted settings memo success");
    run_settings_memo_clear_case(
        ...
        temp_root,
        ".lbx",
        "deleted_settings_memo_stable_clear.lbx",
        true,
        "label",
        "#2776: report/label stable deleted settings memo success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_settings_memo_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_settings_memo_stable_selection(argv[1]);
