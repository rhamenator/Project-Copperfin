...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2745: duplicate-batch rollback layout fixture should be created");
}
...
void run_duplicate_batch_rollback_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_duplicate_batch_rollback" + extension);
    write_synthetic_report_table_for_layout_reorder_json(asset_path);

    const auto list_result = copperfin::vfp::list_visual_objects(asset_path.string());
    expect(list_result.ok, issue_prefix + " fixture should be listable");
    const std::size_t before_count = list_result.objects.size();
    ...
    const auto after_list_result = copperfin::vfp::list_visual_objects(asset_path.string());
    expect(after_list_result.ok && after_list_result.objects.size() == before_count,
           issue_prefix + " should restore the original object count");
}

void test_studio_host_json_preserves_object_duplicate_batch_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_object_duplicate_batch_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_duplicate_batch_rollback_case(
            ...
            temp_root,
            extension,
            "#2745: object duplicate-batch rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_object_duplicate_batch_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_object_duplicate_batch_errors(argv[1]);
