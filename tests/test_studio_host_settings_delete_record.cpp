...
void write_synthetic_report_table_for_unsupported_expr_layout_json(const std::filesystem::path& report_path) {
    const std::vector<copperfin::vfp::DbfFieldDescriptor> fields{
        {.name = "OBJTYPE", .type = 'N', .length = 8U},
        {.name = "OBJCODE", .type = 'N', .length = 8U},
        {.name = "EXPR", .type = 'M', .length = 4U},
    ...
    const auto create_result = copperfin::vfp::create_dbf_table_file(report_path.string(), fields, records);
    expect(create_result.ok, "#3096: record settings delete fixture should create unsupported EXPR layout");
    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#3096: record settings delete fixture should mark deleted objects");
}
...
void run_unsupported_expr_settings_delete_case(
    ...
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / file_name;
    write_synthetic_report_table_for_unsupported_expr_layout_json(asset_path);
    ...
    if (delete_process.exit_code != 0) {
        std::cerr << "studio host " << label << " record unsupported EXPR settings delete stdout:\n"
                  << delete_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " record unsupported EXPR settings delete stderr:\n"
                  << delete_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
...
void test_studio_host_json_preserves_settings_delete_record_selection(const std::string& studio_host_path) {
    ...
    run_unsupported_expr_settings_delete_case(
        ...
        "report",
        "#3096: record-selected report settings delete should preserve unsupported EXPR lines");
    run_unsupported_expr_settings_delete_case(
        studio_host_path,
        temp_root,
        "mailing_unsupported.lbx",
        "label",
        "#3096: record-selected label settings delete should preserve unsupported EXPR lines");
