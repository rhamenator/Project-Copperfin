    ...
    if (fs::exists(expected_output) && native_symbol_dump_is_available()) {
        ...
        if (extension == "dll") {
            ...
            expect(wrapper_source.find("return stub_emission_return_surface.emitted_int_value;") != std::string::npos,
                   "build host DLL wrapper should route DLL stub emission through the shared stub-emission return-surface helper.");
    ...
    if (fs::exists(expected_output) && native_symbol_dump_is_available()) {
        ...
        if (extension == "fll") {
            ...
            expect(wrapper_source.find("return _RetInt(stub_emission_return_surface.emitted_int_value);") != std::string::npos,
                   "build host FLL wrapper should route FLL stub emission through the shared stub-emission return-surface helper.");
            expect(wrapper_source.find("static int copperfin_runtime_bridge_emit_stub_return(") != std::string::npos,
