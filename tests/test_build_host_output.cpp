...
void run_library_build_host_smoke(
    ...
    if (fs::exists(expected_output) && native_symbol_dump_is_available()) {
        ...
        if (extension == "dll") {
            ...
            expect(wrapper_source.find("copperfin_runtime_bridge_build_stub_emission_return_surface(") != std::string::npos,
                   "build host DLL wrapper should route shared stub-emission output through the shared return-surface helper.");
            expect(wrapper_source.find("copperfin_build_runtime_bridge_native_int_return_surface());") != std::string::npos,
                   "build host DLL wrapper should build the DLL stub-emission return surface from the native int return-surface contract.");
            ...
            expect(wrapper_source.find("return copperfin_runtime_bridge_apply_stub_emission_output(") != std::string::npos,
                   "build host DLL wrapper should route DLL stub emission through the shared output-application helper.");
            expect(wrapper_source.find("copperfin_runtime_bridge_return_native_int);") != std::string::npos,
                   "build host DLL wrapper should apply the DLL stub-emission result through the native-int return adapter.");
    ...
    if (fs::exists(expected_output) && native_symbol_dump_is_available()) {
        ...
        if (extension == "fll") {
            ...
            expect(wrapper_source.find("copperfin_runtime_bridge_build_stub_emission_return_surface(") != std::string::npos,
                   "build host FLL wrapper should route shared stub-emission output through the shared return-surface helper.");
            expect(wrapper_source.find("copperfin_build_runtime_bridge_fll_int_return_surface());") != std::string::npos,
                   "build host FLL wrapper should build the FLL stub-emission return surface from the FLL int return-surface contract.");
            expect(wrapper_source.find("return copperfin_runtime_bridge_apply_stub_emission_output(") != std::string::npos,
                   "build host FLL wrapper should route FLL stub emission through the shared output-application helper.");
            expect(wrapper_source.find("_RetInt);") != std::string::npos,
                   "build host FLL wrapper should apply the FLL stub-emission result through the `_RetInt` adapter.");
            expect(wrapper_source.find("static int copperfin_runtime_bridge_emit_stub_return(") != std::string::npos,
