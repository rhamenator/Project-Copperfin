...
void run_library_build_host_smoke(
    ...
    if (fs::exists(expected_output) && native_symbol_dump_is_available()) {
        ...
        if (extension == "dll") {
            ...
            expect(wrapper_source.find("CopperfinRuntimeBridgeStubEmissionWrapper stub_emission_wrapper;") != std::string::npos,
                   "build host DLL wrapper should carry the stub-emission wrapper contract through the payload plan.");
            ...
            expect(wrapper_source.find("const auto stub_emission_wrapper =\n        copperfin_runtime_bridge_build_stub_emission_wrapper(") != std::string::npos,
                   "build host DLL wrapper should build a shared stub-emission wrapper before building the payload plan.");
            ...
            expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.request_write_plan.request_artifact.response_validation_plan.failure_policy_plan.interpretation_plan.payload_plan.stub_emission_wrapper.native_return_surface") != std::string::npos,
                   "build host DLL wrapper should read the stub-emission return surface through the payload plan.");
            expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.request_write_plan.request_artifact.response_validation_plan.failure_policy_plan.interpretation_plan.payload_plan.stub_emission_wrapper.return_adapter") != std::string::npos,
                   "build host DLL wrapper should read the stub-emission return adapter through the payload plan.");
            expect(wrapper_source.find("copperfin_build_runtime_bridge_native_int_return_surface(),") != std::string::npos,
            ...
            expect(wrapper_source.find("dispatch_plan,\n        stub_emission_wrapper);") != std::string::npos,
                   "build host DLL wrapper should pass the built wrapper into the payload-plan builder.");
    ...
    if (fs::exists(expected_output) && native_symbol_dump_is_available()) {
        ...
        if (extension == "fll") {
            ...
            expect(wrapper_source.find("CopperfinRuntimeBridgeStubEmissionWrapper stub_emission_wrapper;") != std::string::npos,
                   "build host FLL wrapper should carry the stub-emission wrapper contract through the payload plan.");
            expect(wrapper_source.find("static CopperfinRuntimeBridgeReturnActivationAdmission copperfin_runtime_bridge_admit_return_activation(") != std::string::npos,
            ...
            expect(wrapper_source.find("const auto stub_emission_wrapper =\n        copperfin_runtime_bridge_build_stub_emission_wrapper(") != std::string::npos,
                   "build host FLL wrapper should build a shared stub-emission wrapper before building the payload plan.");
            expect(wrapper_source.find("return copperfin_runtime_bridge_emit_stub_return_shared(") != std::string::npos,
                   "build host FLL wrapper should route FLL stub emission through the shared emitter helper.");
            expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.request_write_plan.request_artifact.response_validation_plan.failure_policy_plan.interpretation_plan.payload_plan.stub_emission_wrapper.native_return_surface") != std::string::npos,
                   "build host FLL wrapper should read the stub-emission return surface through the payload plan.");
            expect(wrapper_source.find("final_return_adoption_plan.return_emission_plan.return_materialization_plan.outcome_selection_plan.native_return_plan.interpreted_result_plan.response_parse_plan.response_artifact.response_read_plan.request_write_plan.request_artifact.response_validation_plan.failure_policy_plan.interpretation_plan.payload_plan.stub_emission_wrapper.return_adapter") != std::string::npos,
                   "build host FLL wrapper should read the stub-emission return adapter through the payload plan.");
            ...
            expect(wrapper_source.find("dispatch_plan,\n        stub_emission_wrapper);") != std::string::npos,
                   "build host FLL wrapper should pass the built wrapper into the payload-plan builder.");
            expect(wrapper_source.find("const auto failure_policy = copperfin_build_runtime_bridge_failure_policy_plan(\n        interpretation_plan,\n        placeholder_return_binding.value_representation);") != std::string::npos,
