...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2738: rename-batch layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2738: rename-batch fixture should mark target rows deleted");
}
...
void run_rename_batch_case(
    ...
    const auto rename_batch_process = run_process_capture(
        ...
        temp_root);
    expect(rename_batch_process.exit_code == 0, issue_prefix + " should succeed");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host rename-batch reopen " << extension << " stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host rename-batch reopen " << extension << " stderr:\n"
    ...
    expect_reopened_deleted_row_section_context(reopen_process.stdout_text, "middle-renamed-guid", issue_prefix);
}

void test_studio_host_json_preserves_deleted_object_rename_batch_sections(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_object_rename_batch_section_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_rename_batch_case(
            studio_host_path,
            temp_root,
            extension,
            "#2738: deleted object rename-batch");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_rename_batch_sections <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_rename_batch_sections(argv[1]);
