...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2765: deleted duplicate-batch success layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2765: deleted duplicate-batch success fixture should mark target rows deleted");
}

void run_deleted_duplicate_batch_success_case(
    ...
    if (duplicate_batch_process.exit_code != 0) {
        std::cerr << "studio host " << label << " deleted duplicate-batch stdout:\n"
                  << duplicate_batch_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " deleted duplicate-batch stderr:\n"
    ...
    expect(visual_object_count(asset_path) == before_count + 2U &&
           ...
           issue_prefix + " should append deleted duplicate rows");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host " << label << " deleted duplicate-batch reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " deleted duplicate-batch reopen stderr:\n"
...
void test_studio_host_json_preserves_deleted_object_duplicate_batch(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_deleted_object_duplicate_batch_tests";
    ...
    run_deleted_duplicate_batch_success_case(
        studio_host_path,
        temp_root,
        ".frx",
        "deleted_object_duplicate_batch.frx",
        "report",
        "#2765: deleted report/label duplicate-batch success");
    run_deleted_duplicate_batch_success_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "deleted_object_duplicate_batch.lbx",
        "label",
        "#2765: deleted report/label duplicate-batch success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_duplicate_batch <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_duplicate_batch(argv[1]);
