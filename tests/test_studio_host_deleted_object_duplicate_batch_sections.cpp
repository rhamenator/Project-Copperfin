...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2739: duplicate-batch layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2739: duplicate-batch fixture should mark target rows deleted");
}

void expect_reopened_deleted_copy_section_context(
    const std::string& stdout_text,
    const std::string& issue_prefix) {
    expect_contains(stdout_text, "\"selectedReportObjectAvailable\": true",
                    issue_prefix + " should keep the copied deleted row selected");
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should preserve selected copied-row containing-section metadata");
    expect_contains_in_order(
        stdout_text,
        {
...
void run_duplicate_batch_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("deleted_object_duplicate_batch" + extension);
    ...
    const auto duplicate_batch_process = run_process_capture(
        ...
        temp_root);
    expect(duplicate_batch_process.exit_code == 0, issue_prefix + " should succeed");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host duplicate-batch reopen " << extension << " stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host duplicate-batch reopen " << extension << " stderr:\n"
    ...
    expect_reopened_deleted_copy_section_context(reopen_process.stdout_text, issue_prefix);
}

void test_studio_host_json_preserves_deleted_object_duplicate_batch_sections(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_object_duplicate_batch_section_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_duplicate_batch_case(
            studio_host_path,
            temp_root,
            extension,
            "#2739: deleted object duplicate-batch");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_duplicate_batch_sections <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_duplicate_batch_sections(argv[1]);
