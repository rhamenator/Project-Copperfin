...
void write_synthetic_report_table_for_column_spacing_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2787: column-spacing settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_column_spacing_field_json(
    ...
    expect(delete_result.ok, "#2787: deleted column-spacing settings fixture should mark settings deleted");
}
...
void run_column_spacing_update_case(
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " column spacing field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " column spacing field update " << extension << " stderr:\n"
...
void run_column_spacing_clear_case(
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " column spacing field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " column spacing field clear " << extension << " stderr:\n"
    ...
    if (!deleted) {
        ...
        expect_not_contains(clear_process.stdout_text,
                            ...
                            issue_prefix + " clear should remove direct column-spacing settings");
    }
}

void test_studio_host_json_preserves_column_spacing_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_column_spacing_record_settings_tests";
    ...
    run_column_spacing_update_case(
        ...
        false,
        "report",
        "#2787: report/label record-selected column-spacing settings success");
    run_column_spacing_update_case(
        studio_host_path,
        temp_root,
        "column_spacing",
    ...
    run_column_spacing_update_case(
        studio_host_path,
        temp_root,
        "deleted_column_spacing",
        ".frx",
        "180",
        true,
        "report",
        "#2787: report/label record-selected deleted column-spacing settings success");
    run_column_spacing_update_case(
        studio_host_path,
        temp_root,
        "deleted_column_spacing",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_column_spacing_record_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_column_spacing_record_selection(argv[1]);
