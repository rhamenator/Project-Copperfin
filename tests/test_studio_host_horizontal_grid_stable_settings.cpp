...
void write_synthetic_report_table_for_grid_horizontal_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2773: horizontal-grid settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_grid_horizontal_field_json(
    ...
    expect(delete_result.ok, "#2773: deleted horizontal-grid settings fixture should mark settings deleted");
}
...
void write_synthetic_report_table_for_stable_grid_horizontal_field_json(
    ...
    expect(unique_id_result.ok, "#2773: stable horizontal-grid fixture should seed a settings unique id");
    expect(!dbf_record_deleted(report_path, 0U),
           "#2773: stable horizontal-grid fixture should preserve the live settings state");
}
...
void write_synthetic_report_table_for_stable_deleted_grid_horizontal_field_json(
    ...
    expect(unique_id_result.ok,
           "#2773: stable deleted horizontal-grid fixture should seed a settings unique id");
    expect(dbf_record_deleted(report_path, 0U),
           "#2773: stable deleted horizontal-grid fixture should preserve the deleted settings state");
}

void run_horizontal_grid_update_case(
    ...
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("grid_horizontal_update") + extension);
    if (deleted) {
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable horizontal-grid field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable horizontal-grid field update " << extension << " stderr:\n"
...
void run_horizontal_grid_clear_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& title,
    ...
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("grid_horizontal_clear") + extension);
    if (deleted) {
        write_synthetic_report_table_for_stable_deleted_grid_horizontal_field_json(asset_path);
    } else {
        write_synthetic_report_table_for_stable_grid_horizontal_field_json(asset_path);
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable horizontal-grid field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable horizontal-grid field clear " << extension << " stderr:\n"
    ...
    expect_not_contains(clear_process.stdout_text,
                        ...
                        issue_prefix + " clear should remove direct GRIDH provenance");
}

void test_studio_host_json_preserves_horizontal_grid_stable_settings(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_horizontal_grid_stable_settings_tests";
    ...
    run_horizontal_grid_update_case(
        studio_host_path,
        temp_root,
        ".frx",
        "grid_horizontal_update.frx",
        "16",
        false,
        "report",
        "#2773: report/label stable horizontal-grid settings success");
    run_horizontal_grid_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "grid_horizontal_update.lbx",
        "18",
        false,
        "label",
        "#2773: report/label stable horizontal-grid settings success");
    run_horizontal_grid_clear_case(
        studio_host_path,
        temp_root,
        ".frx",
        "grid_horizontal_clear.frx",
        false,
        "report",
        "#2773: report/label stable horizontal-grid settings success");
    run_horizontal_grid_clear_case(
    ...
    run_horizontal_grid_update_case(
        ...
        temp_root,
        ".frx",
        "deleted_grid_horizontal_update.frx",
        "16",
        true,
        "report",
        "#2773: report/label stable deleted horizontal-grid settings success");
    run_horizontal_grid_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "deleted_grid_horizontal_update.lbx",
        "18",
        true,
        "label",
        "#2773: report/label stable deleted horizontal-grid settings success");
    run_horizontal_grid_clear_case(
        ...
        temp_root,
        ".frx",
        "deleted_grid_horizontal_clear.frx",
        true,
        "report",
        "#2773: report/label stable deleted horizontal-grid settings success");
    run_horizontal_grid_clear_case(
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_horizontal_grid_stable_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_horizontal_grid_stable_settings(argv[1]);
