...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2764: duplicate-batch success layout fixture should be created");
}

void run_duplicate_batch_success_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_duplicate_batch" + extension);
    ...
    if (duplicate_batch_process.exit_code != 0) {
        std::cerr << "studio host " << label << " duplicate-batch stdout:\n"
                  << duplicate_batch_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " duplicate-batch stderr:\n"
                  << duplicate_batch_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }

    expect(duplicate_batch_process.exit_code == 0, issue_prefix + " should exit successfully");
    ...
    expect(visual_object_count(asset_path) == before_count + 2U &&
           ...
           issue_prefix + " should append duplicates after original objects");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host " << label << " duplicate-batch reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " duplicate-batch reopen stderr:\n"
    ...
    expect_contains(reopen_process.stdout_text, "\"uniqueId\": \"left-copy-guid\"",
                    issue_prefix + " should preserve selected duplicate identity after reopen");
    expect_contains_in_order(
        ...
        issue_prefix + " should refresh selected duplicate metadata after reopen");
}

void test_studio_host_json_preserves_object_duplicate_batch(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_object_duplicate_batch_tests";
    ...
    run_duplicate_batch_success_case(
        studio_host_path,
        temp_root,
        ".frx",
        "object_duplicate_batch.frx",
        "report",
        "#2764: report/label duplicate-batch success");
    run_duplicate_batch_success_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "object_duplicate_batch.lbx",
        "label",
        "#2764: report/label duplicate-batch success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_object_duplicate_batch <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_object_duplicate_batch(argv[1]);
