...
void test_report_expression_total_type_provenance() {
    ...
    document.display_name = "total-type.frx";
    ...
    expect(layout.available && layout.sections.size() == 1U && layout.sections[0].objects.size() == 3U,
        "#4525: report-expression and label objects should remain in their containing section");
    if (layout.sections.size() == 1U && layout.sections[0].objects.size() == 3U) {
        ...
        const auto live_highlight = std::find_if(
            ...
            [](const auto& highlight) {
                return highlight.name == "TOTALTYPE";
            });
        expect(live_highlight != live.highlights.end(),
            "#4525: live report-expression highlights should include TOTALTYPE");
        if (live_highlight != live.highlights.end()) {
            expect(live_highlight->value == "2" &&
                   live_highlight->field_index == 2U &&
                   live_highlight->memo_block_number == 618U,
                "#4525: live TOTALTYPE should preserve value and provenance");
        }
        ...
        expect(std::none_of(
            ...
            "#4525: blank TOTALTYPE should remain an observable blank without a fabricated highlight");
        ...
        expect(std::none_of(
            ...
            "#4525: label TOTALTYPE should not be treated as a calculated expression contract");
    }
    ...
    expect(deleted != layout.deleted_objects.end(),
        "#4525: deleted report-expression objects should remain available for inspection");
    if (deleted != layout.deleted_objects.end()) {
        ...
        expect(deleted_highlight != deleted->highlights.end() &&
               deleted_highlight->value == "5" &&
               deleted_highlight->field_index == 2U &&
               deleted_highlight->memo_block_number == 619U,
            "#4525: deleted TOTALTYPE should preserve value and provenance");
    }
}
