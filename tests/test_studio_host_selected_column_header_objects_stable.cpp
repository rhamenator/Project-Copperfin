...
void write_synthetic_report_table_for_stable_column_header_object_json(
    ...
    expect(create_result.ok, "#2822: stable column-header object fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_column_header_object_json(
    ...
    expect(delete_result.ok, "#2822: deleted column-header object fixture should mark column-header object deleted");
}

void run_column_header_object_selection(
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose the containing column-header metadata");
}

void run_deleted_column_header_object_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose containing column-header metadata");
    expect_contains(object_process.stdout_text, "\"left\": 200",
                    issue_prefix + " should expose selected-object left bounds");
    expect_contains(object_process.stdout_text, "\"top\": 100",
                    issue_prefix + " should expose selected-object top bounds");
...
void test_studio_host_json_preserves_selected_column_header_objects_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_column_header_objects_stable_tests";
    ...
    run_column_header_object_selection(
        studio_host_path,
        temp_root,
        "selected_column_header_object_stable.frx",
        "report",
        "#2822: stable live report column-header object selection");
    run_column_header_object_selection(
        studio_host_path,
        temp_root,
        "selected_column_header_object_stable.lbx",
        "label",
        "#2822: stable live label column-header object selection");
    run_deleted_column_header_object_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_column_header_object_stable.frx",
        "report",
        "#2822: stable deleted report column-header object selection");
    run_deleted_column_header_object_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_column_header_object_stable.lbx",
        "label",
        "#2822: stable deleted label column-header object selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_column_header_objects_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_column_header_objects_stable_selection(argv[1]);
