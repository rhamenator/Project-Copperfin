...
void write_synthetic_report_table_for_stable_title_section_json(
    ...
    expect(create_result.ok, "#2811: record-selected title section fixture should be created");
}

void run_title_section_record_selection(
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host record-selected " << label << " title section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host record-selected " << label << " title section stderr:\n"
                  << process.stderr_text << "\n";
...
void run_deleted_title_section_record_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    if (process.exit_code != 0) {
        std::cerr << "studio host record-selected deleted " << label << " title section stdout:\n"
                  << process.stdout_text << "\n";
        std::cerr << "studio host record-selected deleted " << label << " title section stderr:\n"
                  << process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose deleted title metadata");
    expect_contains_in_order(
        process.stdout_text,
        {
            "\"sections\": [",
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose selected deleted title metadata");
}

void test_studio_host_json_preserves_selected_title_sections_record_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_title_sections_record_tests";
    ...
    run_title_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_title_section_record.frx",
        "report",
        "#2811: record-selected live report title section selection");
    run_title_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_title_section_record.lbx",
        "label",
        "#2811: record-selected live label title section selection");
    run_deleted_title_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_title_section_record.frx",
        "report",
        "#2811: record-selected deleted report title section selection");
    run_deleted_title_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_title_section_record.lbx",
        "label",
        "#2811: record-selected deleted label title section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_title_sections_record <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_title_sections_record_selection(argv[1]);
