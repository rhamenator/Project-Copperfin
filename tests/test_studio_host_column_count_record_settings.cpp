...
void write_synthetic_report_table_for_column_setup_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2785: column-count settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_column_setup_field_json(
    ...
    expect(delete_result.ok, "#2785: deleted column-count settings fixture should mark settings deleted");
}

void run_column_count_update_case(
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " column count field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " column count field update " << extension << " stderr:\n"
    ...
    expect(column_count_property.ok && column_count_property.exists &&
           ...
           issue_prefix + " update should persist the COLS field");
    ...
    if (!deleted) {
        ...
        expect_contains(update_process.stdout_text, "\"columnWidthAvailable\": true",
                        issue_prefix + " update should preserve effective column-width availability");
        expect_contains(update_process.stdout_text, "\"columnWidth\": 3600",
                        issue_prefix + " update should preserve the effective column width");
...
void run_column_count_clear_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& asset_stem,
    const std::string& extension,
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " column count field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " column count field clear " << extension << " stderr:\n"
    ...
    expect(column_count_property.ok && column_count_property.exists &&
           ...
           issue_prefix + " clear should blank the COLS field");
    ...
    if (!deleted) {
        ...
        expect_contains(clear_process.stdout_text, "\"columnCount\": 0",
                        issue_prefix + " clear should clear column counts");
        ...
        expect_contains(clear_process.stdout_text, "\"columnWidthAvailable\": true",
                        issue_prefix + " clear should preserve effective column-width availability");
        expect_contains(clear_process.stdout_text, "\"columnWidth\": 3600",
                        issue_prefix + " clear should preserve the effective column width");
        ...
        expect_not_contains(clear_process.stdout_text,
                            ...
                            issue_prefix + " clear should remove direct column-count settings");
    }
}

void test_studio_host_json_preserves_column_count_record_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_column_count_record_settings_tests";
    ...
    run_column_count_update_case(
        ...
        false,
        "report",
        "#2785: report/label record-selected column-count settings success");
    run_column_count_update_case(
        studio_host_path,
        temp_root,
        "column_count",
    ...
    run_column_count_update_case(
        ...
        true,
        "report",
        "#2785: report/label record-selected deleted column-count settings success");
    run_column_count_update_case(
        studio_host_path,
        temp_root,
        "deleted_column_count",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_column_count_record_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_column_count_record_selection(argv[1]);
