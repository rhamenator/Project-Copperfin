...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2767: deleted reorder-batch success layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2767: deleted reorder-batch success fixture should mark target rows deleted");
}

void run_deleted_reorder_batch_success_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("deleted_object_reorder_batch" + extension);
    ...
    if (reorder_batch_process.exit_code != 0) {
        std::cerr << "studio host " << label << " deleted reorder-batch stdout:\n"
                  << reorder_batch_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " deleted reorder-batch stderr:\n"
                  << reorder_batch_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }

    expect(reorder_batch_process.exit_code == 0, issue_prefix + " should exit successfully");
    ...
    expect(visual_object_order(asset_path) == "right-field-guid,middle-field-guid,left-field-guid" &&
           ...
           issue_prefix + " should move deleted rows without changing deleted state");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host " << label << " deleted reorder-batch reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " deleted reorder-batch reopen stderr:\n"
    ...
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectAvailable\": true",
                    issue_prefix + " should select the moved deleted row");
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectSectionAvailable\": true",
                    issue_prefix + " should preserve containing sections");
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should preserve selected moved-row containing-section metadata");
    expect_contains_in_order(
        reopen_process.stdout_text,
        {
...
void test_studio_host_json_preserves_deleted_object_reorder_batch(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_deleted_object_reorder_batch_tests";
    ...
    run_deleted_reorder_batch_success_case(
        studio_host_path,
        temp_root,
        ".frx",
        "deleted_object_reorder_batch.frx",
        "report",
        "#2767: deleted report/label reorder-batch success");
    run_deleted_reorder_batch_success_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "deleted_object_reorder_batch.lbx",
        "label",
        "#2767: deleted report/label reorder-batch success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_reorder_batch <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_reorder_batch(argv[1]);
