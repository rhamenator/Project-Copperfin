...
void write_synthetic_report_table_for_paper_size_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2775: paper-size settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_paper_size_field_json(
    ...
    expect(delete_result.ok, "#2775: deleted paper-size settings fixture should mark settings deleted");
}
...
void write_synthetic_report_table_for_stable_paper_size_field_json(
    ...
    expect(unique_id_result.ok, "#2775: stable paper-size fixture should seed a settings unique id");
    expect(!dbf_record_deleted(report_path, 0U),
           "#2775: stable paper-size fixture should preserve the live settings state");
}
...
void write_synthetic_report_table_for_stable_deleted_paper_size_field_json(
    ...
    expect(unique_id_result.ok,
           "#2775: stable deleted paper-size fixture should seed a settings unique id");
    expect(dbf_record_deleted(report_path, 0U),
           "#2775: stable deleted paper-size fixture should preserve the deleted settings state");
}
...
void run_paper_size_update_case(
    ...
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("paper_size_update") + extension);
    if (deleted) {
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable paper-size field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable paper-size field update " << extension << " stderr:\n"
...
void run_paper_size_clear_case(
    ...
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("paper_size_clear") + extension);
    if (deleted) {
        write_synthetic_report_table_for_stable_deleted_paper_size_field_json(asset_path);
    } else {
        write_synthetic_report_table_for_stable_paper_size_field_json(asset_path);
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable paper-size field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable paper-size field clear " << extension << " stderr:\n"
    ...
    if (!deleted) {
        expect_contains(clear_process.stdout_text, "\"orientationCode\": 0",
                        issue_prefix + " clear should preserve orientation codes");
    ...
    expect_not_contains(clear_process.stdout_text,
                        ...
                        issue_prefix + " clear should remove direct PAPERSIZE provenance");
}

void test_studio_host_json_preserves_paper_size_stable_settings(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_paper_size_stable_settings_tests";
    ...
    run_paper_size_update_case(
        ...
        temp_root,
        ".frx",
        "paper_size_update.frx",
        "9",
        false,
        "report",
        "#2775: report/label stable paper-size settings success");
    run_paper_size_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "paper_size_update.lbx",
        "5",
        false,
        "label",
        "#2775: report/label stable paper-size settings success");
    ...
    run_paper_size_update_case(
        ...
        temp_root,
        ".frx",
        "deleted_paper_size_update.frx",
        "9",
        true,
        "report",
        "#2775: report/label stable deleted paper-size settings success");
    run_paper_size_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "deleted_paper_size_update.lbx",
        "5",
        true,
        "label",
        "#2775: report/label stable deleted paper-size settings success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_paper_size_stable_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_paper_size_stable_settings(argv[1]);
