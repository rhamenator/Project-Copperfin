...
require_text("${script}" "invoke-visual-studio-command-window.ps1" "registered-command helper")
...
require_text("${script}" "Start-Sleep -Milliseconds 10000" "bounded Command-window lazy-load interval")
require_text("${script}" "'-Mode', 'Activate'" "separate bounded Command-window activation")
require_text("${script}" "'-Mode', 'Input'" "separate bounded Command-window input")
require_text("${script}" "external sender exits" "hosted input-dispatch boundary")
require_text("${script}" "Visual Studio Command-window lazy-load activation" "lazy-load activation phase")
require_text("${script}" "Visual Studio loaded Command-window activation" "loaded-surface activation phase")
...
string(FIND "${script_contents}" "Visual Studio Command-window lazy-load activation" lazy_load_activation_offset)
string(FIND "${script_contents}" "Visual Studio loaded Command-window activation" loaded_activation_offset)
string(FIND "${script_contents}" "Copperfin Command-window input" command_input_offset)
if(lazy_load_activation_offset EQUAL -1 OR loaded_activation_offset EQUAL -1 OR
        command_input_offset EQUAL -1 OR
        NOT lazy_load_activation_offset LESS loaded_activation_offset OR
        NOT loaded_activation_offset LESS command_input_offset)
    message(FATAL_ERROR "separate Command-window activation and input processes must remain ordered")
endif()
