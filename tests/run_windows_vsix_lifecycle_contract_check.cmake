...
require_text("${script}" "RequireToolWindowSurface" "tool-window-specific observation mode")
require_text("${script}" "ControlType]::Pane.Id" "tool-window pane control identity")
require_text("${script}" "ControlType]::TabItem.Id" "docked tool-window tab identity")
require_text("${script}" "ControlType]::Window.Id" "floating tool-window identity")
require_text("${script}" "observed_control_type" "retained UI control-type evidence")
require_text("${script}" "Copperfin.ShowCommandWindow" "registered canonical command identity")
require_text("${script}" "'/Embedding', '/NoSplash', '/Log'" "automation-hosted IDE with retained activity log")
require_text("${script}" "GetRunningObjectTable" "PID-bound Visual Studio automation discovery")
require_text("${script}" "name.EndsWith(suffix" "exact Visual Studio process binding")
require_text("${script}" "$dte.MainWindow.Visible = $true" "automation-hosted IDE visibility")
require_text("${script}" "$dte.UserControl = $true" "IDE lifetime transfer after automation")
require_text("${script}" "$dte.Solution.Open($result.solution_path)" "runner-owned solution request")
require_text("${script}" "$dte.ItemOperations.OpenFile($result.prg_path)" "runner-owned PRG request")
require_text("${script}" "$dte.Commands.Raise" "public Visual Studio semantic command dispatch")
require_text("${script}" "4b56ff76-d352-4027-bb18-ef4c759d260b" "exact Copperfin command group")
require_text("${script}" "0x0300" "exact Copperfin command ID")
require_text("${script}" "vsix-automation-dispatch.json" "machine-readable semantic dispatch evidence")
require_text("${script}" "command_raise_completed" "semantic command completion evidence")
require_text("${script}" "lifecycle-smoke.sln" "runner-owned empty solution fixture")
...
require_text("${workflow}" "vsix-automation-dispatch.json" "retained PID-bound dispatch diagnostics")
...
string(FIND "${script_contents}" "VSIX lifecycle phase: drive exact installed command through PID-bound automation" command_submission_offset)
