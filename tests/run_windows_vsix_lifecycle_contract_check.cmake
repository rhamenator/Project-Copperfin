...
require_text("${script}" "invoke-visual-studio-automation.ps1" "registered-command and document helper")
require_text("${script}" "-CommandName', 'Copperfin.ShowCommandWindow'" "exact invariant command name")
require_text("${script}" "VisualStudio.DTE.$VisualStudioRegistryVersion`:$ExpectedProcessId" "exact-process Visual Studio automation identity")
require_text("${script}" "GetWindowThreadProcessId" "automation-window process ownership lookup")
require_text("${script}" "ownerProcessId -eq [uint32]$ExpectedProcessId" "automation process identity verification")
require_text("${script}" "'/Command', 'View.CommandWindow'" "IDE-owned built-in Command-window startup")
require_text("${script}" "CopperfinRunningObjectTable" "Visual Studio Running Object Table lookup")
require_text("${script}" "displayName.EndsWith(expectedDisplayName, StringComparison.Ordinal)" "exact process-suffixed automation moniker lookup")
require_text("${script}" "$dte.Commands.Item($CommandName, 0)" "registered-command lookup")
require_text("${script}" "$dte.ExecuteCommand($CommandName)" "registered-command execution")
require_text("${script}" "Registered Copperfin Command surface" "same-process command-surface proof")
require_text("${script}" "$dte.ItemOperations.OpenFile" "explicit running-IDE document-open request")
...
string(FIND "${script_contents}" "Copperfin exact-process registered-command automation" command_automation_offset)
...
string(FIND "${script_contents}" "VSIX lifecycle phase: invoke registered Copperfin command through exact-process automation" command_phase_offset)
