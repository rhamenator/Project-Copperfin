...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2798: synthetic report/label layout settings fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2798: synthetic report/label layout settings fixture should mark deleted objects");
}
...
void run_settings_restore_case(
    ...
    if (restore_process.exit_code != 0) {
        std::cerr << "studio host " << label << " record settings restore stdout:\n"
                  << restore_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " record settings restore stderr:\n"
...
void test_studio_host_json_preserves_settings_restore_record_selection(const std::string& studio_host_path) {
    ...
    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_settings_restore_record_selection_tests";
    ...
    run_settings_restore_case(
        ...
        "summary.frx",
        "report",
        "#2798: record-selected report settings restore");
    run_settings_restore_case(
        studio_host_path,
        temp_root,
        "mailing.lbx",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_settings_restore_record <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_settings_restore_record_selection(argv[1]);
