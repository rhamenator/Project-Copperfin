...
namespace copperfin::runtime_surface_tests
{
    void test_inherited_external_parent_declarative_derived_startup_external_base_child_identity_metadata_resists_mutation()
    {
        ...
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_inherited_external_parent_declarative_derived_startup_external_base_identity";
        ...
        const fs::path main_path = temp_root / "inherited_external_parent_declarative_derived_startup_external_base_identity.prg";
        write_text(
            main_path,
            ...
            "lDictSet = SETPEM(oDict, 'comparemode', 132)\n"
        ...
        expect(state.completed,
               std::string("inherited external-parent declarative derived-startup external-base identity script should complete: ") + state.message +
        ...
        check("ldictset", "true");
        check("ndictcompare", "132");

        expect(state.ole_objects.size() == 5U,
               "inherited external-parent declarative derived-startup external-base identity should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            ...
            expect(create_parent.prog_id == "ChildForm",
                   "inherited external-parent declarative derived-startup external-base identity should preserve CREATEOBJECT parent identity");
            expect(create_child.prog_id == "SaveButton",
                   "inherited external-parent declarative derived-startup external-base identity should preserve CREATEOBJECT child identity");
            expect(create_child.base_class_name == "ParentButton",
                   "inherited external-parent declarative derived-startup external-base identity should preserve the CREATEOBJECT child immediate external base");
            expect(create_child.source == main_path.string(),
                   "inherited external-parent declarative derived-startup external-base identity should preserve CREATEOBJECT child source identity");
            expect(create_child.class_library == button_library_path.string(),
                   "inherited external-parent declarative derived-startup external-base identity should preserve CREATEOBJECT child ClassLibrary path");
            expect(create_child.class_hierarchy.size() == 5U,
                   "inherited external-parent declarative derived-startup external-base identity should preserve CREATEOBJECT child class hierarchy");
            expect(!create_child.properties.contains("class"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a CREATEOBJECT child Class shadow");
            expect(!create_child.properties.contains("baseclass"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a CREATEOBJECT child BaseClass shadow");
            expect(!create_child.properties.contains("parentclass"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a CREATEOBJECT child ParentClass shadow");
            expect(!create_child.properties.contains("classlibrary"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a CREATEOBJECT child ClassLibrary shadow");
            expect(leaf_parent.prog_id == "LeafForm",
                   "inherited external-parent declarative derived-startup external-base identity should preserve NEWOBJECT leaf identity");
            expect(leaf_child.prog_id == "SaveButton",
                   "inherited external-parent declarative derived-startup external-base identity should preserve leaf child identity");
            expect(leaf_child.base_class_name == "ParentButton",
                   "inherited external-parent declarative derived-startup external-base identity should preserve the leaf child immediate external base");
            expect(leaf_child.source == main_path.string(),
                   "inherited external-parent declarative derived-startup external-base identity should preserve leaf child source identity");
            expect(leaf_child.class_library == button_library_path.string(),
                   "inherited external-parent declarative derived-startup external-base identity should preserve leaf child ClassLibrary path");
            expect(leaf_child.class_hierarchy.size() == 5U,
                   "inherited external-parent declarative derived-startup external-base identity should preserve leaf child class hierarchy");
            expect(!leaf_child.properties.contains("class"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a leaf child Class shadow");
            expect(!leaf_child.properties.contains("baseclass"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a leaf child BaseClass shadow");
            expect(!leaf_child.properties.contains("parentclass"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a leaf child ParentClass shadow");
            expect(!leaf_child.properties.contains("classlibrary"),
                   "inherited external-parent declarative derived-startup external-base identity should not materialize a leaf child ClassLibrary shadow");
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM behavior should remain stable while inherited external-parent declarative derived-startup external-base identity lands");
        }
        ...
        expect(has_save_invoke_event,
               "inherited external-parent declarative derived-startup external-base identity should keep child owner dispatch usable after rejected mutations");
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_inherited_external_parent_declarative_derived_startup_external_base_dotted";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "inherited_external_parent_declarative_derived_startup_external_base_dotted.prg";
        write_text(
            main_path,
            "oCreate = CREATEOBJECT('ChildForm')\n"
            "oLeaf = NEWOBJECT('LeafForm')\n"
            ...
            "oDict = NEWOBJECT('Scripting.Dictionary', 'vbscript.dll')\n"
            "lDictSet = SETPEM(oDict, 'comparemode', 136)\n"
            "nDictCompare = GETPEM(oDict, 'comparemode')\n"
            "RETURN\n"
            "DEFINE CLASS ChildForm AS ParentForm OF widgetlib.prg\n"
            "ENDDEFINE\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("inherited external-parent declarative derived-startup external-base dotted script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        check("ldictset", "true");
        check("ndictcompare", "136");

        expect(state.ole_objects.size() == 5U,
               "inherited external-parent declarative derived-startup external-base dotted script should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            const auto &create_parent = state.ole_objects[0];
            const auto &create_child = state.ole_objects[1];
            ...
            const auto &leaf_child = state.ole_objects[3];
            expect(create_parent.prog_id == "ChildForm",
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve CREATEOBJECT parent identity");
            expect(create_parent.source == main_path.string(),
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve CREATEOBJECT parent source");
            expect(create_child.prog_id == "SaveButton",
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve CREATEOBJECT child identity");
            expect(create_child.base_class_name == "ParentButton",
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve the CREATEOBJECT child immediate external base");
            expect(create_child.source == main_path.string(),
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve CREATEOBJECT child derived-startup source identity");
            expect(create_child.class_library == button_library_path.string(),
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve CREATEOBJECT child ClassLibrary path");
            expect(leaf_parent.prog_id == "LeafForm",
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve NEWOBJECT leaf identity");
            expect(leaf_parent.source == main_path.string(),
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve NEWOBJECT leaf source");
            expect(leaf_child.prog_id == "SaveButton",
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve leaf child identity");
            expect(leaf_child.base_class_name == "ParentButton",
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve the leaf child immediate external base");
            expect(leaf_child.source == main_path.string(),
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve leaf child derived-startup source identity");
            expect(leaf_child.class_library == button_library_path.string(),
                   "inherited external-parent declarative derived-startup external-base dotted access should preserve leaf child ClassLibrary path");
            ...
            if (create_parent_caption != create_parent.properties.end())
            {
                expect(copperfin::runtime::format_value(create_parent_caption->second) == "Done-Saved",
                       "inherited external-parent declarative derived-startup external-base dotted access should let dotted child method calls update the CREATEOBJECT owner");
            }
            else
            {
                expect(false, "inherited external-parent declarative derived-startup external-base dotted access should preserve the CREATEOBJECT owner caption");
            }
            if (create_child_caption != create_child.properties.end())
            {
                expect(copperfin::runtime::format_value(create_child_caption->second) == "Go",
                       "inherited external-parent declarative derived-startup external-base dotted access should preserve the CREATEOBJECT child dotted assignment");
            }
            else
            {
                expect(false, "inherited external-parent declarative derived-startup external-base dotted access should preserve the CREATEOBJECT child caption");
            }
            if (leaf_parent_caption != leaf_parent.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_parent_caption->second) == "Ready-Saved",
                       "inherited external-parent declarative derived-startup external-base dotted access should let dotted child method calls update the leaf owner");
            }
            else
            {
                expect(false, "inherited external-parent declarative derived-startup external-base dotted access should preserve the leaf owner caption");
            }
            if (leaf_child_caption != leaf_child.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_child_caption->second) == "Ship",
                       "inherited external-parent declarative derived-startup external-base dotted access should preserve the leaf child dotted assignment");
            }
            else
            {
                expect(false, "inherited external-parent declarative derived-startup external-base dotted access should preserve the leaf child caption");
            }
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM behavior should remain stable while inherited external-parent declarative derived-startup external-base dotted access lands");
        }
        ...
        expect(has_addobject_event,
               "inherited external-parent declarative derived-startup external-base dotted access should emit child materialization events");
        ...
        expect(has_owner_caption_invoke_event,
               "inherited external-parent declarative derived-startup external-base dotted access should dispatch direct dotted child method calls");
        ...
        expect(has_save_invoke_event,
               "inherited external-parent declarative derived-startup external-base dotted access should keep child THISFORM owner dispatch stable");
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_inherited_external_parent_object_block_derived_startup_child";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "inherited_external_parent_object_block_derived_startup_child.prg";
        write_text(
            main_path,
            "oCreate = CREATEOBJECT('ChildForm')\n"
            "oLeaf = NEWOBJECT('LeafForm')\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("inherited external-parent object-block derived-startup child script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        check("cleafchildcaption", "Commit");
        check("ccreateownercaption", "MainForm");
        check("cleafownercaption", "MainForm");
        check("ccreatesavedcaption", "MainForm-Saved");
        check("cleafsavedcaption", "MainForm-Saved");
        ...
        expect(leaf_child_class_library != state.globals.end() &&
               ...
               "inherited external-parent object-block derived-startup child should leave the leaf child ClassLibrary empty on ordinary reads");
        ...
        expect(leaf_child_class_library_reflect != state.globals.end() &&
               ...
               "inherited external-parent object-block derived-startup child should leave the leaf child ClassLibrary empty through GETPEM()");
        ...
        expect(leaf_members_union != state.globals.end() &&
               ...
               "inherited external-parent object-block derived-startup child should keep leaf union member enumeration including inherited child methods");

        expect(state.ole_objects.size() == 5U,
               "inherited external-parent object-block derived-startup child script should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            const auto &create_parent = state.ole_objects[0];
            const auto &create_child = state.ole_objects[1];
            ...
            const auto &leaf_child = state.ole_objects[3];
            expect(create_parent.prog_id == "ChildForm",
                   "inherited external-parent object-block derived-startup child should preserve CREATEOBJECT parent identity");
            expect(create_child.prog_id == "SaveButton",
                   "inherited external-parent object-block derived-startup child should materialize the CREATEOBJECT child");
            expect(create_child.source == main_path.string(),
                   "inherited external-parent object-block derived-startup child should resolve the CREATEOBJECT child against the derived startup PRG when the external parent library lacks the class");
            expect(leaf_parent.prog_id == "LeafForm",
                   "inherited external-parent object-block derived-startup child should preserve NEWOBJECT leaf identity");
            expect(leaf_child.prog_id == "SaveButton",
                   "inherited external-parent object-block derived-startup child should materialize leaf inherited children");
            expect(leaf_child.source == main_path.string(),
                   "inherited external-parent object-block derived-startup child should preserve derived-startup provenance for leaf child instances");
            expect(leaf_child.base_class_name == "Custom",
                   "inherited external-parent object-block derived-startup child should preserve the leaf child immediate builtin base");
            expect(leaf_child.class_hierarchy.size() == 3U,
                   "inherited external-parent object-block derived-startup child should preserve the leaf child class hierarchy");
            if (leaf_child.class_hierarchy.size() == 3U)
            {
                expect(leaf_child.class_hierarchy[0] == "SAVEBUTTON",
                       "inherited external-parent object-block derived-startup child should store the leaf child class first");
                expect(leaf_child.class_hierarchy[1] == "CUSTOM",
                       "inherited external-parent object-block derived-startup child should store the leaf builtin base second");
                expect(leaf_child.class_hierarchy[2] == "OBJECT",
                       "inherited external-parent object-block derived-startup child should store the leaf terminal object token third");
            }
            expect(!leaf_child.properties.contains("classlibrary"),
                   "inherited external-parent object-block derived-startup child should not materialize a leaf child ClassLibrary shadow");
            const auto create_child_caption = create_child.properties.find("caption");
            const auto leaf_child_caption = leaf_child.properties.find("caption");
            if (create_child_caption != create_child.properties.end())
            {
                expect(copperfin::runtime::format_value(create_child_caption->second) == "Commit",
                       "inherited external-parent object-block derived-startup child should preserve object-block caption overrides on the CREATEOBJECT child");
            }
            else
            {
                expect(false, "inherited external-parent object-block derived-startup child should materialize the CREATEOBJECT child caption override");
            }
            if (leaf_child_caption != leaf_child.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_child_caption->second) == "Commit",
                       "inherited external-parent object-block derived-startup child should preserve object-block caption overrides on the leaf child");
            }
            else
            {
                expect(false, "inherited external-parent object-block derived-startup child should materialize the leaf child caption override");
            }
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM behavior should remain stable while inherited external-parent object-block derived-startup child activation lands");
        }
        ...
        expect(has_addobject_event,
               "inherited external-parent object-block derived-startup child should emit child materialization events");
        ...
        expect(has_save_invoke_event,
               "inherited external-parent object-block derived-startup child should keep materialized children fully usable through THISFORM owner dispatch");
