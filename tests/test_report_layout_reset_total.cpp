...
void test_report_expression_reset_total_provenance() {
    ...
    document.display_name = "reset-total.frx";
    ...
    document.table_preview.records = {
        ...
        },
        {
            ...
            .values = {
                value("OBJTYPE", "8"),
                value("EXPR", "customer.deleted_total"),
                value("RESETTOTAL", "80", 623U),
                value("HPOS", "900.000"),
                value("VPOS", "200.000"),
                value("WIDTH", "800.000"),
                value("HEIGHT", "600.000")
    ...
    expect(layout.available && layout.sections.size() == 1U && layout.sections[0].objects.size() == 3U,
        "#4526: report-expression and label objects should remain in their containing section");
    if (layout.sections.size() == 1U && layout.sections[0].objects.size() == 3U) {
        ...
        const auto live_highlight = std::find_if(
            ...
            [](const auto& highlight) {
                return highlight.name == "RESETTOTAL";
            });
        expect(live_highlight != live.highlights.end(),
            "#4526: live report-expression highlights should include RESETTOTAL");
        if (live_highlight != live.highlights.end()) {
            expect(live_highlight->value == "2" &&
                   live_highlight->field_index == 2U &&
                   live_highlight->memo_block_number == 622U,
                "#4526: live RESETTOTAL should preserve value and provenance");
        }
        ...
        expect(std::none_of(
            ...
            "#4526: blank RESETTOTAL should remain an observable blank without a fabricated highlight");
        ...
        expect(std::none_of(
            ...
            "#4526: label RESETTOTAL should not be treated as a calculated expression contract");
    }
    ...
    expect(deleted != layout.deleted_objects.end(),
        "#4526: deleted report-expression objects should remain available for inspection");
    if (deleted != layout.deleted_objects.end()) {
        ...
        expect(deleted_highlight != deleted->highlights.end() &&
               deleted_highlight->value == "80" &&
               deleted_highlight->field_index == 2U &&
               deleted_highlight->memo_block_number == 623U,
            "#4526: deleted RESETTOTAL should preserve value and provenance");
    }
}
