...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2741: reorder-batch rollback layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2741: reorder-batch rollback fixture should mark target rows deleted");
}

void run_reorder_batch_rollback_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("deleted_object_reorder_batch_rollback" + extension);
    ...
    if (rollback_process.exit_code != 4) {
        std::cerr << "studio host reorder-batch rollback " << extension << " stdout:\n"
                  << rollback_process.stdout_text << "\n";
        std::cerr << "studio host reorder-batch rollback " << extension << " stderr:\n"
    ...
    expect_contains(rollback_process.stdout_text,
                    ...
                    issue_prefix + " should report the current missing-target error");
    const std::string restored_order = "left-field-guid,middle-field-guid,right-field-guid";
    expect(visual_object_order(asset_path) == restored_order,
           issue_prefix + " should roll back earlier reorder mutations");
}

void test_studio_host_json_preserves_deleted_object_reorder_batch_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_object_reorder_batch_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_reorder_batch_rollback_case(
            studio_host_path,
            temp_root,
            extension,
            "#2741: deleted object reorder-batch rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_reorder_batch_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_reorder_batch_errors(argv[1]);
