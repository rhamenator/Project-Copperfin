...
void write_synthetic_report_table_for_stable_summary_section_json(
    ...
    expect(create_result.ok, "#2805: stable summary section fixture should be created");
}

void run_summary_section_record_selection(
    ...
    if (section_process.exit_code != 0) {
        std::cerr << "studio host " << label << " record-selected summary section stdout:\n"
                  << section_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " record-selected summary section stderr:\n"
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose selected summary metadata");
}

void run_deleted_summary_section_record_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    expect(delete_result.ok && dbf_record_deleted(asset_path, 2U),
           issue_prefix + " fixture should mark the summary section deleted");
    ...
    if (section_process.exit_code != 0) {
        std::cerr << "studio host " << label << " record-selected deleted summary section stdout:\n"
                  << section_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " record-selected deleted summary section stderr:\n"
                  << section_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose deleted summary metadata");
    expect_contains_in_order(
        section_process.stdout_text,
        {
            "\"selectedReportSection\": {",
        ...
        issue_prefix + " should expose selected deleted summary metadata");
    expect_contains_in_order(
        section_process.stdout_text,
        {
            "\"sections\": [",
        ...
        issue_prefix + " should preserve live detail metadata");
}

void test_studio_host_json_preserves_selected_summary_sections_record_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_summary_sections_record_tests";
    ...
    run_summary_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_summary_section_record.frx",
        "report",
        "#2805: record-selected live report summary section selection");
    run_summary_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_summary_section_record.lbx",
        "label",
        "#2805: record-selected live label summary section selection");
    run_deleted_summary_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_summary_section_record.frx",
        "report",
        "#2805: record-selected deleted report summary section selection");
    run_deleted_summary_section_record_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_summary_section_record.lbx",
        "label",
        "#2805: record-selected deleted label summary section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_summary_sections_record <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_summary_sections_record_selection(argv[1]);
