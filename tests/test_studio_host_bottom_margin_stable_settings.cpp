...
void write_synthetic_report_table_for_bottom_margin_field_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2771: bottom-margin settings fixture should be created");
}
...
void write_synthetic_report_table_for_deleted_bottom_margin_field_json(
    ...
    expect(delete_result.ok, "#2771: deleted bottom-margin settings fixture should mark settings deleted");
}
...
void write_synthetic_report_table_for_stable_bottom_margin_field_json(
    ...
    expect(unique_id_result.ok, "#2771: stable bottom-margin fixture should seed a settings unique id");
    expect(!dbf_record_deleted(report_path, 0U),
           "#2771: stable bottom-margin fixture should preserve the live settings state");
}
...
void write_synthetic_report_table_for_stable_deleted_bottom_margin_field_json(
    ...
    expect(unique_id_result.ok,
           "#2771: stable deleted bottom-margin fixture should seed a settings unique id");
    expect(dbf_record_deleted(report_path, 0U),
           "#2771: stable deleted bottom-margin fixture should preserve the deleted settings state");
}

void run_bottom_margin_update_case(
    ...
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("bottom_margin_update") + extension);
    if (deleted) {
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable bottom margin field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable bottom margin field update " << extension << " stderr:\n"
    ...
    expect(margin_property.ok && margin_property.exists && margin_property.value == updated_margin,
           issue_prefix + " update should persist the BOTMARGIN field");
...
void run_bottom_margin_clear_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& title,
    ...
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path =
        temp_root / ((deleted ? "deleted_" : "") + std::string("bottom_margin_clear") + extension);
    if (deleted) {
        write_synthetic_report_table_for_stable_deleted_bottom_margin_field_json(asset_path);
    } else {
        write_synthetic_report_table_for_stable_bottom_margin_field_json(asset_path);
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable bottom margin field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable bottom margin field clear " << extension << " stderr:\n"
    ...
    expect(margin_property.ok && margin_property.exists && margin_property.value.empty(),
           issue_prefix + " clear should blank the BOTMARGIN field");
    ...
    if (!deleted) {
        ...
        expect_contains(clear_process.stdout_text, "\"bottomMargin\": 0",
                        issue_prefix + " clear should clear bottom margins");
    ...
    expect_not_contains(clear_process.stdout_text,
                        ...
                        issue_prefix + " clear should remove direct BOTMARGIN provenance");
}

void test_studio_host_json_preserves_bottom_margin_stable_settings(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_bottom_margin_stable_settings_tests";
    ...
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        ".frx",
        "bottom_margin_update.frx",
        "34",
        false,
        "report",
        "#2771: report/label stable bottom-margin settings success");
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "bottom_margin_update.lbx",
        "36",
        false,
        "label",
        "#2771: report/label stable bottom-margin settings success");
    run_bottom_margin_clear_case(
        studio_host_path,
        temp_root,
        ".frx",
        "bottom_margin_clear.frx",
        false,
        "report",
        "#2771: report/label stable bottom-margin settings success");
    run_bottom_margin_clear_case(
    ...
    run_bottom_margin_update_case(
        ...
        temp_root,
        ".frx",
        "deleted_bottom_margin_update.frx",
        "34",
        true,
        "report",
        "#2771: report/label stable deleted bottom-margin settings success");
    run_bottom_margin_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "deleted_bottom_margin_update.lbx",
        "36",
        true,
        "label",
        "#2771: report/label stable deleted bottom-margin settings success");
    run_bottom_margin_clear_case(
        ...
        temp_root,
        ".frx",
        "deleted_bottom_margin_clear.frx",
        true,
        "report",
        "#2771: report/label stable deleted bottom-margin settings success");
    run_bottom_margin_clear_case(
        ...
        temp_root,
        ".lbx",
        "deleted_bottom_margin_clear.lbx",
        true,
        "label",
        "#2771: report/label stable deleted bottom-margin settings success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_bottom_margin_stable_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_bottom_margin_stable_settings(argv[1]);
