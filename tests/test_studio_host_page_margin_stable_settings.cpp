...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2777: synthetic report/label layout settings fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2777: synthetic report/label layout settings fixture should mark deleted objects");
}
...
void write_synthetic_report_table_for_deleted_settings_json(const std::filesystem::path& report_path) {
    ...
    expect(delete_result.ok, "#2777: deleted settings fixture should mark report settings deleted");
}
...
void write_synthetic_report_table_for_stable_settings_json(const std::filesystem::path& report_path) {
    ...
    expect(unique_id_result.ok, "#2777: stable settings fixture should seed a settings unique id");
    expect(!dbf_record_deleted(report_path, 0U),
           "#2777: stable settings fixture should preserve the live settings state");
}
...
void write_synthetic_report_table_for_stable_deleted_settings_json(const std::filesystem::path& report_path) {
    ...
    expect(unique_id_result.ok, "#2777: stable deleted settings fixture should seed a deleted settings unique id");
    expect(dbf_record_deleted(report_path, 0U),
           "#2777: stable deleted settings fixture should preserve the deleted settings state");
}
...
void run_page_margin_update_case(
    ...
    if (update_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable page margin field update " << extension << " stdout:\n"
                  << update_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable page margin field update " << extension << " stderr:\n"
...
void run_page_margin_clear_case(
    ...
    if (clear_process.exit_code != 0) {
        std::cerr << "studio host " << label << " stable page margin field clear " << extension << " stdout:\n"
                  << clear_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " stable page margin field clear " << extension << " stderr:\n"
    ...
    expect_not_contains(clear_process.stdout_text,
                        ...
                        issue_prefix + " clear should remove direct TOPMARGIN provenance");
    if (deleted) {
...
void test_studio_host_json_preserves_page_margin_stable_settings(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_page_margin_stable_settings_tests";
    ...
    run_page_margin_update_case(
        ...
        false,
        "report",
        "#2777: report/label stable page-margin settings success");
    run_page_margin_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
    ...
    run_page_margin_update_case(
        ...
        true,
        "report",
        "#2777: report/label stable deleted page-margin settings success");
    run_page_margin_update_case(
        studio_host_path,
        temp_root,
        ".lbx",
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_page_margin_stable_settings <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_page_margin_stable_settings(argv[1]);
