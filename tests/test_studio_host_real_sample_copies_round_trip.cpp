...
struct RealSamplePair {
    ...
    std::string title;
    std::string copies_value;
    int copies_memo_block_number = 0;
    bool is_label = false;
};
...
void expect_common_reopen_json(const std::string& json_text, const RealSamplePair& sample) {
    expect_contains(json_text,
                    ...
                    "#3773: real-sample COPIES round trip should preserve document title");
    if (sample.is_label) {
        expect_contains(json_text,
                        "\"isLabel\": true",
                        "#3773: label COPIES round trip should preserve label identity");
    } else {
        expect_contains(json_text,
                        "\"isLabel\": false",
                        "#3773: report COPIES round trip should preserve report identity");
    }
    expect_contains(json_text,
                    "\"selectedReportSettingsAvailable\": true",
                    "#3773: real-sample COPIES round trip should preserve selected-settings availability");
    expect_contains(json_text,
                    "\"selectedReportSelectionKind\": \"settings\"",
                    "#3773: real-sample COPIES round trip should preserve settings selection kind");
    expect_contains(json_text,
                    "\"pageSetupAvailable\": true",
                    "#3773: real-sample COPIES round trip should preserve page setup availability");
    expect_contains(json_text,
                    "\"orientationCode\": 0",
                    "#3773: real-sample COPIES round trip should preserve orientation");
    expect_contains(json_text,
                    "\"paperSizeCode\": 1",
                    "#3773: real-sample COPIES round trip should preserve paper size");
    expect_contains(json_text,
                    "\"gridVerticalAvailable\": true",
                    "#3773: real-sample COPIES round trip should preserve grid-vertical availability");
    expect_contains(json_text,
                    "\"gridVertical\": 12",
                    "#3773: real-sample COPIES round trip should preserve grid-vertical value");
}

void exercise_real_sample_copies_round_trip(
    ...
    expect(!copy_error, "#3773: should copy the real primary asset into temp space");
    copy_error.clear();
    fs::copy_file(sample.sidecar, copied_sidecar, fs::copy_options::overwrite_existing, copy_error);
    expect(!copy_error, "#3773: should copy the real sidecar asset into temp space");
    expect(make_writable(copied_primary), "#3773: copied primary asset should become writable");
    expect(make_writable(copied_sidecar), "#3773: copied sidecar asset should become writable");
    ...
    const auto initial_copies_property = copperfin::vfp::query_visual_object_property({
        .path = copied_primary.string(),
        .record_index = 0U,
        .object_name = {},
        .unique_id = {},
        .property_name = "COPIES"
    });
    expect(initial_copies_property.ok &&
               !initial_copies_property.exists &&
               initial_copies_property.value.empty(),
           "#3773: real sample COPIES query should treat the installed setting as materializable when missing");

    const auto set_copies_process = run_process_capture(
        studio_host_path,
        {
            "--path", copied_primary.string(),
            "--set-property",
            ...
            "--property-value", sample.copies_value,
            "--json"
        },
        temp_root);
    if (set_copies_process.exit_code != 0) {
        std::cerr << "studio host sample COPIES update stdout:\n" << set_copies_process.stdout_text << "\n";
        std::cerr << "studio host sample COPIES update stderr:\n" << set_copies_process.stderr_text << "\n";
    }
    expect(set_copies_process.exit_code == 0, "#3773: real sample COPIES update should succeed");

    const auto copies_property = copperfin::vfp::query_visual_object_property({
        .path = copied_primary.string(),
        .record_index = 0U,
        .object_name = {},
        .unique_id = {},
    ...
    expect(copies_property.ok && copies_property.exists && copies_property.value == sample.copies_value,
           "#3773: real sample COPIES update should persist the memo-backed setting");
    ...
    expect(after_set_primary_bytes != original_primary_bytes,
           "#3773: real sample COPIES update should change the primary asset bytes");
    expect(after_set_sidecar_bytes != original_sidecar_bytes,
           "#3773: real sample COPIES update should change the memo sidecar bytes");
    ...
    if (reopen_after_set.exit_code != 0) {
        std::cerr << "studio host sample COPIES reopen stdout:\n" << reopen_after_set.stdout_text << "\n";
        std::cerr << "studio host sample COPIES reopen stderr:\n" << reopen_after_set.stderr_text << "\n";
    }
    ...
    const auto clear_copies_process = run_process_capture(
        studio_host_path,
        {
            "--path", copied_primary.string(),
            "--clear-property",
        ...
        temp_root);
    if (clear_copies_process.exit_code != 0) {
        std::cerr << "studio host sample COPIES clear stdout:\n" << clear_copies_process.stdout_text << "\n";
        std::cerr << "studio host sample COPIES clear stderr:\n" << clear_copies_process.stderr_text << "\n";
    }
    expect(clear_copies_process.exit_code == 0, "#3773: real sample COPIES clear should succeed");

    const auto cleared_copies_property = copperfin::vfp::query_visual_object_property({
        .path = copied_primary.string(),
        .record_index = 0U,
        .object_name = {},
        .unique_id = {},
    ...
    expect(cleared_copies_property.ok &&
               !cleared_copies_property.exists &&
               cleared_copies_property.value.empty(),
           "#3773: real sample COPIES clear should restore the missing-setting state");
    ...
    expect(cleared_primary_bytes != after_set_primary_bytes,
           "#3773: real sample COPIES clear should change the primary asset bytes again");
    expect(cleared_sidecar_bytes != after_set_sidecar_bytes,
           "#3773: real sample COPIES clear should change the memo sidecar bytes again");
    expect(cleared_primary_bytes != original_primary_bytes,
           "#3773: real sample COPIES clear should not rewind the primary bytes to the original sample");
    expect(cleared_sidecar_bytes != original_sidecar_bytes,
           "#3773: real sample COPIES clear should not rewind the sidecar bytes to the original sample");
    ...
    if (reopen_after_clear.exit_code != 0) {
        std::cerr << "studio host sample COPIES clear reopen stdout:\n" << reopen_after_clear.stdout_text << "\n";
        std::cerr << "studio host sample COPIES clear reopen stderr:\n" << reopen_after_clear.stderr_text << "\n";
    }
    ...
    if (!selected_settings.empty()) {
        expect_not_contains(selected_settings,
                            "\"name\": \"COPIES\"",
                            "#3773: real sample COPIES clear should remove COPIES from selected settings");
    }
}

void test_real_vfp9_report_and_label_copies_round_trip(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path reports_root = copperfin::test_support::find_vfp9_reports_root();
    if (reports_root.empty()) {
        std::cerr << "SKIP: #3773 real VFP9 report samples were not found\n";
        return;
    }

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_real_vfp9_copies_round_trip_tests";
    ...
    exercise_real_sample_copies_round_trip(
        studio_host_path,
        temp_root,
        {
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "usage: test_studio_host_real_sample_copies_round_trip <studio-host>\n";
        return 1;
    }

    test_real_vfp9_report_and_label_copies_round_trip(argv[1]);
