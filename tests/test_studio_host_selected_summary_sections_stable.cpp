...
void write_synthetic_report_table_for_stable_summary_section_json(
    ...
    expect(create_result.ok, "#2806: stable summary section fixture should be created");
}

void run_summary_section_stable_selection(
...
void run_deleted_summary_section_stable_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
...
void test_studio_host_json_preserves_selected_summary_sections_stable_selection(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_summary_sections_stable_tests";
    ...
    run_summary_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_summary_section_stable.frx",
        "report",
        "#2806: stable-selected live report summary section selection");
    run_summary_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_summary_section_stable.lbx",
        "label",
        "#2806: stable-selected live label summary section selection");
    run_deleted_summary_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_summary_section_stable.frx",
        "report",
        "#2806: stable-selected deleted report summary section selection");
    run_deleted_summary_section_stable_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_summary_section_stable.lbx",
        "label",
        "#2806: stable-selected deleted label summary section selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_summary_sections_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_summary_sections_stable_selection(argv[1]);
