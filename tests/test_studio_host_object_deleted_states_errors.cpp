...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2747: deleted-states rollback layout fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2747: deleted-states rollback layout fixture should seed deleted rows");
}
...
void run_deleted_states_rollback_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_deleted_states_rollback" + extension);
    ...
    if (rollback_process.exit_code != 4) {
        std::cerr << "studio host deleted-states rollback " << extension << " stdout:\n"
                  << rollback_process.stdout_text << "\n";
        std::cerr << "studio host deleted-states rollback " << extension << " stderr:\n"
    ...
    expect(!visual_object_deleted(asset_path, "field-guid") &&
           ...
           issue_prefix + " should roll back earlier object deletions");
...
void test_studio_host_json_preserves_object_deleted_states_errors(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_object_deleted_states_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_deleted_states_rollback_case(
            ...
            temp_root,
            extension,
            "#2747: object deleted-states rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_object_deleted_states_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_object_deleted_states_errors(argv[1]);
