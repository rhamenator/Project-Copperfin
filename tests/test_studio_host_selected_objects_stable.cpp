...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2802: selected object layout fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2802: selected object layout fixture should mark deleted objects");
}
...
void write_synthetic_report_table_for_stable_deleted_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(unique_id_result.ok, "#2802: stable deleted layout fixture should seed a deleted object unique id");
    expect(dbf_record_deleted(report_path, 6U),
           "#2802: stable deleted layout fixture should preserve the deleted object state");
}

void run_live_object_selection(
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose selected object geometry and provenance");
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose the containing live-section metadata");
}

void run_deleted_object_selection(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& file_name,
    const std::string& label,
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should expose selected deleted object geometry with section metadata");
...
void test_studio_host_json_preserves_selected_objects_stable_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_selected_objects_stable_tests";
    ...
    run_live_object_selection(
        studio_host_path,
        temp_root,
        "selected_object_stable.frx",
        "report",
        "#2802: stable live report object selection");
    run_live_object_selection(
        studio_host_path,
        temp_root,
        "selected_object_stable.lbx",
        "label",
        "#2802: stable live label object selection");
    run_deleted_object_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_object_stable.frx",
        "report",
        "#2802: stable deleted report object selection");
    run_deleted_object_selection(
        studio_host_path,
        temp_root,
        "selected_deleted_object_stable.lbx",
        "label",
        "#2802: stable deleted label object selection");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_selected_objects_stable <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_selected_objects_stable_selection(argv[1]);
