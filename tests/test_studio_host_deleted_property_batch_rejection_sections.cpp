...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2737: batch rejection layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2737: batch rejection fixture should mark target rows deleted");
}
...
void run_batch_rejection_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::vector<std::string>& action_args,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("deleted_property_batch_rejection" + extension);
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host batch rejection reopen " << extension << " stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host batch rejection reopen " << extension << " stderr:\n"
...
void test_studio_host_json_preserves_deleted_property_batch_rejection_sections(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_property_batch_rejection_section_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_batch_rejection_case(
            ...
            "#2737: deleted property rename-batch rejection");
        run_batch_rejection_case(
            studio_host_path,
            temp_root,
            extension,
            ...
            "#2737: deleted property reorder-batch rejection");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_property_batch_rejection_sections <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_property_batch_rejection_sections(argv[1]);
