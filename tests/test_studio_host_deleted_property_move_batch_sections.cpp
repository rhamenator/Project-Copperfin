...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2735: move-batch layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2735: move-batch fixture should mark target rows deleted");
}
...
void run_deleted_property_move_batch_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension) {
    const std::filesystem::path asset_path = temp_root / ("deleted_property_move_batch" + extension);
    ...
    if (move_batch_process.exit_code != 0) {
        std::cerr << "studio host deleted property move-batch " << extension << " stdout:\n"
                  << move_batch_process.stdout_text << "\n";
        std::cerr << "studio host deleted property move-batch " << extension << " stderr:\n"
                  << move_batch_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }

    expect(move_batch_process.exit_code == 0, "#2735: deleted property move-batch execution should exit successfully");
    expect(visual_object_deleted(asset_path, "middle-field-guid") && visual_object_deleted(asset_path, "right-field-guid"),
           "#2735: deleted property move-batch should preserve the deleted state");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host deleted property move-batch reopen " << extension << " stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host deleted property move-batch reopen " << extension << " stderr:\n"
                  << reopen_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }

    expect(reopen_process.exit_code == 0, "#2735: deleted property move-batch reopen should exit successfully");
    if (extension == ".lbx") {
        expect_contains(reopen_process.stdout_text, "\"isLabel\": true",
                        "#2735: deleted property move-batch reopen should preserve label identity");
    }
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectAvailable\": true",
                    "#2735: deleted property move-batch reopen should keep the deleted target row selected");
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectSectionAvailable\": true",
                    "#2735: deleted property move-batch reopen should preserve containing-section availability");
    expect_contains_in_order(
        ...
        "#2735: deleted property move-batch reopen should preserve target deleted-row containing-section metadata");
    expect_contains_in_order(
        reopen_process.stdout_text,
        {
        ...
        "#2735: deleted property move-batch reopen should expose containing detail-band metadata");
}

void test_studio_host_json_preserves_deleted_property_move_batch_sections(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_property_move_batch_section_tests";
    ...
    run_deleted_property_move_batch_case(studio_host_path, temp_root, ".frx");
    run_deleted_property_move_batch_case(studio_host_path, temp_root, ".lbx");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_property_move_batch_sections <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_property_move_batch_sections(argv[1]);
