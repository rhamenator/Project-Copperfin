...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2792: stable section delete layout fixture should be created");
}
...
void write_synthetic_report_table_for_stable_section_json(const std::filesystem::path& report_path) {
    ...
    expect(unique_id_result.ok, "#2792: stable section fixture should seed a section unique id");
    expect(!dbf_record_deleted(report_path, 1U),
           "#2792: stable section fixture should preserve the live section state");
}
...
void run_section_delete_case(
    ...
    expect_contains(delete_process.stdout_text, "\"containingSectionId\": \"section-guid\"",
...
void test_studio_host_json_preserves_section_delete_stable_selection(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_section_delete_stable_selection_tests";
    ...
    run_section_delete_case(
        ...
        "report",
        "#2792: stable-selected report section delete");
    run_section_delete_case(
        studio_host_path,
        temp_root,
        "section_delete_stable.lbx",
        "label",
        "#2792: stable-selected label section delete");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_section_delete_stable_selection <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_section_delete_stable_selection(argv[1]);
