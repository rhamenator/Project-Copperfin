...
void test_report_label_spacing_provenance() {
    ...
    document.display_name = "spacing.lbx";
    ...
    document.table_preview.records = {
        ...
        },
        {
            ...
            .values = {
                value("OBJTYPE", "5"),
                value("EXPR", "Deleted customer name"),
                value("SPACING", "2", 627U),
                value("HPOS", "900.000"),
                value("VPOS", "200.000"),
                value("WIDTH", "800.000"),
                value("HEIGHT", "600.000")
    ...
    expect(layout.available && layout.sections.size() == 1U && layout.sections[0].objects.size() == 3U,
        "#4527: label and expression objects should remain in their containing section");
    if (layout.sections.size() == 1U && layout.sections[0].objects.size() == 3U) {
        ...
        const auto live_highlight = std::find_if(
            ...
            [](const auto& highlight) {
                return highlight.name == "SPACING";
            });
        expect(live_highlight != live.highlights.end(),
            "#4527: live label highlights should include SPACING");
        if (live_highlight != live.highlights.end()) {
            expect(live_highlight->value == "1" &&
                   live_highlight->field_index == 2U &&
                   live_highlight->memo_block_number == 626U,
                "#4527: live SPACING should preserve value and provenance");
        }
        ...
        expect(std::none_of(
            ...
            "#4527: blank SPACING should remain an observable blank without a fabricated highlight");
        ...
        expect(std::none_of(
            ...
            "#4527: expression SPACING should not be treated as a label-only contract");
    }
    ...
    expect(deleted != layout.deleted_objects.end(),
        "#4527: deleted label objects should remain available for inspection");
    if (deleted != layout.deleted_objects.end()) {
        ...
        expect(deleted_highlight != deleted->highlights.end() &&
               ...
               deleted_highlight->memo_block_number == 627U,
            "#4527: deleted SPACING should preserve value and provenance");
    }
}
