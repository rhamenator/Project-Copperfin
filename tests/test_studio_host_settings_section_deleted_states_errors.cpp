...
void write_synthetic_report_table_for_layout_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2749: deleted-states rollback layout fixture should be created");

    const auto delete_result = copperfin::vfp::set_record_deleted_flag(report_path.string(), 6U, true);
    expect(delete_result.ok, "#2749: deleted-states rollback layout fixture should seed deleted rows");
}
...
void write_synthetic_report_table_for_stable_settings_and_section_json(
    ...
    expect(settings_unique_id_result.ok,
           "#2749: settings+section rollback fixture should seed a settings unique id");
    ...
    expect(section_unique_id_result.ok,
           "#2749: settings+section rollback fixture should seed a section unique id");
    expect(!dbf_record_deleted(report_path, 0U) && !dbf_record_deleted(report_path, 1U),
           "#2749: settings+section rollback fixture should preserve live settings and section rows");
}

void run_deleted_states_rollback_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension,
    const std::string& issue_prefix) {
    const std::filesystem::path asset_path = temp_root / ("settings_section_deleted_states_rollback" + extension);
    ...
    if (rollback_process.exit_code != 4) {
        std::cerr << "studio host settings+section deleted-states rollback " << extension << " stdout:\n"
                  << rollback_process.stdout_text << "\n";
        std::cerr << "studio host settings+section deleted-states rollback " << extension << " stderr:\n"
    ...
    expect(!dbf_record_deleted(asset_path, 0U) && !dbf_record_deleted(asset_path, 1U),
           issue_prefix + " should roll back earlier settings and section deletions");
...
void test_studio_host_json_preserves_settings_section_deleted_states_errors(
    const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_settings_section_deleted_states_error_tests";
    ...
    for (const auto& extension : {std::string(".frx"), std::string(".lbx")}) {
        run_deleted_states_rollback_case(
            studio_host_path,
            temp_root,
            extension,
            "#2749: settings+section deleted-states rollback");
    }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_settings_section_deleted_states_errors <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_settings_section_deleted_states_errors(argv[1]);
