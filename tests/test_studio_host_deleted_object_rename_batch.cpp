...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2763: deleted rename-batch success layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2763: deleted rename-batch success fixture should mark target rows deleted");
}

void run_deleted_rename_batch_success_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("deleted_object_rename_batch" + extension);
    ...
    if (rename_batch_process.exit_code != 0) {
        std::cerr << "studio host " << label << " deleted rename-batch stdout:\n"
                  << rename_batch_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " deleted rename-batch stderr:\n"
    ...
    expect(visual_object_count(asset_path) == before_count &&
           ...
           issue_prefix + " should replace deleted-row identities without changing order");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host " << label << " deleted rename-batch reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " deleted rename-batch reopen stderr:\n"
    ...
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectAvailable\": true",
                    issue_prefix + " should select the renamed deleted row");
...
void test_studio_host_json_preserves_deleted_object_rename_batch(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root =
        fs::temp_directory_path() / "copperfin_studio_host_deleted_object_rename_batch_tests";
    ...
    run_deleted_rename_batch_success_case(
        studio_host_path,
        temp_root,
        ".frx",
        "deleted_object_rename_batch.frx",
        "report",
        "#2763: deleted report/label rename-batch success");
    run_deleted_rename_batch_success_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "deleted_object_rename_batch.lbx",
        "label",
        "#2763: deleted report/label rename-batch success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_object_rename_batch <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_object_rename_batch(argv[1]);
