...
namespace copperfin::runtime_surface_tests
{
    void test_inherited_external_base_object_block_deeper_external_child_external_base_provenance_stays_coherent()
    {
        ...
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_external_base_inherited_object_block_deeper_external_child_external_base_provenance";
        ...
        const fs::path main_path = temp_root / "external_base_inherited_object_block_deeper_external_child_external_base_provenance.prg";
        write_text(
            main_path,
        ...
        expect(state.completed,
               std::string("external-base inherited object-block deeper external child external-base provenance script should complete: ") + state.message +
        ...
        if (state.ole_objects.size() == 5U)
        {
            ...
            expect(create_parent.prog_id == "ChildForm",
                   "external-base inherited object-block deeper external child external-base provenance should preserve CREATEOBJECT parent identity");
            expect(create_parent.source == main_path.string(),
                   "external-base inherited object-block deeper external child external-base provenance should preserve CREATEOBJECT parent source");
            expect(create_child.prog_id == "SaveButton",
                   "external-base inherited object-block deeper external child external-base provenance should preserve CREATEOBJECT child identity");
            expect(create_child.base_class_name == "ParentButton",
                   "external-base inherited object-block deeper external child external-base provenance should preserve immediate child base-class identity");
            expect(create_child.source == widget_library_path.string(),
                   "external-base inherited object-block deeper external child external-base provenance should preserve the defining child-class source path");
            expect(create_child.class_library == button_library_path.string(),
                   "external-base inherited object-block deeper external child external-base provenance should preserve the immediate external ClassLibrary path");
            expect(create_child.class_hierarchy.size() == 5U,
                   "external-base inherited object-block deeper external child external-base provenance should preserve the deeper runtime child class hierarchy");
            expect(!create_child.properties.contains("classlibrary"),
                   "external-base inherited object-block deeper external child external-base provenance should not materialize a CREATEOBJECT child ClassLibrary shadow");
            expect(leaf_parent.prog_id == "LeafForm",
                   "external-base inherited object-block deeper external child external-base provenance should preserve NEWOBJECT leaf identity");
            expect(leaf_parent.source == main_path.string(),
                   "external-base inherited object-block deeper external child external-base provenance should preserve NEWOBJECT leaf source");
            expect(leaf_child.prog_id == "SaveButton",
                   "external-base inherited object-block deeper external child external-base provenance should preserve leaf child identity");
            expect(leaf_child.base_class_name == "ParentButton",
                   "external-base inherited object-block deeper external child external-base provenance should preserve the leaf child immediate external base");
            expect(leaf_child.source == widget_library_path.string(),
                   "external-base inherited object-block deeper external child external-base provenance should preserve the leaf child defining source path");
            expect(leaf_child.class_library == button_library_path.string(),
                   "external-base inherited object-block deeper external child external-base provenance should preserve the leaf child immediate external ClassLibrary path");
            expect(!leaf_child.properties.contains("classlibrary"),
                   "external-base inherited object-block deeper external child external-base provenance should not materialize a leaf child ClassLibrary shadow");
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM NEWOBJECT should remain stable while external-base inherited object-block deeper external child external-base provenance lands");
        }
        ...
        expect(has_addobject_event,
               "external-base inherited object-block deeper external child external-base provenance should emit child materialization events");
        ...
        expect(has_save_invoke_event,
               "external-base inherited object-block deeper external child external-base provenance should keep child owner dispatch usable after ordinary reads and reflection");
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_external_base_inherited_object_block_deeper_external_child_external_base_dotted";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "external_base_inherited_object_block_deeper_external_child_external_base_dotted.prg";
        write_text(
            main_path,
            "oCreate = CREATEOBJECT('ChildForm')\n"
            "oLeaf = NEWOBJECT('LeafForm')\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("external-base inherited object-block deeper external child external-base dotted script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        expect(state.ole_objects.size() == 5U,
               "external-base inherited object-block deeper external child external-base dotted should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            const auto &create_parent = state.ole_objects[0];
            const auto &create_child = state.ole_objects[1];
            ...
            const auto &leaf_child = state.ole_objects[3];
            expect(create_parent.prog_id == "ChildForm",
                   "external-base inherited object-block deeper external child external-base dotted should preserve CREATEOBJECT parent identity");
            expect(create_parent.source == main_path.string(),
                   "external-base inherited object-block deeper external child external-base dotted should preserve CREATEOBJECT parent source");
            expect(create_child.prog_id == "SaveButton",
                   "external-base inherited object-block deeper external child external-base dotted should preserve CREATEOBJECT child identity");
            expect(create_child.base_class_name == "ParentButton",
                   "external-base inherited object-block deeper external child external-base dotted should preserve CREATEOBJECT immediate child base-class identity");
            expect(create_child.source == widget_library_path.string(),
                   "external-base inherited object-block deeper external child external-base dotted should preserve the CREATEOBJECT defining child-class source path");
            expect(create_child.class_library == button_library_path.string(),
                   "external-base inherited object-block deeper external child external-base dotted should preserve the CREATEOBJECT immediate external ClassLibrary path");
            expect(create_child.class_hierarchy.size() == 5U,
                   "external-base inherited object-block deeper external child external-base dotted should preserve CREATEOBJECT class hierarchy");
            expect(leaf_parent.prog_id == "LeafForm",
                   "external-base inherited object-block deeper external child external-base dotted should preserve NEWOBJECT leaf identity");
            expect(leaf_parent.source == main_path.string(),
                   "external-base inherited object-block deeper external child external-base dotted should preserve NEWOBJECT leaf source");
            expect(leaf_child.prog_id == "SaveButton",
                   "external-base inherited object-block deeper external child external-base dotted should preserve leaf child identity");
            expect(leaf_child.base_class_name == "ParentButton",
                   "external-base inherited object-block deeper external child external-base dotted should preserve leaf immediate child base-class identity");
            expect(leaf_child.source == widget_library_path.string(),
                   "external-base inherited object-block deeper external child external-base dotted should preserve the leaf defining child-class source path");
            expect(leaf_child.class_library == button_library_path.string(),
                   "external-base inherited object-block deeper external child external-base dotted should preserve the leaf immediate external ClassLibrary path");
            expect(leaf_child.class_hierarchy.size() == 5U,
                   "external-base inherited object-block deeper external child external-base dotted should preserve leaf class hierarchy");
            ...
            if (create_parent_caption != create_parent.properties.end())
            {
                expect(copperfin::runtime::format_value(create_parent_caption->second) == "Done-Saved",
                       "external-base inherited object-block deeper external child external-base dotted should let dotted child method calls update the CREATEOBJECT owner");
            }
            else
            {
                expect(false, "external-base inherited object-block deeper external child external-base dotted should preserve the CREATEOBJECT owner caption");
            }
            if (create_child_caption != create_child.properties.end())
            {
                expect(copperfin::runtime::format_value(create_child_caption->second) == "Go",
                       "external-base inherited object-block deeper external child external-base dotted should preserve the CREATEOBJECT child dotted assignment");
            }
            else
            {
                expect(false, "external-base inherited object-block deeper external child external-base dotted should preserve the CREATEOBJECT child caption");
            }
            if (leaf_parent_caption != leaf_parent.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_parent_caption->second) == "Ready-Saved",
                       "external-base inherited object-block deeper external child external-base dotted should let dotted child method calls update the leaf owner");
            }
            else
            {
                expect(false, "external-base inherited object-block deeper external child external-base dotted should preserve the leaf owner caption");
            }
            if (leaf_child_caption != leaf_child.properties.end())
            {
                expect(copperfin::runtime::format_value(leaf_child_caption->second) == "Ship",
                       "external-base inherited object-block deeper external child external-base dotted should preserve the leaf child dotted assignment");
            }
            else
            {
                expect(false, "external-base inherited object-block deeper external child external-base dotted should preserve the leaf child caption");
            }
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM NEWOBJECT should remain stable while external-base inherited object-block deeper external child external-base dotted lands");
        }
        ...
        expect(has_addobject_event,
               "external-base inherited object-block deeper external child external-base dotted should emit child materialization events");
        ...
        expect(has_owner_caption_invoke_event,
               "external-base inherited object-block deeper external child external-base dotted should dispatch direct dotted child method calls");
        ...
        expect(has_save_invoke_event,
               "external-base inherited object-block deeper external child external-base dotted should keep child THISFORM owner dispatch stable");
    ...
    {
        namespace fs = std::filesystem;
        const fs::path temp_root = fs::temp_directory_path() / "copperfin_external_base_inherited_object_block_deeper_external_child_external_base_mutation";
        std::error_code ignored;
        fs::remove_all(temp_root, ignored);
        fs::create_directories(temp_root);
        ...
        const fs::path main_path = temp_root / "external_base_inherited_object_block_deeper_external_child_external_base_mutation.prg";
        write_text(
            main_path,
            "oCreate = CREATEOBJECT('ChildForm')\n"
            "oLeaf = NEWOBJECT('LeafForm')\n"
        ...
        const auto state = session.run(copperfin::runtime::DebugResumeAction::continue_run);
        expect(state.completed,
               std::string("external-base inherited object-block deeper external child external-base mutation script should complete: ") + state.message +
                   " @line=" + std::to_string(state.location.line));
        ...
        expect(state.ole_objects.size() == 5U,
               "external-base inherited object-block deeper external child external-base mutation should register CREATEOBJECT parent/child, NEWOBJECT parent/child, and COM objects");
        if (state.ole_objects.size() == 5U)
        {
            const auto &create_parent = state.ole_objects[0];
            const auto &create_child = state.ole_objects[1];
            ...
            const auto &leaf_child = state.ole_objects[3];
            expect(create_parent.prog_id == "ChildForm",
                   "external-base inherited object-block deeper external child external-base mutation should preserve CREATEOBJECT parent identity");
            expect(create_child.prog_id == "SaveButton",
                   "external-base inherited object-block deeper external child external-base mutation should preserve CREATEOBJECT child identity");
            expect(create_child.base_class_name == "ParentButton",
                   "external-base inherited object-block deeper external child external-base mutation should preserve CREATEOBJECT immediate child base-class identity");
            expect(create_child.source == widget_library_path.string(),
                   "external-base inherited object-block deeper external child external-base mutation should preserve the CREATEOBJECT defining child-class source path");
            expect(create_child.class_library == button_library_path.string(),
                   "external-base inherited object-block deeper external child external-base mutation should preserve the CREATEOBJECT immediate external ClassLibrary path");
            expect(create_child.class_hierarchy.size() == 5U,
                   "external-base inherited object-block deeper external child external-base mutation should preserve CREATEOBJECT class hierarchy");
            expect(!create_child.properties.contains("class"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a CREATEOBJECT child Class shadow");
            expect(!create_child.properties.contains("baseclass"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a CREATEOBJECT child BaseClass shadow");
            expect(!create_child.properties.contains("parentclass"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a CREATEOBJECT child ParentClass shadow");
            expect(!create_child.properties.contains("classlibrary"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a CREATEOBJECT child ClassLibrary shadow");
            expect(leaf_parent.prog_id == "LeafForm",
                   "external-base inherited object-block deeper external child external-base mutation should preserve NEWOBJECT leaf identity");
            expect(leaf_child.prog_id == "SaveButton",
                   "external-base inherited object-block deeper external child external-base mutation should preserve leaf child identity");
            expect(leaf_child.base_class_name == "ParentButton",
                   "external-base inherited object-block deeper external child external-base mutation should preserve leaf immediate child base-class identity");
            expect(leaf_child.source == widget_library_path.string(),
                   "external-base inherited object-block deeper external child external-base mutation should preserve the leaf defining child-class source path");
            expect(leaf_child.class_library == button_library_path.string(),
                   "external-base inherited object-block deeper external child external-base mutation should preserve the leaf immediate external ClassLibrary path");
            expect(leaf_child.class_hierarchy.size() == 5U,
                   "external-base inherited object-block deeper external child external-base mutation should preserve leaf class hierarchy");
            expect(!leaf_child.properties.contains("class"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a leaf child Class shadow");
            expect(!leaf_child.properties.contains("baseclass"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a leaf child BaseClass shadow");
            expect(!leaf_child.properties.contains("parentclass"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a leaf child ParentClass shadow");
            expect(!leaf_child.properties.contains("classlibrary"),
                   "external-base inherited object-block deeper external child external-base mutation should not materialize a leaf child ClassLibrary shadow");
            expect(state.ole_objects[4].prog_id == "Scripting.Dictionary",
                   "COM NEWOBJECT should remain stable while external-base inherited object-block deeper external child external-base mutation lands");
        }
        ...
        expect(has_save_invoke_event,
               "external-base inherited object-block deeper external child external-base mutation should keep child owner dispatch usable after rejected mutations");
