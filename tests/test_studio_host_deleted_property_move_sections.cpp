...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2734: move layout fixture should be created");
}
...
void mark_deleted(const std::filesystem::path& asset_path, const std::string& unique_id) {
    ...
    expect(delete_result.ok, "#2734: move fixture should mark target rows deleted");
}
...
void run_deleted_property_move_case(
    const std::string& studio_host_path,
    const std::filesystem::path& temp_root,
    const std::string& extension) {
    const std::filesystem::path asset_path = temp_root / ("deleted_property_move" + extension);
    ...
    if (move_process.exit_code != 0) {
        std::cerr << "studio host deleted property move " << extension << " stdout:\n"
                  << move_process.stdout_text << "\n";
        std::cerr << "studio host deleted property move " << extension << " stderr:\n"
                  << move_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }

    expect(move_process.exit_code == 0, "#2734: deleted property move execution should exit successfully");
    expect(visual_object_deleted(asset_path, "middle-field-guid") && visual_object_deleted(asset_path, "right-field-guid"),
           "#2734: deleted property move should preserve the deleted state");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host deleted property move reopen " << extension << " stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host deleted property move reopen " << extension << " stderr:\n"
                  << reopen_process.stderr_text << "\n";
        std::cerr << "fixture root: " << temp_root << "\n";
    }

    expect(reopen_process.exit_code == 0, "#2734: deleted property move reopen should exit successfully");
    if (extension == ".lbx") {
        expect_contains(reopen_process.stdout_text, "\"isLabel\": true",
                        "#2734: deleted property move reopen should preserve label identity");
    }
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectAvailable\": true",
                    "#2734: deleted property move reopen should keep the deleted target row selected");
    expect_contains(reopen_process.stdout_text, "\"selectedReportObjectSectionAvailable\": true",
                    "#2734: deleted property move reopen should preserve containing-section availability");
    expect_contains_in_order(
        ...
        "#2734: deleted property move reopen should preserve target deleted-row containing-section metadata");
    expect_contains_in_order(
        reopen_process.stdout_text,
        {
        ...
        "#2734: deleted property move reopen should expose containing detail-band metadata");
}

void test_studio_host_json_preserves_deleted_property_move_sections(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_deleted_property_move_section_tests";
    ...
    run_deleted_property_move_case(studio_host_path, temp_root, ".frx");
    run_deleted_property_move_case(studio_host_path, temp_root, ".lbx");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_deleted_property_move_sections <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_deleted_property_move_sections(argv[1]);
