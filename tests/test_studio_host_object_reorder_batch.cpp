...
void write_synthetic_report_table_for_layout_reorder_json(const std::filesystem::path& report_path) {
    ...
    expect(create_result.ok, "#2766: reorder-batch success layout fixture should be created");
}

void run_reorder_batch_success_case(
    ...
    const std::filesystem::path asset_path = temp_root / ("object_reorder_batch" + extension);
    ...
    if (reorder_batch_process.exit_code != 0) {
        std::cerr << "studio host " << label << " reorder-batch stdout:\n"
                  << reorder_batch_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " reorder-batch stderr:\n"
    ...
    expect(visual_object_order(asset_path) == "right-field-guid,middle-field-guid,left-field-guid",
           issue_prefix + " should apply ordered stable-selector moves");
    ...
    if (reopen_process.exit_code != 0) {
        std::cerr << "studio host " << label << " reorder-batch reopen stdout:\n"
                  << reopen_process.stdout_text << "\n";
        std::cerr << "studio host " << label << " reorder-batch reopen stderr:\n"
    ...
    expect_contains_in_order(
        ...
        issue_prefix + " should refresh selected reordered object metadata after reopen");
}

void test_studio_host_json_preserves_object_reorder_batch(const std::string& studio_host_path) {
    namespace fs = std::filesystem;

    const fs::path temp_root = fs::temp_directory_path() / "copperfin_studio_host_object_reorder_batch_tests";
    ...
    run_reorder_batch_success_case(
        studio_host_path,
        temp_root,
        ".frx",
        "object_reorder_batch.frx",
        "report",
        "#2766: report/label reorder-batch success");
    run_reorder_batch_success_case(
        studio_host_path,
        temp_root,
        ".lbx",
        "object_reorder_batch.lbx",
        "label",
        "#2766: report/label reorder-batch success");
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: test_studio_host_object_reorder_batch <studio_host_path>\n";
        return 1;
    }

    test_studio_host_json_preserves_object_reorder_batch(argv[1]);
