...
- Python now has one practical out-of-process sidecar workflow, not a general embedded runtime. Copperfin admits the interpreter and the exact script separately, binds the script to its command-line position, launches with isolated startup and an explicit environment, and lets ordinary PRG control the leaf through the existing polyglot contract. R and broader language adapters remain planning surfaces.
