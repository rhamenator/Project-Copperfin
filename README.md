...
- Current .NET support is an early modernization path, not a general managed-runtime surface: the build host can publish a generated C# launcher/stub that the native runtime pipeline starts as a child process, while generated C# transpilation output is only an emitted artifact today. The portable route executor composes admitted executable identity, deterministic request serialization, one bounded stdin/stdout/stderr attempt, strict response admission, routing/fallback, outcome reporting, and migration telemetry. It does not identify or host a managed runtime.
- Polyglot capability use does not require foreign-language syntax inside FP/VFP source files. The host-injected `CFPOLYGLOTDISPATCH()` boundary accepts a canonical capability, bounded JSON-object arguments, and a deterministic route sample, then returns invariant JSON evidence without exposing mutable runtime state. PRG can place that call in `SPAWN` and supervise it through nonblocking `CFTASKSTATUS()`, `CFTASKCANCEL()`, `CFTASKRESULT()`, and `CFTASKOUTPUT()` calls. The trusted portable runtime-host composition can supply this callback from an explicit validated route registry and admitted-artifact bindings; unconfigured dispatch still fails closed, and no language-specific runtime is bundled or discovered.
