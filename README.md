...
- Version 1 is anchored on Visual FoxPro 9 compatibility. VFP 6, VFP 7, and VFP 8 assets may work through shared DBF/FPT/CDX/DBC readers, but they are best-effort and untested rather than separate targets. Interpreting older Fox Software and xBase-family assets (issue #1076) and producing binaries for those products (issue #1077) are both required Version 1 scope, not a later release. Database/container interchange, the Access migration bridge, and a bounded .NET execution/interop bridge (issues #137-141, #139) are likewise required Version 1 scope, not deferred work.
- Current .NET support is an early modernization path, not a general managed-runtime surface. In addition to the generated launcher and Windows-only bounded .NET Framework `DECLARE` path, Copperfin now includes a real cross-platform C# leaf-candidate template. The template publishes as one self-contained Native AOT executable; the portable host admits its exact SHA-256, invokes it once through bounded stdin/stdout/stderr, strictly validates its response, and returns the result through the existing route and telemetry contracts. Copperfin does not discover a CLR, load arbitrary assemblies, or execute emitted C# transpilation output.
- Polyglot capability use does not require foreign-language syntax inside FP/VFP source files. The host-injected `CFPOLYGLOTDISPATCH()` boundary accepts a canonical capability, bounded JSON-object arguments, and a deterministic route sample, then returns invariant JSON evidence without exposing mutable runtime state. PRG can place that call in `SPAWN` and supervise it through nonblocking `CFTASKSTATUS()`, `CFTASKCANCEL()`, `CFTASKRESULT()`, and `CFTASKOUTPUT()` calls. The trusted portable runtime-host composition supplies this callback from an explicit validated route registry and admitted-artifact bindings; unconfigured dispatch still fails closed. Checked-in .NET, Python, and R leaf samples exercise this boundary; Copperfin does not bundle or discover any of those language runtimes.
...
- [`docs/27-native-test-isolation.md`](docs/27-native-test-isolation.md)
- [`docs/28-repository-ontology.md`](docs/28-repository-ontology.md)
- [`docs/29-package-trust-contract.md`](docs/29-package-trust-contract.md)
- [`docs/30-runtime-stub-inventory.md`](docs/30-runtime-stub-inventory.md)
- [`docs/31-specification-compliance-gap-analysis.md`](docs/31-specification-compliance-gap-analysis.md)
...
- [`docs/50-portable-public-path-boundary.md`](docs/50-portable-public-path-boundary.md)
- [`docs/51-portable-public-environment-boundary.md`](docs/51-portable-public-environment-boundary.md)
- [`docs/52-portable-executable-search-default.md`](docs/52-portable-executable-search-default.md)
- [`docs/53-private-sqlite-native-api-boundary.md`](docs/53-private-sqlite-native-api-boundary.md)
- [`docs/54-portable-clr-host-boundary.md`](docs/54-portable-clr-host-boundary.md)
- [`docs/55-native-declare-loader-boundary.md`](docs/55-native-declare-loader-boundary.md)
- [`docs/56-native-declare-invocation-boundary.md`](docs/56-native-declare-invocation-boundary.md)
- [`docs/57-portable-printer-shell-boundary.md`](docs/57-portable-printer-shell-boundary.md)
- [`docs/58-portable-file-version-boundary.md`](docs/58-portable-file-version-boundary.md)
- [`docs/59-portable-code-page-boundary.md`](docs/59-portable-code-page-boundary.md)
- [`docs/60-portable-disk-space-boundary.md`](docs/60-portable-disk-space-boundary.md)
- [`docs/61-portable-exclusive-file-boundary.md`](docs/61-portable-exclusive-file-boundary.md)
- [`docs/62-portable-file-stream-boundary.md`](docs/62-portable-file-stream-boundary.md)
- [`docs/63-portable-font-directory-boundary.md`](docs/63-portable-font-directory-boundary.md)
- [`docs/64-workspace-agent-access-policy.md`](docs/64-workspace-agent-access-policy.md)
  — POSIX/macOS/Windows private-executable materialization, retention, and
  bounded launch policy for the workspace agent, including the macOS
  code-signed retained-image transition
...
- [`docs/70-foxbase-0xfb-investigation.md`](docs/70-foxbase-0xfb-investigation.md)
