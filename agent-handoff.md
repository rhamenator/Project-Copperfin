...
- Last shipped localization child: `#2618`, the remaining `Studio.ToolboxDispatch.CatalogEntry.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused context-editor-actions coverage now proves translated and pseudo-localized catalog-entry errors without changing toolbox item ids, selection-context tokens, command tokens, execution flags, or other machine-readable toolbox-dispatch contracts.
