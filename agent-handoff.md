...
- Last shipped localization child: `#2724`, `apps/copperfin_studio_host/main.cpp` no longer carries raw English fallback labels for the catalog-backed object-assignment usage lines, so pseudo-localized and non-English usage output now depends directly on the shipped `StudioHost.LaunchParse.ObjectAssignment.*` catalogs in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`; direct qps-ploc host verification now proves the selected-back-color, OLE drop-mode, and WhatsThis help ID usage labels route through localization without changing CLI switches, invariant placeholders, or other machine-readable host contracts.
...
- Next action: continue live GitHub triage for `#2348` with a fresh runtime/host/native hard-coded-string audit now that the obvious `prg_engine_dispatch.inl` backlog and Studio host usage-label fallback cluster have been cleared; pick the next prompt-sized child issue from any remaining user-facing prose that still bypasses shipped catalogs, keep E3/#24 open for report/label fidelity, and preserve localization as a standing requirement for every new user-facing surface after the hard-coded backlog reaches zero.
