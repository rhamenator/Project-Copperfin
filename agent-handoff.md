...
- Last shipped slice: `#3279`, the next prompt-sized child under `#3217`, taught native PRG-defined objects to surface direct `Class` identity through `GETPEM()` / `PEMSTATUS()` as read-only metadata, with focused runtime-surface and control-flow regressions proving same-PRG and external-base native instances expose coherent direct class identity without disturbing the previously-shipped `ACLASS()` chain, `BaseClass` / `ClassLibrary` metadata, or COM/plain-object reflection.
...
- Next action: execute `#3280`, the next prompt-sized `#3217` child, so same-PRG and external-base native objects include `Class` / `BaseClass` / `ClassLibrary` coherently in `AMEMBERS(..., 1|3)` where applicable, without disturbing the newly-shipped `GETPEM()` / `PEMSTATUS()` metadata, `ACLASS()` chain, inherited declarative children, inherited `THIS.AddObject()`, helper-method resolution, `DODEFAULT()`, accessor surfaces, or the stack-frugal runtime frame machine.
