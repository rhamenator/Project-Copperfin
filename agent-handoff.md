...
- Last shipped localization child: `#2613`, the remaining `Studio.ToolboxDispatch.Execution.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused context-editor-actions coverage now proves translated and pseudo-localized execution errors without changing toolbox item ids, selection-context tokens, command tokens, execution flags, or other machine-readable toolbox-dispatch contracts.
