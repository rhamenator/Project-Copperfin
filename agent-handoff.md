...
- Last shipped E3 child: `#2784`, report/label record-selected settings memo success evidence; the focused `test_studio_host_settings_memo_record` host regression now proves FRX/LBX record-selected `EXPR` settings-memo update and clear success for live and deleted settings rows preserve the current page-setup/settings counts, selected-settings provenance, direct-field carrier behavior after clear, and full live/deleted preview and section/object metadata for both reports and labels, without changing production behavior.
