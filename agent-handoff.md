...
- Last shipped slice: `#3224`, the seventh prompt-sized child under `#3217`, now preserves `@` by-reference updates through same-PRG native object member calls and automatic native `Init` execution, and it also hardens the expression parser so inter-argument whitespace no longer prevents `@identifier` arguments from taking the by-reference path. Focused runtime-surface and control-flow regressions prove constructor-style `Init` write-back, native object method write-back, and existing COM object activation all remain stable under the same stack-frugal iterative PRG frame machine.
...
- Next action: execute `#3225`, the next prompt-sized `#3217` child, so same-PRG native `DEFINE CLASS ... AS BaseClass` activation starts applying inherited default properties and inherited methods without disturbing the existing stack-frugal PRG frame machine, constructor/by-reference behavior, or COM/plain-object activation.
