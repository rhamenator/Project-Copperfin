...
- Last shipped E3 child: `#2986`, real FRX duplicate reopen continuity; Studio document opens now load full DBF previews for report/label assets instead of truncating FRX/LBX table previews to eight rows, so a fresh read-only reopen preserves persisted real layout rows such as duplicated `by_author.FRX` objects and the shared VSIX/standalone designer model now sees the same report/label row set that host-backed mutation commands already returned.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2986` closed the real FRX duplicate reopen seam, `#2985` closed the real unplaced FRX placement-transition seam, `#2984` closed the real unplaced FRX continuity seam, `#2983` closed the real memo-backed write-back seam, `#2982` closed the shared property-grid direct-field seam, `#2981` closed the host-level real reload+undo seam, and `#2980` closed non-Windows real-asset smoke fallback resolution, prefer the next real-asset gap that still lacks evidence such as a real deleted FRX/LBX continuity fixture if one can be sourced, richer supported property families beyond single-row edits, or section-aware multi-row write-back proof where live repo state still shows a report/label gap, and treat the real-project debugger path as a separate Windows-first follow-up only if fresh evidence shows a real implementation seam rather than the current runtime build-execution environment limit.
