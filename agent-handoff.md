...
- Last shipped E3 child: `#3199`, the shared real `STYLE3V.FRX` host-backed live placement smoke now captures the source preview-bounds geometry and fails if `PREVIEWBOUNDS` drift while an unplaced object is moved into a live section, after reload, or after undo, so live placement coverage can no longer accept host-JSON preview regressions hidden behind correct VPOS, section continuity, and unplaced-object counts alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3199`, the host-backed real preview-metadata seams for deleted-object workflows plus live duplicate/rename/reorder/placement now have explicit real-asset evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, remaining live-object property or batch preview-metadata seams, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed live placement preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedPlacementRoundTrip` under the resolved local toolchain against real `STYLE3V.FRX`, proving source `PREVIEWBOUNDS` geometry stays stable through live placement, reload, and undo in the current native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
