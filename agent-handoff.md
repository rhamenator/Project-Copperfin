...
- Last shipped slice: `#3277`, the next prompt-sized child under `#3217`, taught native `ACLASS()` to return the real inheritance chain for same-PRG and external-PRG-base PRG-defined objects instead of the previous hard-coded `[class, OBJECT]` pair, with focused runtime-surface and control-flow regressions proving native instances now surface derived-to-base class identity coherently while COM/plain-object reflection remains stable.
...
- Next action: execute `#3278`, the next prompt-sized `#3217` child, so native PRG-defined objects surface coherent `BaseClass` identity for same-PRG inheritance and coherent `BaseClass` plus `ClassLibrary` identity for external-base instances through `GETPEM()` / `PEMSTATUS()` without disturbing the newly-shipped `ACLASS()` chain, existing `AMEMBERS()` reflection, inherited declarative children, inherited `THIS.AddObject()`, helper-method resolution, `DODEFAULT()`, accessor surfaces, or the stack-frugal runtime frame machine.
