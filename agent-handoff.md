...
- Last shipped localization child: `#2593`, the shared federation AI planner fallback wrapper now has shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused shipped-catalog plus runtime-host coverage proves localized planner-fallback prose without changing `runtime.mode: federation-query-plan`, exit codes, permission gating, unresolved machine-readable payload keys, or other runtime-host contract fields.
