...
- Last shipped localization child: `#2715`, the remaining direct hard-coded PRG `DECLARE DLL` missing-function/path, DLL-load, function-not-found, and non-Windows guard failures now route through `Runtime.Prg.Dispatch.Error.*` catalog entries with shipped parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization plus PRG runtime coverage now proves pseudo-localized DECLARE failures without changing PRG command syntax, DLL/function identifiers, downstream OS error payloads, or other machine-readable runtime contracts.
...
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, which is down to 14 direct `last_error_message` literals after `#2715`; the next likely slices are the top-level runtime-fault wrappers, the object-helper `SCATTER NAME`/object-assignment fallbacks, and the smaller residual precondition guards now that `APPEND FROM`, `SCATTER`, `GATHER`, `CREATE CURSOR/TABLE`, `ALTER TABLE`, `SET FILTER`, `DIMENSION/DECLARE`, `SLEEP`, `DECLARE DLL`, `COPY TO`, `SAVE TO` / `RESTORE FROM`, and file-operation failures are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
