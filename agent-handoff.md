...
- Last shipped slice: `#3445`, the next prompt-sized child under `#3217`, shipped as evidence closure: focused runtime-surface regressions now prove representative same-PRG native `DEFINE CLASS ... AS Separator ... ENDDEFINE` classes can be instantiated through `CREATEOBJECT()` and `NEWOBJECT()` with ordinary property initialization, method invocation, `BaseClass`/`Class` identity surfaces, native class-hierarchy storage, empty same-PRG `ClassLibrary` semantics, and stable COM behavior, without additional runtime changes.
...
- Next action: execute `#3446`, the next prompt-sized `#3217` child, to validate and, only if needed, harden representative same-PRG `DEFINE CLASS ... AS ToolBar ... ENDDEFINE` activation so `CREATEOBJECT()` and `NEWOBJECT()` can instantiate another builtin container/control base without regressing stack-frugal execution, native identity surfaces, localization constraints for new diagnostics, or the already-shipped same-PRG builtin-base native class runtime behavior.
