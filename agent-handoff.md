...
- Last shipped E3 child: `#3017`, shared side-margin settings parity; the focused VSIX/standalone designer smokes now prove live and deleted FRX/LBX root-settings selections surface `LEFTMARGIN` and `RIGHTMARGIN`, localize their display labels through `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and round-trip side-margin edits through the existing shared property-grid and Studio-host refresh paths without changing invariant property names, host JSON keys, or record/field provenance contracts.
