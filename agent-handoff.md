...
- Last shipped localization child: `#2716`, the remaining dispatch-layer out-of-memory, filesystem/system fault, generic exception, and unknown-exception wrappers now route through the existing `Runtime.Prg.Core.Error.*` catalog entries instead of hard-coded English, and focused PRG control-flow coverage now proves runtime-fault wrappers honor the active locale without changing exception detail propagation, event categories, pause reasons, or other machine-readable runtime contracts.
...
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, which is down to 12 direct `last_error_message` literals after `#2716`; the next likely slices are the object-helper `SCATTER NAME`/object-assignment fallbacks and the smaller residual precondition guards (`TEXT`, `TRY`, `REPLACE`, `UPDATE`, `INSERT INTO`, `UNLOCK RECORD`, and `SLEEP cancelled`) now that `APPEND FROM`, `SCATTER`, `GATHER`, `CREATE CURSOR/TABLE`, `ALTER TABLE`, `SET FILTER`, `DIMENSION/DECLARE`, `SLEEP` preconditions, `DECLARE DLL`, top-level runtime faults, `COPY TO`, `SAVE TO` / `RESTORE FROM`, and file-operation failures are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
