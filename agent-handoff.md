...
- Last shipped E3 child: `#2764`, report/label duplicate-batch success evidence; the focused `test_studio_host_object_duplicate_batch` host regression now proves stable FRX/LBX `--visual-object-duplicate-batch` success preserves the current batch result payload, appends selected duplicates without changing original rows, and keeps the refreshed reopen selection and preview metadata for both reports and labels, without changing production behavior.
