...
- Last shipped slice: `#3276`, the next prompt-sized child under `#3217`, closed the external-base method-reflection evidence gap: focused runtime-surface and control-flow regressions now prove inherited methods loaded from external PRG base libraries already remain visible through `GETPEM()`, `PEMSTATUS()`, and `AMEMBERS()` on derived instances, while follow-on inherited and derived method invocation stays stable.
...
- Next action: execute `#3277`, the next prompt-sized `#3217` child, so `ACLASS()` returns the actual native inheritance chain for same-PRG and external-PRG-base instances instead of the current hard-coded `[class, OBJECT]` shape, without disturbing existing `GETPEM()` / `PEMSTATUS()` / `AMEMBERS()` reflection, inherited declarative children, inherited `THIS.AddObject()`, helper-method resolution, `DODEFAULT()`, accessor surfaces, or the stack-frugal runtime frame machine.
