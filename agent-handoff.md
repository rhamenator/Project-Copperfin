...
- Last shipped E3 child: `#2744`, report/label object-reorder-batch rollback payload evidence; the stable FRX/LBX `--visual-object-reorder-batch` rollback regressions now pin `en-US` host locale state before asserting English missing-target prose, the broad `#1845` rollback assertions now track the current missing-target payload deterministically instead of drifting against leaked locale state, and the focused `test_studio_host_object_reorder_batch_errors` host regression now proves report/label reorder-batch rollback failures keep earlier reorder mutations rolled back while reporting the current missing-target JSON error for both reports and labels without changing production behavior.
