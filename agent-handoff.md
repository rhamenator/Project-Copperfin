...
- Last shipped localization child: `#2598`, the existing database federation profile connector, query-path, and guardrail keys under `Platform.Database.*` now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused platform-model coverage proves localized database profile prose without changing connector ids, connector family values, query-path ids, policy booleans, or other machine-readable profile contracts.
