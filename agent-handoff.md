...
- Last shipped localization child: `#2419`, Studio host direct toolbox batch-create parser diagnostics route through the portable C++ localization catalog while preserving CLI options, command names, toolbox-context tokens, toolbox item ids, JSON fields/status contracts, `name=value` syntax, and exit codes.
