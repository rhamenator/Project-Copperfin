...
- Last shipped E3 child: `#3060`, non-top real FRX/LBX sibling-row continuity evidence; shared host-backed and shared-editor real-asset section `HEIGHT` and grouped `EXPR` smokes now prove non-top section edits preserve untouched sibling rows for `STYLELBL.LBX` and `by_author.FRX` across update, reload, and undo, including grouped-header metadata continuity for the real report sample, without changing host JSON field names, invariant property names, or machine-readable contracts.
