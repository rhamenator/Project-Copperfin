...
- Last shipped localization child: `#2589`, the remaining shared runtime pause/session `Runtime.Prg.Session.*` messages now exist in `es-419`, `pt-BR`, and `qps-ploc`, `PrgRuntimeSession` now routes pause/session prose through shared runtime catalogs, and focused runtime-host plus shipped-catalog coverage proves localized step/event-loop/completed/breakpoint session messages without changing `debug.reason`, `status` fields, `READ EVENTS`, file paths, asset paths, or other machine-readable status data.
