...
- Last shipped E3 child: `#3185`, `CopperfinProjectWorkflow` now exposes runtime-manifest path plus structured build warning count/details from the managed project-build path, and the shared real `addlabel.pjx` build smoke now asserts a materialized manifest, zero workflow warnings, zero manifest warning lines, and the staged `wzcommon/registry.vcx` dependency so missing-asset regressions fail managed workflow coverage instead of hiding behind a superficially successful build.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3185`, the real `addlabel` project workflow path is now covered for build success, replay/debug success, warning-free packaging, and staged shared-class dependency fidelity, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, shared build/replay workflow, shared UI debugger path, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 managed-workflow hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeProjectBuildWorkflowWithRealAsset` directly against real `addlabel.pjx`, proving the managed workflow now exposes the runtime manifest path plus structured build warnings and that the shared real-project build smoke enforces a warning-free manifest with staged `wzcommon/registry.vcx` instead of accepting artifact existence alone. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
