...
- Last shipped localization child: `#2616`, the remaining `Studio.DesignerDispatch.CatalogEntry.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused designer-context coverage now proves translated and pseudo-localized catalog-entry errors without changing selection-context tokens, action ids, builder ids, toolbox item ids, execution flags, or other machine-readable designer-dispatch contracts.
