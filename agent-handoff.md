...
- Last shipped slice: `#3292`, the next prompt-sized child under `#3217`, shipped as evidence closure: focused regressions now prove native `ParentClass` already stays protected from ordinary direct-assignment shadowing on same-PRG and direct external-base objects because the shared reserved native identity-member guard already covers that path, while the shipped reflection, ordinary reads, and `AMEMBERS(..., 1|3)` enumeration stay stable.
...
- Next action: execute `#3293`, the next prompt-sized `#3217` child, to validate and, only if needed, harden contained native child objects so `GETPEM()` / `PEMSTATUS()` surface coherent `Parent` references across same-PRG and external-base child definitions, while preserving ordinary `Parent` reads, `PARENT` locals inside child methods, identity metadata behavior, inherited child behavior, COM/plain-object reflection, and the stack-frugal runtime frame machine.
