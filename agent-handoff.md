...
- Last shipped E3 child: `#3004`, shared report/label settings scope exposure; the shared VSIX/standalone FRX/LBX explorer now exposes a localized settings scope alongside sections and unplaced objects, the shared property-grid model now surfaces editable root settings with invariant update targets plus localized display labels for `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, the managed snapshot settings model now preserves root-setting record/field/memo provenance, and focused designer smokes now prove settings-scope explorer selection, localization, and host-backed root-setting updates without changing host JSON keys or machine-readable property names.
