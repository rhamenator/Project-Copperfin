...
- Last shipped E3 child: `#3015`, left/right page-margin stable-mutation evidence; the focused Studio-host page-setup regression suite now proves live and deleted FRX/LBX stable-selected `LEFTMARGIN` and `RIGHTMARGIN` update/clear flows persist on disk, refresh through the existing selected-settings JSON path, preserve selected-settings provenance/order, and do not add new machine-readable left/right summary keys beyond the existing shared report-layout contract.
