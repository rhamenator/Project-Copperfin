...
- Last shipped E3 child: `#3016`, left/right page-margin record-mutation evidence; the focused Studio-host record-settings regression lane now proves live and deleted FRX/LBX record-selected `LEFTMARGIN` and `RIGHTMARGIN` update/clear flows persist on disk, refresh through the existing `settings`/`deletedSettings`/`selectedReportSettings` JSON paths, preserve preview and section/object metadata continuity, and do not add new machine-readable left/right summary keys beyond the existing shared report-layout contract.
