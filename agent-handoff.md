...
- Last shipped E3 child: `#2761`, deleted report/label update-batch rollback evidence; the focused `test_studio_host_deleted_update_batch_errors` host regression now proves deleted FRX/LBX `--visual-object-update-batch` rollback on a missing stable selector preserves the current null-batch JSON error payload, omits stale success-only fields, restores earlier property values, and preserves deleted-state membership for both reports and labels, without changing production behavior.
