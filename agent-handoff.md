...
- Last shipped slice: `#3259`, the seventeenth prompt-sized child under `#3217`, now walks contained native child-object references segment by segment for ordinary dotted reads and method calls, including `Parent` hops, so expressions like `oForm.cmdSave.Parent.Caption` and calls like `oForm.cmdSave.OwnerCaption()` resolve against the contained target object instead of treating the full dotted suffix as one flat leaf on the root object. Focused runtime-surface, parser-class, and control-flow regressions prove direct dotted child-property reads, dotted child-method calls, dotted parent-chain reads, preserved owning-form state updates, and runtime stability all remain under the same stack-frugal execution model.
...
- Current lane: high-priority runtime parity blocker `#3217` continues to supersede E3/#24 because native VFP class/object behavior remains common-in-the-wild execution work; Localization/#2348 remains a standing architectural constraint for all new user-facing text.
- Next action: execute `#3260`, the next prompt-sized `#3217` child, so ordinary dotted assignment can walk through contained native child-object references without requiring intermediate variables, while preserving the existing stack-frugal PRG frame machine, same-PRG/external-PRG activation, and newly shipped `ADDOBJECT()` / `PARENT` / `THISFORM` / dotted-read containment behavior.
