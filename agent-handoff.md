...
- Last shipped E3 child: `#3208`, the shared real `by_author.FRX` and `STYLELBL.LBX` deleted rename command smokes now reuse captured source live preview bounds, compare section continuity from the section model instead of brittle list text, and fail if deleted preview metadata drifts after delete, rename, undo, or reload or if restored live preview metadata drifts after restore, so the shared asset-editor deleted-row rename workflow can no longer accept preview-metadata regressions hidden behind superficially correct deleted/live identity alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3208`, the shared asset-editor deleted rename seam joins the existing shared live reorder/rename/duplicate and host-backed settings-root, deleted-object, live duplicate/rename/reorder/placement, representative live property/batch, and representative section-property preview-metadata evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across shared deleted duplicate or other remaining deleted/live command paths, report/label editing, shared host JSON fidelity, project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, or recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 deleted-rename preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono probe that invoked `SmokeAssetEditorDeletedRenameCommandWithRealAsset` for the real `by_author.FRX` deleted-rename case under the resolved local toolchain, proving deleted preview metadata stays aligned after delete/rename/undo/reload and restored live preview metadata stays aligned after restore in the current shared editor/native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
