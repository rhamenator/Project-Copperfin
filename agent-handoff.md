...
- Last shipped localization child: `#2707`, the remaining hard-coded PRG `COPY TO` / `COPY TO ARRAY` runtime failures now route through `Runtime.Prg.Dispatch.Error.*` catalog entries with shipped parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization plus PRG data-IO coverage now proves pseudo-localized `COPY TO` failures without changing PRG command syntax, output-type tokens, schema selection rules, OS error propagation, or other machine-readable runtime contracts.
...
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, with the adjacent `APPEND FROM` family and other remaining dispatch/runtime faults as the next likely slices now that `SAVE TO` / `RESTORE FROM`, file-operation failures, and `COPY TO` are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
