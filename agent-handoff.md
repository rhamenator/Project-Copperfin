...
- Last shipped slice: `#3271`, the next prompt-sized child under `#3217`, now resolves bare helper-method calls from inherited external PRG base methods against the defining method PRG instead of always the derived object's source PRG. Focused runtime-surface and control-flow regressions prove derived classes can inherit external-base `Init` and ordinary methods that call helper methods from the defining library, by-reference flow and follow-on state updates remain stable, and existing same-PRG helper-method resolution plus COM/plain-object behavior remain stable.
...
- Next action: execute `#3272`, the next prompt-sized `#3217` child, so `DODEFAULT()` can base-call parent methods loaded from external PRG libraries while preserving `THIS`, by-reference flow, and follow-on child logic, without disturbing same-PRG `DODEFAULT()`, inherited declarative children, inherited `THIS.AddObject()`, inherited bare helper calls, external base-class inheritance, or the stack-frugal runtime frame machine.
