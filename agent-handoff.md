...
- Last shipped E3 child: `#3009`, shared report/label document-title settings exposure; the shared VSIX/standalone FRX/LBX root-settings property-grid selection now surfaces localized read-only document-title metadata from the existing managed report-layout model while preserving stable host JSON keys, invariant editable setting update targets, and shared behavior between Visual Studio and standalone Studio, and focused designer smokes now prove synthetic and host-backed settings continuity plus `en-US`, `es-419`, `pt-BR`, and `qps-ploc` routing for the new visible document-title metadata.
