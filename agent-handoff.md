...
- Last shipped E3 child: `#2789`, report/label record-selected column-setup settings success evidence; the focused `test_studio_host_column_setup_record_settings` host regression now proves FRX/LBX record-selected `EXPR` column-setup memo update and clear success for live and deleted settings rows preserve the current column-setup/settings counts, selected-settings provenance, direct-field carrier behavior after clear, and non-fabricated preview state for both reports and labels, without changing production behavior.
