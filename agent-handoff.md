...
- Last shipped localization child: `#2582`, `copperfin_studio_host` now has `es-419`, `pt-BR`, and `qps-ploc` catalog parity for the remaining `StudioHost.ToolboxPaletteParse.Error.*` keys, with focused locale checks proving representative toolbox-palette parse failures no longer fall back to raw English outside `en-US` while preserving CLI option names, selection-context tokens, toolbox-context tokens, JSON field names, exit codes, and other machine-readable contracts unchanged.
