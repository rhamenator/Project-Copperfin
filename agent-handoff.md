...
- Last shipped E3 child: `#2759`, report/label update-batch success evidence; the focused `test_studio_host_update_batch` host regression now proves stable FRX/LBX `--visual-object-update-batch` success preserves the current batch result payload, persisted direct and memo-backed property updates, live/deleted preview bounds, and refreshed selected-object metadata for both reports and labels, without changing production behavior.
