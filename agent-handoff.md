...
- Last shipped slice: `#3268`, the next prompt-sized child under `#3217`, now supports native `THIS.AddObject('<name>', '<class>', '<file>.prg')` activation through the existing external PRG class-library path. Focused runtime-surface and control-flow regressions prove external PRG child classes materialize before follow-on parent logic reads them, child `PARENT` methods still work, child `Init` still receives trailing constructor arguments, and existing same-PRG `THIS.AddObject()` plus COM/plain-object behavior remain stable.
...
- Next action: execute `#3269`, the next prompt-sized `#3217` child, so inherited declarative child definitions from external PRG base classes resolve same-PRG child class names against the lineage class's defining PRG source rather than the derived object's PRG source, without disturbing explicit child `OF <file>.prg` activation, same-PRG declarative children, runtime `THIS.AddObject()`, external base-class inheritance, or the stack-frugal runtime frame machine.
