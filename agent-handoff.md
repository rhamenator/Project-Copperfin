...
- Next action: inspect live `#24` children or create the next prompt-sized E3 slice from current real gaps, with remaining likely seams around typed sorting metadata exposure from the already-parsed root report settings stream, shared host/designer round-trip fidelity, report/label runtime-preview hooks, and any other FRX/LBX behavior the live issue tree still shows as unproven now that explicit grouping summaries and section grouping context plus stable-selected, record-selected, deleted, and nested group-section EXPR edit evidence are in place.
