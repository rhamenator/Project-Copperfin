...
- Last shipped E3 child: `#3007`, shared report/label grouping explorer selection; the shared VSIX/standalone FRX/LBX explorer now exposes root grouping definitions from the existing managed report-layout model, and the shared property-grid path now surfaces localized read-only grouping index, nesting depth, expression provenance, and header/footer linkage metadata without changing host JSON keys or machine-readable update targets, while focused designer smokes now prove synthetic and real grouped-asset continuity plus `en-US`, `es-419`, `pt-BR`, and `qps-ploc` routing for the new visible grouping rows and property labels.
