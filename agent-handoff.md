...
- Last shipped localization child: `#2394`, Studio host usage prefixes, alternate-command labels, object-entry labels, and selection-context token summary text routed through the portable C++ localization catalog while preserving command names, CLI flags, placeholders, selection-context tokens, JSON contracts, and host parser behavior.
