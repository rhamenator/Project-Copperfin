...
- Last shipped E3 child: `#3065`, FRX/LBX root-settings unsupported EXPR-line preservation; the shared native report/label root-settings write path now preserves unsupported and non-edited `EXPR` memo lines when a supported FRX/LBX root setting is added, updated, or removed, and focused native host regressions now prove `DRIVER` add/remove edits keep unsupported line ordering plus supported setting `sourceLineIndex` provenance stable for both reports and labels without changing host JSON field names, invariant property names, or machine-readable contracts.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#3065`, `#3064`, `#3063`, `#3062`, `#3061`, `#3060`, `#3059`, `#3058`, and `#3057` closed the unsupported root-settings EXPR-line preservation seam, the absent memo-backed printer-setting add/remove seam, the memo-backed real root-settings write seam, and the grouped real-section key-expression, grouped multi-object section-height, real section-sibling, grouped section-top, and non-top section continuity seams on top of `#3056` label axis-specific snap, `#3055` vertical snap, `#3054` horizontal snap, `#3053` vertical distribution, `#3052` top-alignment, `#3049` resize-to-anchor height, `#3048` resize-to-anchor width, `#3047` resize-to-anchor size, `#3046` snap-to-grid, `#3045` horizontal distribution, `#3044` left-alignment, `#3043` post-nudge property-grid geometry refresh, and `#3042` real-asset report/label nudge continuity coverage, treat the shared live layout-action path as covered for those command families unless fresh evidence shows a broader section/object authoring gap; prefer the next unshipped host-backed shared-model or real-asset round-trip seam around remaining section-aware edit fidelity, deeper memo-backed settings parity, or unsupported-property preservation rather than reopening already-covered continuity lanes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; this slice stayed in the shared native FRX/LBX root-settings write path and used `cmake --build build --target test_studio_host_orientation_record_settings`, `ctest --test-dir build -R '^test_studio_host_orientation_record_settings$' --output-on-failure`, and focused synthetic FRX/LBX host regressions that now prove `DRIVER` add/remove edits preserve unsupported root `EXPR` memo lines plus supported-setting `sourceLineIndex` provenance for both reports and labels without changing host JSON or shared-shell contracts.
