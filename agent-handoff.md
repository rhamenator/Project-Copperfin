...
- Last shipped E3 child: `#3028`, real label document-title continuity evidence; focused shared smoke coverage now explicitly requires a real LBX candidate and proves the shared VSIX/standalone settings selection surfaces `DOCUMENTTITLE` from the real `STYLELBL.LBX` sample instead of passing on an earlier report asset, without changing host JSON keys or machine-readable setting names.
