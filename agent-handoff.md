...
- Last shipped localization child: `#2586`, the remaining `RuntimeHost.Debug.Error.*` keys now exist in `es-419`, `pt-BR`, and `qps-ploc`, with focused runtime-host coverage proving localized invalid-breakpoint, unknown-breakpoint, and watch-paused debug failures without changing breakpoint grammar, file paths, line numbers, debug command tokens, prefixes, exit codes, or machine-readable status fields.
