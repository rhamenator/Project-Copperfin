...
- Last shipped E3 child: `#2739`, deleted object-duplicate-batch section-context evidence; the stable deleted FRX/LBX `--visual-object-duplicate-batch` reopen regressions now assert the current shared report-layout behavior instead of stale null containing sections, and the focused `test_studio_host_deleted_object_duplicate_batch_sections` host regression now proves deleted copied rows reopened after stable-identity duplicate-batch keep live detail-band ids and per-section deleted-object counts for both reports and labels without changing production behavior.
