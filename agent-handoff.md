...
- Last shipped E3 child: `#2735`, deleted property-move-batch section-context evidence; the stable deleted FRX/LBX `--visual-property-move-batch` regressions now assert the current shared report-layout behavior instead of stale null containing sections, and the focused `test_studio_host_deleted_property_move_batch_sections` host regression now proves deleted placed rows reopened after a move-batch keep live detail-band ids, section-relative geometry, and per-section deleted-object counts for both reports and labels without changing production behavior.
