...
- Last shipped E3 child: `#3006`, shared report/label preview-metadata settings exposure; the shared VSIX/standalone FRX/LBX root-settings property-grid selection now surfaces localized live and deleted preview bounds from the existing managed report-layout model while preserving invariant update targets, stable host JSON keys, and shared behavior between Visual Studio and standalone Studio, and focused designer smokes now prove settings-scope explorer selection, host-backed refresh continuity, and `en-US`, `es-419`, `pt-BR`, and `qps-ploc` localization for the new visible preview metadata.
