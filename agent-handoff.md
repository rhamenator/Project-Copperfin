...
- Last shipped localization child: `#2554`, `DO` and `CALL` target resolution errors route through the portable C++ localization catalog while preserving VFP command names, target paths/names, parser behavior, event categories, machine-readable state, and default en-US output.
