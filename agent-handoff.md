...
- Last shipped localization child: `#2576`, `copperfin_studio_host` now localizes the remaining usage-wrapper prose by adding `es-419`, `pt-BR`, and `qps-ploc` catalog coverage for `StudioHost.Usage.Primary`, `StudioHost.Usage.Alternate`, `StudioHost.Usage.ObjectEntry`, and `StudioHost.Usage.SelectionContextTokens`, so `Usage:`, `or:`, `object:`, and selection-context token wrapper text no longer fall back to raw English outside `en-US` while CLI switches, option names, placeholder values, selection-context tokens, JSON field names, parser tokens, and other machine-readable contracts remain unchanged.
