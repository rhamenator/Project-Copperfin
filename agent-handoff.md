...
- Last shipped E3 child: `#2966`, FRX/LBX no-op property-write byte preservation; focused native visual-asset regressions now prove unchanged report/label direct-field and memo-backed property edits short-circuit as no-ops without rewriting `.frx/.frt` or `.lbx/.lbt` bytes and without creating undo history, and the shared direct-field update path now uses field-aware no-op detection so report/label safe round-trip continuity matches existing SCX-style unchanged-write behavior.
