...
- Last shipped localization child: `#2493`, extensibility .NET interop policy decision reason prose routes through the portable C++ localization catalog while preserving `DotNetInteropDecision` enum values, execution path tokens, capability ids, allowlist/denylist ids, latency budgets, policy behavior, and default en-US output.
