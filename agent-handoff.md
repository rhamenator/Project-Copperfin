...
- Last shipped slice: `#3221`, the fourth prompt-sized child under `#3217`, now runs a discovered same-PRG native `Init` method automatically during `CREATEOBJECT()` / bare `NEWOBJECT()` after default property materialization, preserving `THIS` binding and the iterative PRG frame machine so constructor-style class initialization can mutate instance state during creation without disturbing plain-object or COM creation paths. Focused runtime-surface regressions prove both native `Init` state changes and older dictionary/plain-object flows remain stable.
...
- Next action: create the next prompt-sized `#3217` child after `#3221` and move from zero-argument native lifecycle support into constructor-parameter parity, most likely passing `CREATEOBJECT()` arguments through to native `Init` while preserving the stack-frugal iterative runtime model and keeping plain-object/COM activation semantics stable.
