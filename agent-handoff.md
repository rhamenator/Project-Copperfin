...
- Last shipped E3 child: `#3195`, the shared real `by_author.FRX` and `STYLELBL.LBX` host-backed deleted rename and deleted duplicate smokes now capture the source live preview-bounds geometry, assert that deleted preview bounds stay aligned with current deleted layout-object geometry through rename/duplicate/reload, and fail if restore leaves the live `PREVIEWBOUNDS` or remaining `DELETEDPREVIEWBOUNDS` inconsistent with the real host snapshot contract, so deleted identity coverage can no longer accept host-JSON preview drift hidden behind correct UNIQUEID/count behavior alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3195`, the host-backed real FRX/LBX deleted identity paths now prove rename and duplicate preserve preview metadata according to the real host contract, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, delete/restore and deleted-edit preview-metadata seams, remaining deleted reorder preview-metadata seams, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed deleted-identity preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedDeletedRenameRoundTrip` plus `SmokeRealAssetHostBackedDeletedDuplicateRoundTrip` under the resolved local toolchain against real `by_author.FRX` and `STYLELBL.LBX`, proving deleted preview bounds stay aligned with current deleted layout-object geometry through rename/duplicate/reload and that restore preserves the expected live `PREVIEWBOUNDS` plus any remaining deleted preview metadata according to the native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
