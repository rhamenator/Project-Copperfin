...
- Last shipped E3 child: `#2688`, report/label per-section deleted-object counts; `StudioReportSectionSnapshot` now carries additive deleted placed-object counts for each FRX/LBX section, `copperfin_studio_host --json` exposes the same field on live/deleted sections and selected containing sections, and focused report-layout plus direct host validation now proves live/deleted sections count deleted placed objects within their own geometry while unplaced deleted-object selections keep containing-section JSON absent.
