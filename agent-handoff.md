...
- Last shipped E3 child: `#2778`, report/label record-selected page-margin setting success evidence; the focused `test_studio_host_page_margin_record_settings` host regression now proves FRX/LBX record-selected `TOPMARGIN` direct-field update and clear success for live and deleted settings rows preserve the current page-setup/settings counts, selected-settings provenance, cleared-field behavior, and full live/deleted preview bounds for both reports and labels, without changing production behavior.
