...
- Last shipped E3 child: `#3187`, the shared real `addlabel.pjx` project debugger UI smoke now inspects the embedded editor control's active `CopperfinRuntimeDebugSession`, asserts a preserved runtime-manifest path plus zero inherited build warnings, and fails if the surfaced manifest contains warning lines or loses the staged `wzcommon/registry.vcx` dependency, so the UI-facing debugger path can no longer hide packaging regressions behind a successful pause summary alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3187`, the real `addlabel` project workflow path is now covered for build success, replay/debug success, UI-debugger success, warning-free packaging, and staged shared-class dependency fidelity across the shared build, replay, and embedded debugger surfaces, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 UI-debugger hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeProjectDebuggerWithRealAsset` directly against real `addlabel.pjx`, proving the embedded shared debugger surface now preserves runtime-manifest path plus inherited build warning metadata and that the UI-facing real-project smoke enforces a warning-free manifest with staged `wzcommon/registry.vcx` instead of accepting pause-summary success alone. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
