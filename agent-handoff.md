...
- Last shipped E3 child: `#3194`, the shared real `by_author.FRX` and `STYLELBL.LBX` host-backed deleted-property and deleted-batch smokes now capture the original live preview-bounds geometry, assert that deleted snapshots keep `DELETEDPREVIEWBOUNDS` aligned with the current deleted layout-object geometry through update/reload/undo, and fail if restore leaves stale deleted preview metadata or loses the original live `PREVIEWBOUNDS`, so deleted-object property coverage can no longer accept host-JSON preview drift that is inconsistent with either the deleted object state or the restored live asset state.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3194`, the host-backed real FRX/LBX deleted-property path now proves deleted preview bounds stay aligned with deleted layout-object geometry through update/reload/undo and that restore returns to the original live preview bounds without stale deleted metadata, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, delete/restore and deleted-edit preview-metadata seams, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed deleted-property preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedDeletedPropertyRoundTrip` plus `SmokeRealAssetHostBackedDeletedBatchPropertyRoundTrip` under the resolved local toolchain against real `by_author.FRX` and `STYLELBL.LBX`, proving deleted snapshots keep `DELETEDPREVIEWBOUNDS` aligned with current deleted layout-object geometry through update/reload/undo and that restore returns to the original live `PREVIEWBOUNDS` without stale deleted metadata. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
