...
- Last shipped localization child: `#2610`, the remaining `Studio.ReportLayout.Section.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused report-layout coverage now proves translated and pseudo-localized section titles without changing invariant `band_kind` values, section ordering, provenance fields, or other machine-readable report-layout contracts.
