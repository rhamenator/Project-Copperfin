...
- Last shipped slice: `#3223`, the sixth prompt-sized child under `#3217`, now forwards trailing bare same-PRG native `NEWOBJECT()` arguments into the automatic native `Init` call when the activation still looks like an in-program class construction instead of an explicit classlib/COM target, bringing constructor-argument parity with `CREATEOBJECT()` without disturbing library/server-targeted COM `NEWOBJECT()` activation or the iterative PRG runtime model. Focused runtime-surface regressions prove native `NEWOBJECT()` constructor-argument state changes and the existing `CREATEOBJECT()`/COM object paths remain stable.
...
- Next action: create the next prompt-sized `#3217` child after `#3223` and move from constructor-argument parity into native object-method/by-reference argument parity so `THIS`-bound methods and constructor-style `Init` flows can propagate `@` updates through the same stack-frugal iterative runtime model.
