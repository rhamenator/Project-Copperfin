...
- Last shipped localization child: `#2605`, the remaining `Runtime.PrgStaticAnalysis.Diagnostic.PRG0001.Message`, `PRG1001.Message`, and `PRG1002.Message` strings now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, the shared PRG static-analysis helper now routes human-facing diagnostics through the runtime locale helper instead of a one-time static catalog snapshot, and focused control-flow analyzer coverage now proves locale-specific key presence, pseudo-localized resolution, and in-process locale switching without changing diagnostic codes, severities, file paths, line numbers, or other machine-readable/runtime contracts.
