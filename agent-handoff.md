...
- Last shipped localization child: `#2474`, Studio launch record-source text target selector value diagnostics route through the portable C++ localization catalog while preserving CLI option names, record-source/text binding selector assignment semantics, JSON/status contracts, parser behavior, and default en-US output.
