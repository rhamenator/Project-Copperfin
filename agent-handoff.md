...
- Last shipped E3 child: `#3193`, the shared real `by_author.FRX` and `STYLELBL.LBX` host-backed delete/restore smokes now capture the source preview-bounds geometry, assert that both the immediate restore result and the reload from disk preserve live `PREVIEWBOUNDS`, and fail if stale `DELETEDPREVIEWBOUNDS` remain after restore, so host-JSON delete/restore coverage can no longer accept a superficially successful object restore while deleted preview metadata still leaks through the shared native snapshot contract.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3193`, both the embedded shared editor path and the host-backed native snapshot path now prove delete/restore clears stale deleted-preview metadata while preserving live preview bounds for real FRX/LBX assets, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, delete/restore preview-metadata seams, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed delete-restore preview-bounds hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedDeleteRestoreRoundTrip` under the resolved local toolchain against real `by_author.FRX` and `STYLELBL.LBX`, proving both the immediate restore result and the reload from disk now preserve source `PREVIEWBOUNDS` geometry while clearing stale `DELETEDPREVIEWBOUNDS` in the shared native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
