...
- Last shipped E3 child: `#2901`, report column-setup broad-suite retirement; the broad `test_studio_host_json` target no longer compiles `tests/test_studio_host_json_settings_column_setup.cpp`, and the report/label column-setup exposure plus stable/deleted `COLS` host JSON mutation coverage now lives in the focused `test_studio_host_report_column_setup` executable while the existing `test_studio_host_report_direct_setting_fields` executable keeps the invalid direct column-setup field lane without host JSON or production-behavior changes.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; `#2901` removes the report column-setup shard from the broad `test_studio_host_json` target, so the next step should reassess whether the remaining report/label broad-target compile units still lack focused ownership after the recent timeout-driven refactors and otherwise pivot to the highest-value direct FRX/LBX fidelity gap once the broad-target cleanup stops yielding narrow evidence-backed slices.
