...
- Last shipped E3 child: `#3067`, shared missing direct-field `TAG` root-setting parity; the shared VSIX/standalone report-settings selection model now materializes absent FRX/LBX root `TAG` as an editable setting when the asset lacks a current value, using the existing host-backed direct-field update path, and focused shared plus host-backed real-asset smokes now prove `TAG` add/reload/undo continuity on `by_author.FRX` and `STYLELBL.LBX` without changing host JSON field names, invariant property names, or machine-readable contracts.
