...
- Last shipped E3 child: `#3205`, the shared real `STYLE2V.FRX` live reorder command smoke now captures the source live preview-bounds geometry before opening the editor, asserts grouped-section continuity from the section model instead of brittle decorated list text, and fails if `PREVIEWBOUNDS` drift after reorder or on reload, so the shared asset-editor live-row reorder workflow can no longer accept preview-metadata regressions hidden behind superficially correct object ordering and identity alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3205`, both shared asset-editor reorder seams now have explicit real-asset preview-metadata evidence alongside the existing host-backed settings-root, deleted-object, live duplicate/rename/reorder/placement, representative live property/batch, and representative section-property coverage, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, or recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 live-reorder preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono probe that invoked `SmokeAssetEditorReorderCommandWithRealAsset` for the real `STYLE2V.FRX` live-reorder case under the resolved local toolchain, proving live `PREVIEWBOUNDS` stays aligned after reorder and on reload in the current shared editor/native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
