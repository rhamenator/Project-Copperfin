...
- Last shipped E3 child: `#3189`, the shared asset editor now retains the latest non-debug `CopperfinProjectExecutionResult`, and the real `addlabel.pjx` project editor smoke now clicks `Run Copperfin Project`, asserts preserved output/manifest/debug-manifest/launcher paths plus zero inherited build warnings, and fails if the surfaced runtime manifest contains warning lines or loses the staged `wzcommon/registry.vcx` dependency, so the editor-shell run path can no longer hide packaging regressions behind a superficially successful launcher start.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3189`, the real `addlabel` project workflow path is now covered for project-editor open plus editor-shell run success, managed build/run success, replay/debug success, UI-debugger success, warning-free packaging, and staged shared-class dependency fidelity across the shared build/run/replay and embedded debugger surfaces, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 editor-shell run hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeProjectEditorWithRealAsset` directly against real `addlabel.pjx`, proving the shared asset editor's `Run Copperfin Project` command now preserves output/manifest/debug-manifest/launcher paths plus inherited build warning metadata and that the editor-shell real-project smoke enforces a warning-free manifest with staged `wzcommon/registry.vcx` instead of accepting button visibility or launcher-start success alone. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
