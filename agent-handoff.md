...
- Last shipped localization child: `#2476`, Studio launch row-source-type and list index target selector value diagnostics route through the portable C++ localization catalog while preserving CLI option names, row-source-type/list selector assignment semantics, JSON/status contracts, parser behavior, and default en-US output.
