...
- Last shipped E3 child: `#3023`, real report print-profile settings continuity; the shared VSIX/standalone designer path now routes invariant `DEFAULTSOURCE`, `PRINTQUALITY`, `YRESOLUTION`, and `TTOPTION` report-setting names through localized numeric property handling plus localized display labels, focused shared smokes prove live and deleted settings-root continuity plus host-refresh continuity for those print-profile settings, and a focused real-asset smoke now proves the same shared selection model surfaces those values from `STYLE1H.FRX` without changing host JSON keys or machine-readable setting names.
