...
- Last shipped localization child: `#2612`, the remaining `Studio.BuilderDispatch.Execution.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused builder-registry coverage now proves translated and pseudo-localized execution errors without changing builder ids, context tokens, command tokens, execution flags, or other machine-readable builder-dispatch contracts.
