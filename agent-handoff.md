...
- Last shipped localization child: `#2588`, the remaining runtime-host manifest verification `RuntimeHost.Error.*` keys now exist in `es-419`, `pt-BR`, and `qps-ploc`, with focused runtime-host and shipped-catalog coverage proving localized missing-`runtime_host_sha256` and missing-extension-payload failures without changing exit codes, `status: error`, filenames, manifest fields, or other machine-readable status data.
