...
- Last shipped E3 child: `#3202`, the shared real host-backed section smoke now supports explicit preview-bounds assertions, and representative `by_author.FRX` plus `STYLELBL.LBX` section cases now fail if `PREVIEWBOUNDS` drift across section property update, reload, or undo for both geometry-changing and non-geometry edits, so section-edit coverage can no longer accept host-JSON preview regressions hidden behind correct section identity and persisted property values alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3202`, the host-backed real preview-metadata seams for deleted-object workflows, live duplicate/rename/reorder/placement, representative live property/batch cases, and representative section property cases now have explicit real-asset evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, or recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed section preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedSectionRoundTrip` for representative real `by_author.FRX` and `STYLELBL.LBX` section cases under the resolved local toolchain, proving `PREVIEWBOUNDS` stays stable or changes only as expected through section property update, reload, and undo in the current native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
