...
- Last shipped E3 child: `#3010`, real-asset report/label settings preview-metadata continuity; the shared FRX/LBX settings-root property-grid selection now has real-asset smoke proof that live preview-bounds metadata from actual FRX snapshots survives into the shared property grid with invariant `PREVIEWBOUNDS` values while deleted-preview-bounds coverage emits an explicit skip when no current real asset exposes the needed settings-plus-deleted-preview combination, so no host JSON or shared contract changes were required.
