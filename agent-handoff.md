...
- Last shipped E3 child: `#2920`, shared report refresh-selection continuity; the shared WinForms FRX/LBX shell now preserves section-scoped explorer selection across section-list rebuilds during shared refresh paths and only reselects the design-surface/object row when the prior selection was actually object-rooted, keeping section-rooted property-grid editing intact through update/undo refreshes for both the VSIX and standalone Studio without changing host JSON keys or shared designer-model contracts; focused designer smokes now prove section-rooted refresh continuity and that object-rooted reselection still survives the same refresh path.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2920` made section-rooted refresh continuity stable, prefer the next direct FRX/LBX gap around deleted/unplaced preview metadata, section-aware round-trip editing seams beyond `TOP`/`HEIGHT`, or other evidence-backed report/label editing gaps unless build-pressure evidence still requires another narrowly owned shard extraction first.
