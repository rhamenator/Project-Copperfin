...
- Last shipped localization child: `#2555`, `SEEK`, `LOCATE`, and `SCAN` target work-area errors route through the portable C++ localization catalog with `{command}` placeholders while preserving VFP command names, cursor targeting semantics, parser behavior, event categories, machine-readable state, and default en-US output.
