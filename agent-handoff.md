...
- Last shipped E3 child: `#2788`, report/label stable-selected column-spacing setting success evidence; the focused `test_studio_host_column_spacing_stable_settings` host regression now proves FRX/LBX stable-selected `COLSPACING` direct-field update and clear success for live and deleted settings rows preserve the current column-setup/settings counts, selected-settings provenance, cleared-field behavior, and non-fabricated preview state for both reports and labels, without changing production behavior.
