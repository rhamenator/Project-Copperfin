...
- Last shipped localization child: `#2624`, the remaining `Studio.ToolboxCreation.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused toolbox-creation coverage now proves translated and pseudo-localized request errors without changing toolbox item ids, designer-context filtering, admitted-dispatch semantics, object-identity handling, asset paths, or other machine-readable toolbox-creation contracts.
