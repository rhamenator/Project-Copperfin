...
- Last shipped E3 child: `#3063`, memo-backed real FRX/LBX page-setup setting writes; the shared native report/label visual-property path now reads and writes real root `EXPR` memo settings such as `ORIENTATION` from FRX/LBX settings records, and the shared host-backed plus VSIX/standalone real-asset ORIENTATION smokes now prove update, reload, and undo continuity against the stable `reportLayout.settings` contract without changing host JSON field names, invariant property names, or machine-readable contracts.
