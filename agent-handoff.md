...
- Last shipped E3 child: `#2918`, shared report explorer section-scoping parity; the shared WinForms FRX/LBX explorer now filters object rows to the selected section instead of always showing every snapshot object, and it adds a localized unplaced-objects row that scopes the object list to `reportLayout.unplacedObjects` for both the VSIX and standalone Studio without changing host JSON keys or shared designer-model contracts; focused designer smokes now prove section-scoped filtering, unplaced-object filtering, and `en-US`/`es-419`/`pt-BR`/`qps-ploc` routing for the new explorer row.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2918` made the shared explorer genuinely section-aware for live and unplaced report objects, prefer the next direct FRX/LBX gap around deleted/unplaced preview metadata, section-aware round-trip editing seams, or other evidence-backed report/label editing gaps unless build-pressure evidence still requires another narrowly owned shard extraction first.
