...
- Last shipped E3 child: `#2763`, deleted report/label rename-batch success evidence; the focused `test_studio_host_deleted_object_rename_batch` host regression now proves deleted FRX/LBX `--visual-object-rename-batch` success preserves the current batch result payload, replaces deleted selected identities without changing order, counts, or deleted-state membership, and keeps the refreshed reopen selection, containing-section, and preview metadata for both reports and labels, without changing production behavior.
