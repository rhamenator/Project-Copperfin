...
- Last shipped slice: `#3278`, the next prompt-sized child under `#3217`, taught native PRG-defined objects to surface immediate `BaseClass` identity through `GETPEM()` / `PEMSTATUS()`, and direct external-base native objects to surface resolved `ClassLibrary` provenance through that same read-only reflection path, with focused runtime-surface and control-flow regressions proving the new metadata stays coherent without disturbing the previously-shipped `ACLASS()` chain or COM/plain-object reflection.
...
- Next action: execute `#3279`, the next prompt-sized `#3217` child, so native PRG-defined objects surface coherent direct `Class` identity through `GETPEM()` / `PEMSTATUS()` without disturbing the newly-shipped `ACLASS()` chain, `BaseClass` / `ClassLibrary` metadata, existing `AMEMBERS()` reflection, inherited declarative children, inherited `THIS.AddObject()`, helper-method resolution, `DODEFAULT()`, accessor surfaces, or the stack-frugal runtime frame machine.
