...
- Last shipped localization child: `#2600`, the existing native security profile mode/policy, permission, role, identity-provider, feature, and hardening-profile keys under `Security.Profile.*` now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused platform-model coverage proves localized native-security-profile prose without changing permission ids, role ids, identity-provider kinds, feature ids, audit event ids, or other machine-readable profile contracts.
