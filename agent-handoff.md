...
- Last shipped localization child: `#2540`, numeric runtime domain errors for `LOG()`, `LOG10()`, `ASIN()`, and `ACOS()` route through the portable C++ localization catalog while preserving function names, numeric argument values, parser behavior, runtime result semantics, event contracts, and default en-US output.
