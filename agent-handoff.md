...
- Last shipped E3 child: `#2786`, report/label record-selected column-width setting success evidence; the focused `test_studio_host_column_width_record_settings` host regression now proves FRX/LBX record-selected `COLWIDTH` direct-field update and clear success for live and deleted settings rows preserve the current column-setup/settings counts, selected-settings provenance, cleared-field behavior, and non-fabricated preview state for both reports and labels, without changing production behavior.
