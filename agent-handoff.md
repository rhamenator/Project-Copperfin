...
- Last shipped slice: `#3216`, native PRG expression-level user-defined `FUNCTION`/`PROCEDURE` calls now route through the same iterative frame machine used by statement-level `DO`/`CALL` instead of falling back to string literals, so ordinary Fox/VFP patterns such as `x = MyFunc(...)`, `IF SomePredicate(...)`, nested user-routine return expressions, and `@` by-reference caller updates now work without reintroducing recursive host-stack growth. The focused runtime regressions also prove both `FUNCTION` and `PROCEDURE` return values, IF-predicate use, nested user-routine chaining, and by-reference argument write-back under the stack-frugal PRG engine.
...
- Current lane: high-priority runtime parity blockers `#3216` and `#3217` temporarily supersede E3/#24 because they affect common-in-the-wild Fox/VFP code execution; Localization/#2348 remains a standing architectural constraint for all new user-facing text.
- Next action: split `#3217` into the first prompt-sized child issue and start the native `DEFINE CLASS` / `ENDDEFINE` parser-and-model foundation slice before returning to the deferred E3/#24 designer backlog.
