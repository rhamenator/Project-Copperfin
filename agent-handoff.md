...
- Last shipped slice: `#3267`, the next prompt-sized child under `#3217`, now supports native `DEFINE CLASS <name> AS <base> OF <file>.prg` inheritance across loaded PRG files. Focused parser, runtime-surface, and control-flow regressions prove the external base source stays distinct from the base class identifier, derived classes can inherit parent defaults and methods from external `.prg` class libraries through both `CREATEOBJECT()` and bare `NEWOBJECT()`, inherited `Init` still sees child-overridden state, and same-PRG inheritance plus COM/plain-object behavior remain stable.
...
- Next action: execute `#3268`, the next prompt-sized `#3217` child, so `THIS.AddObject('<name>', '<class>', '<file>.prg')` can materialize external native child classes through the existing PRG class-library path without disturbing the current same-PRG `THIS.AddObject()` path, declarative child `OF <file>.prg` activation, external base-class inheritance, or the stack-frugal runtime frame machine.
