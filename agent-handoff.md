...
- Last shipped E3 child: `#3020`, shared column-setup settings continuity; the focused VSIX/standalone designer smokes now prove live and deleted FRX/LBX root-settings selections surface `COLS`, `COLWIDTH`, and `COLSPACING`, localize their display labels through `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and preserve settings-rooted shared host-refresh continuity after column-setup edits without changing invariant property names, host JSON keys, or record/field provenance contracts.
