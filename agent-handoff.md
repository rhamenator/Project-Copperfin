...
- Last shipped E3 child: `#3008`, shared report/label sort-metadata settings exposure; the shared VSIX/standalone FRX/LBX root-settings property-grid selection now surfaces localized read-only sort metadata from the existing `TAG` root setting, including active sort expression plus provenance field/memo details, while preserving invariant editable `TAG` update targets, stable host JSON keys, and shared behavior between Visual Studio and standalone Studio, and focused designer smokes now prove synthetic and host-backed settings continuity plus `en-US`, `es-419`, `pt-BR`, and `qps-ploc` routing for the new visible sort metadata.
