...
- Last shipped localization child: `#2717`, the remaining direct hard-coded `TEXT`, `TRY`, `REPLACE`, `UPDATE`, `INSERT INTO`, `UNLOCK RECORD`, and `SLEEP cancelled` dispatch failures now route through `Runtime.Prg.Dispatch.Error.*` catalog entries with shipped parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization plus PRG control-flow coverage now proves pseudo-localized command failures and spawned-task sleep cancellation without changing command syntax, cancellation semantics, event categories, or other machine-readable runtime contracts.
- Current lane: Localization/#2348 remains active for the remaining runtime/host hard-coded user-facing prose; E3/#24 is still open, but new user-facing work must stay localized by default and the next prompt-sized slice should come from the live localization/runtime backlog unless a higher-priority E3 blocker appears in live issue state.
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, which is down to 5 direct `last_error_message` literals after `#2717`; the remaining cluster is the object-helper fallback path for OLE property assignment failure, generic object-target assignment failure, and `SCATTER NAME` object creation failure, now that `APPEND FROM`, `SCATTER`, `GATHER`, `CREATE CURSOR/TABLE`, `ALTER TABLE`, `SET FILTER`, `DIMENSION/DECLARE`, both `SLEEP` failure paths, `DECLARE DLL`, the smaller residual command precondition guards, top-level runtime faults, `COPY TO`, `SAVE TO` / `RESTORE FROM`, and file-operation failures are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
