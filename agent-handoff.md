...
- Last shipped localization child: `#2539`, build-host usage, parse diagnostics, and process-hardening warning labels have portable C++ catalog entries across `en-US`, `es-419`, `pt-BR`, and `qps-ploc` while preserving command names, CLI options, configuration tokens, machine-readable status fields, output-field labels, placeholder values, and default en-US output.
