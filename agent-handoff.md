...
- Last shipped E3 child: `#2977`, shared asset-editor object identity rename parity; the shared VSIX/standalone report/label editor now routes object id regeneration through the existing Studio host `--rename-object` contract with invariant `--record`, stable optional `--unique-id`, and replacement `--new-unique-id` selectors, exposes localized rename command chrome, and focused designer smokes now prove live report plus deleted label rename flows refresh shell summary details while preserving explorer/property-grid/design-surface continuity without changing host or snapshot machine-readable contracts.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2977` closed shared rename continuity, `#2976` closed front/back reorder continuity, `#2975` closed duplicate continuity, and `#2974` closed delete/restore continuity, prefer the next direct FRX/LBX gap around shared reparent parity if a target-picking seam can stay prompt-sized, otherwise move to deeper real-asset round-trip coverage or richer section-aware multi-row write-back evidence where live repo state still shows a report/label gap.
