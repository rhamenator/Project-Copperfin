...
- Last shipped E3 child: `#3192`, the shared real `by_author.FRX` and `STYLELBL.LBX` delete/restore smokes now assert that a restored asset keeps live `PREVIEWBOUNDS`, clears stale `DELETEDPREVIEWBOUNDS`, and reselects the shared settings scope to verify the property grid surfaces only the live preview metadata after restore, so report/label delete-restore coverage can no longer accept a superficially successful object restore while stale deleted-preview state still leaks through the shared designer selection model.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3192`, the real FRX/LBX delete-restore path now proves restored assets keep live preview bounds while clearing stale deleted-preview metadata all the way through the shared settings selection, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 delete-restore preview-bounds hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and focused repo-local net472/Mono reflection-probe coverage against real `by_author.FRX` and `STYLELBL.LBX`, proving the shared restore path now preserves live `PREVIEWBOUNDS`, clears stale `DELETEDPREVIEWBOUNDS`, and surfaces only the live preview metadata when the shared settings scope is reselected after restore. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
