...
- Last shipped E3 child: `#2746`, report/label object-rename-batch rollback payload evidence; the stable FRX/LBX `--visual-object-rename-batch` rollback regressions now pin `en-US` host locale state before asserting English collision prose, the broad `#1843` rollback assertions now track the current identity-value collision payload deterministically instead of drifting against leaked locale state, and the focused `test_studio_host_object_rename_batch_errors` host regression now proves report/label rename-batch rollback failures keep earlier identity mutations rolled back while reporting the current identity-value collision JSON error for both reports and labels without changing production behavior.
