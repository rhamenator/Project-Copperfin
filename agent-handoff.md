...
- Last shipped localization child: `#2602`, the remaining native `Vfp.AssetInspector.*`, `Vfp.CdxHeader.*`, `Vfp.DbfHeader.*`, `Vfp.DbfTable.*`, and `Vfp.IndexProbe.*` user-facing strings now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused VFP/DBF coverage now proves locale-specific key presence, placeholder preservation, and pseudo-localized resolution without changing DBF/CDX/index identifiers, parser behavior, or other machine-readable/runtime contracts.
