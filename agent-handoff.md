...
- Last shipped E3 child: `#3066`, shared missing EXPR-backed root-setting parity; the shared VSIX/standalone report-settings selection model now materializes the remaining add-safe absent FRX/LBX root settings that the current host path can create through the shared `EXPR` update seam, keeps optional numeric fields blank in the property grid until the user edits them, and focused shared plus host-backed real-asset smokes now prove `DEFAULTSOURCE` add/reload/undo continuity on `by_author.FRX` and `STYLELBL.LBX` without changing host JSON field names, invariant property names, or machine-readable contracts.
