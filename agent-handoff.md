...
- Next action: execute `#3450`, the next prompt-sized `#3217` child, to validate and, only if needed, harden representative same-PRG `DEFINE CLASS ... AS CursorAdapter ... ENDDEFINE` activation so `CREATEOBJECT()` and `NEWOBJECT()` can instantiate another common-in-the-wild builtin object base without regressing stack-frugal execution, native identity surfaces, localization constraints for new diagnostics, or the already-shipped same-PRG builtin-base native class runtime behavior.
