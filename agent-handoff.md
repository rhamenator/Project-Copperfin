...
- Last shipped localization child: `#2584`, the remaining top-level shared `Command.*`, `Diagnostic.*`, `Error.*`, and `Help.*` keys now exist in `es-419`, `pt-BR`, and `qps-ploc`, with focused localization coverage proving the real shipped catalogs preserve placeholders, parser tokens, locale identifiers, and pseudo-localization behavior without falling back to raw English for the covered shared-core surfaces.
