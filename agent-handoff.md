...
- Last shipped slice: `#3241`, the thirteenth prompt-sized child under `#3217`, now routes `SETPem()` writes for same-PRG native accessor-backed properties through matching `*_ASSIGN` methods before any raw-property fallback, so dynamic property updates observe the same effective behavior as ordinary member assignment while preserving existing COM/plain-object `SETPem()` behavior. Focused runtime-surface, parser-class, and control-flow regressions prove assigner-backed writes update backing state visible through ordinary reads plus `GETPEM()`, access-only properties stay read-only to `SETPem()`, and runtime stability remains under the same stack-frugal execution model.
...
- Next action: execute `#3256`, the next prompt-sized `#3217` child, so `NEWOBJECT('<class>', '<file>.prg')` treats explicit `.prg` library targets as native PRG class sources instead of bare constructor arguments while preserving the existing stack-frugal PRG frame machine, same-PRG native-class behavior, and COM/server-targeted `NEWOBJECT()` activation.
