...
- Last shipped E3 child: `#3191`, the shared real `addlabel.pjx` debugger UI smoke now clicks the embedded `Restart` command after the first session loads, asserts a fresh restarted debug session with preserved runtime/debug manifest paths plus zero inherited build warnings, and fails if restart drops the staged `wzcommon/registry.vcx` dependency or leaves stale command history in place, so the editor-shell debugger restart path can no longer hide packaging regressions behind a successful first pause state.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3191`, the real `addlabel` project workflow path is now covered for project-editor open plus editor-shell build/run success, managed build/run success, replay/debug success, UI-debugger initial-session success, and UI-debugger restart success, all with warning-free packaging and staged shared-class dependency fidelity across the shared build/run/replay and embedded debugger surfaces, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 debugger-restart hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeProjectDebuggerWithRealAsset` directly against real `addlabel.pjx`, proving the embedded shared debugger surface now restarts into a fresh session with preserved runtime/debug manifest paths plus inherited build warning metadata and that the UI-facing real-project smoke enforces a warning-free manifest with staged `wzcommon/registry.vcx` instead of accepting only the first pause state. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
