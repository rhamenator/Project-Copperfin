...
- Last shipped E3 child: `#2757`, report/label subtree deleted-state success evidence; the stable FRX/LBX `--subtree-deleted-state` delete assertions in `test_studio_host_json` now track the current shared report-layout behavior instead of stale null containing sections, and the focused `test_studio_host_subtree_deleted_state` host regression now proves report/label subtree delete and restore success preserve the current preview bounds, section-aware selection payloads, object counts, and stable identities for both reports and labels, without changing production behavior.
