...
- Last shipped localization child: `#2623`, the remaining `Studio.Toolbox.Category.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused context-editor-actions coverage now proves translated and pseudo-localized toolbox category labels without changing toolbox item ids, VFP/base-class metadata, default name prefixes, report-context filtering, or other machine-readable toolbox palette contracts.
