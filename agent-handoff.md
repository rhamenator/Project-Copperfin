...
- Last shipped E3 child: `#2743`, deleted object-rename-batch rollback payload evidence; the stable deleted FRX/LBX `--visual-object-rename-batch` rollback regressions now pin `en-US` host locale state before asserting English identity-collision prose, the broad `#1862` rollback assertions now track the current identity-value collision payload deterministically instead of drifting against leaked locale state, and the focused `test_studio_host_deleted_object_rename_batch_errors` host regression now proves deleted placed rename-batch rollback failures keep earlier rename mutations rolled back while reporting the current collision JSON error for both reports and labels without changing production behavior.
