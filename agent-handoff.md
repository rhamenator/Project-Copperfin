...
- Last shipped localization child: `#2620`, the remaining `Studio.BuilderDispatch.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused builder-registry coverage now proves translated and pseudo-localized request errors without changing builder ids, context tokens, command tokens, launch entry points, execution flags, or other machine-readable builder-dispatch contracts.
