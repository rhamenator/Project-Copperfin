...
- Last shipped localization child: `#2416`, Studio host selection-toolbox batch-create parser diagnostics route through the portable C++ localization catalog while preserving CLI options, command names, selection-context tokens, toolbox item ids, JSON fields/status contracts, `name=value` syntax, and exit codes.
