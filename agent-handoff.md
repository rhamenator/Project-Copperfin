...
- Last shipped E3 child: `#3021`, real report paper-dimension settings continuity; the shared VSIX/standalone designer path now treats `PAPERLENGTH` and `PAPERWIDTH` as localized numeric report-settings fields, focused shared smokes prove live and deleted settings-root continuity plus host-refresh continuity for those invariant property names, and a focused real-asset smoke now proves the same shared selection model surfaces `PAPERLENGTH` and `PAPERWIDTH` from `DATAEXPLORERQUICKREPORT.FRX` without changing host JSON keys or machine-readable setting names.
