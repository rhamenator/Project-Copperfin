...
- Last shipped localization child: `#2591`, the interactive runtime-host quit-confirm prompt now routes through `RuntimeHost.Prompt.QuitConfirm` catalog entries in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused shipped-catalog plus runtime-host coverage proves localized quit-confirm prose without changing quit callback semantics, success exit behavior, or the live `[y/N]` confirmation tokens.
