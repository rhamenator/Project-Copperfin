...
- Last shipped E3 child: `#2738`, deleted object-rename-batch section-context evidence; the stable deleted FRX/LBX `--visual-object-rename-batch` reopen regressions now assert the current shared report-layout behavior instead of stale null containing sections, and the focused `test_studio_host_deleted_object_rename_batch_sections` host regression now proves deleted placed rows reopened after stable-identity rename-batch keep live detail-band ids, section-relative geometry, and per-section deleted-object counts for both reports and labels without changing production behavior.
