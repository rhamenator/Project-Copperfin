...
- Last shipped E3 child: `#2939`, shared report grouping-expression provenance exposure; the shared FRX/LBX section property-grid path now exposes read-only grouping expression field-index and memo-block metadata for grouped report and label sections in both the VSIX and standalone Studio while preserving invariant raw `groupingExpressionFieldIndex` and `groupingExpressionMemoBlockNumber` snapshot/host contract values; focused designer smokes now prove `es-419`, `pt-BR`, and `qps-ploc` routing for the new provenance labels and that the managed section snapshot keeps the same machine-readable provenance fields.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2939` exposed shared grouping-expression provenance on section property grids, prefer the next direct FRX/LBX gap around section/write-back round-trip fidelity, remaining hard-coded report/label labels or summaries, unplaced-object preview context, object-selection drag/scope continuity, or other section-aware property/editing seams unless build-pressure evidence still requires another narrowly owned shard extraction first.
