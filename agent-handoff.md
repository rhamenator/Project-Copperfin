...
- Last shipped E3 child: `#2736`, deleted property-rejection section-context evidence; the stable deleted FRX/LBX `--visual-property-rename` and `--visual-property-reorder` rejection regressions now assert the current shared report-layout behavior instead of stale null containing sections, and the focused `test_studio_host_deleted_property_rejection_sections` host regression now proves deleted placed rows reopened after direct-field rejection keep live detail-band ids, section-relative geometry, and per-section deleted-object counts for both reports and labels without changing production behavior.
