...
- Last shipped E3 child: `#3204`, the shared real `bandedm.FRX` deleted reorder command smoke now captures the source live preview-bounds geometry before opening the editor, asserts the deleted host snapshot keeps live `PREVIEWBOUNDS` stable after delete and reorder operations, and fails if `DELETEDPREVIEWBOUNDS` drift out of sync with reordered deleted-object geometry after delete, reorder, or reload, so the shared asset-editor deleted-row workflow can no longer accept preview-metadata regressions hidden behind superficially correct design-surface ordering alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3204`, the shared asset-editor deleted-reorder preview seam is now covered alongside the existing host-backed settings-root, deleted-object, live duplicate/rename/reorder/placement, representative live property/batch, and representative section-property real-asset evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, or recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 deleted-reorder preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono probe that invoked `SmokeAssetEditorDeletedReorderCommandWithRealAsset` for the grouped real `bandedm.FRX` deleted-reorder case under the resolved local toolchain, proving live `PREVIEWBOUNDS` and reordered `DELETEDPREVIEWBOUNDS` stay aligned through delete, reorder, and reload in the current shared editor/native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
