...
- Last shipped localization child: `#2603`, the remaining shared `Runtime.IndexSeek.*` optimizer strings now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, the shared index-seek optimizer now routes human-facing prose through the runtime locale helper instead of a one-time static catalog snapshot, and focused optimizer coverage now proves locale-specific key presence, placeholder preservation, pseudo-localized resolution, and in-process locale switching without changing optimizer enums, selected order names, execution strategies, or other machine-readable/runtime contracts.
