...
- Last shipped E3 child: `#2908`, report/label `TAG` sort-metadata exposure; `build_report_layout(...)` now includes nonblank FRX/LBX root `TAG` fields in live/deleted settings snapshots, the focused native `test_report_layout_sort_settings` target locks down live/deleted/blank behavior, and the focused selected-settings FRX/LBX host regressions now prove `reportLayout.settings`, `deletedSettings`, and `selectedReportSettings` carry the stable `TAG` provenance without changing host JSON keys.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2908` closed the missing root `TAG` sort-setting exposure, the next slice should stay on evidence-backed direct FRX/LBX fidelity gaps rather than helper-only broad-suite surgery unless a remaining broad-owned report/label shard proves materially expensive again.
