...
- Last shipped E3 child: `#2937`, shared report grouping-index property exposure; the shared FRX/LBX section property-grid path now exposes read-only grouping index and localized grouping nesting-depth metadata for grouped report and label sections in both the VSIX and standalone Studio while preserving invariant raw `groupingIndex` and `groupingNestingDepth` snapshot/host contract values; focused designer smokes now prove `es-419`, `pt-BR`, and `qps-ploc` routing for the new visible grouping metadata and that the managed section snapshot keeps the same machine-readable grouping fields.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2937` exposed shared grouping index and nesting-depth metadata on section property grids, prefer the next direct FRX/LBX gap around section/write-back round-trip fidelity, remaining hard-coded report/label labels or summaries, unplaced-object preview context, object-selection drag/scope continuity, or other section-aware property/editing seams unless build-pressure evidence still requires another narrowly owned shard extraction first.
