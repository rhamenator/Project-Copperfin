...
- Last shipped E3 child: `#3057`, real report/label section-top continuity; the shared FRX/LBX section-top write path now reparses the full table when a band record’s `VPOS` is edited and batches contained placed-object `VPOS` shifts in the same command so real section-top edits preserve section-relative geometry, while the shared report-layout overlap rule now prefers the deepest overlapping section for both host snapshots and write-back targeting, the shared VSIX/standalone undo path explicitly rebinds non-object explorer selections after command undo, and focused native plus real-asset designer regressions now cover `by_author.FRX`, `STYLELBL.LBX`, and the real `bandedm.FRX` deleted-reorder overlap seam without changing host JSON field names, invariant property names, or machine-readable contracts.
