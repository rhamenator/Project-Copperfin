...
- Last shipped localization child: `#2604`, the remaining `Runtime.XAsset.Error.TablePreviewMissing` and `Runtime.XAsset.Error.UnsupportedExecutableFamily` strings now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, the shared xAsset executable-model helper now routes human-facing diagnostics through the runtime locale helper instead of a one-time static catalog snapshot, and focused xAsset coverage now proves locale-specific key presence, pseudo-localized resolution, and in-process locale switching without changing asset kinds, action ids, routine names, source-field ordinals, memo-block provenance, or other machine-readable/runtime contracts.
