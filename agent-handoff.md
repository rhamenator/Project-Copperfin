...
- Last shipped E3 child: `#3022`, real report printer-identity settings continuity; the shared VSIX/standalone designer path now routes invariant `DRIVER`, `DEVICE`, and `OUTPUT` report-setting names through localized display labels, focused shared smokes prove live and deleted settings-root continuity plus host-refresh continuity for those string settings, and a focused real-asset smoke now proves the same shared selection model surfaces printer-identity values from `STYLE1H.FRX` without changing host JSON keys or machine-readable setting names.
