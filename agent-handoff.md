...
- Last shipped localization child: `#2608`, the remaining `Runtime.Prg.Cursor.Error.*` diagnostics now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization coverage now proves translated prose, placeholder preservation, and pseudo-localized resolution without changing `USE`/`SEEK` behavior, alias/work-area resolution, runtime identifiers, or other machine-readable/runtime contracts.
