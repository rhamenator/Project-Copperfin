...
- Last shipped localization child: `#2587`, the remaining `RuntimeHost.Bridge.Error.*` keys now exist in `es-419`, `pt-BR`, and `qps-ploc`, with focused runtime-host coverage proving localized bridge request-artifact failures without changing `runtime.mode: bridge-invocation`, request/response paths, media types, schema versions, exit codes, or machine-readable status fields.
