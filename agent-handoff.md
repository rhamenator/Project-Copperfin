...
- Last shipped localization child: `#2706`, the remaining hard-coded PRG `ERASE`, `COPY FILE`, and `RENAME` runtime failures now route through `Runtime.Prg.Dispatch.Error.*` catalog entries with shipped parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization plus PRG control-flow coverage now proves pseudo-localized file-operation failures without changing PRG command syntax, OS error propagation, path handling, or other machine-readable runtime contracts.
...
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, with the adjacent `COPY TO` / `APPEND FROM` families and other remaining dispatch/runtime faults as the next likely slices now that `SAVE TO` / `RESTORE FROM` and file-operation failures are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
