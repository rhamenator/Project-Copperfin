...
- Last shipped E3 child: `#2690`, live report-object deleted-section fidelity; the shared report-layout snapshot now preserves containing deleted-section identity, relative geometry, and section-local object order/count metadata for live FRX/LBX objects whose geometry belongs to deleted bands, deleted sections now retain that live object membership instead of empty object arrays, `copperfin_studio_host --json` now resolves `selectedReportObjectSection` for those selections, and focused report-layout plus direct host validation now prove live objects inside deleted sections stay placed with deleted-preview context while truly unplaced objects still keep containing-section JSON absent.
