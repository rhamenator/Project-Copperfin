...
- Last shipped slice: `#3261`, the nineteenth prompt-sized child under `#3217`, now supports narrow same-PRG native `THIS.RemoveObject('<name>')` detachment, removing the contained child reference from the parent, clearing the detached child object's persisted `PARENT` reference, and making `GETPEM()` / `PEMSTATUS()` on the removed member disappear consistently while leaving any separately held child object reference otherwise intact. Focused runtime-surface, parser-class, and control-flow regressions prove child detachment, removed-member disappearance, preserved detached-child state, and runtime stability all remain under the same stack-frugal execution model.
...
- Next action: execute `#3262`, the next prompt-sized `#3217` child, so native child-object methods can resolve `THISFORMSET` through the current owner chain while preserving the existing stack-frugal PRG frame machine, same-PRG/external-PRG activation, and newly shipped `PARENT` / `THISFORM` / containment behavior.
