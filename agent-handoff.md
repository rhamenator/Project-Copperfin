...
- Last shipped localization child: `#2713`, the remaining direct hard-coded PRG `CREATE CURSOR`, `CREATE TABLE`, and `ALTER TABLE` precondition failures now route through `Runtime.Prg.Dispatch.Error.*` catalog entries with shipped parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization plus table-structure coverage now proves pseudo-localized DDL failures without changing PRG command syntax, field-declaration parsing, alter-action semantics, or other machine-readable runtime contracts.
...
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, with the smaller leftover runtime clusters around `DECLARE`, `DIMENSION/DECLARE`, `SET FILTER`, `SLEEP`, and the top-level runtime-fault families as the next likely slices now that `APPEND FROM`, `SCATTER`, `GATHER`, `CREATE CURSOR/TABLE`, `ALTER TABLE`, `COPY TO`, `SAVE TO` / `RESTORE FROM`, and file-operation failures are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
