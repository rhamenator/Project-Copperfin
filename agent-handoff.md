...
- Last shipped localization child: `#2621`, the remaining `Studio.EditorActionDispatch.CatalogEntry.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused context-editor-actions coverage now proves translated and pseudo-localized catalog-entry errors without changing action ids, selection-context tokens, command tokens, execution flags, or other machine-readable editor-action-dispatch contracts.
