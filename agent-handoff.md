...
- Last shipped localization child: `#2599`, the existing extensibility profile AI-feature, language, .NET parity, .NET interop decision, and guardrail keys under `Platform.Extensibility.*` now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused platform-model coverage proves localized extensibility-profile and policy-gateway prose without changing language ids, AI feature ids, parity ids, reason tags, decision enums, execution-path values, or other machine-readable profile contracts.
