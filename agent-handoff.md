...
- Last shipped E3 child: `#3186`, `CopperfinRuntimeDebugSession` now preserves runtime-manifest path plus managed build warning count/details from `CopperfinRuntimeDebugClient.StartSessionAsync`, and the shared real `addlabel.pjx` replay smoke now asserts a warning-free inherited build, zero manifest warning lines, and the staged `wzcommon/registry.vcx` dependency so replay coverage can no longer hide packaging regressions behind a successful pause state.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3186`, the real `addlabel` project workflow path is now covered for build success, replay/debug success, warning-free packaging, and staged shared-class dependency fidelity across both build and replay flows, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, shared build/replay workflow, shared UI debugger path, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 managed-replay hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeProjectDebugReplayWithRealAsset` directly against real `addlabel.pjx`, proving the managed replay path now preserves runtime-manifest path plus inherited build warning metadata and that the shared real-project replay smoke enforces a warning-free manifest with staged `wzcommon/registry.vcx` instead of accepting pause-state success alone. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
