...
- Last shipped E3 child: `#3064`, real memo-backed printer-setting add/remove continuity; the shared VSIX/standalone report-settings selection model now exposes blank editable `DRIVER`, `DEVICE`, and `OUTPUT` fields when those known string-backed root settings are absent from the live FRX/LBX `EXPR` memo, and focused native plus shared host-backed and VSIX/standalone real-asset DRIVER smokes now prove update, reload, and undo continuity for `by_author.FRX` and `STYLELBL.LBX` against the stable `reportLayout.settings` contract without changing host JSON field names, invariant property names, or machine-readable contracts.
