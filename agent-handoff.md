...
- Last shipped E3 child: `#2760`, report/label update-batch rollback evidence; the focused `test_studio_host_update_batch_errors` host regression now proves stable FRX/LBX `--visual-object-update-batch` rollback on a missing stable selector preserves the current null-batch JSON error payload, omits stale success-only fields, and restores earlier direct and memo-backed property values for both reports and labels, without changing production behavior.
