...
- Last shipped localization child: `#2548`, FOXTOOLS load and registered API handle precondition errors route through the portable C++ localization catalog while preserving the `FOXTOOLS` product token, API handle values, interop event categories, function names, parser behavior, machine-readable state, and default en-US output.
