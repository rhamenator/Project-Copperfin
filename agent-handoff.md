...
- Last shipped slice: `#3422`, the next prompt-sized child under `#3217`, shipped as evidence closure: focused runtime-surface regressions now prove representative same-PRG native `DEFINE CLASS ... AS Label ... ENDDEFINE` classes can be instantiated through `CREATEOBJECT()` and `NEWOBJECT()` with ordinary property initialization, method invocation, `BaseClass`/`Class` identity surfaces, native class-hierarchy storage, empty same-PRG `ClassLibrary` semantics, and stable COM behavior, without additional runtime changes.
...
- Next action: execute `#3423`, the next prompt-sized `#3217` child, to validate and, only if needed, harden representative same-PRG `DEFINE CLASS ... AS Container ... ENDDEFINE` activation so `CREATEOBJECT()` and `NEWOBJECT()` can instantiate a common-in-the-wild non-leaf visual base without regressing stack-frugal execution, native identity surfaces, localization constraints for new diagnostics, or the already-shipped same-PRG builtin-base native class runtime behavior.
