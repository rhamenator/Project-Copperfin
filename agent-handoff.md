...
- Last shipped localization child: `#2606`, the remaining `Runtime.Package.Error.*` diagnostics now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, the shared runtime-pipeline helper now routes package diagnostics through the runtime locale helper instead of a one-time static catalog snapshot, and focused runtime-pipeline coverage now proves locale-specific key presence, placeholder preservation, pseudo-localized resolution, and in-process locale switching without changing manifest/debug-manifest field names, output kinds, file paths, security-profile ids, runtime-host names, or other machine-readable/runtime contracts.
