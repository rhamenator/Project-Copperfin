...
- Last shipped E3 child: `#2772`, report/label stable vertical-grid setting success evidence; the focused `test_studio_host_vertical_grid_stable_settings` host regression now proves FRX/LBX stable-selected `GRIDV` direct-field update and clear success for live and deleted settings rows preserve the current page-setup/settings counts, selected-settings provenance, cleared-field behavior, and non-fabricated preview state for both reports and labels, without changing production behavior.
