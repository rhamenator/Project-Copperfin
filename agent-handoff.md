...
- Last shipped localization child: `#2712`, the remaining direct hard-coded PRG `SCATTER` and `GATHER` no-work-area, no-current-record, `FIELDS`-mismatch, and missing-object-variable failures now route through `Runtime.Prg.Dispatch.Error.*` catalog entries with shipped parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused localization plus PRG data-IO coverage now proves pseudo-localized `SCATTER`/`GATHER` failures without changing PRG command syntax, field-filter behavior, object-variable semantics, or other machine-readable runtime contracts.
...
- Next action: continue prompt-sized localization children from the remaining hard-coded PRG/runtime prose in `src/runtime/prg_engine_dispatch.inl`, with the smaller leftover runtime clusters around `SCATTER NAME` object-creation helpers, declaration/DDL preconditions, and the top-level precondition/runtime-fault families as the next likely slices now that `APPEND FROM`, `SCATTER`, `GATHER`, `COPY TO`, `SAVE TO` / `RESTORE FROM`, and file-operation failures are routed through the shared catalog; verify live GitHub state first in case a narrower child already exists.
