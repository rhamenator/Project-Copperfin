...
- Last shipped E3 child: `#3184`, the runtime package pipeline now falls back on a case-insensitive tail-match search under the extracted project source root when a VFP parent-relative asset path still misses after normal resolution, which resolves the real `addlabel.pjx` `..\wzcommon\registry.vcx` dependency from the VFPSource sample layout, stages it safely as `content/wzcommon/registry.vcx`, and removes the last build warning for that sample without changing project-workspace metadata or machine-readable manifest path contracts.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3184`, the `addlabel` shared-class lookup seam is covered end-to-end, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, shared build/replay workflow, shared UI debugger path, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 lookup-fidelity slice used `git diff --check`, `cmake --build build --target test_runtime_pipeline -j4`, `ctest --test-dir build --output-on-failure -R '^test_runtime_pipeline$'`, `cmake --build build --target copperfin_build_host -j4`, and a direct `COPPERFIN_SECURITY_ROLE=build-engineer copperfin_build_host build` repro against real `addlabel.pjx`, proving both the synthetic direct-parent `..\wzcommon\registry.vcx` path and the VFPSource-style `Wizards/wzcommon/REGISTRY.VCX` sample layout now resolve, stage safely under `content/`, and clear the last real-sample missing-asset warning without changing project-workspace metadata or manifest path contracts. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
