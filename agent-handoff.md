...
- Last shipped E3 child: `#2742`, deleted object-duplicate-batch rollback payload evidence; the stable deleted FRX/LBX `--visual-object-duplicate-batch` rollback regressions now pin `en-US` host locale state before asserting English collision prose, the broad `#1860` rollback assertions now track the current replacement-identity collision payload deterministically instead of drifting against leaked locale state, and the focused `test_studio_host_deleted_object_duplicate_batch_errors` host regression now proves deleted placed duplicate-batch rollback failures keep earlier duplicate mutations rolled back while reporting the current collision JSON error for both reports and labels without changing production behavior.
