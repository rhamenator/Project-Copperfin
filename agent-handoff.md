...
- Last shipped E3 child: `#2775`, report/label stable paper-size setting success evidence; the focused `test_studio_host_paper_size_stable_settings` host regression now proves FRX/LBX stable-selected `PAPERSIZE` direct-field update and clear success for live and deleted settings rows preserve the current page-setup/settings counts, selected-settings provenance, cleared-field behavior, and non-fabricated preview state for both reports and labels, without changing production behavior.
