...
- Last shipped E3 child: `#2765`, deleted report/label duplicate-batch success evidence; the focused `test_studio_host_deleted_object_duplicate_batch` host regression now proves deleted FRX/LBX `--visual-object-duplicate-batch` success preserves the current batch result payload, appends deleted duplicates without changing the original rows or deleted-state membership, and keeps the refreshed reopen selection, containing-section, and preview metadata for both reports and labels, without changing production behavior.
