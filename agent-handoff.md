...
- Last shipped E3 child: `#3025`, real report auxiliary print settings continuity; the shared VSIX/standalone designer path now routes invariant `ASCII`, `COLLATE`, and `COPIES` report-setting names through localized numeric property handling plus localized display labels, focused shared smokes prove live and deleted settings-root continuity plus host-refresh continuity for those auxiliary print settings, and a focused real-asset smoke now proves the same shared selection model surfaces those values from `foxrefresultsa4.frx` without changing host JSON keys or machine-readable setting names.
