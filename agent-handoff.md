...
- Last shipped localization child: `#2590`, the remaining shared runtime watch-evaluation `Runtime.Prg.Watch.Error.*` keys now exist in `es-419`, `pt-BR`, and `qps-ploc`, and focused runtime-host plus shipped-catalog coverage proves localized paused-watch failures without changing `debug.watch.ok`, `debug.watch.expression`, `debug.reason`, breakpoint behavior, or other machine-readable runtime-host fields.
