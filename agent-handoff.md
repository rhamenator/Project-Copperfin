...
- Last shipped localization child: `#2595`, the shared aggregate command diagnostics for missing `CALCULATE` assignments, malformed aggregate expressions, and multi-target `COUNT TO` now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused shipped-catalog plus PRG control-flow coverage proves localized aggregate command failures without changing `CALCULATE`/`COUNT TO` behavior, command semantics, or other machine-readable runtime contracts.
