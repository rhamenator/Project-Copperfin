...
- Last shipped E3 child: `#2781`, report/label record-selected horizontal-grid setting success evidence; the focused `test_studio_host_horizontal_grid_record_settings` host regression now proves FRX/LBX record-selected `GRIDH` direct-field update and clear success for live and deleted settings rows preserve the current page-setup/settings counts, selected-settings provenance, cleared-field behavior, and non-fabricated preview state for both reports and labels, without changing production behavior.
