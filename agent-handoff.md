...
- Last shipped E3 child: `#2747`, report/label object deleted-states rollback evidence; the focused `test_studio_host_object_deleted_states_errors` host regression now proves FRX/LBX `--deleted-states` object batches roll back earlier object deletions when a later stable selector is missing while preserving the current invariant error-status payload and asset object counts for both reports and labels, without changing production behavior.
