...
- Last shipped E3 child: `#3012`, broadened real-asset deleted-preview settings discovery; the shared designer-smoke harness now searches the broader resolved FRX/LBX corpus from configured VFP roots and extracted real-asset archives before deciding whether deleted-preview-bounds settings continuity has a qualifying real sample, so the current explicit skip now reflects corpus-wide absence in the available test assets rather than a narrow fixed-file list, and no host JSON or machine-readable contracts changed.
