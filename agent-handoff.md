...
- Last shipped slice: `#3260`, the eighteenth prompt-sized child under `#3217`, now walks contained native child-object references segment by segment for ordinary dotted assignment, including `Parent` hops, so statements like `oForm.cmdSave.Caption = 'OK'` and `oForm.cmdSave.Parent.Caption = 'Done'` update the actual contained target object instead of flattening the full dotted suffix onto the root object. Focused runtime-surface, parser-class, and control-flow regressions prove direct dotted child writes, dotted parent-chain writes, stable follow-on reads/method calls, and runtime stability all remain under the same stack-frugal execution model.
...
- Next action: execute `#3261`, the next prompt-sized `#3217` child, so same-PRG native containment gains narrow `REMOVEOBJECT()` support with child detachment semantics while preserving the existing stack-frugal PRG frame machine, same-PRG/external-PRG activation, and newly shipped `ADDOBJECT()` / `PARENT` / `THISFORM` / dotted containment behavior.
