...
- Last shipped E3 child: `#2766`, report/label reorder-batch success evidence; the focused `test_studio_host_object_reorder_batch` host regression now proves stable FRX/LBX `--visual-object-reorder-batch` success preserves the current batch result payload, applies ordered stable-selector moves without changing identities, and keeps the refreshed reopen selection and preview metadata for both reports and labels, without changing production behavior.
