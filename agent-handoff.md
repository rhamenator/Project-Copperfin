...
- Last shipped E3 child: `#2898`, geometry preview-bounds broad-suite retirement; the broad `test_studio_host_json` target no longer compiles `tests/test_studio_host_json_geometry_preview_bounds.cpp`, instead resolving its shared preview-bound expectations through `tests/test_studio_host_json_geometry_preview_helpers.cpp` while the focused report width/left, height/top, and detail-header/footer section preview-bound executables keep ownership of the geometry preview test bodies without host JSON or production-behavior changes.
