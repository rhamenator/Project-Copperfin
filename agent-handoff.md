...
- Last shipped localization child: `#2596`, the shared SQL runtime diagnostics for missing SQL handles and missing `SQLEXEC` command/prepared statements now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused shipped-catalog plus PRG control-flow coverage proves localized SQL runtime failures without changing `SQLEXEC`/`SQLDISCONNECT` behavior, AERROR payload shape, handle values, or other machine-readable runtime contracts.
