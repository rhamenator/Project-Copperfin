...
- Last shipped E3 child: `#2894`, redundant report direct-setting validation broad-suite shard retirement; the broad `test_studio_host_json` target no longer compiles `tests/test_studio_host_json_settings_direct_field_validation.cpp`, leaving the blank/mixed-invalid/trimmed/fractional/oversized/dot-leading report direct-setting validation coverage owned by the focused `test_studio_host_report_direct_setting_fields` executable while the broad suite and focused direct-setting executable remain green without any host JSON or production-behavior changes.
