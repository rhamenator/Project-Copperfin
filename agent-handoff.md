...
- Last shipped E3 child: `#2758`, deleted report/label update-batch success evidence; the focused `test_studio_host_deleted_update_batch` host regression now proves deleted FRX/LBX `--visual-object-update-batch` success preserves the current batch result payload, persisted property updates, deleted preview bounds, deleted-row section metadata, and stable identities for both reports and labels, without changing production behavior.
