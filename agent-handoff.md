...
- Last shipped localization child: `#2489`, report/label layout section display titles route through the portable C++ localization catalog while preserving `band_kind` values, raw FRX/LBX `OBJCODE` semantics, JSON/status contracts, parser behavior, and default en-US output.
