...
- Last shipped localization child: `#2585`, the remaining top-level `RuntimeHost.Usage.*`, `RuntimeHost.Error.*`, and `RuntimeHost.Launch.Note.*` keys now exist in `es-419`, `pt-BR`, and `qps-ploc`, with focused runtime-host coverage proving localized usage, top-level command errors, federation validation, and compatibility-note surfaces without changing CLI tokens, placeholders, backend ids, permissions, roles, or machine-readable status fields.
