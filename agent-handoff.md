...
- Last shipped E3 child: `#3197`, the shared real `by_author.FRX` and `STYLELBL.LBX` host-backed live duplicate and live rename smokes now capture the source preview-bounds geometry and fail if `PREVIEWBOUNDS` drift during duplicate, rename, reload, or undo, so live identity coverage can no longer accept host-JSON preview regressions hidden behind correct UNIQUEID and section-count behavior alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3197`, the host-backed real preview-metadata seams for deleted-object workflows plus live duplicate/rename now have explicit real-asset evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, remaining live-object property/batch/placement/reorder preview-metadata seams, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed live identity preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedDuplicateRoundTrip` plus `SmokeRealAssetHostBackedRenameRoundTrip` under the resolved local toolchain against real `by_author.FRX` and `STYLELBL.LBX`, proving source `PREVIEWBOUNDS` geometry stays stable through live duplicate, rename, reload, and undo in the current native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
