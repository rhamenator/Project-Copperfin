...
- Last shipped localization child: `#2597`, the shared report output and report-asset runtime diagnostics for missing output paths, output-open failures, output-write failures, and unresolved report assets now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused shipped-catalog plus PRG control-flow coverage proves localized report runtime failures without changing preview semantics, normal `REPORT/LABEL ... TO FILE <path>` behavior, or other machine-readable runtime contracts.
