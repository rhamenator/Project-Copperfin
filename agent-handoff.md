...
- Last shipped localization child: `#2592`, the remaining runtime-host security-denial audit details for `project.open`, `runtime.admin`, and manifest-hash verification denials now reuse localized runtime-host prose in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused shipped-catalog plus runtime-host coverage proves localized console/audit detail text without changing exit codes, `policy.denied`, permission ids, role ids, or immutable-audit chain format.
