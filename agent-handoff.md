...
- Last shipped E3 child: `#3013`, broadened remaining real-asset settings candidate discovery; the shared designer-smoke harness now resolves sort-metadata, document-title, preview-bounds, and deleted-preview-bounds settings continuity candidates from the broader resolved FRX/LBX corpus exposed by configured VFP roots and extracted real-asset archives instead of a narrow preferred-file list, proving live sort/document-title/preview continuity against the available corpus while the deleted-preview path still skips because no current real asset exposes the required settings-plus-deleted-preview combination, and no host JSON or machine-readable contracts changed.
