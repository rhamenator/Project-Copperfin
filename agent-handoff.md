...
- `#3211` tracks two pre-existing bugs discovered while adding the licensing work above (unrelated to licensing itself, both confirmed pre-existing via A/B revert): `tests/test_security_controls.cpp` fails a fresh `/sdl` compile on an unguarded `getenv` call, and `copperfin_studio_host --json` crashes with a stack overflow on any asset. Triage before closing.
