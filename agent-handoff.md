...
- Last shipped E3 child: `#2980`, VFPSource-backed real-asset smoke fallback coverage; the shared designer smoke harness now resolves optional FRX/LBX/PJX/SCX/VCX/MNX evidence from `COPPERFIN_VFPSOURCE_ROOT`, `COPPERFIN_VFPSOURCE_ZIP`, or `~/Downloads/VFPSource.zip`, extracts the containing archive directories so sidecars and neighboring project assets stay available, auto-resolves local Studio/build/runtime host binaries from the repo for non-Windows execution, and proves real-asset editor, project-workspace, and standalone tab coverage against VFPSource samples without needing a full VFP9 install, while the real-project debugger smoke remains Windows-only pending validated runtime build execution support outside the Windows path.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2980` closed non-Windows real-asset smoke fallback resolution, `#2979` closed the shared placed-versus-unplaced transition seam, `#2978` closed the delete/restore lifecycle matrix, `#2977` closed rename continuity, and `#2976` closed front/back reorder continuity, prefer deeper real-asset round-trip coverage or richer section-aware multi-row write-back evidence where live repo state still shows a report/label gap, and treat the real-project debugger path as a separate Windows-first follow-up only if fresh evidence shows a real implementation seam rather than the current runtime build-execution environment limit.
