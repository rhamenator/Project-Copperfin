...
- Last shipped E3 child: `#2737`, deleted property-batch-rejection section-context evidence; the stable deleted FRX/LBX `--visual-property-rename-batch` and `--visual-property-reorder-batch` rejection regressions now assert the current shared report-layout behavior instead of stale null containing sections, and the focused `test_studio_host_deleted_property_batch_rejection_sections` host regression now proves deleted placed rows reopened after direct-field batch rejection keep live detail-band ids, section-relative geometry, and per-section deleted-object counts for both reports and labels without changing production behavior.
