...
- Last shipped E3 child: `#3070`, shared deleted-settings missing sort-setting continuity evidence; focused shared VSIX/standalone deleted-settings host smokes now prove the asset-editor property grid materializes blank editable `TAG` when that deleted FRX/LBX root sort-setting is absent, sends the invariant `--set-property` host update, and preserves deleted settings-rooted selection while surfacing derived sort metadata after the host-backed refresh without changing host JSON field names, invariant property names, or machine-readable contracts.
