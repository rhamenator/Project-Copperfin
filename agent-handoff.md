...
- Last shipped E3 child: `#3188`, the shared real `addlabel.pjx` run smoke now executes `CopperfinProjectWorkflow.ExecuteAsync(..., Run)`, asserts preserved output/manifest/debug-manifest/launcher paths plus zero inherited build warnings, and fails if the surfaced runtime manifest contains warning lines or loses the staged `wzcommon/registry.vcx` dependency, so the managed run path can no longer hide packaging regressions behind a superficially successful launcher start.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3188`, the real `addlabel` project workflow path is now covered for build success, managed run success, replay/debug success, UI-debugger success, warning-free packaging, and staged shared-class dependency fidelity across the shared build/run/replay and embedded debugger surfaces, so the next worthwhile slice should be whichever remaining E3 child is not already covered by the shared VFP9/VFPSource startup paths, fake-host layout-action families, and real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 run-workflow hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeProjectRunWorkflowWithRealAsset` directly against real `addlabel.pjx`, proving the managed shared run path now preserves output/manifest/debug-manifest/launcher paths plus inherited build warning metadata and that the real-project run smoke enforces a warning-free manifest with staged `wzcommon/registry.vcx` instead of accepting launcher-start success alone. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
