...
- Last shipped slice: `#3452`, the next prompt-sized child under `#3217`, shipped as evidence closure: focused runtime-surface regressions now prove representative same-PRG native `DEFINE CLASS ... AS Exception ... ENDDEFINE` classes can be instantiated through `CREATEOBJECT()` and `NEWOBJECT()` with ordinary property initialization, method invocation, `BaseClass`/`Class` identity surfaces, native class-hierarchy storage, empty same-PRG `ClassLibrary` semantics, and stable COM behavior, without additional runtime changes.
...
- Next action: execute `#3453`, the next prompt-sized `#3217` child, to harden `TRY/CATCH` parity so `CATCH TO oErr` binds a native `Exception` object instead of the current plain string fallback, while preserving `AERROR()`, `ERROR()`, `MESSAGE()`, `LINENO()`, `PROGRAM()`, stack-frugal execution, localization constraints for new diagnostics, and the already-shipped same-PRG native class activation behavior.
