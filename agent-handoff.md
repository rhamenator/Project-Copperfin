...
- Last shipped localization child: `#2594`, the shared query-translator `Platform.QueryTranslator.Error.SelectFromOnly` diagnostic now has shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused query-translator, shipped-catalog, and runtime-host coverage proves localized deterministic translator rejection payloads, including planner-fallback composition, without changing translator behavior, `runtime.mode: federation-query-plan`, exit codes, backend ids, or other machine-readable contract fields.
