...
- Last shipped localization child: `#2424`, Studio host direct toolbox plan parser diagnostics route through the portable C++ localization catalog while preserving CLI options, command names, toolbox-context tokens, toolbox item ids, JSON fields/status contracts, boolean tokens, `name=value` syntax, and exit codes.
