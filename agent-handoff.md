...
- Last shipped E3 child: `#2741`, deleted object-reorder-batch rollback payload evidence; the stable deleted FRX/LBX `--visual-object-reorder-batch` rollback regressions now pin `en-US` host locale state before asserting English error prose, the broad `#1861` rollback assertions now match the current missing-target payload shape instead of drifting against leaked locale state, and the focused `test_studio_host_deleted_object_reorder_batch_errors` host regression now proves deleted placed reorder-batch rollback failures keep earlier mutations rolled back while reporting the current missing-target JSON error for both reports and labels without changing production behavior.
