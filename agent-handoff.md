...
- Last shipped E3 child: `#3014`, shared report/label left/right page-margin settings exposure; the shared FRX/LBX report-layout parser now carries nonblank direct `LEFTMARGIN` and `RIGHTMARGIN` root settings through the existing settings snapshot contract for live and deleted roots, preserving invariant machine-readable property names and selected-settings host JSON while reusing the existing localized property-grid labels already present in the shared designer selection model, and focused native plus Studio-host regressions now prove live/deleted provenance and selected-settings parity without introducing new host JSON keys.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#3014` exposed shared left/right page-margin settings in the existing live/deleted settings snapshot and selected-settings host JSON, `#3013` broadened the remaining real-asset settings candidate discovery across the resolved FRX/LBX corpus, `#3011` closed the shared shell-summary deleted-count seam, `#3010` closed the live real-asset settings preview-bounds evidence seam, `#3009` closed the shared document-title settings seam, `#3008` closed the shared sort-metadata settings seam, `#3007` closed the shared grouping explorer/property-grid seam, `#3006` closed the shared root-settings preview-metadata seam, `#3005` closed the shared deleted-settings explorer/property-grid seam, `#3004` closed the shared settings-scope explorer/property-grid seam, `#3003` closed the shared section-expression provenance exposure seam, `#3002` closed the grouped report/footer section-expression parity seam, `#3001` closed the real grouped section continuity seam, `#3000` closed the real live rename continuity seam, `#2999` closed the real live reorder continuity seam, `#2995` closed the real deleted reorder continuity seam, `#2994` closed the real live label duplicate continuity seam, `#2993` closed the real deleted duplicate continuity seam, `#2992` closed the real deleted rename continuity seam, `#2991` closed the real deleted property-edit continuity seam, `#2990` closed the real deleted section-member continuity seam, `#2989` closed the real FRX/LBX single-command batch-undo seam, `#2988` closed the shared batch-refresh seam, `#2987` closed the real section-property continuity seam, `#2986` closed the real FRX duplicate reopen seam, `#2985` closed the real unplaced FRX placement-transition seam, `#2984` closed the real unplaced FRX continuity seam, `#2983` closed the real memo-backed write-back seam, `#2982` closed the shared property-grid direct-field seam, `#2981` closed the host-level real reload+undo seam, and `#2980` closed non-Windows real-asset smoke fallback resolution, prefer the next report/label real-asset or shared-model seam that still lacks evidence or implementation other than deleted-preview settings sample availability, since the current corpus-wide scan still finds no qualifying sample, and treat the real-project debugger path as a separate Windows-first follow-up only if fresh evidence shows a real implementation seam rather than the current runtime build-execution environment limit.
...
- Shell/UI backlog guidance is now captured in `#2996` (VSIX native shell standards), `#2997` (modern dockable standalone shell), and `#2998` (future curated standalone extension support without loading the Copperfin VSIX).
