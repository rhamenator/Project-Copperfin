...
- Last shipped E3 child: `#3024`, real report color settings continuity; the shared VSIX/standalone designer path now routes invariant `COLOR` report-setting names through localized numeric property handling plus localized display labels, focused shared smokes prove live and deleted settings-root continuity plus host-refresh continuity for that color setting, and a focused real-asset smoke now proves the same shared selection model surfaces that value from `by_author.FRX` without changing host JSON keys or machine-readable setting names.
