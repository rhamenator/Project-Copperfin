...
- Last shipped slice: `#3238`, the tenth prompt-sized child under `#3217`, now resolves bare same-PRG helper-method calls from inside native class methods against the current instance before unrelated top-level routines, so helper calls from `Init` and ordinary methods preserve `THIS`, inheritance, and by-reference behavior without requiring explicit `THIS.` syntax. Focused runtime-surface, parser-class, and control-flow regressions prove helper calls beat same-named top-level routines, helper calls from `Init` preserve by-reference write-back, and existing inheritance/`DODEFAULT()`/plain-object/COM activation all remain stable under the same stack-frugal execution model.
...
- Next action: execute `#3239`, the next prompt-sized `#3217` child, so same-PRG native `ACCESS` / `ASSIGN` property-method dispatch starts working through ordinary member syntax without disturbing the existing stack-frugal PRG frame machine, newly shipped inheritance/`DODEFAULT()`/bare-helper behavior, constructor/by-reference behavior, or COM/plain-object activation.
