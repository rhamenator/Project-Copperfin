...
- Next action: inspect live `#24` children or create the next prompt-sized E3 slice from current real gaps, with remaining likely seams around explicit grouping/sorting metadata exposure, shared host/designer round-trip fidelity, report/label runtime-preview hooks, and any other FRX/LBX behavior the live issue tree still shows as unproven now that stable-selected, record-selected, deleted, and nested group-section EXPR edit evidence is in place at both top-level and nested group depths.
