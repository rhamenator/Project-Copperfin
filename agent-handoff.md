...
- Last shipped slice: `#3239`, the eleventh prompt-sized child under `#3217`, now routes ordinary same-PRG native property reads and writes through inherited or local `*_ACCESS` / `*_ASSIGN` methods while preserving raw-property fallback when no accessor exists. Focused runtime-surface, parser-class, and control-flow regressions prove accessor-backed `o.Property` reads/writes, `THIS.Property` reads inside methods, inherited accessor dispatch, raw-property fallback, and existing inheritance/`DODEFAULT()`/bare-helper/plain-object/COM activation all remain stable under the same stack-frugal execution model.
...
- Next action: execute `#3240`, the next prompt-sized `#3217` child, so `GETPEM()` / `PEMSTATUS()` / `AMEMBERS()` reflect accessor-backed same-PRG native properties without disturbing the existing stack-frugal PRG frame machine, newly shipped inheritance/`DODEFAULT()`/bare-helper/`ACCESS`-`ASSIGN` behavior, constructor/by-reference behavior, or COM/plain-object activation.
