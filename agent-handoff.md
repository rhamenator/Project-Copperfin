...
- Last shipped E3 child: `#2975`, shared asset-editor duplicate object command parity; the shared VSIX/standalone report/label editor now routes object duplication through the existing Studio host `--duplicate-object` contract with invariant `--record`, stable `--unique-id`, and replacement `--new-unique-id` selectors, exposes localized duplicate command chrome, and focused designer smokes now prove report and label duplicate flows refresh shell summary details while moving explorer/property-grid/design-surface continuity onto the duplicated object without changing host or snapshot machine-readable contracts.
...
- Next action: continue live GitHub triage under E3/#24 by selecting the next prompt-sized child from the remaining report/label host/designer fidelity backlog; after `#2975` closed shared duplicate continuity and `#2974` closed delete/restore continuity, prefer the next direct FRX/LBX gap around stable-selector rename/reparent/reorder parity in the shared editor, deeper real-asset round-trip coverage, or richer section-aware multi-row write-back evidence where live repo state still shows a report/label gap.
