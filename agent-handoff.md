...
- Last shipped E3 child: `#3207`, the shared real `by_author.FRX` and `STYLELBL.LBX` live duplicate command smokes now capture the source live preview-bounds geometry before opening the editor, compare section continuity from the section model instead of brittle list text, and fail if `PREVIEWBOUNDS` drift after duplicate or on reload, so the shared asset-editor live-row duplicate workflow can no longer accept preview-metadata regressions hidden behind superficially correct duplicated identity and section counts alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3207`, the shared asset-editor live duplicate seam joins the existing shared reorder/rename plus host-backed settings-root, deleted-object, live duplicate/rename/reorder/placement, representative live property/batch, and representative section-property preview-metadata evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across shared deleted-rename/deleted-duplicate command paths, report/label editing, shared host JSON fidelity, project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, or recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 live-duplicate preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono probe that invoked `SmokeAssetEditorDuplicateCommandWithRealAsset` for the real `by_author.FRX` live-duplicate case under the resolved local toolchain, proving live `PREVIEWBOUNDS` stays aligned after duplicate and on reload in the current shared editor/native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
