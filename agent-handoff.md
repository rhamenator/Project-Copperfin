...
- Last shipped slice: `#3417`, the next prompt-sized child under `#3217`, shipped as evidence closure: focused runtime-surface regressions now prove representative same-PRG native `DEFINE CLASS ... AS Session OLEPUBLIC ... ENDDEFINE` classes can be instantiated through `CREATEOBJECT()` and `NEWOBJECT()` with ordinary property initialization, method invocation, `BaseClass`/`Class` identity surfaces, native class-hierarchy storage, empty same-PRG `ClassLibrary` semantics, and stable COM behavior, without additional runtime changes.
...
- Next action: execute `#3418`, the next prompt-sized `#3217` child, to validate and, only if needed, harden representative same-PRG `DEFINE CLASS ... AS Form ... ENDDEFINE` activation so `CREATEOBJECT()` and `NEWOBJECT()` can instantiate builtin-base classes beyond `Custom`/`Session` without regressing stack-frugal execution, native identity surfaces, localization constraints for new diagnostics, or the already-shipped same-PRG native class runtime behavior.
