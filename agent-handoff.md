...
- Last shipped E3 child: `#2909`, report/label `TAG` stable-mutation evidence; the new focused `test_studio_host_report_sort_settings_stable` host regression now proves FRX/LBX stable-selected live/deleted root `TAG` updates and clears survive a fresh host reopen while preserving `reportLayout.settings`, `deletedSettings`, and `selectedReportSettings` counts/provenance, without changing host JSON keys or production behavior.
