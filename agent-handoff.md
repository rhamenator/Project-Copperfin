...
- Last shipped localization child: `#2373`, visual asset editor method/property/object lookup, ambiguity, identity-field, replacement-identity, record-availability, and create/duplicate/reorder identity prose routed through the portable C++ localization catalog while preserving DBF field names, method/property identifiers, object identities, record indexes, operation tokens, and JSON contracts.
...
- Next action: inspect live GitHub children under `#2348`; pick the highest-weight open prompt-sized localization child, or create the next child under `#2348` if none fits. Return to `#24` only after the localization cleanup pass is complete or blocked.
