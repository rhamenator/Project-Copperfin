...
- Last shipped localization child: `#2622`, the remaining `Studio.ProjectWorkspace.BuildTarget.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused project-workspace coverage now proves translated and pseudo-localized build-target labels without changing `output_kind` machine values, project-item ids, field provenance, memo-block provenance, or other machine-readable build-plan contracts.
