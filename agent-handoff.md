...
- Last shipped localization child: `#2611`, the remaining `Studio.ProjectWorkspace.Group.*` and `Studio.ProjectWorkspace.ItemType.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused project-workspace coverage now proves translated and pseudo-localized `group_title`/`type_title` values without changing invariant `group_id` values, type codes, record indexes, provenance fields, or other machine-readable workspace contracts.
