...
- Last shipped E3 child: `#3196`, the shared real `bandedm.FRX` host-backed deleted reorder smoke now captures the source live preview-bounds geometry, asserts that live `PREVIEWBOUNDS` stay unchanged when only deleted rows are reordered, and fails if `DELETEDPREVIEWBOUNDS` drift out of sync with the reordered deleted layout-object geometry after reorder or reload, so deleted reorder coverage can no longer accept host-JSON preview drift hidden behind correct deleted-row identity and ordering alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3196`, the host-backed deleted-object preview-metadata seams for delete/restore, deleted property edits, deleted batch edits, deleted rename/duplicate, and deleted reorder now have real-asset evidence, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across report/label editing, shared host JSON fidelity, or project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, and recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 host-backed deleted reorder preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono reflection probe that invoked `SmokeRealAssetHostBackedDeletedReorderRoundTrip` under the resolved local toolchain against real `bandedm.FRX`, proving live `PREVIEWBOUNDS` stay unchanged when only deleted rows are reordered and that `DELETEDPREVIEWBOUNDS` stay aligned with the reordered deleted layout-object geometry after reorder and reload. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
