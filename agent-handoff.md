...
- Last shipped E3 child: `#3209`, the shared real `by_author.FRX` and `STYLELBL.LBX` deleted duplicate command smokes now capture source live preview bounds before opening the editor, compare section continuity from the section model instead of brittle list text, and fail if deleted preview metadata drifts after delete, duplicate, or reload or if restored live preview plus remaining deleted preview metadata drift after restore, so the shared asset-editor deleted-row duplicate workflow can no longer accept preview-metadata regressions hidden behind superficially correct deleted/live identity alone.
...
- Next action: continue live GitHub triage under E3/#24 by selecting or creating the next prompt-sized child from the remaining report/label host/designer fidelity backlog. After `#3209`, the shared asset-editor deleted duplicate seam joins the existing shared live and deleted rename/reorder/duplicate preview-metadata evidence plus the host-backed settings-root, deleted-object, live duplicate/rename/reorder/placement, representative live property/batch, and representative section-property coverage, so the next worthwhile slice should be whichever remaining E3 child still lacks real-asset evidence across other shared deleted/live command paths, report/label editing, shared host JSON fidelity, project-startup integration beyond the already-covered VFP9/VFPSource startup paths, fake-host layout-action families, or recent real-sample packaging-path/look-up fixes.
- Validation note: on Linux, the full `mono vsix/Copperfin.DesignerSmokeTests/bin/Release/net472/Copperfin.DesignerSmokeTests.exe` run is still not a clean release gate because the unmanaged Mono/libgdiplus tail remains unstable and the suite lacks targeted filtering; the latest E3 deleted-duplicate preview-metadata hardening slice used `git diff --check`, `dotnet build vsix/Copperfin.DesignerSmokeTests/Copperfin.DesignerSmokeTests.csproj -c Release`, and a focused repo-local net472/Mono probe that invoked `SmokeAssetEditorDeletedDuplicateCommandWithRealAsset` for the real `by_author.FRX` deleted-duplicate case under the resolved local toolchain, proving deleted preview metadata stays aligned after delete/duplicate/reload and restored live plus remaining deleted preview metadata stay aligned after restore in the current shared editor/native snapshot contract. Earlier focused validation remains as described below for the shared designer/runtime workflow families.
