...
- Last shipped E3 child: `#3062`, grouped real FRX key-expression continuity evidence; shared host-backed and shared-editor real-asset section `EXPR` smokes now prove grouped `bandedm.FRX` `Group Header` expression edits preserve the paired `Group Footer` sibling row, grouped metadata, explorer title, and contained-object geometry across update, reload, and undo, while the shared real-section smoke helpers now carry explicit update-phase versus undo-phase grouped metadata expectations so memo-block provenance stays covered without changing host JSON field names, invariant property names, or machine-readable contracts.
