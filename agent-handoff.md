...
- Last shipped localization child: `#2615`, the remaining `Studio.BuilderDispatch.CatalogEntry.Error.*` labels now have shipped-catalog parity in `en-US`, `es-419`, `pt-BR`, and `qps-ploc`, and focused builder-registry coverage now proves translated and pseudo-localized catalog-entry errors without changing builder ids, context tokens, command tokens, execution flags, or other machine-readable builder-dispatch contracts.
