...
ToolboxCreatePlanParseResult parse_toolbox_create_plan_arguments(const std::vector<std::string>& args) {
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-plan option: " + argument);
        }
    }
...
ToolboxCreateFromDispatchPlanParseResult parse_toolbox_create_from_dispatch_plan_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-from-dispatch-plan option: " + argument);
        }
    }
...
ToolboxCreateFromDispatchParseResult parse_toolbox_create_from_dispatch_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-from-dispatch option: " + argument);
        }
    }
...
ToolboxCreateDispatchFromDispatchPlanParseResult parse_toolbox_create_dispatch_from_dispatch_plan_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-dispatch-from-dispatch-plan option: " + argument);
        }
    }
...
parse_toolbox_create_batch_dispatch_from_dispatch_plan_arguments(const std::vector<std::string>& args) {
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-batch-dispatch-from-dispatch-plan option: " + argument);
        }
    }
...
ToolboxCreateBatchPlanParseResult parse_toolbox_create_batch_plan_arguments(const std::vector<std::string>& args) {
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-batch-plan option: " + argument);
        }
    }
...
SelectionToolboxCreateBatchDispatchPlanParseResult parse_selection_toolbox_create_batch_dispatch_plan_arguments(
    ...
    auto require_current_item = [&]() -> copperfin::studio::StudioToolboxObjectCreateBatchItem* {
        if (result.request.batch_request.items.empty()) {
            fail("Selection toolbox batch dispatch item options require a preceding --toolbox-item.");
            return nullptr;
        }
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown selection-toolbox-create-batch-dispatch-plan option: " + argument);
        }
    }
...
ToolboxCreateBatchDispatchPlanParseResult parse_toolbox_create_batch_dispatch_plan_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-batch-dispatch-plan option: " + argument);
        }
    }
...
ToolboxCreatePlanCatalogParseResult parse_toolbox_create_plan_catalog_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-plan-catalog option: " + argument);
        }
    }
...
SelectionToolboxCreatePlanCatalogParseResult parse_selection_toolbox_create_plan_catalog_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown selection-toolbox-create-plan-catalog option: " + argument);
        }
    }
...
ToolboxCreateDispatchCatalogParseResult parse_toolbox_create_dispatch_catalog_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-dispatch-catalog option: " + argument);
        }
    }
...
SelectionToolboxCreateDispatchCatalogParseResult parse_selection_toolbox_create_dispatch_catalog_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown selection-toolbox-create-dispatch-catalog option: " + argument);
        }
    }
...
ToolboxCreateBatchDispatchCatalogParseResult parse_toolbox_create_batch_dispatch_catalog_arguments(
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown toolbox-create-batch-dispatch-catalog option: " + argument);
        }
    }
...
parse_selection_toolbox_create_batch_dispatch_catalog_arguments(const std::vector<std::string>& args) {
    ...
    for (std::size_t index = 0U; index < args.size() && result.ok; ++index) {
        ...
        if (argument == "--path") {
            ...
            fail("Unknown selection-toolbox-create-batch-dispatch-catalog option: " + argument);
        }
    }
...
int main(int argc, char** argv) {
    ...
    const auto toolbox_create_batch_dispatch_catalog_parse =
        parse_toolbox_create_batch_dispatch_catalog_arguments(args);
    ...
    const auto selection_toolbox_create_batch_dispatch_catalog_parse =
        parse_selection_toolbox_create_batch_dispatch_catalog_arguments(args);
    ...
    const auto toolbox_create_dispatch_catalog_parse = parse_toolbox_create_dispatch_catalog_arguments(args);
    ...
    const auto selection_toolbox_create_dispatch_catalog_parse =
        parse_selection_toolbox_create_dispatch_catalog_arguments(args);
    ...
    const auto toolbox_create_plan_catalog_parse = parse_toolbox_create_plan_catalog_arguments(args);
    ...
    const auto selection_toolbox_create_plan_catalog_parse =
        parse_selection_toolbox_create_plan_catalog_arguments(args);
    ...
    const auto toolbox_create_batch_plan_parse = parse_toolbox_create_batch_plan_arguments(args);
    ...
    const auto selection_toolbox_create_batch_dispatch_plan_parse =
        parse_selection_toolbox_create_batch_dispatch_plan_arguments(args);
    ...
    const auto toolbox_create_batch_dispatch_from_dispatch_plan_parse =
        parse_toolbox_create_batch_dispatch_from_dispatch_plan_arguments(args);
    ...
    const auto toolbox_create_batch_dispatch_plan_parse = parse_toolbox_create_batch_dispatch_plan_arguments(args);
    ...
    const auto toolbox_create_from_dispatch_parse = parse_toolbox_create_from_dispatch_arguments(args);
    ...
    const auto toolbox_create_from_dispatch_plan_parse = parse_toolbox_create_from_dispatch_plan_arguments(args);
    ...
    const auto toolbox_create_dispatch_from_dispatch_plan_parse =
        parse_toolbox_create_dispatch_from_dispatch_plan_arguments(args);
    ...
    const auto toolbox_create_plan_parse = parse_toolbox_create_plan_arguments(args);
