...
{
    private static void SmokeAssetEditorAlignLeftObjectCommandRefreshesLabelShellSummary()
    {
        ...
        try
        {
            ...
            CreateFakeStudioHostScript(scriptPath, BuildAlignLeftLabelObjectHostResponseJson());
            ...
            Expect(objectListView.Items.Cast<ListViewItem>().Select(item => item.Text).SequenceEqual(new[] { "first.value", "middle.value", "last.value" }) &&
                   ...
                   string.Equals(ReadSelectionPropertyValue(initialSelection, "HPOS"), "1400", StringComparison.Ordinal),
                "A label align-left smoke should start from a multi-selection with the anchor row selected in the shared property grid");
            ...
            Expect(invocationStartCount == 1,
                "Aligning label objects left through the shared asset editor should invoke the Studio host exactly once");
            ...
            Expect(invocationArguments.Contains("--from-vs") &&
                ...
                "Aligning label objects left through the shared asset editor should send one invariant align-object command through the host contract");
            ...
            Expect(HasLabelTextContaining(control, "Aligned objects left. Snapshot loaded: 3 object rows, 5 fields.") &&
                   ...
                   string.Equals(ReadSelectionPropertyValue(refreshedSelection, "HPOS"), "1400", StringComparison.Ordinal) &&
                   alignLeftButton.Visible &&
                   alignLeftButton.Enabled &&
                   !duplicateButton.Visible &&
                   !reorderFrontButton.Visible &&
                   ...
                   string.Equals(TryGetSnapshotObjectPropertyValue(alignedObject, "HPOS"), "1400", StringComparison.Ordinal) &&
    ...
    private static void SmokeAssetEditorAlignTopObjectCommandRefreshesLabelShellSummary()
    {
        var snapshot = BuildAssetEditorAlignTopLabelObjectSmokeSnapshot();
        var tempRoot = Path.Combine(Path.GetTempPath(), "CopperfinDesignerSmoke-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        var assetPath = CreateSmokeAssetFile(tempRoot, "cust.lbx");
        var scriptPath = CreateFakeStudioHostScriptPath(tempRoot);
        ...
        {
            File.WriteAllText(logPath, string.Empty);
            CreateFakeStudioHostScript(scriptPath, BuildAlignTopLabelObjectHostResponseJson());
            Environment.SetEnvironmentVariable("COPPERFIN_STUDIO_HOST_PATH", scriptPath);
            Environment.SetEnvironmentVariable("COPPERFIN_SMOKE_LOG", logPath);
            ...
            Expect(objectListView.Items.Cast<ListViewItem>().Select(item => item.Text).SequenceEqual(new[] { "first.value", "middle.value", "last.value" }) &&
                ...
                "A label align-top smoke should start from a multi-selection with the anchor row selected in the shared property grid");
            ...
            var invocationStartCount = logLines.Count(line => string.Equals(line, "BEGIN", StringComparison.Ordinal));
            Expect(invocationStartCount == 1,
                "Aligning label objects top through the shared asset editor should invoke the Studio host exactly once");
            ...
            Expect(invocationArguments.Contains("--from-vs") &&
                   ...
                   invocationArguments.Contains("--path") &&
                   invocationArguments.Contains(assetPath),
                "Aligning label objects top through the shared asset editor should send one invariant align-object command through the host contract");
            ...
            Expect(HasLabelTextContaining(control, "Aligned objects top. Snapshot loaded: 3 object rows, 5 fields.") &&
                   ...
                   propertyGrid.SelectedObject is CopperfinDesignerSelection refreshedSelection &&
                   refreshedSelection.RecordIndex == 6 &&
                   string.Equals(ReadSelectionPropertyValue(refreshedSelection, "VPOS"), "2600", StringComparison.Ordinal) &&
                   alignTopButton.Visible &&
                   alignTopButton.Enabled &&
                   alignedObject is not null &&
                   string.Equals(TryGetSnapshotObjectPropertyValue(alignedObject, "VPOS"), "2600", StringComparison.Ordinal) &&
                   string.Equals(TryGetSnapshotObjectPropertyValue(alignedObject, "HPOS"), "2500", StringComparison.Ordinal) &&
                   string.Equals(ReadPrivateStringField(surface, "assetFamily"), "label", StringComparison.Ordinal) &&
                   ReadPrivateNullableInt(surface, "selectedRecordIndex") == 6 &&
                   ReadPrivateNullableInt(surface, "selectedReportSectionRecordIndex") == 42 &&
                   !ReadPrivateBoolField(surface, "unplacedReportObjectsSelected"),
                "Aligning label objects top through the shared asset editor should preserve anchor selection continuity and refresh the aligned target geometry");
        }
        finally
        {
            Environment.SetEnvironmentVariable("COPPERFIN_STUDIO_HOST_PATH", previousHostPath);
    ...
    private static void SmokeAssetEditorMatchWidthObjectCommandRefreshesLabelShellSummary()
    {
        var snapshot = BuildAssetEditorResizeLabelObjectSmokeSnapshot();
        var tempRoot = Path.Combine(Path.GetTempPath(), "CopperfinDesignerSmoke-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        ...
        {
            File.WriteAllText(logPath, string.Empty);
            CreateFakeStudioHostScript(scriptPath, BuildResizeToAnchorWidthLabelObjectHostResponseJson());
            Environment.SetEnvironmentVariable("COPPERFIN_STUDIO_HOST_PATH", scriptPath);
            Environment.SetEnvironmentVariable("COPPERFIN_SMOKE_LOG", logPath);
            ...
            Expect(objectListView.Items.Cast<ListViewItem>().Take(2).Select(item => item.Text).SequenceEqual(new[] { "first.value", "middle.value" }) &&
                   ...
                   string.Equals(ReadSelectionPropertyValue(initialSelection, "WIDTH"), "3400", StringComparison.Ordinal) &&
                   string.Equals(ReadSelectionPropertyValue(initialSelection, "HEIGHT"), "600", StringComparison.Ordinal),
                "A label match-width smoke should start from a multi-selection with the anchor row selected in the shared property grid");
            ...
            var invocationStartCount = logLines.Count(line => string.Equals(line, "BEGIN", StringComparison.Ordinal));
            Expect(invocationStartCount == 1,
                "Matching label object widths through the shared asset editor should invoke the Studio host exactly once");
            ...
            Expect(invocationArguments.Contains("--from-vs") &&
                   ...
                   invocationArguments.Contains("--path") &&
                   invocationArguments.Contains(assetPath),
                "Matching label object widths through the shared asset editor should send one invariant resize-object command through the host contract");
            ...
            Expect(HasLabelTextContaining(control, "Matched objects to the anchor width. Snapshot loaded: 2 object rows, 5 fields.") &&
                   ...
                   ReadPrivateNullableInt(surface, "selectedReportSectionRecordIndex") == 42 &&
                   !ReadPrivateBoolField(surface, "unplacedReportObjectsSelected"),
                "Matching label object widths through the shared asset editor should preserve anchor selection continuity and refresh the resized target geometry");
        }
        finally
        {
            Environment.SetEnvironmentVariable("COPPERFIN_STUDIO_HOST_PATH", previousHostPath);
    ...
    private static void SmokeAssetEditorMatchHeightObjectCommandRefreshesLabelShellSummary()
    {
        var snapshot = BuildAssetEditorResizeLabelObjectSmokeSnapshot();
        var tempRoot = Path.Combine(Path.GetTempPath(), "CopperfinDesignerSmoke-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(tempRoot);
        ...
        {
            File.WriteAllText(logPath, string.Empty);
            CreateFakeStudioHostScript(scriptPath, BuildResizeToAnchorHeightLabelObjectHostResponseJson());
            Environment.SetEnvironmentVariable("COPPERFIN_STUDIO_HOST_PATH", scriptPath);
            Environment.SetEnvironmentVariable("COPPERFIN_SMOKE_LOG", logPath);
            ...
            Expect(objectListView.Items.Cast<ListViewItem>().Take(2).Select(item => item.Text).SequenceEqual(new[] { "first.value", "middle.value" }) &&
                   ...
                   string.Equals(ReadSelectionPropertyValue(initialSelection, "WIDTH"), "3400", StringComparison.Ordinal) &&
                   string.Equals(ReadSelectionPropertyValue(initialSelection, "HEIGHT"), "600", StringComparison.Ordinal),
                "A label match-height smoke should start from a multi-selection with the anchor row selected in the shared property grid");
            ...
            var invocationStartCount = logLines.Count(line => string.Equals(line, "BEGIN", StringComparison.Ordinal));
            Expect(invocationStartCount == 1,
                "Matching label object heights through the shared asset editor should invoke the Studio host exactly once");
            ...
            Expect(invocationArguments.Contains("--from-vs") &&
                   ...
                   invocationArguments.Contains("--path") &&
                   invocationArguments.Contains(assetPath),
                "Matching label object heights through the shared asset editor should send one invariant resize-object command through the host contract");
            ...
            Expect(HasLabelTextContaining(control, "Matched objects to the anchor height. Snapshot loaded: 2 object rows, 5 fields.") &&
                   ...
                   ReadPrivateNullableInt(surface, "selectedReportSectionRecordIndex") == 42 &&
                   !ReadPrivateBoolField(surface, "unplacedReportObjectsSelected"),
                "Matching label object heights through the shared asset editor should preserve anchor selection continuity and refresh the resized target geometry");
        }
        finally
        {
            Environment.SetEnvironmentVariable("COPPERFIN_STUDIO_HOST_PATH", previousHostPath);
