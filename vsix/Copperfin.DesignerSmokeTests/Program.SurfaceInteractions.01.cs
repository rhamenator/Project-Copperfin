    ...
    private static void SmokeReportObjectPropertyGridLocalization()
    {
        var snapshotObject = new CopperfinStudioSnapshotObject
        {
            ...
            Properties = new List<CopperfinStudioSnapshotProperty>
            {
                ...
                new() { Name = "USER", Value = "Object user comment" },
                ...
                new() { Name = "SUPVALCHNG", Value = "false" },
                new() { Name = "SUPRPCOL", Value = "0" },
                new() { Name = "SUPOVFLOW", Value = "false" },
                new() { Name = "BOTTOM", Value = "false" },
                new() { Name = "TOP", Value = "false" },
                new() { Name = "HPOS", Value = "1200" },
                new() { Name = "VPOS", Value = "2600" },
                new() { Name = "WIDTH", Value = "4000" },
                new() { Name = "HEIGHT", Value = "500" },
        ...
        var imageSnapshot = new CopperfinStudioSnapshotObject
        {
            RecordIndex = 11,
            Title = "images.logo",
            Subtitle = "picture",
        ...
        var deletedImageSelection = CopperfinDesignerSelection.FromSnapshot(
            ...
            new CopperfinStudioSnapshotObject
            {
                RecordIndex = 12,
                Deleted = true,
                Title = "deleted.images.logo",
                Subtitle = "picture",
                Properties = imageSnapshot.Properties
            },
        ...
        Expect(imageSelection is not null &&
            ...
            "Live report image selection should expose localized PICTURE");
        Expect(deletedImageSelection is not null &&
            ...
            "Deleted report image selection should expose localized PICTURE");
        Expect(imageSelection is not null &&
            ...
            "Live report image selection should expose localized GENERAL");
        Expect(deletedImageSelection is not null &&
            ...
            "Deleted report image selection should expose localized GENERAL");
        Expect(imageSelection is not null &&
               TypeDescriptor.GetProperties(imageSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Modo de origen de imagen", StringComparison.Ordinal)),
            "Live report image selection should expose localized image-source OFFSET");
        Expect(deletedImageSelection is not null &&
        ...
        if (imageSelection is not null && deletedImageSelection is not null)
        {
            ...
            ExpectSelectionUpdate(imageSelection, "OFFSET", 2, "2",
        ...
        Expect(spanishSelection is not null &&
               ...
               TypeDescriptor.GetProperties(spanishSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Tipo de cálculo", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(spanishSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Restablecer total", StringComparison.Ordinal)) &&
               ...
               TypeDescriptor.GetProperties(spanishSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Imprimir valores repetidos", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(spanishSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Imprimir solo cuando cambia el valor", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(spanishSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "En la primera banda completa de la página/columna nueva", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(spanishSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Cuando el detalle desborda a una página/columna nueva", StringComparison.Ordinal)) &&
        ...
        Expect(portugueseSelection is not null &&
               ...
               TypeDescriptor.GetProperties(portugueseSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Tipo de cálculo", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(portugueseSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Redefinir total", StringComparison.Ordinal)) &&
               ...
               TypeDescriptor.GetProperties(portugueseSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Imprimir valores repetidos", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(portugueseSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Imprimir somente quando o valor muda", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(portugueseSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Na primeira banda inteira da nova página/coluna", StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(portugueseSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, "Quando o detalhe transborda para uma nova página/coluna", StringComparison.Ordinal)) &&
        ...
        Expect(pseudoSelection is not null &&
               ...
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.UserComments"), StringComparison.Ordinal)) &&
               ...
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.CalculationType"), StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.ResetTotal"), StringComparison.Ordinal)) &&
               ...
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.PrintWhenRepeated"), StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.PrintWhenValueChanges"), StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.PrintWhenNewPageColumn"), StringComparison.Ordinal)) &&
               TypeDescriptor.GetProperties(pseudoSelection).Cast<PropertyDescriptor>().Any(property => string.Equals(property.DisplayName, pseudoLocalization.Text("AssetEditor.Property.PrintWhenOverflow"), StringComparison.Ordinal)) &&
        ...
        if (liveSelection is not null)
        {
            ...
            ExpectSelectionUpdate(liveSelection, "USER", "Updated object user comment", "Updated object user comment",
                "Live report object property-grid selection should serialize USER edits through the shared update path");
            ...
            ExpectSelectionUpdate(liveSelection, "SUPALWAYS", false, "false",
                "Live report object property-grid selection should serialize SUPALWAYS edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "SUPVALCHNG", true, "true",
                "Live report object property-grid selection should serialize SUPVALCHNG edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "SUPRPCOL", 3, "3",
                "Live report object property-grid selection should serialize SUPRPCOL edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "SUPOVFLOW", true, "true",
                "Live report object property-grid selection should serialize SUPOVFLOW edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "BOTTOM", true, "true",
                "Live report object property-grid selection should serialize BOTTOM edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "TOP", true, "true",
                "Live report object property-grid selection should serialize TOP edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "RULERLINES", 2, "2",
                "Live report-expression property-grid selection should serialize RULERLINES edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "OFFSET", 2, "2",
                "Live report-expression property-grid selection should serialize OFFSET edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "FILLCHAR", "D", "D",
                "Live report-expression property-grid selection should serialize FILLCHAR edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "TOTALTYPE", 3, "3",
                "Live report-expression property-grid selection should serialize TOTALTYPE edits through the shared update path");
            ExpectSelectionUpdate(liveSelection, "RESETTOTAL", 2, "2",
                "Live report-expression property-grid selection should serialize RESETTOTAL edits through the shared update path");
        }
        ...
        if (deletedSelection is not null)
        {
            ...
            ExpectSelectionUpdate(deletedSelection, "SUPALWAYS", false, "false",
                "Deleted report object property-grid selection should preserve the invariant repeated-value update target");
            ExpectSelectionUpdate(deletedSelection, "SUPVALCHNG", true, "true",
                "Deleted report object property-grid selection should preserve the invariant value-change update target");
            ExpectSelectionUpdate(deletedSelection, "SUPRPCOL", 3, "3",
                "Deleted report object property-grid selection should preserve the invariant page-column update target");
            ExpectSelectionUpdate(deletedSelection, "SUPOVFLOW", true, "true",
                "Deleted report object property-grid selection should preserve the invariant overflow update target");
            ExpectSelectionUpdate(deletedSelection, "BOTTOM", true, "true",
                "Deleted report object property-grid selection should preserve the invariant bottom-position update target");
            ExpectSelectionUpdate(deletedSelection, "TOP", true, "true",
                "Deleted report object property-grid selection should preserve the invariant top-position update target");
            ...
            ExpectSelectionUpdate(deletedSelection, "TOTALTYPE", 4, "4",
                "Deleted report-expression property-grid selection should serialize TOTALTYPE edits through the shared update path");
            ExpectSelectionUpdate(deletedSelection, "RESETTOTAL", 6, "6",
                "Deleted report-expression property-grid selection should serialize RESETTOTAL edits through the shared update path");
            ExpectSelectionUpdate(deletedSelection, "MODE", 6, "6",
                "Deleted report object property-grid selection should serialize MODE edits through the shared update path");
