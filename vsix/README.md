...
- now surfaces a shared grouped read-only `Project Explorer`, plus `Task List`, `Code References`, `Data Explorer`, `Object Browser`, `Toolbox`, `Builders`, `Coverage`, and `Database` panes for project workspaces so Visual Studio and standalone Studio share more VFP-style utility-window behavior; Project Explorer preserves project group and entry order, filters matching entries, and opens eligible real assets, while Task List and Code References open findings at source lines, Data Explorer filters/opens data assets, Object Browser filters/opens eligible source assets, Builders resolve selection-aware native launch plans, and Database independently filters and details connector/query-path contracts
...
- most new project utility panes are still summary-first panes, not yet the full interactive VFP 9-equivalent Builders/Coverage toolchain; Project Explorer is currently grouped, filtered, read-only navigation, while Task List, Code References, Data Explorer, Object Browser, Database selection/filtering, Builders launch planning, and the debugger detail tables are the current inspectable/navigable exceptions. Builder execution is now available as an explicit, configured-command action through `COPPERFIN_BUILDER_LAUNCH_COMMAND`; builder mutation and a default builder executable remain separate.
