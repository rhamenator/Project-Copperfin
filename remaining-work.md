...
| CDX/DCX inspection | Partial | First-pass real directory-leaf tag-name parsing now replaces whole-file tag-name heuristics, and page-local per-tag key/`FOR` expression hints are surfaced for the shared `CDX/DCX` path, but deeper metadata, write fidelity, and richer order/collation behavior remain incomplete. |
