...
- 2026-04-12: local indexed-search now evaluates a constrained first-pass `STR(<numeric>, <width>[, <decimals>])` tag-expression path on shipped command/function `SEEK` flows. Loaded tags such as `UPPER(STR(AGE, 3))` now produce fixed-width right-aligned keys that match FoxPro-style space-padded probes, with focused regression coverage locking down command and function seek behavior.
