...
- 2026-04-30: Command-level `FIELDS` parsing widened beyond the first `SCATTER` / `GATHER` fix. Single-field forms such as `SCATTER FIELDS NAME MEMVAR`, `SCATTER FIELDS NAME NAME oRow`, `GATHER MEMVAR FIELDS NAME`, and `GATHER NAME oRow FIELDS NAME` now keep treating `NAME` as the selected field when appropriate instead of misparsing it as the command's `NAME` clause, and the same parser hardening now applies to adjacent `BROWSE`, `COPY TO`, `COPY TO ARRAY`, `APPEND FROM`, and `APPEND FROM ARRAY` command families so keyword-named fields such as `TYPE` still survive `FIELDS <name>` extraction. Focused `test_prg_engine_data_io` coverage now locks down both the `SCATTER` / `GATHER` regressions and adjacent array-copy/import keyword-field cases.
