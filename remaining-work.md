...
| CDX/DCX inspection | Partial | First-pass real directory-leaf tag-name parsing now replaces whole-file tag-name heuristics, and expression hints are still surfaced, but deeper per-tag metadata, write fidelity, and richer order/collation behavior remain incomplete. |
