...
| CDX/DCX inspection | Partial | First-pass real directory-leaf tag-name parsing now preserves conservative per-tag page hints, page-local per-tag key/`FOR` expression binding prefers hinted tag-page neighborhoods before whole-file fallback, and focused `.cdx`/`.dcx` adversarial coverage now locks down that shared parser entry point, but deeper metadata, write fidelity, and richer order/collation behavior remain incomplete. |
