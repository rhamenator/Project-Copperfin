...
| Security/policy controls | Partial | Project/workspace security models exist, and runtime/build hosts now include first-pass process hardening (`SetDllDirectory("" )` + `SetDefaultDllDirectories`) plus security-enabled runtime-host path validation and safer launcher publish invocation, but deeper RBAC/policy/secret/audit enforcement is not complete. |
