...
flowchart TD
	...
	class B4,C1,C2,C3,D1,D2,D3,D4,D5,E1,E3 partial;
...
| PRG execution engine | Partial | Structured stepping, breakpoints, fault containment, configurable runtime guardrails (max call depth, executed statements, loop-iteration caps), cooperative scheduler yielding, first-pass config-file controls (`CONFIG.FPW`/`CONFIG.FP`) for limits/temp behavior, first-pass `DO CASE/CASE/OTHERWISE/ENDCASE`, `DO WHILE/ENDDO`, `LOOP`/`CONTINUE`/`EXIT`, first-pass `ELSEIF`, `DO ... WITH` argument binding with `PARAMETERS`/`LPARAMETERS`, literal `TEXT/ENDTEXT` plus `TEXT ... TEXTMERGE`, first-pass `EVAL()` / `SET()` / `&macro`, `PRIVATE` / `STORE`, parser/analyzer module splits, and richer first-pass `ON ERROR DO ... WITH` metadata (`MESSAGE()` / `PROGRAM()` / `LINENO()` / `ERROR()`) now exist, but practical VFP parity still needs additional control-flow, error-model, and call-semantics depth. |
