...
- 2026-04-26: String expression coverage gained first-pass memo-line helpers `MEMLINES()` and `MLINE()` using default MEMOWIDTH-style wrapping. The implementation honors carriage-return hard breaks, treats bare line feeds as content, trims returned wrapped lines, and supports `MLINE(..., startOffset)` extraction. Focused string/math coverage validates CR/LF behavior, default-width word wrapping, missing-line output, and start-offset reads. Later work should wire `SET MEMOWIDTH` and `_MLINE` state updates.
