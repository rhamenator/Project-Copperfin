...
- 2026-04-17: Linux-side narrow validation is now available from a repo-local `.codex-venv` CMake install, with local generated artifacts ignored. The first Linux run exposed portability issues in Windows-only process includes, PowerShell test launching, path-separator-specific exporter assertions, and DBF memo-sidecar directory reads; those were fixed without changing the Windows-first product direction. `test_dbf_table` and `test_prg_engine` build and run under the local Linux CMake/Ninja toolchain.
