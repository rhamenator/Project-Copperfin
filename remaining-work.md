...
- 2026-04-30: `SCATTER` / `GATHER` field-filter parsing closed a real `NAME` keyword collision. Single-field forms such as `SCATTER FIELDS NAME MEMVAR`, `SCATTER FIELDS NAME NAME oRow`, `GATHER MEMVAR FIELDS NAME`, and `GATHER NAME oRow FIELDS NAME` now keep treating `NAME` as the selected field when appropriate instead of misparsing it as the command's `NAME` clause. Focused `test_prg_engine_data_io` coverage now locks down both memvar and object-target regressions.
