...
| DBF local-table mutation core | Partial | Native DBF append/field-update/delete-flag writes now exist for local table-backed runtime flows, including first-pass shared memo-backed `M` field persistence via FoxPro-compatible sidecars, first-pass fixed-width `I`/`Y`/`T` field create/replace/append support, and a fail-fast safety guard that blocks indexed-table mutation until structural index writes exist, but broader field-type coverage, repair, and transactional safety still need work. |
