...
- keep expression-function additions in small reference-derived batches; after shipping `FORCEEXT()`, `FORCEPATH()`, `CHRTRANC()`, and the first date/time helper batch (`DOW()`, `CDOW()`, `CMONTH()`, `GOMONTH()`, `SECONDS()`, `MDY()`), continue with adjacent date/time and conversion helpers from the reference inventory
