...
std::string build_native_wrapper_source(const RuntimePackagePlan& plan) {
    ...
    stream << "}\n\n";
    stream << "static std::string copperfin_runtime_bridge_quote_command_argument(const std::string& argument) {\n";
    stream << "    if (argument.empty()) {\n";
    stream << "        return \"\\\"\\\"\";\n";
    stream << "    }\n";
    stream << "    const bool needs_quotes = argument.find_first_of(\" \\t\\r\\n\\\"\") != std::string::npos;\n";
    stream << "    if (!needs_quotes) {\n";
    stream << "        return argument;\n";
    stream << "    }\n";
    stream << "    std::ostringstream quoted;\n";
    stream << "    quoted << '\"';\n";
    stream << "    for (const char ch : argument) {\n";
    stream << "        if (ch == '\"') {\n";
    stream << "            quoted << \"\\\\\\\"\";\n";
    stream << "        } else {\n";
    stream << "            quoted << ch;\n";
    ...
    stream << "    }\n";
    stream << "    quoted << '\"';\n";
    stream << "    return quoted.str();\n";
    stream << "}\n\n";
    stream << "static std::string copperfin_runtime_bridge_build_process_command(\n";
    stream << "    const CopperfinRuntimeBridgeDispatchExecution& dispatch_execution) {\n";
    stream << "    if (dispatch_execution.executable_path.empty()) {\n";
    stream << "        return {};\n";
    stream << "    }\n";
    stream << "    std::ostringstream command;\n";
    stream << "    if (!dispatch_execution.working_directory.empty()) {\n";
    stream << "#if defined(_WIN32)\n";
    stream << "        command << \"cd /d \"\n";
    stream << "#else\n";
    stream << "        command << \"cd \"\n";
    ...
    stream << "    }\n";
    stream << "    if (dispatch_execution.capture_stdout && !dispatch_execution.stdout_log_path.empty()) {\n";
    stream << "        command << \" > \" << copperfin_runtime_bridge_quote_command_argument(dispatch_execution.stdout_log_path.string());\n";
    stream << "    }\n";
    stream << "    if (dispatch_execution.capture_stderr && !dispatch_execution.stderr_log_path.empty()) {\n";
    stream << "        command << \" 2> \" << copperfin_runtime_bridge_quote_command_argument(dispatch_execution.stderr_log_path.string());\n";
    stream << "    }\n";
    stream << "    return command.str();\n";
    stream << "}\n\n";
    stream << "static CopperfinRuntimeBridgeProcessLaunch copperfin_runtime_bridge_launch_process(\n";
    stream << "    const CopperfinRuntimeBridgeDispatchExecution& dispatch_execution) {\n";
    stream << "    const auto command_line = copperfin_runtime_bridge_build_process_command(dispatch_execution);\n";
    stream << "    const bool launch_attempted = !command_line.empty();\n";
    stream << "    const int exit_code = launch_attempted ? std::system(command_line.c_str()) : dispatch_execution.expected_exit_code;\n";
    stream << "    const bool launch_succeeded = launch_attempted && exit_code == dispatch_execution.expected_exit_code;\n";
    ...
    stream << "    return response_document.find(field_token) != std::string::npos;\n";
    stream << "}\n\n";
