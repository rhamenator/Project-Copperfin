...
bool is_native_form_keypreview_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_form_keypreview_member_name_matches(runtime_object, normalized_member_name);
}

bool is_native_form_mdiform_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_form_mdiform_member_name_matches(runtime_object, normalized_member_name);
}
...
{
    return native_form_forecolor_member_name_matches(runtime_object, normalized_member_name);
}
...
bool is_native_visual_dynamicfontbold_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
...
bool is_native_column_dynamicbackcolor_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_column_dynamicbackcolor_member_name_matches(runtime_object, normalized_member_name);
}

bool is_native_column_dynamicforecolor_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_column_dynamicforecolor_member_name_matches(runtime_object, normalized_member_name);
}
...
{
    return native_commandbutton_picture_layout_member_name_matches(runtime_object, normalized_member_name);
}
...
{
    if (!native_visual_righttoleft_runtime_object(runtime_object)) {
        return;
    }

    const auto right_to_left = runtime_object.properties.find("righttoleft");
    if (right_to_left == runtime_object.properties.end()) {
        return;
    }
...
{
    if (!native_header_column_statusbartext_runtime_object(runtime_object)) {
        return;
    }

    const auto status_bar_text = runtime_object.properties.find("statusbartext");
    if (status_bar_text == runtime_object.properties.end()) {
        return;
    }
...
{
    if (!native_commandbutton_picture_layout_runtime_object(runtime_object)) {
        return;
    }

    const auto normalize_nonnegative_pixel_value = [&](const char* property_name) {
        const auto property = runtime_object.properties.find(property_name);
        if (property == runtime_object.properties.end()) {
            return;
        }
        ...
        const long long rounded = std::isfinite(value) ? std::llround(value) : -1LL;
        const long long normalized = rounded >= 0LL && rounded <= 65535LL ? rounded : 0LL;
        property->second = make_number_value(static_cast<double>(normalized));
    };

    normalize_nonnegative_pixel_value("picturemargin");
    normalize_nonnegative_pixel_value("picturespacing");

    const auto picture_position = runtime_object.properties.find("pictureposition");
    if (picture_position == runtime_object.properties.end()) {
        return;
    }

    const double value = value_as_number(picture_position->second);
    const long long rounded = std::isfinite(value) ? std::llround(value) : -1LL;
    const long long normalized = rounded >= 0LL && rounded <= 14LL ? rounded : 13LL;
    picture_position->second = make_number_value(static_cast<double>(normalized));
}
...
bool is_native_movable_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_movable_member_name_matches(runtime_object, normalized_member_name);
}
...
{
    return native_grid_linkmaster_member_name_matches(runtime_object, normalized_member_name);
}
...
{
    return native_relation_onetomany_member_name_matches(runtime_object, normalized_member_name);
}
...
{
    return native_grid_relativecolumn_member_name_matches(runtime_object, normalized_member_name);
}

bool is_native_grid_relativerow_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_grid_relativerow_member_name_matches(runtime_object, normalized_member_name);
}
