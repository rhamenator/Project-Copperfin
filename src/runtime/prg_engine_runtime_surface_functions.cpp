...
bool native_olecontrol_provenance_member_name_matches(
    const RuntimeOleObjectState& runtime_object,
    const std::string& normalized_member_name) {
    ...
    if (!is_olecontrol || normalized_member_name != "oleclass") {
        return false;
    }
    return runtime_object.properties.contains("oleclass");
}
...
bool is_native_olecontrol_provenance_member_name(const RuntimeOleObjectState& runtime_object, const std::string& normalized_member_name)
{
    return native_olecontrol_provenance_member_name_matches(runtime_object, normalized_member_name);
}
...
std::optional<PrgValue> evaluate_runtime_surface_function(
    ...
    if (function == "pemstatus" && arguments.size() >= 3U) {
        ...
        if (attribute == 5) {
            const bool readonly =
                ...
                is_native_olecontrol_provenance_member_name(*runtime_object, member_name) ||
    ...
    if (function == "addproperty" && arguments.size() >= 2U) {
        ...
        if (is_native_identity_member_name(*runtime_object, property_name) ||
            is_native_olecontrol_provenance_member_name(*runtime_object, property_name) ||
