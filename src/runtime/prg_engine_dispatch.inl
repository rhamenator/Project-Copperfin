        ...
        try
        {
            ...
            case StatementKind::declare_dll:
            {
                ...
                const NativeDeclaredLibraryLoadResult load_result = load_native_declared_library(
                    dll_fspath,
                    is_win32api_designator,
                    fn_name,
                    param_types_str);
                if (load_result.kind == NativeDeclaredLibraryKind::cannot_load)
                {
                    last_error_message = runtime_text(
                        "Runtime.Prg.Dispatch.Error.DeclareCannotLoadDll",
                        {
                            {"path", declfn.dll_path},
                            {"errorMessage", load_result.system_error_message},
                        });
                    last_fault_location = statement.location;
                    last_fault_statement = statement.text;
                    if (dispatch_error_handler())
                ...
                if (load_result.kind == NativeDeclaredLibraryKind::function_not_found)
                {
                    last_error_message = runtime_text(
                ...
                declfn.loaded_module_path = load_result.loaded_module_path;
                declfn.resolved_function_name = load_result.resolved_function_name;
                declfn.native_module_handle = load_result.module_handle;
                declfn.native_function_address = load_result.function_address;
                declfn.native_cdecl = load_result.native_cdecl;
                if (load_result.kind == NativeDeclaredLibraryKind::managed)
                {
                ...
                if (const auto existing = declared_dll_functions.find(alias_key);
                    existing != declared_dll_functions.end())
                {
            ...
            case StatementKind::export_database_command:
            {
                ...
                if (destination_path.extension().empty())
                {
                    destination_path += (is_sql || is_access || is_postgresql || is_sqlite || is_sqlserver || is_oracle || is_mysql) ? ".sql" : ".json";
                }
                ...
                {
                    const auto export_result = vfp::export_database_as_postgresql_sql(
                        copperfin::platform::path_to_utf8_string(source_path));
                    if (!export_result.ok)
                    {
                        last_error_message = runtime_text(
                            "Runtime.Prg.Dispatch.Error.ExportDatabasePostgresqlSqlFailed",
                            {{"errorMessage", export_result.error}});
                        last_fault_location = statement.location;
                        last_fault_statement = statement.text;
                        return {.ok = false, .message = last_error_message};
                ...
                else if (is_sqlserver)
                {
                    const auto export_result = vfp::export_database_as_sqlserver_sql(
                        copperfin::platform::path_to_utf8_string(source_path));
                    if (!export_result.ok)
                    {
                        last_error_message = runtime_text(
                            "Runtime.Prg.Dispatch.Error.ExportDatabaseSqlserverSqlFailed",
                            {{"errorMessage", export_result.error}});
                        last_fault_location = statement.location;
                        last_fault_statement = statement.text;
                        return {.ok = false, .message = last_error_message};
                ...
                {
                    const auto export_result = vfp::export_database_as_mysql_sql(
                        copperfin::platform::path_to_utf8_string(source_path));
                    if (!export_result.ok)
                    {
                        last_error_message = runtime_text(
                            "Runtime.Prg.Dispatch.Error.ExportDatabaseMysqlSqlFailed",
                            {{"errorMessage", export_result.error}});
                        last_fault_location = statement.location;
                        last_fault_statement = statement.text;
                        return {.ok = false, .message = last_error_message};
                ...
                else
                {
                    ...
                    {
                        last_error_message = runtime_text(
                            "Runtime.Prg.Dispatch.Error.ExportDatabaseSqlFailed",
                            {{"errorMessage", export_result.error}});
                        last_fault_location = statement.location;
                        last_fault_statement = statement.text;
                        return {.ok = false, .message = last_error_message};
                ...
                const char* open_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseJsonOpenOutputFailed";
                const char* write_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseJsonWriteOutputFailed";
                const char* event_category = "runtime.export_database_json";
                if (is_access)
                {
                    open_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseAccessSqlOpenOutputFailed";
                    write_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseAccessSqlWriteOutputFailed";
                    event_category = "runtime.export_database_access_sql";
                }
                else if (is_postgresql)
                {
                ...
                {
                    open_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseMysqlSqlOpenOutputFailed";
                    write_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseMysqlSqlWriteOutputFailed";
                    event_category = "runtime.export_database_mysql_sql";
                }
                else if (is_sql)
                {
                    open_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseSqlOpenOutputFailed";
                    write_failed_key = "Runtime.Prg.Dispatch.Error.ExportDatabaseSqlWriteOutputFailed";
                    event_category = "runtime.export_database_sql";
                }

                std::ofstream output(destination_path, std::ios::binary | std::ios::trunc);
                if (!output.good())
                {
                    last_error_message = runtime_text(open_failed_key);
                    last_fault_location = statement.location;
                    last_fault_statement = statement.text;
                    return {.ok = false, .message = last_error_message};
                }
                ...
                if (!output.good())
                {
                    last_error_message = runtime_text(write_failed_key);
                    last_fault_location = statement.location;
                    last_fault_statement = statement.text;
                    return {.ok = false, .message = last_error_message};
                }
                events.push_back({.category = event_category,
                                  .detail = copperfin::platform::path_to_utf8_string(destination_path),
                                  .location = statement.location});
                return {};
            }
            ...
            case StatementKind::append_from_command:
            {
                ...
                // truncated away when it is still the last record; if the FOR
                // callback appended more records after it, it is marked deleted
                // instead; if the callback already removed it (PACK/ZAP), there
                // is nothing left to roll back.
                const auto reject_local_provisional_record = [&](std::size_t provisional_recno) -> bool
                {
                    if (cursor->record_count == provisional_recno)
                    {
                        const auto truncated = vfp::truncate_dbf_table_file(cursor->source_path, provisional_recno - 1U);
                        if (!truncated.ok)
                        {
                            last_error_message = runtime_text(
                    ...
                    else if (provisional_recno <= cursor->record_count)
                    {
                        const auto flagged = vfp::set_record_deleted_flag(cursor->source_path, provisional_recno - 1U, true);
                        if (!flagged.ok)
                        {
                            last_error_message = runtime_text(
                                "Runtime.Prg.Dispatch.Error.AppendFromFailed",
                                {{"errorMessage", flagged.error}});
                            return false;
                        }
                    }
                    cursor->recno = cursor->record_count == 0U ? 0U : cursor->record_count;
                ...
                if (cursor->remote && cursor->source_path.empty())
                {
                        ...
                        [&](vfp::DbfRecord appended_record, bool evaluate_on_target) -> RemoteAppendRowOutcome
                    {
                        ...
                        std::vector<vfp::DbfRecordValue> candidate_values;
                        if (evaluate)
                        {
                            candidate_values = appended_record.values;
                        }
                        ...
                        // The FOR callback can reshape the surviving target (append,
                        // DELETE+PACK, ZAP), so the provisional row is located again
                        // by its values rather than trusted to still be at its index.
                        const auto roll_back_provisional_row = [&]()
                        {
                            auto &rows = cursor->remote_records;
                            const auto is_candidate = [&](const vfp::DbfRecord &row)
                            {
                                if (row.values.size() != candidate_values.size())
                                {
                                    return false;
                                }
                                for (std::size_t index = 0U; index < row.values.size(); ++index)
                                {
                                    if (row.values[index].display_value != candidate_values[index].display_value ||
                                        row.values[index].is_null != candidate_values[index].is_null)
                                    {
                                        return false;
                                    }
                                }
                            ...
                            if (provisional_index < rows.size() && is_candidate(rows[provisional_index]))
                            {
                                found = provisional_index;
                            }
                            else
                            {
                                for (std::size_t index = std::min(provisional_index + 1U, rows.size()); index > 0U; --index)
                                {
                                    if (is_candidate(rows[index - 1U]))
                                    {
                                        found = index - 1U;
                                        break;
                                    }
                ...
                if (append_from_sdf)
                {
                    ...
                    for (const std::string &line : split_sdf_lines(buffer))
                    {
                        ...
                        }
                        {
                            ...
                            {
                                if (!reject_local_provisional_record(provisional_recno))
                                {
                                    last_fault_location = statement.location;
                                    last_fault_statement = statement.text;
                                    return {.ok = false, .message = last_error_message};
