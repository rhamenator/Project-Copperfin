...
std::vector<ParsedSqlExportTable> write_oracle_tables_and_data(
    ...
    for (const auto& rt : snapshot.resolved_tables) {
        ...
        // #5717 PR review (chatgpt-codex-connector, P1) originally found
        // that a genuinely non-null empty value and a genuinely null value
        // in a *nullable* field were indistinguishable in this codebase's
        // own in-memory representation (`_NullFlags` was never decoded and
        // applied back to the fields it governs), so the empty-character
        // check below was scoped to tables with no nullable fields at all.
        // #6047 fixed the root cause -- `parse_dbf_table_from_file()` now
        // decodes `_NullFlags` and sets each governed field's own
        // `is_null` correctly -- so `rv.is_null` is authoritative for
        // every table now, and the check below (only reached once
        // `rv.is_null` has already been ruled out, see the dispatch above)
        // is unambiguous unconditionally; the nullable-fields carve-out is
        // no longer needed.
        ...
        for (const auto& rec : tbl.table.records) {
            ...
            for (std::size_t vi = 0U; vi < rec.values.size(); ++vi) {
                ...
                if (rv.is_null) {
                    ...
                    // -- see this loop's own comment above for why this is
                    // unambiguous now that #6047 decodes `_NullFlags`.
                    if (rv.display_value.empty()) {
                        hard_failure_error = asset_inspector_text(
