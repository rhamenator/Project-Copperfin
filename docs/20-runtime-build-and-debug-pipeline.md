...
and pixel-level behavior remain separate. #4773 is implementation-complete:
Claude review seq790 passed, and exact-head Windows validation seq789 rebuilt
the product and passed the focused runtime-surface CTest `1/1`; the remaining
macOS/full-RC matrix is separate evidence.

Runtime child #4774 extends that default forward Tab path through nested
`Container` descendants. An explicit worklist carries ancestor `Visible` and
`Enabled` state without recursive host-stack growth; discovered controls remain
deduplicated and use the existing `TabIndex`/`TabStop` ordering and focus
transition. The strengthened focused test passes `1/1` on Linux at product
`29391e3d2`; Claude seq795 and exact-head Windows validation seq794 also pass.
...
TabIndex while preserving group/child Value state. The mode is selected
explicitly through `RuntimeSessionOptions.keyboard_compatibility`, with
Windows as the default; DOS-compatible KEYCOMP remains unsupported. Its Linux
focused test passes, with Claude and Windows focused behavior corroborated;
full current-head Windows/macOS workflows remain separate RC evidence. Reverse
Shift+Tab, keyboard buffering, and hosted pixel UI remain separate contracts.
...
- runnable `MNX/MNT` startup assets can now execute setup code and activate shortcut/menu event loops through a dedicated menu bootstrap model
  - non-shortcut MNX bootstraps derive a deterministic internal menu identifier from the logical asset path, including when the table bytes are opened from a verified temporary snapshot, and use it consistently for definition, activation, deactivation, and release; the original path and filename stem remain model metadata. Shortcut popup names from asset records must meet VFP identifier syntax before entering generated source. Empty filename stems and invalid popup names produce localized model errors (#6459; `RQ-CF-XASSET-MENU-NAME-001`).
