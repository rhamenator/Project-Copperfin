...
`SIGPIPE` only in the writer thread, consuming only a signal generated by that
writer's own `EPIPE` while preserving the prior thread state. Windows shutdown cancels outstanding synchronous
