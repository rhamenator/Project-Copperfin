...
The current implementation head is `ff9417a4a` (synchronized coordination
head `e8b857f95`), which includes runtime child #4753's defined-menu lifecycle
slice. Fresh hosted Linux Native `30322581213`, Windows Native
`30322581189`, and macOS Native `30322581188` runs are in progress against
that head, along with the managed UI, VSIX, installer, launcher, and Windows
environment validation workflows. The prior corrected-head evidence remains
valid: hosted Linux Native `30320046770` passed all `316/316` CTest cases in
`175.76s`, with only the two expected platform-conditional skips; hosted macOS
Native `30320046738` passed `316/316` in `218.31s`, and Windows Environment
and Executable Path Validation `30320046758` passed. The earlier Windows
...
The new current-head runs are still partial release evidence until they finish.
