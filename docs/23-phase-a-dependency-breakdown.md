...
- Latest implemented E3 slice: `#1755`, which covers invalid-first duplicate report/label root settings precedence. Studio host regressions create FRX/LBX live and deleted root-settings rows where duplicate `EXPR` memo-derived `ORIENTATION`/`COLS` entries start invalid and later memo/direct duplicates are valid, proving page/column summaries do not fabricate availability from later duplicates while every invalid and valid entry remains inspectable with source-order and provenance. `#1754` covered first-valid duplicate precedence; recent prior slices `#1753`-`#1750` cover malformed memo lines, blank memo values, fractional memo values, and invalid memo values. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
