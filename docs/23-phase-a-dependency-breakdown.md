...
- Latest implemented E3 slice: `#4059`, fallback report/label settings-root write parity. The shared model and visual-asset editor now share the `OBJTYPE == 1` settings-root classification, so fallback FRX/LBX root rows without `OBJCODE` can update and reopen exposed memo-backed `GRIDV` values. Focused editor and report-layout coverage passes; host JSON field names and all visible/machine contracts remain unchanged.
