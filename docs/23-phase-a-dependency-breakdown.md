...
- Latest implemented E3 slice: `#1512`, which proves Studio host selected live `.frx` report section rows preserve section selection while explicitly clearing selected-object, containing-object-section, and selected-settings JSON surfaces. `#1511` is the immediately preceding selected unplaced report object no-section/settings JSON parity slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
