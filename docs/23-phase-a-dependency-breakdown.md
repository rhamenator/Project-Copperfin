...
- Latest implemented E2 slice: `#1231`, which adds non-executing per-context builder/wizard invocation dispatch catalog planning for admitted builder invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
