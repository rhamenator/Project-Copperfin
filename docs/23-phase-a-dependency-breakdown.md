...
- Latest implemented E3 slice: `#2689`, deleted report-object containing-section fidelity; the shared report-layout snapshot now assigns containing-section identity, relative geometry, and section-local deleted-object order/count metadata to deleted placed FRX/LBX objects when their geometry belongs to a live or deleted band, `copperfin_studio_host --json` now resolves `selectedReportObjectSection` for those deleted placed-object selections, and focused report-layout plus direct host validation now prove live-section and deleted-section deleted-object selections retain section-aware context while deleted unplaced objects still keep containing-section JSON absent without changing existing machine-readable designer contracts.
