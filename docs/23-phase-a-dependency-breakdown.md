...
- Recent native PRG parity slice: `#4660` under #3217 adds the documented `Column.DynamicFontBold` raw-expression state to explicit, derived, and materialized grid-column objects. Preserve the documented `.T.` true-default exception, direct/PEM access, reflection, and built-in protection; exact-head Windows validation is being re-requested against the corrected head. Expression evaluation, per-row bold resolution, font lookup, layout, rendering, host UI integration, and designer serialization remain separate.
