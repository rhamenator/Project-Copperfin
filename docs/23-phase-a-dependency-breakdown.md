...
- Latest implemented E3 slice: `#1620`, which proves live FRX/LBX layout objects edited outside section bands can be duplicated with edited HPOS/VPOS/HEIGHT fields preserved and refreshed `selectedReportObject` JSON pointing at the unplaced duplicate while selected containing-section availability/object JSON stays false/null. `#1619` is the immediately preceding closed live-edited duplicate-to-section slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
