...
- Latest implemented E3 slice: `#1797`, which covers stable-selected live Detail Header/Footer object distribution containment for FRX/LBX label/field objects. Studio host regressions prove Detail Header and Detail Footer horizontal distribution persists refreshed HPOS geometry, preserves VPOS/WIDTH/HEIGHT geometry, keeps containing-section ids/record indexes, section-relative coordinates, object indexes/counts, selected containing-section JSON, non-failing inspection, and label identity. `#1796` covered deleted object nudge containment, `#1795` covered live object nudge containment, `#1794` covered deleted object snap containment, `#1793` covered live object snap containment, `#1792` covered deleted object resize containment, `#1791` covered live object resize containment, `#1790` covered deleted object alignment containment, and `#1789` covered live object alignment containment. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
