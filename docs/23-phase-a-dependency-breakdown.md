...
- Latest implemented E2 slice: `#1414`, which exposes toolbox palette query filtering through Studio host JSON. `#1413` is the immediately preceding menu command editor action slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
