...
- Latest implemented E3 slice: `#1653`, which proves a deleted FRX/LBX layout object can be edited into out-of-section geometry and restored by stable unique id without a record selector while the DBF deleted flag clears, edited unplaced geometry persists, preview bounds/counts refresh, and selected-object containing-section metadata remains unavailable/null. `#1652` is the immediately preceding closed stable-selector deleted restore slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
