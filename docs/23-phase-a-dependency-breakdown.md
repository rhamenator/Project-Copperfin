...
- The live execution lane is now back on E3/#24 report/label designer fidelity. Localization/#2348 child #1779 has landed and the shipped catalog parity sweep now acts as a standing requirement for new user-facing text rather than the default implementation lane. E2/#23 remains open with many prompt-sized children still open in GitHub; treat them as evidence-audit/closure cleanup unless fresh implementation evidence shows a real remaining local miss.
- Latest implemented E3 slice: `#2690`, live report-object deleted-section fidelity; the shared report-layout snapshot now preserves containing deleted-section identity, relative geometry, and section-local object order/count metadata for live FRX/LBX objects whose geometry belongs to deleted bands, deleted sections now retain corresponding live object membership instead of empty object arrays, `copperfin_studio_host --json` now resolves `selectedReportObjectSection` for those selections, and focused report-layout plus direct host validation now prove live objects inside deleted sections stay placed with deleted-preview context while truly unplaced objects still keep containing-section JSON absent without changing existing machine-readable designer contracts.
...
- Current continuation shape: continue prompt-sized E3/#24 children in report/label designer fidelity, especially shared host/designer round-trip fidelity, section-aware editing, output preview metadata, and layout/runtime parity seams, while keeping any newly introduced user-facing text localized by default; explicit grouping summaries, section grouping context, resolved grouping-expression provenance, per-section deleted-object counts, deleted placed-object containing-section fidelity, live-object deleted-section membership, and stable-selected, deleted, record-selected, and nested group-section EXPR edit evidence now exist at both top-level and nested group depths, so prefer the next unproven seam rather than re-covering group-section grouping or EXPR mutation cases; use host exposure where useful, focused tests, docs, validation, commit, push, and issue closure per slice.
- Next work: create or pick the next prompt-sized `#24` child from a real remaining FRX/LBX gap unless the live issue tree shows a higher-weight blocker. A broader visual-asset editor API scan found remaining unwrapped batch APIs after `#1447`; `#1448` duplicate batches, `#1449` rename batches, `#1450` reorder batches, and `#1451` subtree duplication are now closed with evidence-backed approval. The `#1078`-`#1086`, `#1087`-`#1101`, `#1102`-`#1202`, and `#1203`-`#1447` E2 evidence-audit ranges are closed; only older E2 parent/roadmap rows remain open in the local issue snapshot and must not be orphaned. Do not open more E2 wrapper slices unless new APIs are added or the scanner regresses.
