...
- Latest implemented E2 slice: `#1424`, which exposes visual object method updates/appends through Studio host JSON. `#1423` is the immediately preceding visual object method-query host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
