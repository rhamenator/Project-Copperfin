...
- Latest implemented E2 slice: `#1306`, which adds selected-context ordered toolbox object batch create-dispatch planning through the shared selected-context batch create-plan seam. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
