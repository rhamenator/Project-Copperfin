...
- Latest implemented E2 slice: `#1228`, which exposes non-executing per-context editor action dispatch catalog planning through Studio host JSON for admitted editor invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
