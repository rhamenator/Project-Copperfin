...
- Latest implemented E3 slice: `#1491`, which proves Studio host `.lbx` label layout object identity rename mutation through stable selectors, stable LBX object count/order, replacement `UNIQUEID` persistence, and refreshed live section membership JSON while preserving label identity. `#1490` is the immediately preceding label layout object duplicate mutation coverage slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
