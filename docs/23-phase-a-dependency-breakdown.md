...
- Latest implemented E3 slice: `#1506`, which proves Studio host selected live `.lbx` label layout object rows preserve object selection and containing-section metadata while explicitly clearing selected-section and selected-settings JSON surfaces. `#1505` is the immediately preceding selected deleted label settings no-section/object JSON parity slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
