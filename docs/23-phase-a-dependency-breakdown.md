...
- Latest implemented E2 slice: `#1314`, which exposes single-object toolbox creation from admitted palette dispatches through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
