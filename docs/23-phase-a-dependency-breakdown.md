...
- Latest implemented E3 slice: `#1770`, which covers stable-selected Detail Header/Footer object expression edits and clears for live FRX/LBX label/field objects. Studio host regressions prove `EXPR` set/clear operations by `UNIQUEID` persist refreshed or blank memo data, refresh selected-object expression text/provenance plus containing-section metadata, keep inspection non-failing, and retain label identity. `#1769` covered stable-selected object expression provenance, `#1768` covered stable live Detail Header/Footer section expression selections, `#1767` covered stable deleted Detail Header/Footer section selection, and `#1766` covered deleted Detail Header/Footer section-expression edits. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
