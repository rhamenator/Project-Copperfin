...
- Latest implemented E3 slice: `#4061`, shared fractional geometry conversion for report/label layout and section movement. Finite bounded `VPOS`/`HEIGHT` conversion now truncates toward zero in both paths, so an object at a fractional band boundary moves with the same section that displays it and retains relative position after FRX/FRT or LBX/LBT reopen. Focused GCC/Clang editor, layout, and localization coverage passes; host JSON fields and all visible/machine contracts remain unchanged. `#4060` remains the preceding memo-settings lifecycle fix.
