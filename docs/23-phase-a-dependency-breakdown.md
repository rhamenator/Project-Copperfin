...
- Latest implemented E2 slice: `#1245`, which adds non-mutating toolbox object batch creation planning with batch-wide generated-name reservation. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
