...
- Latest implemented E2 slice: `#1267`, which exposes non-executing toolbox object batch creation dispatch planning from admitted palette dispatch through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
