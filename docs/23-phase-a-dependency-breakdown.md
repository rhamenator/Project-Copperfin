...
- Latest implemented E3 slice: `#2685`, report/label grouping summary JSON; `StudioReportLayoutSnapshot` now derives explicit FRX/LBX grouping summaries from live and deleted group-header/group-footer bands, `copperfin_studio_host --json` exposes ordered `groupings` metadata plus `groupingCount`, and focused report-layout plus host validation now proves nested live/deleted pairing, expression provenance, stable header/footer identities, and label identity without changing existing machine-readable designer contracts.
