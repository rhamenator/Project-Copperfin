...
- Latest implemented E2 slice: `#1411`, which adds non-mutating toolbox palette search/category query planning. `#1410` is the immediately preceding data-environment property-grid action slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
