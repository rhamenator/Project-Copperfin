...
- Latest implemented E3 slice: `#1772`, which covers stable-selected deleted Detail Header/Footer object expression edits and clears for FRX/LBX label/field objects. Studio host regressions prove `EXPR` set/clear operations by `UNIQUEID` preserve deleted state, refresh deleted-object and selected-object expression text/provenance, keep containing-section metadata unavailable/null instead of fabricating live section membership, keep inspection non-failing, and retain label identity. `#1771` covered stable-selected deleted Detail Header/Footer object expression selections, `#1770` covered live Detail Header/Footer object expression edits and clears, and `#1769` covered live object expression provenance. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
