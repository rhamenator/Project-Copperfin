...
- Latest implemented E3 slice: `#2691`, report/label layout font-option edit fidelity; focused Studio host JSON regressions now prove FRX/LBX layout-object `FONTSIZE` updates and `MODE` clears across record-selected, stable-selected, and deleted record-selected flows, and the deleted layout-object font regressions now align with current placed deleted-object containing-section semantics without changing existing machine-readable designer contracts.
