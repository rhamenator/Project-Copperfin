...
- Latest implemented E2 slice: `#1425`, which exposes visual object method deletes through Studio host JSON. `#1424` is the immediately preceding visual object method-update host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
