...
- Latest implemented E3 slice: `#4060`, memo-backed `TOPMARGIN` and `TAG` settings lifecycle. Both names now use the existing writable EXPR settings path, supporting absent/create, existing/update, clear, and re-add operations while retaining unrelated assignments for FRX/FRT and LBX/LBT. Focused GCC/Clang editor and localization coverage passes; host JSON fields and all visible/machine contracts remain unchanged. `#4059` remains the preceding fallback-settings-root classification fix.
