...
- Latest implemented E2 slice: `#1227`, which adds non-executing per-context editor action dispatch catalog planning for admitted editor invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
