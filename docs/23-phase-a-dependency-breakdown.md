...
- Latest implemented E2 slice: `#1251`, which adds non-mutating toolbox object batch creation dispatch planning for deterministic host command arguments. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
