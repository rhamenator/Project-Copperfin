...
- Latest implemented E3 slice: `#1631`, which proves live FRX/LBX layout object expressions can be cleared by stable unique id without a record selector while the EXPR memo blanks and refreshed `selectedReportObject` JSON points at the cleared object with containing-section metadata. `#1630` is the immediately preceding closed stable-selector expression update slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
