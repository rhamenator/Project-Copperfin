...
- Latest implemented E2 slice: `#1420`, which exposes visual object descendant listings through Studio host JSON. `#1419` is the immediately preceding visual object immediate-child host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
