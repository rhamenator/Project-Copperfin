...
- Latest implemented E2 slice: `#1258`, which exposes non-mutating toolbox object batch creation plan catalogs through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
