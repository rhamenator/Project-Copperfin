...
- Latest implemented E2 slice: `#1441`, which exposes visual object property renames through Studio host JSON. `#1440` is the immediately preceding visual object property move-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
