...
- Recent native PRG parity slice: `#4763` under #3217 extends modeled pointer input to `WM_LBUTTONDBLCLK`/`DblClick`, preserving VFP left-button, Shift/Ctrl, and signed client-coordinate arguments, stable telemetry, independent missing-handler behavior, and event-loop restoration. Preserve existing bindings and left-button message behavior; double-click timing policy, hit testing, capture, rendering, and hosted Windows UI remain separate. Focused runtime-surface coverage passes in a fresh out-of-tree build.
