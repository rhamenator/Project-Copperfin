...
- Latest implemented E2 slice: `#1417`, which exposes visual property lists through Studio host JSON. `#1416` is the immediately preceding visual property point-read host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
