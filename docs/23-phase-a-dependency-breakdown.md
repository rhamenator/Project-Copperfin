...
- Latest implemented E3 slice: `#1776`, which covers stable-selected deleted Detail Header/Footer object font edits and clears for FRX/LBX label/field objects. Studio host regressions prove `FONTFACE` set/clear operations by `UNIQUEID` preserve deleted state, persist refreshed or blank memo data, refresh deleted-object and selected-object highlight provenance, keep containing-section metadata unavailable/null instead of fabricating live section membership, keep inspection non-failing, and retain label identity. `#1775` covered live Detail Header/Footer object font edits and clears, `#1774` covered deleted font/highlight metadata selections, and `#1773` covered live font/highlight metadata selections. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
