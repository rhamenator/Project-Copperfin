...
- Latest implemented E3 slice: `#1778`, which covers stable-selected deleted Detail Header/Footer object font-option edits for FRX/LBX label/field objects. Studio host regressions prove `FONTSIZE` set and `MODE` clear operations by `UNIQUEID` preserve deleted state, persist refreshed field values or blank clears, refresh deleted-object and selected-object highlight provenance, keep containing-section metadata unavailable/null instead of fabricating live section membership, remove stale cleared-option highlights, keep inspection non-failing, and retain label identity. `#1777` covered live Detail Header/Footer object font-option edits, `#1776` covered deleted `FONTFACE` edits and clears, and `#1775` covered live `FONTFACE` edits and clears. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
