...
- Latest implemented E2 slice: `#1418`, which exposes visual object outline listings through Studio host JSON. `#1417` is the immediately preceding visual property-list host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
