...
- Latest implemented E3 slice: `#1766`, which covers deleted Detail Header and deleted Detail Footer report/label section-expression edits. Studio host regressions prove `--set-property` on deleted detail-header `EXPR` records and `--clear-property` on deleted detail-footer `EXPR` records refresh deleted-section and selected-section expression JSON, persist or clear memo provenance correctly, preserve live Detail sibling metadata, keep inspection non-failing, and retain label identity. `#1765` covered live Detail Header/Footer section-expression edits, `#1764` covered Detail Header/Footer object containment, `#1763` covered Detail Header/Footer section kinds, and `#1762` covered extended object kinds. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
