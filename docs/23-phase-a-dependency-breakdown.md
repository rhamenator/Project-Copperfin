...
- Latest implemented E2 slice: `#1247`, which adds Studio-layer toolbox object batch creation with all-or-nothing mutation through planned toolbox descriptors. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
