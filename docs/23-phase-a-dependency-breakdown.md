...
- Latest implemented E2 slice: `#1428`, which exposes visual object method moves through Studio host JSON. `#1427` is the immediately preceding visual object method-copy host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
