...
- Latest implemented E2 slice: `#1443`, which exposes visual object property reorders through Studio host JSON. `#1442` is the immediately preceding visual object property rename-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
