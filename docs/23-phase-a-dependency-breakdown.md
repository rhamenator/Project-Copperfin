...
- Latest implemented E3 slice: `#2687`, resolved report/label grouping-expression JSON; `StudioReportSectionSnapshot` now carries resolved FRX/LBX grouping-expression metadata for group-header/group-footer bands, `copperfin_studio_host --json` exposes additive grouping-expression fields on `sections`, `deletedSections`, and `selectedReportSection`, and focused report-layout plus host validation now proves blank-footer live/deleted FRX/LBX sections inherit stable grouping-expression provenance from paired headers while non-group selected object sections keep that metadata absent without changing existing machine-readable designer contracts.
