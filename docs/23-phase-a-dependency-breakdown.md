...
- Latest implemented E3 slice: `#1781`, which covers stable-selected deleted Detail Header/Footer object geometry edits for FRX/LBX label/field objects. Studio host regressions prove `VPOS` set and `HEIGHT` clear operations by `UNIQUEID` preserve deleted state, persist refreshed or blank geometry fields, refresh deleted-object and selected-object bounds, keep containing-section metadata unavailable/null instead of fabricating live section membership, keep inspection non-failing, and retain label identity. `#1780` covered live Detail Header/Footer object geometry edits, `#1778` covered deleted font-option edits, and `#1777` covered live font-option edits. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
