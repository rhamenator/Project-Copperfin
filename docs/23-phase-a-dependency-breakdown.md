...
- Latest implemented E2 slice: `#1410`, which exposes the shared property-grid editor action for data-environment selection contexts. `#1409` is the immediately preceding Studio host usage/discoverability slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
