...
- Latest implemented E2 slice: `#1334`, which preserves nested editor-action, builder/wizard, and toolbox dispatch command tokens plus dispatch arguments in `copperfin_studio_host --designer-dispatch-execution-catalog` JSON so aggregate designer execution-catalog consumers can inspect exact non-mutating child launch contracts without executing UI/processes or mutating assets. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
