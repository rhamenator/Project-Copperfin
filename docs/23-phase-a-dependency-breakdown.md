...
- Latest implemented E2 slice: `#1422`, which exposes visual object method listings through Studio host JSON. `#1421` is the immediately preceding visual object ancestor host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
