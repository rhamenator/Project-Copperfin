...
- Latest implemented E2 slice: `#1440`, which exposes visual object property move batches through Studio host JSON. `#1439` is the immediately preceding visual object property-move host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
