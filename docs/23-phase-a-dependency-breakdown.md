...
- Latest implemented E3 slice: `#1765`, which covers Detail Header and Detail Footer report/label section-expression edits. Studio host regressions prove `--set-property` and `--clear-property` on detail-header/detail-footer `EXPR` records refresh selected-section expression JSON, persist or clear memo provenance correctly, preserve sibling section expressions, and retain label identity. `#1764` covered Detail Header/Footer object containment, `#1763` covered Detail Header/Footer section kinds, `#1762` covered extended object kinds, `#1761` covered negative classification codes, `#1760` covered dot-leading classification codes, `#1759` covered trimmed classification codes, and `#1758` covered fractional classification codes. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
