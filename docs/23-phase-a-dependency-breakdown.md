...
- Latest implemented E2 slice: `#1303`, which exposes selected-context single-item toolbox object create-dispatch planning through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
