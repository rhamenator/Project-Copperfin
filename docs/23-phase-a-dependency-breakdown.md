...
- Latest implemented E2 slice: `#1299`, which exposes selected-context toolbox object batch create-dispatch catalogs through Studio host JSON via the shared toolbox palette launch seam. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
