...
- Latest implemented E3 slice: `#1504`, which proves Studio host selected `.lbx` label settings rows preserve settings selection while explicitly clearing selected-section, selected-object, and containing-object-section JSON surfaces. `#1503` is the immediately preceding selected deleted label section no-object JSON parity slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
