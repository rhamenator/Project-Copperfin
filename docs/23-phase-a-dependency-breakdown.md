...
- Latest implemented E2 slice: `#1239`, which adds non-executing aggregate designer invocation dispatch catalog planning across all known Studio selection contexts. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
