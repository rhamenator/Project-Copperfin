...
- Latest implemented E2 slice: `#1230`, which exposes non-executing builder/wizard invocation dispatch planning through Studio host JSON for admitted builder invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
