...
- Latest implemented E2 slice: `#1439`, which exposes visual object property moves through Studio host JSON. `#1438` is the immediately preceding visual object property copy-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
