...
- Latest implemented E3 slice: `#1458`, which carries containing-section metadata directly on report/label layout objects and through Studio host JSON. `#1457` is the immediately preceding report-selection-kind JSON slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
