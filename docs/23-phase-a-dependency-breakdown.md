...
- Latest implemented E3 slice: `#3602`, mounted real-sample FRX/LBX root sort-setting round-trip coverage; focused Studio host regressions now prove writable copies of the mounted VFP9 `invoice.frx` / `invoice.frt` and `cust.lbx` / `cust.lbt` samples persist root `TAG` updates through the shared host, refresh selected-settings JSON with stable record/field/memo provenance, and clear back to the original five live root settings without fabricating deleted settings, while explicitly documenting the current memo-allocation behavior after clear.
