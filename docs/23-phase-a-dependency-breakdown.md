...
- Latest implemented E2 slice: `#1232`, which exposes non-executing per-context builder/wizard invocation dispatch catalog planning through Studio host JSON for admitted builder invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
