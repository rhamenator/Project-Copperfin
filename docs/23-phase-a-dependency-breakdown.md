...
- Latest implemented E2 slice: `#1432`, which exposes visual object method copy batches through Studio host JSON. `#1431` is the immediately preceding visual object method rename-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
