...
- Latest implemented E2 slice: `#1272`, which routes builder/wizard dispatch catalog planning through the shared invocation-admission catalog seam. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
