...
- Latest implemented E3 slice: `#1767`, which covers stable `--unique-id` selection for deleted Detail Header and deleted Detail Footer report/label sections. Studio host regressions prove deleted Detail Header/Footer selections expose deleted-section and selected-section metadata, expression provenance, section counts, null unrelated report-selection surfaces, non-failing inspection, and label identity. `#1766` covered deleted Detail Header/Footer section-expression edits, `#1765` covered live Detail Header/Footer section-expression edits, `#1764` covered Detail Header/Footer object containment, and `#1763` covered Detail Header/Footer section kinds. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
