...
- Latest implemented E2 slice: `#1434`, which exposes visual object method reorder batches through Studio host JSON. `#1433` is the immediately preceding visual object method move-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
