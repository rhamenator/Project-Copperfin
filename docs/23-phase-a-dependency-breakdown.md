...
- Latest implemented E3 slice: `#1615`, which proves live FRX/LBX layout objects edited through host geometry properties can then be deleted with edited HPOS/WIDTH/HEIGHT fields preserved in deleted-object and selected-object JSON while selected containing-section metadata stays unavailable/null. `#1614` is the immediately preceding closed edited deleted-object restore-as-unplaced slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
