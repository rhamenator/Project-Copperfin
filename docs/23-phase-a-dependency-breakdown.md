...
- Latest implemented E3 slice: `#1460`, which carries section-order metadata directly on report/label sections and through Studio host JSON. `#1459` is the immediately preceding object section-order metadata slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
