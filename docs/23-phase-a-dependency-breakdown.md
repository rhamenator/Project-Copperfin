...
- Latest implemented E2 slice: `#1436`, which exposes visual object property copies through Studio host JSON. `#1435` is the immediately preceding visual object property-clear host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
