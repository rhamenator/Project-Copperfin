...
- Recent native PRG parity slice: `#4655` under #3217 adds the documented `Header`/`Column` `MouseIcon` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic empty-string defaults, raw filename state, direct/PEM access, reflection, and built-in protection; exact-head Windows validation is requested for `41ac354b`. Cursor/icon loading, rendering, host UI integration, and designer serialization remain separate.
