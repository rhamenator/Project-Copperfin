...
- Latest implemented E3 slice: `#1616`, which proves live FRX/LBX layout objects edited into out-of-section geometry can then be deleted with edited HPOS/VPOS/HEIGHT fields preserved in deleted-object and selected-object JSON while selected containing-section metadata stays unavailable/null. `#1615` is the immediately preceding closed edited live-object delete geometry slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
