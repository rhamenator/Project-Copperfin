...
- Latest implemented E2 slice: `#1308`, which adds selected-context toolbox object creation execution through the shared selected-context create-plan seam. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
