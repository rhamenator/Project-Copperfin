...
- Latest implemented E3 slice: `#1760`, which covers dot-leading report/label layout classification codes. Studio host regressions prove dot-leading FRX/LBX `OBJTYPE`/`OBJCODE` values keep inspection non-failing, do not fabricate root settings, live/deleted bands, layout objects, or preview bounds, and degrade explicit record selection to no selected report metadata while retaining label identity. `#1759` covered trimmed classification codes, `#1758` covered fractional classification codes, and `#1757` covered mixed-case root settings memo names. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
