...
- Latest implemented E2 slice: `#1429`, which exposes visual object method reorders through Studio host JSON. `#1428` is the immediately preceding visual object method-move host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
