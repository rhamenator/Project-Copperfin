...
- The live execution lane remains broader than a single parent: choose the next prompt-sized child from live GitHub state across E3/#24 report/label designer fidelity, native runtime parity `#3217`, and still-evidence-backed hardening follow-ons under `#3214` or `#110`, with localization/#2348 acting as a standing requirement for new user-facing text rather than the default implementation lane. E2/#23 remains open with many prompt-sized children still open in GitHub; treat them as evidence-audit/closure cleanup unless fresh implementation evidence shows a real remaining local miss.
- Preserve stack-frugal PRG execution as a hard runtime constraint. `#4068` now owns the pre-existing synchronous nested expression-level UDF path that recursively re-enters the host evaluator despite heap-backed PRG frames; fix it with a continuation/trampoline slice, not by inflating the process stack. Shipped `#3918` owns session-wide UDFPARMS value/reference semantics and must retain its UDF-only call classification, live scalar aliases, canonical array aliases, and independent `DO ... WITH` behavior.
- Latest implemented native runtime slice: `#3869`, VFP9 `Init RETURN .F.` construction rejection. Native `CREATEOBJECT()` and `NEWOBJECT()` now return `.NULL.` after an explicit boolean false from `Init`, remove the pending object subtree without `Destroy` or object-created events, and retain iterative expression-invoked frame execution. Installed VFP9 help says `.F.` prevents creation and skips `Destroy`; focused Container-with-child coverage proves both entry points, null identity, continued execution, cleanup, and event suppression under GCC/Clang, with ASan/UBSan runtime-surface coverage passing. The local runner injects an incompatible ASan runtime into the separate localization executable, so it is not sanitizer evidence. Do not turn this contract into a runtime fault or reintroduce a synthetic OLE fallback.
- Latest implemented E3 slice: `#4058`, CP-marked FRX/LBX text transcoding at shared designer boundaries. Marked C/V and M values decode to UTF-8 for shared model and host JSON; editor C/M writes strictly encode back to the original table page, rejecting invalid UTF-8 and unrepresentable values, and schema rewrites retain the source mark with their preserved raw fields/memos. Unmarked writes remain UTF-8-compatible, while opaque raw structural fields/memos keep byte-for-byte preservation and malformed legacy displays retain their sanitizing fallback. CP1252/CP1251 codec, FRX/FRT and LBX/LBT direct/memo round-trip, failure-atomic rejection, raw-byte, schema-rewrite, and Studio-host JSON tests pass under GCC and Clang. Keep all future user-facing diagnostics in four catalogs and all machine JSON keys/IDs invariant.
- Latest implemented localization slice: `#2628`, remaining `StudioHost.LaunchParse.ObjectCommand.*` label parity. Keep new user-facing work localized by default, but prefer ordinary E3/#24 feature children unless live GitHub state raises a higher-priority localization or release blocker.
...
| `#3217` | `#4069` open | Dispatch Form `Load` once during native PRG instantiation through the iterative method-frame path without changing `Init`, child materialization, `THISFORM`, or `THISFORMSET` behavior |
