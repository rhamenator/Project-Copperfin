...
- Latest implemented E2 slice: `#1242`, which exposes non-mutating toolbox object creation planning through Studio host JSON ahead of the existing mutating toolbox-create path. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
