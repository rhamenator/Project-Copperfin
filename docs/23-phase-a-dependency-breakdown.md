...
- Latest implemented E3 slice: `#4057`, opaque-byte-safe FRX/LBX structural editing. Single/batch create, duplicate, subtree duplicate, and reorder operations now preserve existing raw DBF headers, flags/code-page marks, reserved and trailing bytes, deleted markers, complete record spans, memo blocks, and physical memo slack; new memo payloads append without rewriting old content, malformed preservation boundaries fail before publication, and table-only/pair transactions recover before multi-step rollback snapshots. Synthetic FRX/FRT and LBX/LBT fixtures plus mounted VFP9 `invoice.frx` and `cust.lbx` copies prove exact primary/sidecar behavior across no-op, single, batch, subtree, rollback, and malformed cases. `#4058` is the next live E3 child and owns CP-marked text transcoding at designer boundaries.
