...
- Latest implemented E2 slice: `#1450`, which exposes visual object reorder batches through Studio host JSON. `#1449` is the immediately preceding rename-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
