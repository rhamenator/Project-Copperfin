...
- Latest implemented E2 slice: `#1347`, which normalizes Studio host child process `observedExitCode` values from `std::system(...)` wait statuses across individual builder, editor-action, toolbox, and aggregate designer execution JSON. `#1346` is the immediately preceding failed-child error summary host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
