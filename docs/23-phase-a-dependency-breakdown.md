...
- Windows runtime-host debug fixture slice `#4048` normalizes captured CRLF only at the test assertion boundary, JSON-escapes native bridge request paths, and passes Unicode filesystem-valued locale roots through `ScopedEnvironmentPath` rather than a lossy narrow path conversion. Keep the per-case/subcase progress markers and the focused audit workflow's explicit `test_runtime_host_debug_output` build/CTest selection; hosted Linux GCC, macOS Clang, and Windows MSVC all pass.
