...
- Latest implemented E2 slice: `#1423`, which exposes visual object method queries through Studio host JSON. `#1422` is the immediately preceding visual object method-list host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
