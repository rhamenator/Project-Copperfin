...
- Latest implemented E2 slice: `#1226`, which exposes non-executing editor action invocation dispatch planning through Studio host JSON for admitted editor invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
