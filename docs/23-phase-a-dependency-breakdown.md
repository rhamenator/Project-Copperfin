...
- Latest implemented E2 slice: `#1237`, which adds non-executing aggregate designer invocation dispatch planning across editor actions, builders, and toolbox palettes for one selected Studio context. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
