...
- Latest implemented E2 slice: `#1437`, which exposes visual object property clear batches through Studio host JSON. `#1436` is the immediately preceding visual object property-copy host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
