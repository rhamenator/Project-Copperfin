...
- Recent native PRG parity slice: `#4658` under #3217 adds the documented `Column.Format` state to explicit, derived, and materialized grid-column objects. Preserve the deterministic empty-string default, raw direct/PEM access, reflection, and built-in protection; exact-head Windows validation is requested for `e6ede3db`. Format-code evaluation, display conversion, editing behavior, rendering, and designer serialization remain separate.
