...
- Recent native PRG parity slice: `#4771` under #3217 completes the modeled keyboard default-action boundary. After `KeyPress` permits default processing, ENTER selects the owning Form's deterministic `CommandButton.Default` candidate and ESC selects `CommandButton.Cancel`; missing handlers are skipped, `Click` receives no synthetic arguments, and `NODEFAULT`, event-loop restoration, and invariant telemetry remain intact. Focus traversal, keyboard buffering, toolbar arbitration, pixel UI, and hosted Windows/macOS validation remain separate. Focused runtime-surface CTest passes locally.
