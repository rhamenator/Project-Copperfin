...
- Latest implemented E2 slice: `#1221`, which adds aggregate non-executing designer invocation admission planning across editor actions, builders/wizards, and toolbox palettes. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
