...
- Latest implemented E3 slice: `#1783`, which covers stable-selected Detail Header/Footer object duplicate containment for FRX/LBX label/field objects. Studio host regressions prove Detail Header and Detail Footer duplicates append replacement-`UNIQUEID` records, preserve object metadata, remain placed in the correct containing sections, refresh section object indexes/counts, keep inspection non-failing, and retain label identity. `#1782` covered delete/restore containment transitions, and `#1781` covered deleted object geometry edits. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
