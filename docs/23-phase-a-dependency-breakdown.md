...
- Latest implemented E2 slice: `#1444`, which exposes visual object property reorder batches through Studio host JSON. `#1443` is the immediately preceding visual object property-reorder host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
