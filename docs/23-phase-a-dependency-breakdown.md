...
- Latest implemented E3 slice: `#1487`, which proves Studio host deleted `.lbx` label layout object restore mutation through stable selectors, cleared LBX delete flags, restored live section membership, and selected containing-section JSON while preserving label identity. `#1486` is the immediately preceding label layout object delete-state mutation coverage slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
