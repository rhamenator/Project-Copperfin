...
- Latest implemented E2 slice: `#1348`, which exposes per-child aggregate designer execution `launchCommand` fields for editor-action, builder/wizard, and toolbox entries in `copperfin_studio_host --designer-execute --json`. `#1347` is the immediately preceding normalized observed-exit-code host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
