...
- Latest implemented E2 slice: `#1320`, which adds the injected editor-action dispatch execution seam for admitted context-editor action dispatch contracts. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
