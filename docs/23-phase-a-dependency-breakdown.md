...
- Latest implemented E3 slice: `#1455`, which exposes the containing section for selected report/label layout objects through Studio host JSON. `#1454` is the immediately preceding selected-object JSON slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
