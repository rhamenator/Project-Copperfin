...
- Latest implemented E2 slice: `#1256`, which exposes non-mutating toolbox object batch creation dispatch catalogs through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
