...
- Latest implemented E3 slice: `#1774`, which covers stable-selected deleted Detail Header/Footer object font/highlight metadata for FRX/LBX label/field objects. Studio host regressions prove deleted selected objects expose `FONTFACE`, `FONTSIZE`, and `MODE` provenance plus highlight counts while preserving deleted state, expression metadata, unavailable/null containing-section metadata, non-failing inspection, and label identity. `#1773` covered live Detail Header/Footer object font/highlight metadata, `#1772` covered deleted object expression edits and clears, and `#1771` covered deleted object expression selections. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
