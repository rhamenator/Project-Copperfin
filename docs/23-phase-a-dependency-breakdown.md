...
- Latest implemented E2 slice: `#1336`, which preserves nested editor-action, builder/wizard, and toolbox dispatch command tokens plus dispatch arguments in `copperfin_studio_host --designer-dispatch-catalog` JSON so aggregate designer dispatch-catalog consumers can inspect exact non-executing child launch contracts without switching to execution-readiness catalog output. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
