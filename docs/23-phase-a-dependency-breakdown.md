...
- Latest implemented E2 slice: `#1430`, which exposes visual object method delete batches through Studio host JSON. `#1429` is the immediately preceding visual object method-reorder host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
