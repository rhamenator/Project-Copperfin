...
- Recent native PRG parity slice: `#4657` under #3217 adds the documented `Column.InputMask` state to explicit, derived, and materialized grid-column objects. Preserve the deterministic empty-string default, raw direct/PEM access, reflection, and built-in protection; exact-head Windows validation is requested for `7783baef`. Mask parsing, editing behavior, rendering, and designer serialization remain separate.
