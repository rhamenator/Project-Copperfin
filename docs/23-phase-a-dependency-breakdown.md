...
- Latest implemented E3 slice: `#1456`, which exposes selected report/label root settings through Studio host JSON. `#1455` is the immediately preceding selected-object containing-section JSON slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
