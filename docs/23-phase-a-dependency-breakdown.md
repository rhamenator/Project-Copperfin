...
- Latest E3 report/label slice: `#4556` under `#24` carries FRX/LBX variable-record (`OBJTYPE = 18`) memo-backed `TAG` initial values through native layout highlights, additive host JSON, and the shared localized property grid. Runtime expression evaluation, rendering, and Report Builder behavior remain separate.
- Latest portable UI harness regression: `#4557` under `#23` fixes standalone Command/Terminal tab removal by selecting a surviving tool window before removing the selected page. This prevents the Mono/WinForms invalid selected-index crash found by Linux CI while preserving shell-layout state and dock/restore behavior.
