...
- Latest implemented E2 slice: `#1431`, which exposes visual object method rename batches through Studio host JSON. `#1430` is the immediately preceding visual object method delete-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
