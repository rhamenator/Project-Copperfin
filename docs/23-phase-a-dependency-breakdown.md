...
- Latest implemented E3 slice: `#1768`, which covers stable `--unique-id` expression-provenance selection for live Detail Header and Detail Footer report/label sections. Studio host regressions prove live Detail Header/Footer stable selections expose selected-section expression text, field/memo provenance, object-code and geometry metadata, null unrelated report-selection surfaces, non-failing inspection, and label identity. `#1767` covered stable deleted Detail Header/Footer section selection, `#1766` covered deleted Detail Header/Footer section-expression edits, and `#1765` covered live Detail Header/Footer section-expression edits. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
