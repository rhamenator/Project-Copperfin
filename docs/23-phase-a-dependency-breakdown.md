...
- Latest implemented E2 slice: `#1244`, which exposes non-mutating toolbox object creation plan catalogs through Studio host JSON for selected toolbox contexts. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
