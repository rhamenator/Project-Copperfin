...
- Latest implemented E2 slice: `#1271`, which exposes non-executing builder/wizard invocation-admission catalog planning through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
