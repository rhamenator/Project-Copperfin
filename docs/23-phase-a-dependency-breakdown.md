...
- Latest implemented E3 slice: `#1758`, which covers fractional report/label layout classification codes. Studio host regressions prove fractional FRX/LBX `OBJTYPE`/`OBJCODE` values derive root settings, live/deleted bands, and live/deleted layout objects from integer portions while selected settings/sections/objects resolve, inspection stays non-failing, and label identity is retained. `#1757` covered mixed-case root settings memo names, `#1756` covered CR-only memo line separators, and `#1755` covered invalid-first duplicate setting precedence. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
