...
- Latest implemented E2 slice: `#1302`, which adds selected-context single-item toolbox object create-dispatch planning through the shared selected-context create-plan seam. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
