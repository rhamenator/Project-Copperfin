...
- Latest implemented E2 slice: `#1421`, which exposes visual object ancestor listings through Studio host JSON. `#1420` is the immediately preceding visual object descendant host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
