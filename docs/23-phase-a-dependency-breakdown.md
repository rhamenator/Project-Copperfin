...
- Security audit child `#4895` binds the manual signer/guard job to the fixed
  ...
  passes the focused permission/provisioning contract. The VSIX, managed-UI,
...
- Recent G1 signature-highlighting slice: `#4884` anchors Visual Studio
  ...
  expressions retain distinct highlights, while nested calls and quotes remain
  ...
  declaration boundary. Final head `05bd74dc8` passes the local four-mode matrix
  and Windows VSIX run `30531427326`; artifact `8754891262` was uploaded.
...
- Recent F1 test-infrastructure slice: `#4798` adds a final owner-first,
  ...
  and Windows `run-required-designer-smoke.ps1` passed with completed status
  and no remaining Copperfin process. Xvfb remains unavailable on this host.
...
- Recent managed language-service slice: `#4795` under `#27` extends the
  ...
  closed as implementation-complete. Native parser behavior remains the
  separate #4792 contract.
...
- Recent hosted-build correction: `#4593` under `#2996/#110` fixes the confirmed Windows VSIX compiler error from run `30116440637` by importing `System.Runtime.InteropServices` for the terminal pane's `Guid` registration attribute. Hosted run `30117134899` passed the VSIX build, localization, VSIX behavior, language-service, and process-runner tests; the source correction is narrow and contract-neutral.
- Recent standalone shell slice: `#4592` under `#2997` persists Command and Terminal floating bounds in shell-layout version 3 while accepting version-1/version-2 state. Valid bounds survive docking and relaunch; undersized or wholly off-screen rectangles fall back to visible defaults. The focused portable floating-window smoke passes its round-trip and fallback assertions; shared controls, localization, VSIX/runtime contracts, and host JSON are unchanged.
- Recent standalone shell slice: `#4590` under `#2997` adds localized float/re-dock commands for the existing Command and Terminal panes. The controls are reparented rather than recreated, so transcript/session state survives; closing a floating tool window returns it to the dock. Version-2 shell JSON stores independent floating flags while accepting version-1 state, and existing visibility, selected-tool, splitter, and disposal contracts remain intact. Exact and broad `SmokeStandaloneStudio*` portable UI coverage and managed catalog parity pass locally. Keep live Windows/Visual Studio docking and hosted VSSDK validation as release evidence; do not represent this standalone behavior as Visual Studio shell implementation.
- Recent native PRG parity slice: `#4589` under #3217 exposes the documented
...
- Recent native PRG parity slice: `#4588` under #3217 exposes the documented
...
- Recent VSIX Command integration slice: `#4587` under #3051 connects the
...
- Recent UI validation slice: `#4551` under #110 adds `scripts/run-designer-smoke-headless.sh`, which builds and runs the existing net472 WinForms DesignerSmoke executable through a fixed Xvfb screen and provides selector discovery without Xvfb. Preserve selector and exit-code contracts; use it for portable shared/WinForms evidence, while live Visual Studio theme/docking, Win32 rendering, and hosted VFP9 behavior remain Windows evidence gates.
- Recent native PRG parity correction: `#4586`, `#4585`, `#4584`, and `#4583` under #3217 are scoped to the documented `Column.DynamicFontOutline`, `Column.DynamicFontShadow`, `Column.DynamicFontSize`, and `Column.DynamicFontName` properties. Preserve each raw expression through direct/PEM access, reflection, built-in protection, and derived initialization; ordinary visual controls, nonvisual objects, and OLE hosts do not receive them, and the runtime does not evaluate expressions or claim font layout/rendering.
- Recent native PRG parity slice: `#4649` under #3217 adds the documented `Header`/`Column` `FontName`, `FontCharSet`, and `FontSize` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic headless defaults, direct/PEM access, existing normalization, reflection, and built-in protection; platform font lookup, rendering, layout, font styles, `FontCondense`/`FontExtend`, and designer serialization remain separate.
- Recent native PRG parity slice: `#4650` under #3217 adds the documented `Header`/`Column` logical font-style state for `FontBold`, `FontItalic`, `FontUnderline`, `FontStrikeThru`, `FontOutline`, and `FontShadow` to explicit, derived, and lazily materialized grid-header objects. Preserve false defaults, direct/PEM access, reflection, and built-in protection; platform font lookup, rendering, layout, `FontCondense`/`FontExtend`, and designer serialization remain separate.
- Recent native PRG parity slice: `#4651` under #3217 adds the documented `Header`/`Column` `MousePointer` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic zero defaults, direct/PEM access, nonnegative integer normalization, reflection, and built-in protection; exact-head Windows validation passed at `9fab63ee` with artifacts under `artifacts/windows-validation-9fab63ee/`. OS cursor selection, MouseIcon assets, hit testing, rendering, and designer serialization remain separate.
- Recent native PRG parity slice: `#4661` under #3217 adds the documented `Column.DynamicFontItalic`, `Column.DynamicFontStrikeThru`, and `Column.DynamicFontUnderline` raw-expression state to explicit, derived, and materialized grid-column objects. Preserve the documented `.F.` defaults, direct/PEM access, reflection, and built-in protection; focused Linux and exact-head Windows validation passed in coordination seq164 for `9fa13d61`, closing this slice. Expression evaluation, grid refresh, per-row style resolution, font lookup, layout, rendering, host UI integration, and designer serialization remain separate.
- Recent native PRG parity slice: `#4660` under #3217 adds the documented `Column.DynamicFontBold` raw-expression state to explicit, derived, and materialized grid-column objects. Preserve the documented `.T.` true-default exception, direct/PEM access, reflection, and built-in protection; focused Linux runtime-surface CTest and exact-head Windows validation passed at `fc499174` with corrected artifacts under `artifacts/windows-validation-fc499174/`. Expression evaluation, per-row bold resolution, font lookup, layout, rendering, host UI integration, and designer serialization remain separate.
- Recent native PRG parity slice: `#4659` under #3217 adds the documented `Column.DynamicBackColor` and `Column.DynamicForeColor` raw-expression state to explicit, derived, and materialized grid-column objects. Preserve deterministic empty-string defaults, direct/PEM access, reflection, and built-in protection; focused Linux runtime-surface CTest and exact-head Windows validation passed at `31c19fb7` with artifacts under `artifacts/windows-validation-31c19fb7/`. Expression evaluation, grid refresh, per-row color resolution, rendering, and designer serialization remain separate.
- Recent native PRG parity slice: `#4658` under #3217 adds the documented `Column.Format` state to explicit, derived, and materialized grid-column objects. Preserve the deterministic empty-string default, raw direct/PEM access, reflection, and built-in protection; focused Linux runtime-surface CTest and exact-head Windows validation passed at `e6ede3db` with artifacts under `artifacts/windows-validation-e6ede3db/`. Format-code evaluation, display conversion, editing behavior, rendering, and designer serialization remain separate.
- Recent native PRG parity slice: `#4657` under #3217 adds the documented `Column.InputMask` state to explicit, derived, and materialized grid-column objects. Preserve the deterministic empty-string default, raw direct/PEM access, reflection, and built-in protection; exact-head Windows validation passed at `7783baef` with artifacts under `artifacts/windows-validation-7783baef/`. Mask parsing, editing behavior, rendering, and designer serialization remain separate.
- Recent native PRG parity slice: `#4656` under #3217 adds the documented `Column.Enabled` state to explicit, derived, and materialized grid-column objects. Preserve deterministic `.T.` defaults, direct/PEM access, logical normalization, reflection, and built-in protection; exact-head Windows validation passed at `2ba69cdc` with artifacts under `artifacts/windows-validation-2ba69cdc/`. Event routing, child propagation, hit testing, rendering, and designer serialization remain separate.
- Recent native PRG parity slice: `#4655` under #3217 adds the documented `Header`/`Column` `MouseIcon` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic empty-string defaults, raw filename state, direct/PEM access, reflection, and built-in protection; exact-head Windows validation passed at `41ac354b` with artifacts under `artifacts/windows-validation-41ac354b/`. Cursor/icon loading, rendering, host UI integration, and designer serialization remain separate.
- Recent native PRG parity slice: `#4654` under #3217 adds the documented `Header`/`Column` `StatusBarText` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic empty-string defaults, direct/PEM access, reflection, and built-in protection; exact-head Windows validation passed at `901e48a8` with artifacts under `artifacts/windows-validation-901e48a8/`. Status-bar rendering, host UI integration, and designer serialization remain separate.
- Recent native PRG parity slice: `#4653` under #3217 adds the documented `Header`/`Column` `ToolTipText` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic empty-string defaults, direct/PEM access, reflection, and built-in protection; exact-head Windows validation passed at `153d999e` with artifacts under `artifacts/windows-validation-153d999e/`. Tooltip rendering, host UI integration, and designer serialization remain separate.
- Recent native PRG parity slice: `#4652` under #3217 adds the documented `Header`/`Column` `Tag` state to explicit, derived, and lazily materialized grid-header objects. Preserve deterministic empty-string defaults, direct/PEM access, reflection, and built-in protection; exact-head Windows validation passed at `7850d4ed` with artifacts under `artifacts/windows-validation-7850d4ed/`. Tooltip/status-bar behavior, UI rendering, and designer serialization remain separate.
- Recent native PRG parity correction: `#4582` under #3217 is scoped to the documented `Column.DynamicInputMask` property, not `TextBox`. Preserve the raw string through direct/PEM access, reflection, built-in protection, and derived initialization; do not evaluate the expression or claim UI mask application.
- Recent native PRG parity slice: `#4548` under #3217 exposes VFP9 `DisabledItemBackColor` and `DisabledItemForeColor` on native ComboBox/ListBox objects with deterministic integer defaults, direct/reflection normalization, derived-Init coverage, and built-in reflection protection. Preserve machine property names and stack-frugal execution; keep per-row disabled state, rendering, theme translation, and hosted Windows validation separate. Focused runtime-surface coverage passes locally.
- Recent native PRG parity slice: `#4549` under #3217 exposes VFP9 `ItemBackColor` and `ItemForeColor` on native ComboBox/ListBox objects with deterministic control-level integer defaults, direct/reflection normalization, derived-Init coverage, and built-in reflection protection. Preserve machine property names and stack-frugal execution; keep per-row color storage, rendering, theme translation, and hosted platform validation separate. Focused runtime-surface coverage passes locally.
- Recent native PRG parity slice: `#4550` under #3217 exposes VFP9 `SelectedItemBackColor` and `SelectedItemForeColor` on native ComboBox/ListBox objects with deterministic integer defaults matching the native TextBox selected-color fallback, direct/reflection normalization, derived-Init coverage, and built-in reflection protection. Preserve machine property names and stack-frugal execution; keep selected-row rendering, focus/highlight behavior, theme translation, and hosted platform validation separate. Focused runtime-surface coverage passes locally.
- Recent E3 report/label slice: `#4543` exposes header-only FRX/LBX `ORDER` as a binary-safe uppercase `hex:` value through the shared native/managed settings model and additive stable host JSON. Preserve exact memo bytes, field/memo provenance, live/deleted/read-only behavior, generic edit/clear/reopen identity, and localized string-property routing; keep the 32 protection-flag interpretation, report-builder behavior, rendering, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, raw-byte round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4544` exposes memo-backed FRX/LBX `COMMENT` notes for root settings, bands, and layout objects through the shared native/managed model, additive stable host JSON, and localized standalone/VSIX property-grid fields. Preserve invariant field names, field/memo provenance, live/deleted/read-only behavior, and generic edit/reopen identity; keep comments as developer notes and leave rendering, report execution, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, and diff checks are the validation target at the time of this entry.
- Recent E3 report/label slice: `#4545` exposes memo-backed FRX/LBX `USER` comments for root settings, bands, and layout objects through the shared native/managed model, additive stable host JSON, and localized standalone/VSIX `User Comments` property-grid fields. Preserve invariant field names, field/memo provenance, live/deleted/read-only behavior, and generic edit/reopen identity; keep USER as non-executable metadata and leave Report Builder integration, rendering, report execution, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, and diff checks are the validation target at the time of this entry.
- Recent E3 report/label slice: `#4546` exposes band-level FRX/LBX `NOREPEAT` through the shared native/managed section model, additive stable host JSON, and localized standalone/VSIX section property-grid fields. Preserve invariant field names, field/memo provenance, live/deleted/read-only behavior, and generic edit/clear/reopen identity; keep control-level NOREPEAT semantics, detail-header reprinting, rendering, pagination, report execution, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, and diff checks are the validation target at the time of this entry.
- Recent native PRG parity slice: `#4547` under #3217 dispatches VFP9 `ProgrammaticChange` for ComboBox/ListBox direct and reflective selection/value changes, list mutations, and `Requery()` through the existing iterative stack-frugal method path. Preserve no-op/reorder silence, item identity, selected-item identity, bound-value synchronization, and machine event identifiers; keep `InteractiveChange`, keyboard/mouse input, popup/pixel behavior, and hosted Windows validation separate. Focused runtime-surface coverage passes locally.
- Recent native PRG parity slice: `#4758` under #3217 extends the existing modeled `WM_KEYDOWN`/`KeyPress` boundary to ComboBox/ListBox `InteractiveChange`. The runtime compares stable effective selection/value signatures and dispatches only for changed input, while preserving unchanged-input silence, `KeyPress`/`NODEFAULT`, the event loop, and `ProgrammaticChange`. Native keyboard navigation, focus/popup/text input, rendering, and hosted Windows validation remain separate; focused runtime-surface coverage passes locally.
- Recent native PRG parity slice: `#4759` under #3217 extends the modeled `WM_KEYDOWN`/`KeyPress` boundary to Form `KeyPreview`. A child control's owning Form receives `KeyPress` first when preview is enabled; Form `NODEFAULT` suppresses child processing, while the child remains authoritative otherwise. Preserve the existing iterative frame path, key arguments, event-loop restoration, and machine telemetry; native focus traversal, keyboard buffering, modifier expansion, Page/_SCREEN/ToolBar applicability, rendering, and hosted Windows UI validation remain separate. Focused runtime-surface coverage passes locally.
- Recent native PRG parity slice: `#4760` under #3217 extends the modeled window-message boundary to native `WM_LBUTTONUP`/`Click`. A target visual object with a declared `Click` handler now dispatches through the iterative frame path, returns the modeled default result, restores the event loop, and emits invariant click telemetry; a target without `Click` remains unhandled. Preserve existing bindings, WM_CLOSE, and WM_KEYDOWN behavior; hit testing, focus traversal, `MouseDown`/`MouseUp` sequencing, double-click detection, button/modifier state, rendering, and hosted Windows UI remain separate. Focused runtime-surface coverage passes locally.
- Recent native PRG parity slice: `#4761` under #3217 extends the modeled left-button boundary to VFP `MouseDown` and `MouseUp`, preserving the `MouseDown -> MouseUp -> Click` order around native `WM_LBUTTONDOWN`/`WM_LBUTTONUP`. Button, modifier, and signed client-coordinate arguments, stable event telemetry, independent handler dispatch, and event-loop restoration are covered by the focused runtime test. Preserve existing bindings and message behavior; hit testing, capture/focus, double-clicks, other buttons, rendering, and hosted Windows UI remain separate.
- Recent native PRG parity slice: `#4762` under #3217 extends modeled pointer input to `WM_MOUSEMOVE`/`MouseMove`, mapping Win32 left/right/middle button flags to VFP button bits while preserving Shift/Ctrl state, signed client coordinates, stable telemetry, independent missing-handler behavior, and event-loop restoration. Preserve existing bindings and left-button message behavior; hit testing, capture/hover, drag-and-drop, rendering, and hosted Windows UI remain separate. Focused runtime-surface coverage passes in a fresh out-of-tree build.
- Recent native PRG parity slice: `#4763` under #3217 extends modeled pointer input to `WM_LBUTTONDBLCLK`/`DblClick`, preserving VFP's ordinary-control no-argument handler contract, stable telemetry, independent missing-handler behavior, and event-loop restoration. The follow-up correction `#4767` removes synthetic mouse parameters from the native invocation while preserving mouse arguments for `MouseDown`/`MouseUp`/`MouseMove`; double-click timing policy, hit testing, capture, rendering, and hosted Windows UI remain separate. Focused runtime-surface coverage passes in a fresh out-of-tree build.
- Recent native PRG parity slice: `#4764` under #3217 extends modeled pointer input to `WM_RBUTTONUP`/`RightClick`, preserving the VFP no-argument handler contract, stable telemetry, independent missing-handler behavior, and event-loop restoration. Preserve existing bindings and left-button message behavior; context-menu rendering, hit testing, capture, and hosted Windows UI remain separate. Focused runtime-surface coverage passes in a fresh out-of-tree build.
- Recent native PRG parity slice: `#4765` under #3217 extends modeled pointer input to `WM_MBUTTONUP`/`MiddleClick`, preserving the VFP no-argument handler contract, stable telemetry, independent missing-handler behavior, and event-loop restoration. Preserve existing bindings and left/right-button message behavior; middle-button `MouseDown`/`MouseUp`, hit testing, capture, rendering, and hosted Windows UI remain separate. Focused runtime-surface coverage passes in a fresh out-of-tree build.
- Recent native PRG parity slice: `#4766` under #3217 extends modeled middle-button input to `WM_MBUTTONDOWN`/`WM_MBUTTONUP` `MouseDown`/`MouseUp`, preserving VFP button value `4`, modifier and coordinate arguments, `MouseUp -> MiddleClick` ordering, stable telemetry, independent missing-handler behavior, and event-loop restoration. Preserve existing left/right-button and mouse-move behavior; middle-button hit testing, capture, drag-and-drop, rendering, and hosted Windows UI remain separate. Focused runtime-surface coverage passes in a fresh out-of-tree build.
- Recent native PRG parity slice: `#4768` under #3217 extends modeled right-button input to `WM_RBUTTONDOWN`/`WM_RBUTTONUP` `MouseDown`/`MouseUp`, preserving VFP button value `2`, modifier and coordinate arguments, `MouseUp -> RightClick` ordering, stable telemetry, independent missing-handler behavior, and event-loop restoration. Preserve existing left/middle-button and mouse-move behavior; right-button hit testing, capture, context-menu rendering, drag-and-drop, and hosted Windows UI remain separate. Focused runtime-surface coverage will be recorded against the synchronized product head.
- Recent native PRG parity slice: `#4771` under #3217 completes the modeled keyboard default-action boundary. After `KeyPress` permits default processing, ENTER selects the focused enabled `CommandButton` when its host target is available, otherwise the owning Form's deterministic `CommandButton.Default` candidate; ESC selects `CommandButton.Cancel`. Missing handlers and disabled candidates are skipped, `Click` receives no synthetic arguments, and `NODEFAULT`, event-loop restoration, and invariant telemetry remain intact. Ordinary CommandButton synthetic handles, focus traversal, keyboard buffering, toolbar arbitration, pixel UI, and hosted Windows/macOS validation remain separate. Focused runtime-surface CTest passes locally.
- Recent E3 report/label slice: `#4542` exposes header-only FRX/LBX `UNIQUE` through the shared native/managed settings model, additive stable host JSON, and localized standalone/VSIX logical property-grid fields. Preserve invariant values, source-field provenance, blank/deleted/read-only behavior, and generic update/clear/reopen identity; keep report-generation/wizard behavior, rendering, hosted Windows VFP9 validation, and binary `ORDER` protection-flags interpretation separate. Focused native, host, managed, localization, round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4541` exposes header-only FRX/LBX `CURPOS` through the shared native/managed settings model, additive stable host JSON, and localized standalone/VSIX logical property-grid fields. Preserve invariant values, source-field provenance, blank/deleted/read-only behavior, and generic update/clear/reopen identity; keep non-header builder/runtime `CURPOS` selection state, cursor overlays, rendering, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4540` exposes header-only FRX/LBX `ADDALIAS` through the shared native/managed settings model, additive stable host JSON, and localized standalone/VSIX logical property-grid fields. Preserve invariant values, source-field provenance, blank/deleted/read-only behavior, and generic update/clear/reopen identity; keep Expression Builder alias semantics, `PLAIN`, `ORDER`, `CURPOS`, rendering, printer behavior, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4539` preserves header-only FRX/LBX `PICTURE` printer override pairs through the shared native/managed settings model and stable host JSON, using `PICTURE` precedence over duplicate `EXPR` or direct settings while retaining field/memo/source-line provenance and stable setting order. The shared writer updates and clears the selected backing memo without dropping unsupported lines; keep printer execution, object/image/expression `PICTURE` meanings, and hosted Windows VFP9 validation separate. Focused native, host, managed, precedence, round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4538` exposes header-only FRX/LBX `RULERLINES` through the shared native/managed settings model, additive stable host JSON, and localized standalone/VSIX numeric property-grid fields. Preserve invariant values, source-field provenance, blank/deleted/read-only behavior, header versus expression-object context, and generic update identity; keep expression string-trimming semantics, ruler-line rendering, measurement behavior, unsupported header semantics, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, edit/clear/reopen, and diff checks pass locally.
- Recent E3 report/label slice: `#4537` exposes report-header FRX/LBX view settings through the shared native/managed settings model, additive stable host JSON, and localized standalone/VSIX property-grid fields: logical `GRID` and numeric `RULER`. Preserve invariant values, source-field provenance, blank/deleted/read-only behavior, and generic update identity; suppress blank logical `?` settings and keep `CURPOS` selection state, grid/ruler rendering, snapping, measurement conversion, layout recalculation, printer behavior, unsupported header semantics, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, edit/clear/reopen, and diff checks pass locally.
- Recent E3 report/label slice: `#4536` exposes FRX/LBX font-charset fields through the shared native/managed model, additive stable host JSON highlights, and localized standalone/VSIX flag/numeric property-grid fields: `DOUBLE` for header, text/label, expression, and picture records, and `RESOID` for header, text/label, and expression records. Preserve invariant values, field/memo provenance, blank/deleted/read-only behavior, and generic update identity; keep charset interpretation, font lookup, glyph fallback, rendering, font-resource records, unsupported object semantics, and hosted Windows VFP9 validation separate. Focused native, host, managed, localization, font-charset round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4535` exposes FRX/LBX line and shape styling fields through the shared native/managed model, additive stable host JSON highlights, and localized standalone/VSIX numeric property-grid fields, preserving invariant values, field/memo provenance, blank/deleted/read-only behavior, and object-type-specific update identity. Keep rendering, enum validation, line/shape painting, print behavior, unsupported object semantics, and hosted Windows VFP9 validation separate; focused native, host, managed, localization, styling round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4534` exposes FRX/LBX report-control RGB channels through the shared native/managed model, additive stable host JSON highlights, and localized standalone/VSIX numeric property-grid fields, preserving invariant values, field/memo provenance, blank/deleted/read-only behavior, and generic update identity. Keep color interpretation, picker UX, rendering, dynamic color expressions, unsupported object semantics, and hosted Windows VFP9 validation separate; focused native, host, managed, localization, RGB round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4533` exposes memo-backed band-level VFP9 FRX/LBX `TAG` and `TAG2` On Entry/On Exit expressions through the shared native/managed report and label model, additive stable host JSON, and localized standalone/VSIX property-grid fields, preserving invariant values, field/memo provenance, blank/deleted/read-only behavior, and generic update identity. Keep band event execution/compilation, control-object tooltip `TAG2`, pagination, rendering, and hosted Windows VFP9 validation separate; focused native, host, managed, localization, memo round-trip, and diff checks pass locally.
- Recent E3 report/label slice: `#4532` exposes section-level VFP9 FRX/LBX `EJECTBEFOR`, `EJECTAFTER`, and `PLAIN` logical values through the shared native/managed report and label model, additive stable host JSON, and localized standalone/VSIX property-grid fields, preserving invariant values, field/memo provenance, blank/deleted/read-only behavior, and generic update identity. Keep optional-band execution, page-header/page-footer emission, constant-height layout, pagination, rendering, and hosted Windows VFP9 validation separate; focused native, host, managed, localization, and diff checks pass locally.
- Recent E3 report/label slice: `#4531` exposes section-level VFP9 FRX/LBX `PAGEBREAK`, `COLBREAK`, and `RESETPAGE` logical values through the shared native/managed report and label model, additive stable host JSON, and localized standalone/VSIX property-grid fields, preserving invariant values, field/memo provenance, blank/deleted/read-only behavior, and generic update identity. Keep pagination execution, rendering, page/column break behavior, and hosted Windows VFP9 validation separate; focused native, host, managed, localization, and diff checks pass locally.
- Recent E3 report/label slice: `#4530` exposes the existing VFP9 FRX/LBX memo-backed `TAG2` tooltip field on eligible report-control objects (`OBJTYPE` 5, 6, 7, 8, 10, and 17) through the shared native/managed model, preserving invariant value, field/memo provenance, generic update identity, and localized property-grid display. Keep variable records excluded; tooltip rendering, report runtime behavior, and hosted Windows VFP9 validation remain separate work. Focused managed, localization, native report-layout, catalog-parity, and diff checks pass; pushed in `df2acdfb`.
- Recent E3 report/label slice: `#4529` exposes the existing VFP9 FRX/LBX image-source `OFFSET` field on image/bound objects (`OBJTYPE=17`) through the shared native/managed model, preserving invariant value, field/memo provenance, generic update identity, and localized property-grid display. Keep report-expression alignment and image `PICTURE`/`GENERAL` meanings separate; path resolution, General-field loading, expression evaluation, image loading/rendering, and hosted Windows VFP9 validation remain separate work. Focused managed, localization, native report-layout, catalog-parity, and diff checks pass; pushed in `9d20cb8b`.
- Recent E3 report/label slice: `#4528` exposes the existing VFP9 FRX/LBX `GENERAL` image clip/scale field on image objects (`OBJTYPE=17`) through the shared native/managed model, preserving invariant numeric value, field/memo provenance, generic update identity, blank behavior, and localized property-grid display. Keep image loading/scaling/rendering, path resolution, and hosted Windows VFP9 validation separate; focused native/managed gates pass.
...
- Recent E3 report/label slice: `#4525` exposes the existing VFP9 FRX/LBX `TOTALTYPE` calculation-kind field on report-expression objects (`OBJTYPE=8`) through the shared native/managed model, preserving invariant numeric value, field/memo provenance, generic update identity, blank behavior, and localized property-grid display. Keep `RESETTOTAL`, aggregation execution, pagination, rendering, and hosted Windows VFP9 validation separate; focused native/managed gates pass recorded as pending commit/push at the time of this entry.
- Recent E3 report/label slice: `#4526` exposes the existing VFP9 FRX/LBX `RESETTOTAL` reset-timing field on report-expression objects (`OBJTYPE=8`) through the shared native/managed model, preserving invariant numeric value, field/memo provenance, generic update identity, blank behavior, and localized property-grid display. Keep reset execution, aggregation, pagination, rendering, and hosted Windows VFP9 validation separate; focused native/managed gates pass recorded as pending commit/push at the time of this entry.
- Recent E3 report/label slice: `#4527` exposes the existing VFP9 FRX/LBX `SPACING` line-spacing field on label objects (`OBJTYPE=5`) through the shared native/managed model, preserving invariant numeric value, field/memo provenance, generic update identity, blank behavior, and localized property-grid display. Keep expression records excluded and text measurement, line layout, rendering, and hosted Windows VFP9 validation separate; focused native/managed gates pass recorded as pending commit/push at the time of this entry.
- Recent E3 report/label slice: `#4528` exposes the existing VFP9 FRX/LBX `GENERAL` image clip/scale field on image objects (`OBJTYPE=17`) through the shared native/managed model, preserving invariant numeric value, field/memo provenance, generic update identity, blank behavior, and localized property-grid display. Keep image loading/scaling/rendering, path resolution, and hosted Windows VFP9 validation separate; focused native/managed gates pass recorded as pending commit/push at the time of this entry.
- Recent E3 report/label slice: `#4524` exposes the existing VFP9 FRX/LBX `FILLCHAR` expression data-type field on report-expression objects (`OBJTYPE=8`) through the shared native/managed model, preserving invariant value, field/memo provenance, generic update identity, blank behavior, and localized property-grid display. Keep expression evaluation, formatting/rendering, and hosted Windows VFP9 validation separate; focused gates are recorded as pending commit verification at the time of this entry.
...
- Recent native runtime slice: `#4509` adds built-in native PRG Column `Resizable` state with logical default `.F.`, direct/PEM/reflection coverage, declarative and derived `Init` support, logical normalization, and protected property ownership. Preserve the iterative stack-frugal PRG path; native column hit testing, dragging, painting, layout recalculation, Resize event dispatch, and hosted Windows VFP9 validation remain separate evidence or implementation work.
...
- Preserve stack-frugal PRG execution as a hard runtime constraint. Shipped child `#4074` and the completed continuation series under `#4068` remove synchronous evaluator re-entry across the covered expression contexts, including compound and argument-nested expression-level UDF calls. Keep new PRG execution on heap-backed continuations and preserve the configured call-depth diagnostic; do not inflate the process stack as a runtime fix. Shipped `#3918` owns session-wide UDFPARMS value/reference semantics and must retain its UDF-only call classification, live scalar aliases, canonical array aliases, and independent `DO ... WITH` behavior.
...
- Future-facing modernization issues (e.g. the #137 database/container-interchange and legacy-format lanes) are real, committed roadmap work, not optional or maybe-never. They are sequenced after important/basic compatibility, usability, release-readiness, and prerequisite architecture unless live GitHub state shows they have become the highest-weight blocker.
...
- Recent portable UI regression slice: `#4552` under `#3051` corrects the standalone Command-window smoke contract to use supported `? <expression>` input, verify the localized no-active-session response, and preserve empty-input no-op behavior. Keep unsupported command text distinct from no-session/unavailable diagnostics; general VFP Command execution remains future work.
- Recent portable UI harness slice: `#4553` under `#23` adds `scripts/run-designer-smoke-portable.sh` for non-Windows shared WinForms validation, with Linux Xvfb, macOS/existing X display guidance, stable selectors, and display-free discovery. The existing headless script delegates to strict Xvfb mode. Keep live Visual Studio, Win32, installer, and hosted VFP9 evidence on Windows.
- Recent E3 report/label slice: `#4554` under `#24` carries report/label object `FLOAT`, `NOREPEAT`, `STRETCH`, and `STRETCHTOP` through the shared native layout highlights and stable host JSON provenance for live, deleted, and unplaced objects. Focused native/host and managed smoke coverage pass; actual rendering, pagination, floating, stretching, and print behavior remain separate.
- Recent E3 report/label slice: `#4556` under `#24` carries FRX/LBX variable-record (`OBJTYPE = 18`) memo-backed `TAG` initial values through native layout highlights, additive host JSON, and the shared localized property grid. Runtime expression evaluation, rendering, and Report Builder behavior remain separate.
- Recent portable UI harness regression: `#4557` under `#23` fixes standalone Command/Terminal tab removal by selecting a surviving tool window before removing the selected page. This prevents the Mono/WinForms invalid selected-index crash found by Linux CI while preserving shell-layout state and dock/restore behavior.
- Recent supply-chain validation correction: `#4847` under `#4046` explicitly pins Anchore SBOM generation to available Syft `v1.50.0` after the action's stale implicit `v1.42.3` selection returned HTTP 404. Preserve the single explicit `cyclonedx-sbom` uploader, read-only permissions, immutable action pins, and Trivy gate. Hosted run `30490155212` passed at exact head `960fc8eb3` and GitHub reports exactly one non-expired SBOM artifact; #4847 is closed.
