...
- Latest implemented E2 slice: `#1266`, which adds non-executing toolbox object batch creation dispatch planning from admitted palette dispatch. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
