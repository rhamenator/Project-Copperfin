...
- Latest implemented E3 slice: `#1775`, which covers stable-selected live Detail Header/Footer object font edits and clears for FRX/LBX label/field objects. Studio host regressions prove `FONTFACE` set/clear operations by `UNIQUEID` persist refreshed or blank memo data, refresh selected-object highlight provenance, preserve containing-section metadata, keep inspection non-failing, and retain label identity. `#1774` covered deleted Detail Header/Footer object font/highlight metadata, `#1773` covered live font/highlight metadata selections, and `#1772` covered deleted object expression edits and clears. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
