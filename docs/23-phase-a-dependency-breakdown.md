...
- Latest implemented E3 slice: `#1663`, which proves live page-header FRX/LBX layout objects can be selected by stable unique id without a record selector while selected-object availability/kind, label expression/provenance, page-header containing-section metadata, section-relative geometry, label identity, and live/deleted object counts are preserved. `#1662` is the immediately preceding closed stable-selector deleted-unplaced object selection slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
