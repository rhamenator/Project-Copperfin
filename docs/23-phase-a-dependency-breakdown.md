...
- Latest implemented E2 slice: `#1433`, which exposes visual object method move batches through Studio host JSON. `#1432` is the immediately preceding visual object method copy-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
