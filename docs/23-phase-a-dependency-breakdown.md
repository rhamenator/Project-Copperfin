...
- Latest implemented E3 slice: `#1496`, which proves Studio host selected `.lbx` label root/settings JSON parity, selected-settings availability, settings selection kind, settings provenance, and preserved section/object metadata while preserving label identity. `#1495` is the immediately preceding label settings restore mutation coverage slice. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
