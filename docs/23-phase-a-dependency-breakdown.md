...
- Latest implemented E2 slice: `#1225`, which adds non-executing editor action invocation dispatch planning for admitted editor invocations. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
