...
- Latest implemented E2 slice: `#1419`, which exposes visual object immediate-child listings through Studio host JSON. `#1418` is the immediately preceding visual object outline-list host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
