...
- Latest implemented E3 slice: `#1794`, which covers stable-selected deleted Detail Header/Footer object snap containment for FRX/LBX label/field objects. Studio host regressions prove deleted Detail Header and Detail Footer snap-to-grid operations preserve deleted state, persist refreshed HPOS/VPOS geometry, keep containing-section metadata unavailable/null instead of fabricating live section membership, refresh selected deleted-object JSON, keep inspection non-failing, and retain label identity. `#1793` covered live object snap containment, `#1792` covered deleted object resize containment, `#1791` covered live object resize containment, `#1790` covered deleted object alignment containment, `#1789` covered live object alignment containment, `#1788` covered deleted object reorder containment, and `#1787` covered live object reorder containment. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
