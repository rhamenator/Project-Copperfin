...
- Latest implemented E3 slice: `#1773`, which covers stable-selected live Detail Header/Footer object font/highlight metadata for FRX/LBX label/field objects. Studio host regressions prove selected objects expose `FONTFACE`, `FONTSIZE`, and `MODE` provenance plus highlight counts while preserving expression metadata, containing-section metadata, non-failing inspection, and label identity. `#1772` covered deleted Detail Header/Footer object expression edits and clears, `#1771` covered deleted object expression selections, and `#1770` covered live object expression edits and clears. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
