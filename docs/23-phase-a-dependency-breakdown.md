...
- Latest implemented E3 slice: `#1756`, which covers CR-only report/label root settings memo lines. Report layout parsing now treats carriage-return-only separators in `EXPR` memo text as real setting lines, and Studio host regressions prove FRX/LBX live/deleted memo-derived settings remain separate with source-line and memo provenance while valid live settings still drive page/column summaries. `#1755` covered invalid-first duplicate setting precedence, and `#1754` covered first-valid duplicate precedence. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
