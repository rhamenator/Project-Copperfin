...
- Latest implemented E3 slice: `#1754`, which covers duplicate report/label root settings precedence. Studio host regressions create FRX/LBX live and deleted root-settings rows with duplicate `EXPR` memo-derived `ORIENTATION`/`COLS` entries plus same-name direct fields, proving page/column summaries derive from the first live memo setting while every duplicate memo/direct entry remains inspectable with source-order and provenance. Recent prior slices `#1753`-`#1750` cover malformed memo lines, blank memo values, fractional memo values, and invalid memo values; `#1748`-`#1743` cover direct setting-field edge cases. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
