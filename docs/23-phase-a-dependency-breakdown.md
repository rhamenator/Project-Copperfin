...
- Latest implemented E3 slice: `#1753`, which covers malformed report/label root `EXPR` memo setting lines. Studio host regressions create FRX/LBX live and deleted root-settings rows with mixed malformed and valid memo lines, proving no-equals lines and blank-name lines are skipped from settings metadata while later valid page/column settings still parse with their original source-line indexes and memo provenance. `#1752` covered blank and whitespace-only report/label root `EXPR` memo setting values, proving page and column setup availability stays false while live/deleted selected-settings metadata preserves blank values. `#1751` covered whitespace-trimmed fractional report/label root `EXPR` memo setting values, proving setup summaries derive integer portions while live/deleted selected-settings metadata preserves fractional source values. `#1750` covered malformed, oversized, and dot-leading report/label root `EXPR` memo setting values, proving page and column setup availability stays false while live/deleted selected-settings metadata preserves raw invalid values. `#1748` covered dot-leading direct-setting fields, proving page and column setup availability stays false while live/deleted selected-settings metadata preserves dot-leading source values. `#1747` covered oversized direct-setting fields, proving page and column setup availability stays false while live/deleted selected-settings metadata preserves oversized source values. `#1746` covered fractional direct-setting fields, proving setup summaries derive integer portions while live/deleted selected-settings metadata preserves fractional source values. `#1745` covered trimmed direct-setting fields, proving trimmed values still drive setup summaries and live/deleted selected-settings metadata exposes normalized values. `#1744` covered mixed invalid direct-setting fields, proving valid values still drive setup summaries while malformed siblings remain inspectable as raw selected-settings metadata. `#1743` covered blank and whitespace-only direct setup fields. `#1742` covered mixed valid direct settings with sibling unresolved memo placeholders. `#1741` covered all-placeholder direct setup fields. `#1740` covered unresolved section/object geometry memo placeholders. `#1739` covered unresolved live unplaced layout-object memo placeholders. `#1738` covered unresolved deleted layout-object memo placeholders through the deleted-object summary and selected-object paths. `#1737` covered unresolved live/deleted section memo placeholders, proving section expression text/provenance stays suppressed while counts, heights, and band metadata remain available. `#1736` covered unresolved root settings and layout-object memo placeholders, proving placeholder text stays suppressed from summary and selected metadata while root settings/page setup availability stays suppressed and selected-object metadata remains available with empty expression/highlight state. `#1735` covered invalid direct margin/grid fields, proving malformed BOTMARGIN/GRIDV/GRIDH values preserve raw settings without fabricating page setup summaries. `#1734` covered invalid direct column setup fields, proving malformed COLS/COLWIDTH/COLSPACING values preserve raw settings without fabricating available column setup summaries. `#1733` covered invalid direct page setup fields, proving malformed ORIENTATION/PAPERSIZE/TOPMARGIN values preserve raw settings without fabricating available page setup summaries. `#1732` covered unsupported report/label OBJTYPE codes, proving unsupported classification rows do not fabricate root settings, live/deleted sections, layout objects, preview bounds, or selected report metadata. `#1731` covered missing report/label OBJTYPE schema with OBJCODE present, proving OBJCODE-only rows do not infer root settings, sections, objects, preview bounds, or selected report metadata. `#1730` covered missing report/label root OBJCODE schema fallback behavior, proving EXPR-derived grid/column settings, direct page setup fields, selected-settings JSON, and label identity are retained. `#1729` covered missing report/label object OBJCODE schema fallback behavior, proving OBJTYPE-derived field/label kinds, default object-code provenance, deterministic unplaced object accounting, selected-object metadata, and label identity are retained. `#1728` covered missing report/label section OBJCODE schema fallback behavior, proving live/deleted sections default through the Title band metadata path with null object-code provenance while preserving geometry and selected-section metadata. `#1727` covered missing report/label section geometry schema fallback behavior, proving selected-section geometry defaults to top/height/bottom zero with null top/height field provenance. `#1726` covered missing report/label object title schema fallback behavior, proving synthesized `Record N` titles with null title/expression provenance. `#1725` covered missing report/label object EXPR schema fallback behavior, proving object title fallback uses UNIQUEID while expression metadata degrades to empty text with null field provenance. `#1724` covered missing report/label section EXPR schema fallback behavior, proving live/deleted band title/kind metadata and geometry are preserved with null expression provenance and selected-section metadata. `#1723` covered missing report/label root EXPR schema fallback behavior, proving root settings rows without EXPR preserve direct settings metadata, page setup summaries, selected-settings metadata, and label identity. `#1722` covered unknown report/label band-code fallback behavior, proving structurally valid live and deleted band rows with unknown OBJCODE values serialize as `Other Band` / `bandKind: "other"` while preserving geometry, bounds, selected-section metadata, and label identity. `#1721` covered missing report/label layout classification fields, proving rows without OBJTYPE/OBJCODE columns do not infer root settings, sections, layout objects, or preview bounds and explicit selection degrades to no selected report metadata. `#1720` covered missing report/label layout geometry fields, proving HPOS/VPOS/WIDTH/HEIGHT omissions default to zero-origin zero-size geometry with null selected-object field provenance. `#1719` covered fractional report/label layout geometry, proving integer portions are used consistently for selected-object and preview/deleted-preview geometry. `#1718` covered invalid report/label layout classification fields, proving malformed and out-of-range OBJTYPE/OBJCODE rows do not create false sections or layout objects and explicit selection degrades to no selected report metadata. `#1717` covered oversized report/label layout numerics, proving out-of-range geometry values default to non-inverted zero-origin/zero-size selected-object and preview geometry. `#1716` covered malformed report/label layout numerics through the same non-failing fallback path. `#1715` covered negative report/label layout dimensions, proving negative section heights and negative live/deleted object WIDTH/HEIGHT values clamp to non-inverted selected-object and preview geometry. `#1713` clears launch-selection record availability when an FRX/LBX `--record` request is beyond available table records while preserving diagnostics and avoiding stable-selector fallback. `#1712` fixed explicit FRX/LBX `--record` selection beyond the default preview window by parsing at least through the requested record index, bounded by table record count. `#1711` covered missing `UNIQUEID` schema; `#1710` covered whitespace-only selectors not matching blank stored IDs; `#1709` covered live preview/deleted deep duplicate ambiguity; `#1708` covered deep duplicate section/settings ambiguity across live preview/deep rows; `#1707` covered successful deep section/settings selection; `#1706` covered deep duplicate-object ambiguity across preview/deep rows; `#1705` fixed stable selection for larger report/label assets by loading the full table for selector resolution; `#1704` covered explicit-record precedence over conflicting stable selectors; `#1703` covered trim/case-insensitive unique-id matching; `#1702` covered duplicate IDs split across live/deleted rows; `#1701` covered duplicate live section/settings rows; `#1699` covered object ambiguity; `#1698` is the closed missing-selector no-selection slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Interleaved higher-weight evidence-management slice `#1700` is also shipped under `#108`: `test_dbf_table` no longer uses stale issue-number tokens for GAP-01/GAP-02/GAP-03 DBF error-management coverage, and the safety gap analysis now reflects existing malformed-header, numeric-boundary, and staged-write rollback tests. Interleaved F1 error-management slice `#1714` is shipped under `#25`: `test_studio_host` now covers malformed and missing VS launch-contract numeric selection values for `--record`, `--line`, and `--column`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
