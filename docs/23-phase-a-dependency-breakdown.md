...
- Latest implemented E2 slice: `#1294`, which adds selection-context toolbox object create-dispatch catalog planning through the shared toolbox palette launch seam. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
