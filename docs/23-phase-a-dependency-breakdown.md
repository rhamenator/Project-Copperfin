...
- Latest implemented E3 slice: `#1666`, which proves live group-header FRX/LBX sections can be selected by stable unique id without a record selector while selected-section availability/kind, group-header band metadata, expression provenance, sibling detail/footer section counts, null selected object/settings metadata, and label identity are preserved. `#1665` is the immediately preceding closed stable-selector deleted page-header section selection slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
