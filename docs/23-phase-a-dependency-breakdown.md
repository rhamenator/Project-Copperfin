...
- Latest implemented E3 slice: `#1659`, which proves live and deleted FRX/LBX sections can be selected by stable unique id without a record selector while selected-section availability/kind, live/deleted section counts, unplaced object accounting, null settings/object selections, label identity, and section metadata are preserved. `#1658` is the immediately preceding closed stable-selector root-settings selection slice; `#1546` was closed not planned after validation proved its direct-over-memo precedence premise would regress existing memo-edit behavior. Latest implemented E2 slice: `#1451`, which exposes visual object subtree duplication through Studio host JSON. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
