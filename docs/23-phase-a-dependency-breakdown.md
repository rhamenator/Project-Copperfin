...
- Latest implemented E2 slice: `#1442`, which exposes visual object property rename batches through Studio host JSON. `#1441` is the immediately preceding visual object property-rename host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
