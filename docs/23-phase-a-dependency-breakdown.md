...
- Latest implemented E3 slice: `#2684`, stable-selected nested report/label group-section EXPR edit fidelity; focused Studio host JSON coverage now proves FRX/LBX `--unique-id` selection can update and clear live inner nested group-header and deleted inner nested group-footer `EXPR` fields, persist memo-backed changes, refresh selected live/deleted nested section metadata, preserve unaffected outer/inner sibling expressions plus preview/deleted-preview bounds, and keep label identity without changing machine-readable designer contracts.
...
- Current continuation shape: continue prompt-sized E3/#24 children in report/label designer fidelity, especially grouping/sorting metadata, section-aware editing, output preview metadata, and layout/runtime parity seams, while keeping any newly introduced user-facing text localized by default; stable-selected, deleted, record-selected, and nested group-section EXPR edit evidence now exist at both top-level and nested group depths, so prefer the next unproven seam rather than re-covering group-section EXPR mutation cases; use host exposure where useful, focused tests, docs, validation, commit, push, and issue closure per slice.
