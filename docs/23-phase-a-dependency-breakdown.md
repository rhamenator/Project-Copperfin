...
- Latest implemented E2 slice: `#1223`, which adds catalog-level aggregate non-executing designer invocation admission planning across all known Studio contexts. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
