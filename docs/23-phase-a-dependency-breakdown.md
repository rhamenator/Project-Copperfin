...
- Latest implemented E3 slice: `#1791`, which covers stable-selected live Detail Header/Footer object resize containment for FRX/LBX label/field objects. Studio host regressions prove Detail Header and Detail Footer resizes persist refreshed WIDTH/HEIGHT geometry, preserve HPOS/VPOS, preserve containing-section ids/record indexes plus section-relative coordinates and object counts, refresh selected-object/selected-section JSON, keep inspection non-failing, and retain label identity. `#1790` covered deleted object alignment containment, `#1789` covered live object alignment containment, `#1788` covered deleted object reorder containment, `#1787` covered live object reorder containment, `#1786` covered deleted object rename containment, `#1785` covered live object rename containment, and `#1784` covered deleted object duplicate containment. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
