...
- Latest implemented E3 slice: `#1782`, which covers stable-selected Detail Header/Footer object delete/restore containment transitions for FRX/LBX label/field objects. Studio host regressions prove live Detail Header objects delete into the deleted-object collection with containing-section metadata unavailable/null, while deleted Detail Footer objects restore into live containing-section membership with refreshed section-relative metadata, object counts, non-failing inspection, and label identity. `#1781` covered deleted object geometry edits, and `#1780` covered live object geometry edits. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
