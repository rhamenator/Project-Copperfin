...
- Latest implemented E2 slice: `#1326`, which adds non-mutating builder dispatch execution catalog planning so builder/wizard dispatch catalogs can report explicit execution-admission readiness and per-builder execution preflight errors without launching builders. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
