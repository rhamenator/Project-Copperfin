...
- Latest implemented E3 slice: `#2686`, report/label section grouping-context JSON; `StudioReportSectionSnapshot` now carries explicit FRX/LBX grouping context for group-header/group-footer bands, `copperfin_studio_host --json` exposes additive grouping-context fields on `sections`, `deletedSections`, and `selectedReportSection`, and focused report-layout plus host validation now proves live/deleted nested group sections preserve grouping index, nesting depth, role, and partner-band identity without changing existing machine-readable designer contracts.
