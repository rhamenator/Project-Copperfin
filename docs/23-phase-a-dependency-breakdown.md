...
- Latest implemented E2 slice: `#1435`, which exposes visual object property clears through Studio host JSON. `#1434` is the immediately preceding visual object method reorder-batch host JSON slice. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
