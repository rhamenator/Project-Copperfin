...
- Latest implemented E2 slice: `#1241`, which adds non-mutating toolbox object creation planning ahead of the existing mutating toolbox-create path. Issue closure still requires explicit user approval if the standing no-close-without-approval rule is active.
