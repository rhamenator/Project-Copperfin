...
- Latest runtime declaration-fidelity slice: `#3900` makes bare `LOCAL` and `PUBLIC` scalar declarations initialize to logical `.F.` rather than unknown values, matching shipped VFP9 help. Keep unsupplied formal parameters separately scoped, preserve public lifetime and array initialization behavior, and retain debugger exposure before assignment. Focused GCC/Clang core runtime coverage locks this behavior.
