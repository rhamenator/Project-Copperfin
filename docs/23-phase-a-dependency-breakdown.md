...
- Latest implemented E3 slice: `#1769`, which covers stable-selected object expression provenance for live FRX/LBX objects placed inside Detail Header and Detail Footer bands. Studio host regressions prove those selected objects expose expression text, field/memo provenance, object kind, containing-section ids/record indexes, section-relative coordinates, containing-section object counts, non-failing inspection, and label identity. `#1768` covered stable live Detail Header/Footer section expression selections, `#1767` covered stable deleted Detail Header/Footer section selection, and `#1766` covered deleted Detail Header/Footer section-expression edits. Interleaved higher-weight evidence-management slice `#1700` is shipped under `#108`, and F1 error-management slice `#1714` is shipped under `#25`. Latest implemented E2 slice: `#1749`, which covers malformed, oversized, and dot-leading selected-object raw `OBJTYPE`/`OBJCODE` host JSON fallback while preserving raw property metadata. Evidence-backed child issue closure is approved for completed slices; keep parent/lane issues open unless root-level closure evidence is explicit.
