...
| `RQ-CF-AGENT-002` | Derived from `RQ-CF-AGENT-001` and the trusted-host architecture boundary | Before mutable activation exists, the native Studio host shall expose a versioned, read-only policy descriptor whose modes, capabilities, permission/UI/audit gates, warning identity, provider-auth separation, and no-elevation invariant derive from the governing policy. The descriptor shall not activate an agent or manufacture trusted-UI consent, and mixed, duplicate, reordered, or activation-like arguments shall fail without partial output. | `docs/64-workspace-agent-access-policy.md`; `apps/copperfin_studio_host/studio_host_main_workspace_agent_policy.cpp`; shared Studio-host CLI dispatch | `tests/test_studio_host_workspace_agent_policy.cpp`; policy/platform/localization/isolation tests; protected Windows, Ubuntu, and macOS native execution required before closure | `docs/safety/traceability-report-2026-08-12-workspace-agent-access-policy.md`; exact-head hosted result pending | `HZ-system-failure-01`; `HZ-data-corruption-01`; descriptor-only boundary leaves activation, session revocation, audit commits, executor, and sandbox unimplemented | `gap` |
