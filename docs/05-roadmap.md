...
| H | `#113` (`H1`/`#30`, `H2`/`#31`, `H3`/`#32`) | Relational backend translators, document/vector + AI planning, .NET/MCP/polyglot outputs | Portable route/artifact execution, trusted runtime-host composition, PRG dispatch, Native AOT C# leaf, admitted Python/R sidecar leaves, one bounded read-only MCP DBF-header tool, provider-independent workspace-agent access modes, strict read-only native/managed descriptors, a localized non-activating Studio preview, audited session lifecycle, persistent lifecycle sink, product-registry-backed non-executing preflight, existing-file target identity preflight, explicit process/working-directory identity preflight, bounded direct arguments, fixed-key generation-owned logical environment construction, POSIX/Windows native environment and argument serialization, private generation-layout preparation, identity-receipted explicit empty-layout cleanup, a conservative Windows PE process-image gate, exact trusted-host identity-plus-digest Windows child-parser authority, an explicit fail-closed launch-promotion gate, its move-only exact-generation revocation lease, retained target objects, private bounded immutable executable-byte snapshots, opaque prepared launch-candidate composition, one-attempt private exact-snapshot materialization, Windows identity-bound immutable launch transition, and a warned non-elevated Windows unrestricted exact-image bounded executor with paired content-free audit implemented; provider adapters, OAuth, conversation UI, trusted host activation/consent binding, POSIX/macOS launch transition and execution, audit-backed lifecycle cleanup, outcome audit, diff, and undo remain | v1 item 3 |
| I | `#113` (`I1`/`#33`, `I2`/`#34`) | Runtime/project security depth, extension/host/AI-MCP security boundary | RBAC/audit/secrets/signing baseline plus dedicated workspace-agent RBAC, capabilities, localized unrestricted warning, fail-closed activation policy, non-activating descriptor/client boundary, trusted read-only preview, audited session/revocation, contained lifecycle persistence, immutable product-owned tool definitions, exact-generation registered-tool preflight, product-root-bound existing-file containment, non-executing process target containment, direct-argument policy, fixed-key generation-owned isolated-environment construction, bounded native environment and argument serialization, fail-closed POSIX/Windows private-layout preparation, configured process-capable session-start preparation, identity-receipted explicit empty-layout cleanup, structural Windows executable-image compatibility, exact-identity-plus-digest parser authority, fail-closed public launch promotion, a non-executing revocation lease, opaque retained process-target objects, private bounded immutable executable-byte snapshots, opaque prepared launch-candidate composition, one-attempt private exact-snapshot materialization, Windows identity-bound immutable launch transition, and a candidate warned non-elevated Windows unrestricted exact-image bounded executor with paired content-free audit implemented; trusted host activation enforcement, consent binding, POSIX/macOS launch transition and execution, audit-backed lifecycle cleanup and durable receipt recovery, endpoint policy, general tool-outcome audit, public promotion, and real workspace sandbox remain | v1 item 4 |
...
An exact-generation, move-only revocation lease can now delay stop only across a
future direct launch decision. An adjacent move-only boundary now retains the
exact trusted root, executable, and working-directory objects behind opaque
platform handles/descriptors after private provenance and identity checks.
Neither exposes launch authority or weakens the invariantly denying promotion
gate. Private immutable byte snapshots are authenticated, and the controller
now composes the exact internal serialized plan, authenticated pins, and lease
into one opaque candidate. The controller can consume that candidate once into
an opaque private native image while retaining the plan, pins, and lease. A
...
an execution token.
The adjacent process-target preflight admits only the registered workspace or
local process class, exact active session generation, one explicit executable,
and one explicit working directory. Workspace spellings remain physically
under the unchanged root; unrestricted spellings must be absolute with direct
leaves. It performs no PATH lookup, shell interpretation, argument/environment
handling, sandboxing, or launch, and its returned identities are not execution
authority.
The adjacent invocation-shape preflight binds at most 64 direct UTF-8 argument
elements (4,096 bytes each and 8,192 bytes total) to that same exact target and
session, preserves shell metacharacters literally, exposes no caller-selected
...
audit-backed cleanup. A separate identity-receipted primitive can remove only
the exact empty prepared layout, but it is not called by the lifecycle and
never traverses content. These boundaries perform no automatic cleanup, sandboxing, or
...
launch-adjacent target handle pinning and synchronous executor consumption,
trusted-host audit-backed lifecycle cleanup with durable receipt recovery, endpoint
