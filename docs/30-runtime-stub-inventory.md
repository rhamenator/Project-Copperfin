...
| `CURSORTOXML()` / `XMLTOCURSOR()` | `src/runtime/prg_engine_runtime_surface_dispatch_object.inl` | Empty/false result when cursor snapshot/load callbacks are unavailable | Host-capability fallback | Preserve verified-file and machine-contract rules; no silent fallback in strict paths |
| `AFONT()` | `src/runtime/prg_engine_variables.inl` | Returns a fixed headless font set rather than enumerating platform fonts | Deliberate portability fallback | Replace only with a platform-owned font-provider slice |
| Generated runtime bridge exports | `src/runtime/runtime_pipeline_library_export_manifest.cpp` | Emits placeholder return/admission plumbing for unsupported bridge signatures | Deliberate interop boundary | Track against the .NET/native interop roadmap; do not delegate piecemeal within the generator |
