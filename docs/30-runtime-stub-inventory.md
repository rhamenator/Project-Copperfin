...
| `EVENTHANDLER()` COM-event lifecycle | `include/copperfin/runtime/prg_engine.h`; `src/runtime/prg_engine_expression.inl`; `src/runtime/prg_engine.cpp`; `src/runtime/windows_com_event_adapter.{h,cpp}`; governing recovery in `docs/32-recovered-requirements-traceability.md` | Fails closed unless a host explicitly admits a local source identity, interface identity, and required handler methods. The portable registry records deterministic bind/unbind state and removes it on source/handler release. The Windows-only adapter adds a host-owned local `IConnectionPoint` lane that passes only declared method tokens to the runtime; native `BINDEVENT()` / `RAISEEVENT()` and virtual `CREATEOBJECTEX()` provenance are not substitutes. | Deliberate interop boundary | The recovered lane is limited to the documented package-free owned local Windows fixture, with direct evidence from protected run `32794260256`; do not add remote activation, registry discovery, third-party server dependence, or COM resources to PRG/JSON contracts. |
