...
- keep strengthening the shared DBF/FPT mutation path so runtime commands do not regress FoxPro-compatible memo-backed tables, first-pass fixed-width `I`/`Y`/`T` field writes, and other core FoxPro field types, and keep the new indexed-table fail-fast guard in place until real structural `CDX` write/rebuild support exists
