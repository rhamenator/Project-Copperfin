...
The same memory-file lane now also has first-pass array/scope fidelity for `SAVE TO` / `RESTORE FROM`: saved `.mem` payloads now preserve runtime arrays with dimensions plus `PUBLIC` scope markers, additive restore now writes through visible `LOCAL` bindings instead of leaving restored values hidden behind stale locals, and non-additive restore now clears stale arrays and deferred `PRIVATE` shadow state before recreating the saved bindings.
