...
- expand the remaining automation surface beyond the now-deeper `CREATEOBJECT()` / `GETOBJECT()` / `NEWOBJECT()` activation slice, especially where containment or host safety still depends on stubs
