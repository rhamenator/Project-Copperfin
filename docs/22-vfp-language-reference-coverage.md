...
- utility coverage already used by shipped tests and bootstrap paths, including occurrence-aware `AT()` / `RAT()`, first-pass case-insensitive `ATC()` / `RATC()`, line-oriented `ATLINE()` / `ATCLINE()` / `RATLINE()`, `LIKE()`, `INLIST()`, `PROPER()`, first-pass `STRCONV(..., 7/8)` casing modes, first-pass phonetic helpers (`SOUNDEX()` and `DIFFERENCE()`), first-pass memo-line helpers (`MEMLINES()` and `MLINE()` with default-width wrapping), `CHRTRAN()` / first-pass `CHRTRANC()`, `SUBSTR()`, `ALLTRIM()`, `STREXTRACT()` with flag-aware delimiter handling, `STR()` with width/decimal formatting, `PADL()` / `PADR()` / `PADC()` padding and overflow truncation, `GETWORDCOUNT()` / `GETWORDNUM()` with delimiter-set handling, `CHR()`, type/null helpers including first-pass expression-value `ISBLANK()` and single-byte `ISLEADBYTE()`, cursor/schema helpers (`FIELD()` and `FSIZE()` over local DBF-backed and synthetic SQL cursors), numeric helpers (`SIN()`, `COS()`, `TAN()`, `ASIN()`, `ACOS()`, `ATAN()`, `ATN2()`, `DTOR()`, `RTOD()`, `LOG10()`, `RAND()`, `RGB()`, and expression-list `MIN()` / `MAX()`), runtime-surface helpers (`CAST()`, `BITAND()`, `BITOR()`, `BITXOR()`, `BITNOT()`, `BITCLEAR()`, `BITSET()`, `BITTEST()`, `BITLSHIFT()`, `BITRSHIFT()`, `BINTOC()`, `CTOBIN()`, first-pass host probes `HOME()`, `OS()`, `DISKSPACE()`, `DRIVETYPE()`, first-pass `CURSORGETPROP()` / `CURSORSETPROP()`, and lock-key probes such as `NUMLOCK()`), date/time helpers (`DOW()`, `CDOW()`, `CMONTH()`, `GOMONTH()`, `SECONDS()`, `MDY()`, constructor-argument forms of `DATE()` / `DATETIME()`, `STOD()`, `TTOS()`, `CTOT()` including compact sortable datetime input, `DTOC(..., 1)`, `TTOC(..., 1)`, `DTOT()`, `TTOD()` including compact sortable datetime input, `HOUR()`, `MINUTE()`, `SEC()`, first-pass `WEEK()` with optional first-day and first-week mode arguments including mode 2/3 cross-year rollover, `QUARTER()`, `EOMONTH()`, `DTOJ()` / `TTOJ()` / `JTOD()` / `JTOT()` with first-pass invalid-input guards), portable string-parsed path helpers (`JUSTPATH()`, `JUSTFNAME()`, `JUSTSTEM()`, `JUSTEXT()`, `JUSTDRIVE()`, `FORCEEXT()`, `FORCEPATH()`, `DEFAULTEXT()`, and first-pass `CURDIR()`), file metadata helpers (`FILE()` and `FILESIZE()`), `SYS()` including first-pass `SYS(2003)`, `SYS(2004)`, `SYS(2020)`, and `SYS(2023)`, `MESSAGE()`, `PROGRAM()`, `LINENO()`, `ERROR()`, `VERSION()`, `ON()`, `SET()`, and `MESSAGEBOX()`
...
- deepen `MLINE()` / `MEMLINES()` later with compatibility flags and offset semantics beyond the current session-scoped `SET MEMOWIDTH` + `_MLINE` baseline; the current implementation uses width-aware wrapping, CR hard breaks, LF-as-content behavior, and optional start offsets
