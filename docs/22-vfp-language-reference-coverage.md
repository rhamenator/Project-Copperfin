...
- `SCATTER MEMVAR` / `SCATTER TO <array>` — snapshot current record fields to `m.<fieldname>` variables or a first-pass one-dimensional array; `FIELDS` and `BLANK` are honored with typed values, including runtime-facing `MM/DD/YYYY` and `MM/DD/YYYY HH:MM:SS` strings for `D`/`T` fields; first-pass
- `GATHER MEMVAR` / `GATHER FROM <array>` — write `m.<fieldname>` variables or array elements back to current record via direct `replace_record_field_value` calls (type-faithful, no string-quote round-trip); `FIELDS` and `FOR` are honored, and date/datetime/blank runtime strings now restore consistently into DBF storage; first-pass
