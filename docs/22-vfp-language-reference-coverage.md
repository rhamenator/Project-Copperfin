...
- `WAIT`, `KEYBOARD`, `DISPLAY`, `LIST` — first-pass headless interaction/event lane that emits `runtime.wait`, `runtime.keyboard`, `runtime.display`, and `runtime.list` events with captured mode/clause payloads for host integration; `WAIT` now also captures prompt/timeout/flag/target detail for common `WAIT [WINDOW] ... [TIMEOUT ...] [TO <target>] [NOWAIT] [NOCLEAR]` forms and assigns deterministic empty-string headless `TO` results, while `DISPLAY` / `LIST` `RECORDS` forms surface effective cursor/view metadata (`IN`, active filter, and visible fields, including `SET FIELDS TO LIKE ...` and inline `FIELDS EXCEPT ...` behavior) similarly to the shipped `BROWSE` lane
