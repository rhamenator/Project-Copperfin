...
- utility coverage already used by shipped tests and bootstrap paths, including occurrence-aware `AT()` / `RAT()`, first-pass case-insensitive `ATC()` / `RATC()`, line-oriented `ATLINE()` / `ATCLINE()` / `RATLINE()`, `LIKE()`, `INLIST()`, `PROPER()`, `CHRTRAN()` / first-pass `CHRTRANC()`, `SUBSTR()`, `ALLTRIM()`, `STR()`, `CHR()`, portable string-parsed path helpers (`JUSTPATH()`, `JUSTFNAME()`, `JUSTSTEM()`, `JUSTEXT()`, `JUSTDRIVE()`, `FORCEEXT()`, `FORCEPATH()`), `FILE()`, `SYS()`, `MESSAGE()`, `PROGRAM()`, `LINENO()`, `ERROR()`, `VERSION()`, `ON()`, `SET()`, and `MESSAGEBOX()`
...
- keep expression-function additions in small reference-derived batches; the path/string batch now covers `FORCEEXT()`, `FORCEPATH()`, and `CHRTRANC()`, while date/time helper functions remain a natural next family
