...
- `WAIT`, `KEYBOARD`, `DISPLAY`, `LIST`, `INPUT`, `ACCEPT`, `CALCULATE`, `COUNT`, `SUM`, `AVERAGE`, `TOTAL` — first-pass headless interaction/event lane that emits `runtime.wait`, `runtime.keyboard`, `runtime.display`, `runtime.list`, `runtime.input`, `runtime.accept`, and aggregate runtime events with captured mode/clause payloads for host integration; `WAIT` now captures resolved prompt/timeout/target state plus source expressions for common `WAIT [WINDOW] ... [TIMEOUT ...] [TO <target>] [NOWAIT] [NOCLEAR]` forms and assigns deterministic empty-string headless `TO` results, `INPUT` / `ACCEPT` now mirror that resolved prompt/target payload fidelity and deterministic empty-string `TO` behavior, `KEYBOARD` now captures resolved key payloads plus source expressions and common `PLAIN` / `CLEAR` flags, `DISPLAY` / `LIST` now surface adjacent `STRUCTURE` / `STATUS` / `MEMORY` metadata in addition to the richer `RECORDS` lane, `DISPLAY` / `LIST RECORDS` now also retain `WHILE` clause payload detail, `DISPLAY` / `LIST STRUCTURE` now honor `IN <alias|work area>` targeting with raw/resolved target detail, and aggregate command events now also expose targeted cursor/work-area, visible fields, active filter, scope, `FOR` / `WHILE`, output-variable, and raw/resolved `IN` target detail for host/runtime readback
...
- selected synthetic SQL/result cursors now also apply `FOR` filtering during DBF `APPEND FROM` mutation, so only rows matching the clause are materialized into the target cursor
...
Command-level aggregate assignment now also includes first-pass `TO ARRAY` forms for `COUNT`, `SUM`, and `AVERAGE`, using one-dimensional runtime array assignment through the shared array helper path. Focused regression coverage validates scope-clause behavior (`ALL`/`REST`/`NEXT`/`RECORD`), `FOR`/`WHILE` filtering, `IN` alias targeting, `TO &macro` target resolution on the command aggregate lane, and malformed `TO ARRAY` diagnostics. `CALCULATE` now also preserves `WHILE` clause semantics on the same aggregate path. Aggregate expression parity also now includes omitted-expression forms: `COUNT()` returns visible-row counts, and no-expression `SUM()` / `AVG()` / `MIN()` / `MAX()` use the first numeric-compatible field (`N/F/B/I/Y`) with deterministic no-numeric-field fallbacks.
...
- `EVAL()` and deeper first-pass bounded-indirection `&macro` substitution in expression paths (identifier-chain expansion with deterministic recursion/cycle safety), now including alias-qualified field continuations such as `&cAlias..NAME` plus macro-driven suffix forms such as `&cAlias..&cField`
