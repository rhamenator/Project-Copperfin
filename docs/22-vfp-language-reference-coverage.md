...
- utility coverage already used by shipped tests and bootstrap paths, including occurrence-aware `AT()` / `RAT()`, first-pass case-insensitive `ATC()` / `RATC()`, line-oriented `ATLINE()` / `ATCLINE()` / `RATLINE()`, `LIKE()`, `INLIST()`, `PROPER()`, `CHRTRAN()` / first-pass `CHRTRANC()`, `SUBSTR()`, `ALLTRIM()`, `STREXTRACT()` with flag-aware delimiter handling, `STR()` with width/decimal formatting, `PADL()` / `PADR()` / `PADC()` padding and overflow truncation, `GETWORDCOUNT()` / `GETWORDNUM()` with delimiter-set handling, `CHR()`, date/time helpers (`DOW()`, `CDOW()`, `CMONTH()`, `GOMONTH()`, `SECONDS()`, `MDY()`, constructor-argument forms of `DATE()` / `DATETIME()`, `STOD()`, `TTOS()`, `CTOT()`, `DTOC(..., 1)`, `TTOC(..., 1)`, `DTOT()`, `TTOD()`, `HOUR()`, `MINUTE()`, `SEC()`, first-pass `WEEK()` with optional first-day and first-week mode arguments including mode 2/3 cross-year rollover, `QUARTER()`, `EOMONTH()`, `DTOJ()` / `TTOJ()` / `JTOD()` / `JTOT()` with first-pass invalid-input guards), portable string-parsed path helpers (`JUSTPATH()`, `JUSTFNAME()`, `JUSTSTEM()`, `JUSTEXT()`, `JUSTDRIVE()`, `FORCEEXT()`, `FORCEPATH()`), `FILE()`, `SYS()`, `MESSAGE()`, `PROGRAM()`, `LINENO()`, `ERROR()`, `VERSION()`, `ON()`, `SET()`, and `MESSAGEBOX()`
