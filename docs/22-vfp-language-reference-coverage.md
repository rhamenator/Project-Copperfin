...
- `STORE`
- `CREATE TABLE` — first-pass local DBF creation for field declarations (`C`/`N`/`F`/`B`/`L`/`D`/`I`/`Y`/`T`/`M`/`V`/`Q` plus common long-form names); nullable/default annotations are accepted by the parser as declaration syntax, created tables are opened as local cursors, and they can immediately participate in the shared mutation path
- `ALTER TABLE ... ADD COLUMN` — first-pass local DBF schema rewrite that appends one supported field while preserving row values and deleted flags
