...
- deepen `MLINE()` / `MEMLINES()` later with `SET MEMOWIDTH` integration and `_MLINE` side effects; the current implementation uses default-width wrapping, CR hard breaks, LF-as-content behavior, and optional start offsets
