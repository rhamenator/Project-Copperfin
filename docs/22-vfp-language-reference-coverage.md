...
- `EDIT`, `CHANGE`, `INPUT`, `ACCEPT` — first-pass headless interactive command lane that emits runtime events (`runtime.edit`, `runtime.change`, `runtime.input`, `runtime.accept`) with captured prompt/target/field metadata for host-driven UI integration
- `WAIT`, `KEYBOARD`, `DISPLAY`, `LIST` — first-pass headless interaction/event lane that emits `runtime.wait`, `runtime.keyboard`, `runtime.display`, and `runtime.list` events with captured mode/clause payloads for host integration
- `GETFILE`, `PUTFILE`, `GETDIR`, `INPUTBOX` — first-pass headless dialog-helper command lane; parser captures common prompt/title/default/filter/target metadata and dispatcher emits `runtime.getfile`, `runtime.putfile`, `runtime.getdir`, and `runtime.inputbox` events for host/UI integration without blocking runtime execution
...
- `SET REPROCESS`, `SET MULTILOCKS`, `RLOCK()`, `FLOCK()` / `LOCK()`, `ISRLOCKED()`, `ISFLOCKED()`, and `UNLOCK` — first-pass in-process lock-state tracking for open local/runtime cursors, including `SET()` readback and `UNLOCK ALL`; this establishes runtime-visible lock semantics but does not yet claim cross-process OS locking or transaction journaling
- `BEGIN TRANSACTION`, `END TRANSACTION`, `ROLLBACK`, and `TXNLEVEL()` — first-pass in-process transaction nesting counters with per-data-session isolation and runtime event emission (`runtime.transaction.begin` / `runtime.transaction.end` / `runtime.transaction.rollback`); this establishes command/function surface parity without durable transaction log replay or DBF-level undo journaling yet

Command-level aggregate assignment now also includes first-pass `TO ARRAY` forms for `COUNT`, `SUM`, and `AVERAGE`, using one-dimensional runtime array assignment through the shared array helper path. Focused regression coverage validates scope-clause behavior (`ALL`/`REST`/`NEXT`/`RECORD`), `FOR` filtering, `IN` alias targeting, and malformed `TO ARRAY` diagnostics.
...
`ON ERROR` now also has a richer first-pass compatibility lane: `ON ERROR DO <routine> WITH ...` can pass evaluated handler arguments, and handlers can inspect first-pass runtime fault metadata through `MESSAGE()`, `PROGRAM()`, `LINENO()`, `ERROR()`, `AERROR()`, `SYS(2018)`, and `ON('ERROR')` while execution resumes after the handler returns. Normal `AERROR()` rows now preserve the original faulting work area even if the handler changes selection before inspecting the array. Static PRG diagnostics are also now wired through the Studio document-open path so analyzer findings surface to editor-facing consumers instead of staying runtime-only.
