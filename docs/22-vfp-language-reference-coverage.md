...
That same local temporary-order path now also has focused command-path `IN <alias|work area>` parity coverage for ad hoc expressions such as `UPPER(NAME)`, including selected-alias preservation and descending `SET NEAR` behavior on non-selected targeted cursors. Synthetic SQL/result cursors now also honor first-pass ad hoc `FOR` filters on that temporary-order lane, so probes such as `SEEK(..., 'sqlcust', "UPPER(NAME) FOR NAME = 'BRAVO'")` and command-path `SET ORDER TO UPPER(NAME) FOR ...` constrain the candidate set and `SET NEAR` miss positioning instead of treating the `FOR` tail as inert text.
...
- `EVAL()` and deeper first-pass bounded-indirection `&macro` substitution in expression paths (identifier-chain expansion with deterministic recursion/cycle safety), now including alias-qualified field continuations such as `&cAlias..NAME`
...
`RETRY` now complements the fault-isolation model with first-pass session-state restoration. The runtime snapshots the active `DataSessionState` (cursor aliases, work-area selection, active filter) onto the `error_metadata_stack` at the moment a fault fires — covering both normal script-level errors and catastrophic C++ exceptions such as `LOG(-1)`. `RETRY` pops that snapshot and restores the pre-fault session state before re-executing the faulting statement, so recovery loops that use `RETRY` to retry a failing record write or file operation can depend on a stable cursor environment instead of whatever state the `ON ERROR` handler may have mutated. Focused `test_prg_engine_control_flow` coverage validates fault persistence, clean session recovery via `RETRY`, and repeated `RETRY` loop completion.
