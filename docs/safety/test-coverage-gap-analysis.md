...
| GAP-02 | `tests/test_dbf_table.cpp` covers adversarial record counts, descriptor/header bounds, record-width mismatches, malformed memo metadata, and full-width field names. Optional `test_dbf_header_robustness` adds 6,757 bounded deterministic synthetic cases for the fixed header parser, with an ASan/UBSan variant where supported. | Focused and bounded malformed-header coverage is present. Field/record/memo/index randomized campaigns and exhaustive format coverage remain out of scope; this optional lane is not an RC or v1 gate. |
