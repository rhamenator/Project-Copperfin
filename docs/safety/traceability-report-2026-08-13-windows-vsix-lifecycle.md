...
repository-community gates also pass at this head. `RQ-CF-REL-003` is therefore
`defined` for fresh install, registered PRG/command use, package load, uninstall,
and residue containment. Same-version reinstall, previous-version upgrade,
disablement, signing, and human visual review remain separate and explicitly
unclaimed.
