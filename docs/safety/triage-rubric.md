...
| **Required before close** | All seven sections populated; `scripts/validate-safety-traceability.ps1` must pass; review evidence must identify its mode, meaningful verification scope, and exactly one approved overall `result`/`status`. Maintainer self-review must also record `verification result: passed`, meaningful automated evidence, and `automated evidence result: passed`. `high`/`catastrophic` changes require a reviewer-authored structured sign-off that identifies the reviewer, meaningful qualification and verification scope, `verification result: passed`, the exact current issue-body SHA-256, and exactly one approved overall `result`/`status`. Duplicate aliases, placeholder/deferred/unavailable evidence, and failed, cancelled, timed-out, negated, or non-passing outcomes fail closed. Any later issue-body edit requires renewed sign-off. Simulation or walkthrough evidence must be attached or linked. |
