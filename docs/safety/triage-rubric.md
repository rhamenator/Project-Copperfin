...
| **Required before close** | All seven fields populated; `scripts/validate-safety-traceability.ps1` must pass; review evidence must identify its mode and approved result and contain meaningful completed verification rather than placeholder, deferred, unavailable, or negated values. Maintainer self-review must also record meaningful completed automated evidence. `high`/`catastrophic` changes require a reviewer-authored structured sign-off that identifies the reviewer, meaningful non-placeholder and non-negated qualification and verification evidence, the exact current issue-body SHA-256, and an approved result. Any later issue-body edit requires renewed sign-off. Simulation or walkthrough evidence must be attached or linked. |
